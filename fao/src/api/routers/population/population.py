...
@router.get("/")
@cache_result(prefix="population", ttl=86400, exclude_params=["response", "db"])
def get_population(
    response: Response,
    ...
    """
    Get population data with filters and pagination.
    ...
    # Main table columns
    for col in Population.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Population, 'id'):
                columns.append(Population.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Population)
    ...
    if year_code is not None:
        conditions.append(Population.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(Population.year_code == year_code_exact)
    if year is not None:
        conditions.append(Population.year == year)
    if year_min is not None:
        conditions.append(Population.year >= year_min)
    if year_max is not None:
        conditions.append(Population.year <= year_max)
    if unit is not None:
        conditions.append(Population.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(Population.unit == unit_exact)
    if value is not None:
        conditions.append(Population.value == value)
    if value_min is not None:
        conditions.append(Population.value >= value_min)
    if value_max is not None:
        conditions.append(Population.value <= value_max)
    if note is not None:
        conditions.append(Population.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(Population.note == note_exact)
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Population, field_name):
                        order_by_clauses.append(getattr(Population, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Population, sort_field):
                        order_by_clauses.append(getattr(Population, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Population.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Population)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, Population.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, Population.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, Population.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, Population.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_population'))
...
@router.get("/areas")
@cache_result(prefix="population:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="population:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    if not years_data:
        return {"dataset": "population", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="population:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(Population.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(Population.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(Population.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(Population.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(Population.flag_id))).scalar()
