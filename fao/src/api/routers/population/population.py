...
@router.get("/", summary="Get population data")
async def get_population_data(
    ...
    """Get population data with advanced filtering and pagination.
...
@router.get("/aggregate", summary="Get aggregated population data")
async def get_population_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(Population))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in population")
@cache_result(prefix="population:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(Population.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(Population, Population.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'population')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in population")
@cache_result(prefix="population:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(Population, Population.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'population')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in population")
@cache_result(prefix="population:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(Population, Population.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'population')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in population")
@cache_result(prefix="population:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(Population, Flags.id == Population.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(Population.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(Population.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(Population)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in population")
@cache_result(prefix="population:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(Population)
        .group_by(Population.unit)
        .order_by(Population.unit)
    )
...
@router.get("/years", summary="Get available years in population")
@cache_result(prefix="population:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                Population.year,
                Population.year_code,
                func.count(Population.id).label('record_count')
            )
            .group_by(Population.year, Population.year_code)
            .order_by(Population.year_code)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of population dataset")
@cache_result(prefix="population:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(Population.area_code_id)))
        .select_from(Population)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/population/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(Population.item_code_id)))
        .select_from(Population)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/population/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(Population.element_code_id)))
        .select_from(Population)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/population/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(Population.flag_id)))
        .select_from(Population)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/population/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(Population.year).label('min_year'),
            func.max(Population.year).label('max_year'),
            func.count(func.distinct(Population.year)).label('year_count')
        )
        .select_from(Population)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/population/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(Population.value).label('min_value'),
            func.max(Population.value).label('max_value'),
            func.avg(Population.value).label('avg_value')
        )
        .select_from(Population)
        .where(and_(Population.value > 0, Population.value.is_not(None)))
    ).first()
    ...
    overview["endpoints"] = {
        "data": f"/population",
        "aggregate": f"/population/aggregate",
        "summary": f"/population/summary",
        "overview": f"/population/overview",
        "area_codes": f"/population/area_codes",
        "item_codes": f"/population/item_codes",
        "elements": f"/population/elements",
        "flags": f"/population/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the population endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(Population)).scalar()
        return {
            "status": "healthy",
            "dataset": "population",
            "records": result
