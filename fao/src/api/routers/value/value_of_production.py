...
from fao.src.db.pipelines.value_of_production.value_of_production_model import ValueOfProduction
...
from .value_of_production_config import ValueOfProductionConfig
...
router = APIRouter(
    prefix="/value_of_production",
    responses={404: {"description": "Not found"}},
)


config = ValueOfProductionConfig()

@router.get("/", summary="Get value of production data")
async def get_value_of_production_data(
    request: Request,
    response: Response,
    ...
    """Get value of production data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=ValueOfProduction, 
        model_name="ValueOfProduction",
        table_name="value_of_production",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated value of production data")
async def get_value_of_production_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(ValueOfProduction))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in value_of_production")
@cache_result(prefix="value_of_production:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(ValueOfProduction.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(ValueOfProduction, ValueOfProduction.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'value_of_production')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in value_of_production")
@cache_result(prefix="value_of_production:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(ValueOfProduction, ValueOfProduction.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'value_of_production')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in value_of_production")
@cache_result(prefix="value_of_production:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(ValueOfProduction, ValueOfProduction.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'value_of_production')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in value_of_production")
@cache_result(prefix="value_of_production:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(ValueOfProduction, Flags.id == ValueOfProduction.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(ValueOfProduction.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(ValueOfProduction)
                .where(ValueOfProduction.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(ValueOfProduction)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in value_of_production")
@cache_result(prefix="value_of_production:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            ValueOfProduction.unit,
            func.count(ValueOfProduction.id).label('record_count')
        )
        .select_from(ValueOfProduction)
        .group_by(ValueOfProduction.unit)
        .order_by(ValueOfProduction.unit)
    )
...
@router.get("/years", summary="Get available years in value_of_production")
@cache_result(prefix="value_of_production:years", ttl=604800)
    ...
    if include_counts:
        query = (
            select(
                ...
                ValueOfProduction.year_code,
                func.count(ValueOfProduction.id).label('record_count')
            )
            .group_by(ValueOfProduction.year, ValueOfProduction.year_code)
            .order_by(ValueOfProduction.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(ValueOfProduction.year)
            .distinct()
            .order_by(ValueOfProduction.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of value_of_production dataset")
@cache_result(prefix="value_of_production:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(ValueOfProduction.area_code_id)))
        .select_from(ValueOfProduction)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/value_of_production/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(ValueOfProduction.item_code_id)))
        .select_from(ValueOfProduction)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/value_of_production/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(ValueOfProduction.element_code_id)))
        .select_from(ValueOfProduction)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/value_of_production/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(ValueOfProduction.flag_id)))
        .select_from(ValueOfProduction)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/value_of_production/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(ValueOfProduction.year).label('min_year'),
            func.max(ValueOfProduction.year).label('max_year'),
            func.count(func.distinct(ValueOfProduction.year)).label('year_count')
        )
        .select_from(ValueOfProduction)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/value_of_production/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(ValueOfProduction.value).label('min_value'),
            func.max(ValueOfProduction.value).label('max_value'),
            func.avg(ValueOfProduction.value).label('avg_value')
        )
        .select_from(ValueOfProduction)
        .where(and_(ValueOfProduction.value > 0, ValueOfProduction.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/value_of_production",
        "aggregate": f"/value_of_production/aggregate",
        "summary": f"/value_of_production/summary",
        "overview": f"/value_of_production/overview",
        "area_codes": f"/value_of_production/area_codes",
        "item_codes": f"/value_of_production/item_codes",
        "elements": f"/value_of_production/elements",
        "flags": f"/value_of_production/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the value_of_production endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(ValueOfProduction)).scalar()
        return {
            "status": "healthy",
            "dataset": "value_of_production",
            "records": result
