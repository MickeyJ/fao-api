...
@router.get("/")
@cache_result(prefix="value_of_production", ttl=86400, exclude_params=["response", "db"])
def get_value_of_production(
    response: Response,
    ...
    """
    Get value of production data with filters and pagination.
    ...
    # Main table columns
    for col in ValueOfProduction.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(ValueOfProduction, 'id'):
                columns.append(ValueOfProduction.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(ValueOfProduction)
    
    # Add joins
    query = query.outerjoin(AreaCodes, ValueOfProduction.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, ValueOfProduction.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, ValueOfProduction.element_code_id == Elements.id)
    query = query.outerjoin(Flags, ValueOfProduction.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(ValueOfProduction.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(ValueOfProduction.year_code == year_code_exact)
    if year is not None:
        conditions.append(ValueOfProduction.year == year)
    if year_min is not None:
        conditions.append(ValueOfProduction.year >= year_min)
    if year_max is not None:
        conditions.append(ValueOfProduction.year <= year_max)
    if unit is not None:
        conditions.append(ValueOfProduction.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(ValueOfProduction.unit == unit_exact)
    if value is not None:
        conditions.append(ValueOfProduction.value == value)
    if value_min is not None:
        conditions.append(ValueOfProduction.value >= value_min)
    if value_max is not None:
        conditions.append(ValueOfProduction.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(ValueOfProduction, field_name):
                        order_by_clauses.append(getattr(ValueOfProduction, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(ValueOfProduction, sort_field):
                        order_by_clauses.append(getattr(ValueOfProduction, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(ValueOfProduction.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(ValueOfProduction)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, ValueOfProduction.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, ValueOfProduction.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, ValueOfProduction.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, ValueOfProduction.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_value_of_production'))
...
@router.get("/areas")
@cache_result(prefix="value_of_production:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="value_of_production:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(ValueOfProduction.year)
    ...
    if not years_data:
        return {"dataset": "value_of_production", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="value_of_production:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(ValueOfProduction.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(ValueOfProduction.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(ValueOfProduction.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(ValueOfProduction.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(ValueOfProduction.flag_id))).scalar()
