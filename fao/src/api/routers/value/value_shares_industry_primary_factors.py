...
from .value_shares_industry_primary_factors_config import ValueSharesIndustryPrimaryFactorsConfig
...
config = ValueSharesIndustryPrimaryFactorsConfig()

@router.get("/", summary="Get value shares industry primary factors data")
async def get_value_shares_industry_primary_factors_data(
    request: Request,
    response: Response,
    ...
    food_value_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by food_value_code code (comma-separated for multiple)"),
    food_value: Optional[str] = Query(None, description="Filter by food_value description (partial match)"),
    industry_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by industry_code code (comma-separated for multiple)"),
    industry: Optional[str] = Query(None, description="Filter by industry description (partial match)"),
    factor_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by factor_code code (comma-separated for multiple)"),
    factor: Optional[str] = Query(None, description="Filter by factor description (partial match)"),
    ...
    """Get value shares industry primary factors data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=ValueSharesIndustryPrimaryFactors, 
        model_name="ValueSharesIndustryPrimaryFactors",
    ...
    industry_code = router_handler.clean_param(industry_code, "multi")
    industry = router_handler.clean_param(industry, "like")
...
@router.get("/aggregate", summary="Get aggregated value shares industry primary factors data")
async def get_value_shares_industry_primary_factors_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(ValueSharesIndustryPrimaryFactors))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(ValueSharesIndustryPrimaryFactors, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(ValueSharesIndustryPrimaryFactors, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(ValueSharesIndustryPrimaryFactors, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in value_shares_industry_primary_factors")
@cache_result(prefix="value_shares_industry_primary_factors:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(ValueSharesIndustryPrimaryFactors, ValueSharesIndustryPrimaryFactors.area_code_id == AreaCodes.id)
...
@router.get("/elements", summary="Get elements in value_shares_industry_primary_factors")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(ValueSharesIndustryPrimaryFactors, ValueSharesIndustryPrimaryFactors.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in value_shares_industry_primary_factors")
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(ValueSharesIndustryPrimaryFactors)
                .where(ValueSharesIndustryPrimaryFactors.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(ValueSharesIndustryPrimaryFactors)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in value_shares_industry_primary_factors")
@cache_result(prefix="value_shares_industry_primary_factors:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            ValueSharesIndustryPrimaryFactors.unit,
            func.count(ValueSharesIndustryPrimaryFactors.id).label('record_count')
        )
        .select_from(ValueSharesIndustryPrimaryFactors)
        .group_by(ValueSharesIndustryPrimaryFactors.unit)
        .order_by(ValueSharesIndustryPrimaryFactors.unit)
    )
...
@router.get("/years", summary="Get available years in value_shares_industry_primary_factors")
    ...
    if include_counts:
        query = (
            select(
                ...
                ValueSharesIndustryPrimaryFactors.year_code,
                func.count(ValueSharesIndustryPrimaryFactors.id).label('record_count')
            )
            .group_by(ValueSharesIndustryPrimaryFactors.year, ValueSharesIndustryPrimaryFactors.year_code)
            .order_by(ValueSharesIndustryPrimaryFactors.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(ValueSharesIndustryPrimaryFactors.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of value_shares_industry_primary_factors dataset")
@cache_result(prefix="value_shares_industry_primary_factors:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(ValueSharesIndustryPrimaryFactors.area_code_id)))
        .select_from(ValueSharesIndustryPrimaryFactors)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/value_shares_industry_primary_factors/area_codes"
    }

    food_value_code_ids = db.execute(
        select(func.count(func.distinct(ValueSharesIndustryPrimaryFactors.food_value_code_id)))
        .select_from(ValueSharesIndustryPrimaryFactors)
    ).scalar() or 0

    overview["dimensions"]["food_values"] = {
        "count": food_value_code_ids,
        "endpoint": f"/value_shares_industry_primary_factors/food_values"
    }

    industry_code_ids = db.execute(
        select(func.count(func.distinct(ValueSharesIndustryPrimaryFactors.industry_code_id)))
        .select_from(ValueSharesIndustryPrimaryFactors)
    ).scalar() or 0

    overview["dimensions"]["industries"] = {
        "count": industry_code_ids,
        "endpoint": f"/value_shares_industry_primary_factors/industries"
    }

    factor_code_ids = db.execute(
        select(func.count(func.distinct(ValueSharesIndustryPrimaryFactors.factor_code_id)))
        .select_from(ValueSharesIndustryPrimaryFactors)
    ).scalar() or 0

    overview["dimensions"]["factors"] = {
        "count": factor_code_ids,
        "endpoint": f"/value_shares_industry_primary_factors/factors"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(ValueSharesIndustryPrimaryFactors.element_code_id)))
        .select_from(ValueSharesIndustryPrimaryFactors)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/value_shares_industry_primary_factors/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(ValueSharesIndustryPrimaryFactors.flag_id)))
        .select_from(ValueSharesIndustryPrimaryFactors)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/value_shares_industry_primary_factors/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(ValueSharesIndustryPrimaryFactors.year).label('min_year'),
            func.max(ValueSharesIndustryPrimaryFactors.year).label('max_year'),
            func.count(func.distinct(ValueSharesIndustryPrimaryFactors.year)).label('year_count')
        )
        .select_from(ValueSharesIndustryPrimaryFactors)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/value_shares_industry_primary_factors/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(ValueSharesIndustryPrimaryFactors.value).label('min_value'),
            func.max(ValueSharesIndustryPrimaryFactors.value).label('max_value'),
            func.avg(ValueSharesIndustryPrimaryFactors.value).label('avg_value')
        )
        .select_from(ValueSharesIndustryPrimaryFactors)
        .where(and_(ValueSharesIndustryPrimaryFactors.value > 0, ValueSharesIndustryPrimaryFactors.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/value_shares_industry_primary_factors",
        "aggregate": f"/value_shares_industry_primary_factors/aggregate",
        "summary": f"/value_shares_industry_primary_factors/summary",
        "overview": f"/value_shares_industry_primary_factors/overview",
        "area_codes": f"/value_shares_industry_primary_factors/area_codes",
        "elements": f"/value_shares_industry_primary_factors/elements",
        "flags": f"/value_shares_industry_primary_factors/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the value_shares_industry_primary_factors endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(ValueSharesIndustryPrimaryFactors)).scalar()
        return {
            "status": "healthy",
            "dataset": "value_shares_industry_primary_factors",
            "records": result
