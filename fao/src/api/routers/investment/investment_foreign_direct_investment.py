...
@router.get("/")
@cache_result(prefix="investment_foreign_direct_investment", ttl=86400, exclude_params=["response", "db"])
def get_investment_foreign_direct_investment(
    response: Response,
    ...
    """
    Get investment foreign direct investment data with filters and pagination.
    ...
    # Main table columns
    for col in InvestmentForeignDirectInvestment.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InvestmentForeignDirectInvestment, 'id'):
                columns.append(InvestmentForeignDirectInvestment.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InvestmentForeignDirectInvestment)
    
    # Add joins
    query = query.outerjoin(AreaCodes, InvestmentForeignDirectInvestment.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, InvestmentForeignDirectInvestment.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, InvestmentForeignDirectInvestment.element_code_id == Elements.id)
    query = query.outerjoin(Flags, InvestmentForeignDirectInvestment.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(InvestmentForeignDirectInvestment.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InvestmentForeignDirectInvestment.year_code == year_code_exact)
    if year is not None:
        conditions.append(InvestmentForeignDirectInvestment.year == year)
    if year_min is not None:
        conditions.append(InvestmentForeignDirectInvestment.year >= year_min)
    if year_max is not None:
        conditions.append(InvestmentForeignDirectInvestment.year <= year_max)
    if unit is not None:
        conditions.append(InvestmentForeignDirectInvestment.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InvestmentForeignDirectInvestment.unit == unit_exact)
    if value is not None:
        conditions.append(InvestmentForeignDirectInvestment.value == value)
    if value_min is not None:
        conditions.append(InvestmentForeignDirectInvestment.value >= value_min)
    if value_max is not None:
        conditions.append(InvestmentForeignDirectInvestment.value <= value_max)
    if note is not None:
        conditions.append(InvestmentForeignDirectInvestment.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(InvestmentForeignDirectInvestment.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InvestmentForeignDirectInvestment, field_name):
                        order_by_clauses.append(getattr(InvestmentForeignDirectInvestment, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InvestmentForeignDirectInvestment, sort_field):
                        order_by_clauses.append(getattr(InvestmentForeignDirectInvestment, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InvestmentForeignDirectInvestment.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InvestmentForeignDirectInvestment)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InvestmentForeignDirectInvestment.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InvestmentForeignDirectInvestment.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InvestmentForeignDirectInvestment.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InvestmentForeignDirectInvestment.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_investment_foreign_direct_investment'))
...
@router.get("/areas")
@cache_result(prefix="investment_foreign_direct_investment:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="investment_foreign_direct_investment:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InvestmentForeignDirectInvestment.year)
    ...
    if not years_data:
        return {"dataset": "investment_foreign_direct_investment", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="investment_foreign_direct_investment:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InvestmentForeignDirectInvestment.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InvestmentForeignDirectInvestment.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InvestmentForeignDirectInvestment.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InvestmentForeignDirectInvestment.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(InvestmentForeignDirectInvestment.flag_id))).scalar()
