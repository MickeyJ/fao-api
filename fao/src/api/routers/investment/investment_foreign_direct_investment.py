...
from .investment_foreign_direct_investment_config import InvestmentForeignDirectInvestmentConfig
...
router = APIRouter(
    prefix="/investment_foreign_direct_investment",
    responses={404: {"description": "Not found"}},
)


config = InvestmentForeignDirectInvestmentConfig()

@router.get("/", summary="Get investment foreign direct investment data")
async def get_investment_foreign_direct_investment_data(
    request: Request,
    response: Response,
    ...
    """Get investment foreign direct investment data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InvestmentForeignDirectInvestment, 
        model_name="InvestmentForeignDirectInvestment",
        table_name="investment_foreign_direct_investment",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated investment foreign direct investment data")
async def get_investment_foreign_direct_investment_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InvestmentForeignDirectInvestment))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InvestmentForeignDirectInvestment, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InvestmentForeignDirectInvestment, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InvestmentForeignDirectInvestment, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in investment_foreign_direct_investment")
@cache_result(prefix="investment_foreign_direct_investment:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InvestmentForeignDirectInvestment.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InvestmentForeignDirectInvestment, InvestmentForeignDirectInvestment.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'investment_foreign_direct_investment')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="investment_foreign_direct_investment",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in investment_foreign_direct_investment")
@cache_result(prefix="investment_foreign_direct_investment:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InvestmentForeignDirectInvestment, InvestmentForeignDirectInvestment.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'investment_foreign_direct_investment')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in investment_foreign_direct_investment")
@cache_result(prefix="investment_foreign_direct_investment:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InvestmentForeignDirectInvestment, InvestmentForeignDirectInvestment.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'investment_foreign_direct_investment')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in investment_foreign_direct_investment")
@cache_result(prefix="investment_foreign_direct_investment:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InvestmentForeignDirectInvestment, Flags.id == InvestmentForeignDirectInvestment.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InvestmentForeignDirectInvestment.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InvestmentForeignDirectInvestment)
                .where(InvestmentForeignDirectInvestment.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InvestmentForeignDirectInvestment)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in investment_foreign_direct_investment")
@cache_result(prefix="investment_foreign_direct_investment:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InvestmentForeignDirectInvestment.unit,
            func.count(InvestmentForeignDirectInvestment.id).label('record_count')
        )
        .select_from(InvestmentForeignDirectInvestment)
        .group_by(InvestmentForeignDirectInvestment.unit)
        .order_by(InvestmentForeignDirectInvestment.unit)
    )
...
@router.get("/years", summary="Get available years in investment_foreign_direct_investment")
@cache_result(prefix="investment_foreign_direct_investment:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InvestmentForeignDirectInvestment.year,
                InvestmentForeignDirectInvestment.year_code,
                func.count(InvestmentForeignDirectInvestment.id).label('record_count')
            )
            .group_by(InvestmentForeignDirectInvestment.year, InvestmentForeignDirectInvestment.year_code)
            .order_by(InvestmentForeignDirectInvestment.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InvestmentForeignDirectInvestment.year)
            .distinct()
            .order_by(InvestmentForeignDirectInvestment.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of investment_foreign_direct_investment dataset")
@cache_result(prefix="investment_foreign_direct_investment:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentForeignDirectInvestment.area_code_id)))
        .select_from(InvestmentForeignDirectInvestment)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/investment_foreign_direct_investment/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentForeignDirectInvestment.item_code_id)))
        .select_from(InvestmentForeignDirectInvestment)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/investment_foreign_direct_investment/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentForeignDirectInvestment.element_code_id)))
        .select_from(InvestmentForeignDirectInvestment)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/investment_foreign_direct_investment/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InvestmentForeignDirectInvestment.flag_id)))
        .select_from(InvestmentForeignDirectInvestment)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/investment_foreign_direct_investment/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InvestmentForeignDirectInvestment.year).label('min_year'),
            func.max(InvestmentForeignDirectInvestment.year).label('max_year'),
            func.count(func.distinct(InvestmentForeignDirectInvestment.year)).label('year_count')
        )
        .select_from(InvestmentForeignDirectInvestment)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/investment_foreign_direct_investment/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InvestmentForeignDirectInvestment.value).label('min_value'),
            func.max(InvestmentForeignDirectInvestment.value).label('max_value'),
            func.avg(InvestmentForeignDirectInvestment.value).label('avg_value')
        )
        .select_from(InvestmentForeignDirectInvestment)
        .where(and_(InvestmentForeignDirectInvestment.value > 0, InvestmentForeignDirectInvestment.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/investment_foreign_direct_investment",
        "aggregate": f"/investment_foreign_direct_investment/aggregate",
        "summary": f"/investment_foreign_direct_investment/summary",
        "overview": f"/investment_foreign_direct_investment/overview",
        "area_codes": f"/investment_foreign_direct_investment/area_codes",
        "item_codes": f"/investment_foreign_direct_investment/item_codes",
        "elements": f"/investment_foreign_direct_investment/elements",
        "flags": f"/investment_foreign_direct_investment/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the investment_foreign_direct_investment endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InvestmentForeignDirectInvestment)).scalar()
        return {
            "status": "healthy",
            "dataset": "investment_foreign_direct_investment",
            "records": result
