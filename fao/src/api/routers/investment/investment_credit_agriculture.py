...
from .investment_credit_agriculture_config import InvestmentCreditAgricultureConfig
...
config = InvestmentCreditAgricultureConfig()

@router.get("/", summary="Get investment credit agriculture data")
async def get_investment_credit_agriculture_data(
    request: Request,
    response: Response,
    ...
    """Get investment credit agriculture data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InvestmentCreditAgriculture, 
        model_name="InvestmentCreditAgriculture",
        table_name="investment_credit_agriculture",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated investment credit agriculture data")
async def get_investment_credit_agriculture_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InvestmentCreditAgriculture))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InvestmentCreditAgriculture, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InvestmentCreditAgriculture, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InvestmentCreditAgriculture, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in investment_credit_agriculture")
@cache_result(prefix="investment_credit_agriculture:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(InvestmentCreditAgriculture, InvestmentCreditAgriculture.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'investment_credit_agriculture')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in investment_credit_agriculture")
@cache_result(prefix="investment_credit_agriculture:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InvestmentCreditAgriculture, InvestmentCreditAgriculture.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'investment_credit_agriculture')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in investment_credit_agriculture")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(InvestmentCreditAgriculture, InvestmentCreditAgriculture.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'investment_credit_agriculture')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in investment_credit_agriculture")
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(InvestmentCreditAgriculture.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InvestmentCreditAgriculture)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in investment_credit_agriculture")
@cache_result(prefix="investment_credit_agriculture:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InvestmentCreditAgriculture.unit,
            func.count(InvestmentCreditAgriculture.id).label('record_count')
        )
        .select_from(InvestmentCreditAgriculture)
        .group_by(InvestmentCreditAgriculture.unit)
        .order_by(InvestmentCreditAgriculture.unit)
    )
...
@router.get("/years", summary="Get available years in investment_credit_agriculture")
    ...
    if include_counts:
        query = (
            select(
                ...
                InvestmentCreditAgriculture.year_code,
                func.count(InvestmentCreditAgriculture.id).label('record_count')
            )
            .group_by(InvestmentCreditAgriculture.year, InvestmentCreditAgriculture.year_code)
            .order_by(InvestmentCreditAgriculture.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InvestmentCreditAgriculture.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of investment_credit_agriculture dataset")
@cache_result(prefix="investment_credit_agriculture:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCreditAgriculture.area_code_id)))
        .select_from(InvestmentCreditAgriculture)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/investment_credit_agriculture/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCreditAgriculture.item_code_id)))
        .select_from(InvestmentCreditAgriculture)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/investment_credit_agriculture/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCreditAgriculture.element_code_id)))
        .select_from(InvestmentCreditAgriculture)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/investment_credit_agriculture/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InvestmentCreditAgriculture.flag_id)))
        .select_from(InvestmentCreditAgriculture)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/investment_credit_agriculture/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InvestmentCreditAgriculture.year).label('min_year'),
            func.max(InvestmentCreditAgriculture.year).label('max_year'),
            func.count(func.distinct(InvestmentCreditAgriculture.year)).label('year_count')
        )
        .select_from(InvestmentCreditAgriculture)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/investment_credit_agriculture/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InvestmentCreditAgriculture.value).label('min_value'),
            func.max(InvestmentCreditAgriculture.value).label('max_value'),
            func.avg(InvestmentCreditAgriculture.value).label('avg_value')
        )
        .select_from(InvestmentCreditAgriculture)
        .where(and_(InvestmentCreditAgriculture.value > 0, InvestmentCreditAgriculture.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/investment_credit_agriculture",
        "aggregate": f"/investment_credit_agriculture/aggregate",
        "summary": f"/investment_credit_agriculture/summary",
        "overview": f"/investment_credit_agriculture/overview",
        "area_codes": f"/investment_credit_agriculture/area_codes",
        "item_codes": f"/investment_credit_agriculture/item_codes",
        "elements": f"/investment_credit_agriculture/elements",
        "flags": f"/investment_credit_agriculture/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the investment_credit_agriculture endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InvestmentCreditAgriculture)).scalar()
        return {
            "status": "healthy",
            "dataset": "investment_credit_agriculture",
            "records": result
