...
@router.get("/")
@cache_result(prefix="investment_country_investment_statistics_profile", ttl=86400, exclude_params=["response", "db"])
def get_investment_country_investment_statistics_profile(
    ...
    """
    Get investment country investment statistics profile data with filters and pagination.
    ...
    # Main table columns
    for col in InvestmentCountryInvestmentStatisticsProfile.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InvestmentCountryInvestmentStatisticsProfile, 'id'):
                columns.append(InvestmentCountryInvestmentStatisticsProfile.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InvestmentCountryInvestmentStatisticsProfile)
    ...
    if year_code is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.year_code == year_code_exact)
    if year is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.year == year)
    if year_min is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.year >= year_min)
    if year_max is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.year <= year_max)
    if unit is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.unit == unit_exact)
    if value is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.value == value)
    if value_min is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.value >= value_min)
    if value_max is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.value <= value_max)
    if note is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(InvestmentCountryInvestmentStatisticsProfile.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InvestmentCountryInvestmentStatisticsProfile, field_name):
                        order_by_clauses.append(getattr(InvestmentCountryInvestmentStatisticsProfile, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InvestmentCountryInvestmentStatisticsProfile, sort_field):
                        order_by_clauses.append(getattr(InvestmentCountryInvestmentStatisticsProfile, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InvestmentCountryInvestmentStatisticsProfile.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InvestmentCountryInvestmentStatisticsProfile)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InvestmentCountryInvestmentStatisticsProfile.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InvestmentCountryInvestmentStatisticsProfile.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InvestmentCountryInvestmentStatisticsProfile.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InvestmentCountryInvestmentStatisticsProfile.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_investment_country_investment_statistics_profile'))
...
@router.get("/areas")
@cache_result(prefix="investment_country_investment_statistics_profile:areas", ttl=604800)
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(InvestmentCountryInvestmentStatisticsProfile, AreaCodes.id == InvestmentCountryInvestmentStatisticsProfile.area_code_id)
...
@router.get("/items")
@cache_result(prefix="investment_country_investment_statistics_profile:items", ttl=604800)
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(InvestmentCountryInvestmentStatisticsProfile, ItemCodes.id == InvestmentCountryInvestmentStatisticsProfile.item_code_id)
...
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(InvestmentCountryInvestmentStatisticsProfile, Elements.id == InvestmentCountryInvestmentStatisticsProfile.element_code_id)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InvestmentCountryInvestmentStatisticsProfile.year)
    ...
    if not years_data:
        return {"dataset": "investment_country_investment_statistics_profile", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="investment_country_investment_statistics_profile:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InvestmentCountryInvestmentStatisticsProfile.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InvestmentCountryInvestmentStatisticsProfile.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InvestmentCountryInvestmentStatisticsProfile.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InvestmentCountryInvestmentStatisticsProfile.element_code_id))).scalar()
