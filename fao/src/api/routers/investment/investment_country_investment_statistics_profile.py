...
from .investment_country_investment_statistics_profile_config import InvestmentCountryInvestmentStatisticsProfileConfig
...
config = InvestmentCountryInvestmentStatisticsProfileConfig()

@router.get("/", summary="Get investment country investment statistics profile data")
async def get_investment_country_investment_statistics_profile_data(
    request: Request,
    response: Response,
    ...
    """Get investment country investment statistics profile data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InvestmentCountryInvestmentStatisticsProfile, 
        model_name="InvestmentCountryInvestmentStatisticsProfile",
        table_name="investment_country_investment_statistics_profile",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated investment country investment statistics profile data")
async def get_investment_country_investment_statistics_profile_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InvestmentCountryInvestmentStatisticsProfile))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InvestmentCountryInvestmentStatisticsProfile, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InvestmentCountryInvestmentStatisticsProfile, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InvestmentCountryInvestmentStatisticsProfile, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in investment_country_investment_statistics_profile")
@cache_result(prefix="investment_country_investment_statistics_profile:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InvestmentCountryInvestmentStatisticsProfile.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InvestmentCountryInvestmentStatisticsProfile, InvestmentCountryInvestmentStatisticsProfile.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'investment_country_investment_statistics_profile')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="investment_country_investment_statistics_profile",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in investment_country_investment_statistics_profile")
@cache_result(prefix="investment_country_investment_statistics_profile:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InvestmentCountryInvestmentStatisticsProfile, InvestmentCountryInvestmentStatisticsProfile.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'investment_country_investment_statistics_profile')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in investment_country_investment_statistics_profile")
@cache_result(prefix="investment_country_investment_statistics_profile:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InvestmentCountryInvestmentStatisticsProfile, InvestmentCountryInvestmentStatisticsProfile.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'investment_country_investment_statistics_profile')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in investment_country_investment_statistics_profile")
@cache_result(prefix="investment_country_investment_statistics_profile:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InvestmentCountryInvestmentStatisticsProfile, Flags.id == InvestmentCountryInvestmentStatisticsProfile.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InvestmentCountryInvestmentStatisticsProfile.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InvestmentCountryInvestmentStatisticsProfile)
                .where(InvestmentCountryInvestmentStatisticsProfile.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InvestmentCountryInvestmentStatisticsProfile)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in investment_country_investment_statistics_profile")
@cache_result(prefix="investment_country_investment_statistics_profile:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InvestmentCountryInvestmentStatisticsProfile.unit,
            func.count(InvestmentCountryInvestmentStatisticsProfile.id).label('record_count')
        )
        .select_from(InvestmentCountryInvestmentStatisticsProfile)
        .group_by(InvestmentCountryInvestmentStatisticsProfile.unit)
        .order_by(InvestmentCountryInvestmentStatisticsProfile.unit)
    )
...
@router.get("/years", summary="Get available years in investment_country_investment_statistics_profile")
@cache_result(prefix="investment_country_investment_statistics_profile:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InvestmentCountryInvestmentStatisticsProfile.year,
                InvestmentCountryInvestmentStatisticsProfile.year_code,
                func.count(InvestmentCountryInvestmentStatisticsProfile.id).label('record_count')
            )
            .group_by(InvestmentCountryInvestmentStatisticsProfile.year, InvestmentCountryInvestmentStatisticsProfile.year_code)
            .order_by(InvestmentCountryInvestmentStatisticsProfile.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InvestmentCountryInvestmentStatisticsProfile.year)
            .distinct()
            .order_by(InvestmentCountryInvestmentStatisticsProfile.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of investment_country_investment_statistics_profile dataset")
@cache_result(prefix="investment_country_investment_statistics_profile:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCountryInvestmentStatisticsProfile.area_code_id)))
        .select_from(InvestmentCountryInvestmentStatisticsProfile)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/investment_country_investment_statistics_profile/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCountryInvestmentStatisticsProfile.item_code_id)))
        .select_from(InvestmentCountryInvestmentStatisticsProfile)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/investment_country_investment_statistics_profile/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCountryInvestmentStatisticsProfile.element_code_id)))
        .select_from(InvestmentCountryInvestmentStatisticsProfile)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/investment_country_investment_statistics_profile/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InvestmentCountryInvestmentStatisticsProfile.flag_id)))
        .select_from(InvestmentCountryInvestmentStatisticsProfile)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/investment_country_investment_statistics_profile/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InvestmentCountryInvestmentStatisticsProfile.year).label('min_year'),
            func.max(InvestmentCountryInvestmentStatisticsProfile.year).label('max_year'),
            func.count(func.distinct(InvestmentCountryInvestmentStatisticsProfile.year)).label('year_count')
        )
        .select_from(InvestmentCountryInvestmentStatisticsProfile)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/investment_country_investment_statistics_profile/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InvestmentCountryInvestmentStatisticsProfile.value).label('min_value'),
            func.max(InvestmentCountryInvestmentStatisticsProfile.value).label('max_value'),
            func.avg(InvestmentCountryInvestmentStatisticsProfile.value).label('avg_value')
        )
        .select_from(InvestmentCountryInvestmentStatisticsProfile)
        .where(and_(InvestmentCountryInvestmentStatisticsProfile.value > 0, InvestmentCountryInvestmentStatisticsProfile.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/investment_country_investment_statistics_profile",
        "aggregate": f"/investment_country_investment_statistics_profile/aggregate",
        "summary": f"/investment_country_investment_statistics_profile/summary",
        "overview": f"/investment_country_investment_statistics_profile/overview",
        "area_codes": f"/investment_country_investment_statistics_profile/area_codes",
        "item_codes": f"/investment_country_investment_statistics_profile/item_codes",
        "elements": f"/investment_country_investment_statistics_profile/elements",
        "flags": f"/investment_country_investment_statistics_profile/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the investment_country_investment_statistics_profile endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InvestmentCountryInvestmentStatisticsProfile)).scalar()
        return {
            "status": "healthy",
            "dataset": "investment_country_investment_statistics_profile",
            "records": result
