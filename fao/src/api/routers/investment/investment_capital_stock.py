...
@router.get("/")
@cache_result(prefix="investment_capital_stock", ttl=86400, exclude_params=["response", "db"])
def get_investment_capital_stock(
    response: Response,
    ...
    """
    Get investment capital stock data with filters and pagination.
    ...
    # Main table columns
    for col in InvestmentCapitalStock.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InvestmentCapitalStock, 'id'):
                columns.append(InvestmentCapitalStock.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InvestmentCapitalStock)
    
    # Add joins
    query = query.outerjoin(AreaCodes, InvestmentCapitalStock.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, InvestmentCapitalStock.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, InvestmentCapitalStock.element_code_id == Elements.id)
    query = query.outerjoin(Flags, InvestmentCapitalStock.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(InvestmentCapitalStock.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InvestmentCapitalStock.year_code == year_code_exact)
    if year is not None:
        conditions.append(InvestmentCapitalStock.year == year)
    if year_min is not None:
        conditions.append(InvestmentCapitalStock.year >= year_min)
    if year_max is not None:
        conditions.append(InvestmentCapitalStock.year <= year_max)
    if unit is not None:
        conditions.append(InvestmentCapitalStock.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InvestmentCapitalStock.unit == unit_exact)
    if value is not None:
        conditions.append(InvestmentCapitalStock.value == value)
    if value_min is not None:
        conditions.append(InvestmentCapitalStock.value >= value_min)
    if value_max is not None:
        conditions.append(InvestmentCapitalStock.value <= value_max)
    if note is not None:
        conditions.append(InvestmentCapitalStock.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(InvestmentCapitalStock.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InvestmentCapitalStock, field_name):
                        order_by_clauses.append(getattr(InvestmentCapitalStock, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InvestmentCapitalStock, sort_field):
                        order_by_clauses.append(getattr(InvestmentCapitalStock, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InvestmentCapitalStock.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InvestmentCapitalStock)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InvestmentCapitalStock.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InvestmentCapitalStock.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InvestmentCapitalStock.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InvestmentCapitalStock.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_investment_capital_stock'))
...
@router.get("/areas")
@cache_result(prefix="investment_capital_stock:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="investment_capital_stock:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InvestmentCapitalStock.year)
    ...
    if not years_data:
        return {"dataset": "investment_capital_stock", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="investment_capital_stock:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InvestmentCapitalStock.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InvestmentCapitalStock.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InvestmentCapitalStock.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InvestmentCapitalStock.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(InvestmentCapitalStock.flag_id))).scalar()
