...
from .investment_capital_stock_config import InvestmentCapitalStockConfig
...
router = APIRouter(
    prefix="/investment_capital_stock",
    responses={404: {"description": "Not found"}},
)


config = InvestmentCapitalStockConfig()

@router.get("/", summary="Get investment capital stock data")
async def get_investment_capital_stock_data(
    request: Request,
    response: Response,
    ...
    """Get investment capital stock data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InvestmentCapitalStock, 
        model_name="InvestmentCapitalStock",
        table_name="investment_capital_stock",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated investment capital stock data")
async def get_investment_capital_stock_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InvestmentCapitalStock))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InvestmentCapitalStock, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InvestmentCapitalStock, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InvestmentCapitalStock, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in investment_capital_stock")
@cache_result(prefix="investment_capital_stock:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InvestmentCapitalStock.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InvestmentCapitalStock, InvestmentCapitalStock.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'investment_capital_stock')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="investment_capital_stock",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in investment_capital_stock")
@cache_result(prefix="investment_capital_stock:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InvestmentCapitalStock, InvestmentCapitalStock.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'investment_capital_stock')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in investment_capital_stock")
@cache_result(prefix="investment_capital_stock:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InvestmentCapitalStock, InvestmentCapitalStock.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'investment_capital_stock')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in investment_capital_stock")
@cache_result(prefix="investment_capital_stock:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InvestmentCapitalStock, Flags.id == InvestmentCapitalStock.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InvestmentCapitalStock.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InvestmentCapitalStock)
                .where(InvestmentCapitalStock.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InvestmentCapitalStock)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in investment_capital_stock")
@cache_result(prefix="investment_capital_stock:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InvestmentCapitalStock.unit,
            func.count(InvestmentCapitalStock.id).label('record_count')
        )
        .select_from(InvestmentCapitalStock)
        .group_by(InvestmentCapitalStock.unit)
        .order_by(InvestmentCapitalStock.unit)
    )
...
@router.get("/years", summary="Get available years in investment_capital_stock")
@cache_result(prefix="investment_capital_stock:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InvestmentCapitalStock.year,
                InvestmentCapitalStock.year_code,
                func.count(InvestmentCapitalStock.id).label('record_count')
            )
            .group_by(InvestmentCapitalStock.year, InvestmentCapitalStock.year_code)
            .order_by(InvestmentCapitalStock.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InvestmentCapitalStock.year)
            .distinct()
            .order_by(InvestmentCapitalStock.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of investment_capital_stock dataset")
@cache_result(prefix="investment_capital_stock:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCapitalStock.area_code_id)))
        .select_from(InvestmentCapitalStock)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/investment_capital_stock/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCapitalStock.item_code_id)))
        .select_from(InvestmentCapitalStock)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/investment_capital_stock/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentCapitalStock.element_code_id)))
        .select_from(InvestmentCapitalStock)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/investment_capital_stock/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InvestmentCapitalStock.flag_id)))
        .select_from(InvestmentCapitalStock)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/investment_capital_stock/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InvestmentCapitalStock.year).label('min_year'),
            func.max(InvestmentCapitalStock.year).label('max_year'),
            func.count(func.distinct(InvestmentCapitalStock.year)).label('year_count')
        )
        .select_from(InvestmentCapitalStock)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/investment_capital_stock/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InvestmentCapitalStock.value).label('min_value'),
            func.max(InvestmentCapitalStock.value).label('max_value'),
            func.avg(InvestmentCapitalStock.value).label('avg_value')
        )
        .select_from(InvestmentCapitalStock)
        .where(and_(InvestmentCapitalStock.value > 0, InvestmentCapitalStock.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/investment_capital_stock",
        "aggregate": f"/investment_capital_stock/aggregate",
        "summary": f"/investment_capital_stock/summary",
        "overview": f"/investment_capital_stock/overview",
        "area_codes": f"/investment_capital_stock/area_codes",
        "item_codes": f"/investment_capital_stock/item_codes",
        "elements": f"/investment_capital_stock/elements",
        "flags": f"/investment_capital_stock/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the investment_capital_stock endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InvestmentCapitalStock)).scalar()
        return {
            "status": "healthy",
            "dataset": "investment_capital_stock",
            "records": result
