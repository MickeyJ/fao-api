...
@router.get("/")
@cache_result(prefix="investment_government_expenditure", ttl=86400, exclude_params=["response", "db"])
def get_investment_government_expenditure(
    ...
    """
    Get investment government expenditure data with filters and pagination.
    ...
    # Main table columns
    for col in InvestmentGovernmentExpenditure.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InvestmentGovernmentExpenditure, 'id'):
                columns.append(InvestmentGovernmentExpenditure.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InvestmentGovernmentExpenditure)
    ...
    if year_code is not None:
        conditions.append(InvestmentGovernmentExpenditure.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InvestmentGovernmentExpenditure.year_code == year_code_exact)
    if year is not None:
        conditions.append(InvestmentGovernmentExpenditure.year == year)
    if year_min is not None:
        conditions.append(InvestmentGovernmentExpenditure.year >= year_min)
    if year_max is not None:
        conditions.append(InvestmentGovernmentExpenditure.year <= year_max)
    if unit is not None:
        conditions.append(InvestmentGovernmentExpenditure.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InvestmentGovernmentExpenditure.unit == unit_exact)
    if value is not None:
        conditions.append(InvestmentGovernmentExpenditure.value == value)
    if value_min is not None:
        conditions.append(InvestmentGovernmentExpenditure.value >= value_min)
    if value_max is not None:
        conditions.append(InvestmentGovernmentExpenditure.value <= value_max)
    if note is not None:
        conditions.append(InvestmentGovernmentExpenditure.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(InvestmentGovernmentExpenditure.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InvestmentGovernmentExpenditure, field_name):
                        order_by_clauses.append(getattr(InvestmentGovernmentExpenditure, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InvestmentGovernmentExpenditure, sort_field):
                        order_by_clauses.append(getattr(InvestmentGovernmentExpenditure, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InvestmentGovernmentExpenditure.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InvestmentGovernmentExpenditure)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InvestmentGovernmentExpenditure.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InvestmentGovernmentExpenditure.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InvestmentGovernmentExpenditure.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InvestmentGovernmentExpenditure.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_investment_government_expenditure'))
...
@router.get("/areas")
@cache_result(prefix="investment_government_expenditure:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="investment_government_expenditure:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InvestmentGovernmentExpenditure.year)
    ...
    if not years_data:
        return {"dataset": "investment_government_expenditure", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="investment_government_expenditure:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InvestmentGovernmentExpenditure.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InvestmentGovernmentExpenditure.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InvestmentGovernmentExpenditure.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InvestmentGovernmentExpenditure.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(InvestmentGovernmentExpenditure.flag_id))).scalar()
