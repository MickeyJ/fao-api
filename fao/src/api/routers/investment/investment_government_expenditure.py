...
from .investment_government_expenditure_config import InvestmentGovernmentExpenditureConfig
...
config = InvestmentGovernmentExpenditureConfig()

@router.get("/", summary="Get investment government expenditure data")
async def get_investment_government_expenditure_data(
    request: Request,
    response: Response,
    ...
    """Get investment government expenditure data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InvestmentGovernmentExpenditure, 
        model_name="InvestmentGovernmentExpenditure",
        table_name="investment_government_expenditure",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated investment government expenditure data")
async def get_investment_government_expenditure_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InvestmentGovernmentExpenditure))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InvestmentGovernmentExpenditure, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InvestmentGovernmentExpenditure, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InvestmentGovernmentExpenditure, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in investment_government_expenditure")
@cache_result(prefix="investment_government_expenditure:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InvestmentGovernmentExpenditure.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InvestmentGovernmentExpenditure, InvestmentGovernmentExpenditure.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'investment_government_expenditure')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="investment_government_expenditure",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in investment_government_expenditure")
@cache_result(prefix="investment_government_expenditure:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InvestmentGovernmentExpenditure, InvestmentGovernmentExpenditure.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'investment_government_expenditure')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in investment_government_expenditure")
@cache_result(prefix="investment_government_expenditure:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InvestmentGovernmentExpenditure, InvestmentGovernmentExpenditure.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'investment_government_expenditure')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in investment_government_expenditure")
@cache_result(prefix="investment_government_expenditure:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InvestmentGovernmentExpenditure, Flags.id == InvestmentGovernmentExpenditure.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InvestmentGovernmentExpenditure.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InvestmentGovernmentExpenditure)
                .where(InvestmentGovernmentExpenditure.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InvestmentGovernmentExpenditure)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in investment_government_expenditure")
@cache_result(prefix="investment_government_expenditure:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InvestmentGovernmentExpenditure.unit,
            func.count(InvestmentGovernmentExpenditure.id).label('record_count')
        )
        .select_from(InvestmentGovernmentExpenditure)
        .group_by(InvestmentGovernmentExpenditure.unit)
        .order_by(InvestmentGovernmentExpenditure.unit)
    )
...
@router.get("/years", summary="Get available years in investment_government_expenditure")
@cache_result(prefix="investment_government_expenditure:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InvestmentGovernmentExpenditure.year,
                InvestmentGovernmentExpenditure.year_code,
                func.count(InvestmentGovernmentExpenditure.id).label('record_count')
            )
            .group_by(InvestmentGovernmentExpenditure.year, InvestmentGovernmentExpenditure.year_code)
            .order_by(InvestmentGovernmentExpenditure.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InvestmentGovernmentExpenditure.year)
            .distinct()
            .order_by(InvestmentGovernmentExpenditure.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of investment_government_expenditure dataset")
@cache_result(prefix="investment_government_expenditure:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentGovernmentExpenditure.area_code_id)))
        .select_from(InvestmentGovernmentExpenditure)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/investment_government_expenditure/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentGovernmentExpenditure.item_code_id)))
        .select_from(InvestmentGovernmentExpenditure)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/investment_government_expenditure/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentGovernmentExpenditure.element_code_id)))
        .select_from(InvestmentGovernmentExpenditure)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/investment_government_expenditure/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InvestmentGovernmentExpenditure.flag_id)))
        .select_from(InvestmentGovernmentExpenditure)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/investment_government_expenditure/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InvestmentGovernmentExpenditure.year).label('min_year'),
            func.max(InvestmentGovernmentExpenditure.year).label('max_year'),
            func.count(func.distinct(InvestmentGovernmentExpenditure.year)).label('year_count')
        )
        .select_from(InvestmentGovernmentExpenditure)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/investment_government_expenditure/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InvestmentGovernmentExpenditure.value).label('min_value'),
            func.max(InvestmentGovernmentExpenditure.value).label('max_value'),
            func.avg(InvestmentGovernmentExpenditure.value).label('avg_value')
        )
        .select_from(InvestmentGovernmentExpenditure)
        .where(and_(InvestmentGovernmentExpenditure.value > 0, InvestmentGovernmentExpenditure.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/investment_government_expenditure",
        "aggregate": f"/investment_government_expenditure/aggregate",
        "summary": f"/investment_government_expenditure/summary",
        "overview": f"/investment_government_expenditure/overview",
        "area_codes": f"/investment_government_expenditure/area_codes",
        "item_codes": f"/investment_government_expenditure/item_codes",
        "elements": f"/investment_government_expenditure/elements",
        "flags": f"/investment_government_expenditure/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the investment_government_expenditure endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InvestmentGovernmentExpenditure)).scalar()
        return {
            "status": "healthy",
            "dataset": "investment_government_expenditure",
            "records": result
