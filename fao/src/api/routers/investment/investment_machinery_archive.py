...
@router.get("/")
@cache_result(prefix="investment_machinery_archive", ttl=86400, exclude_params=["response", "db"])
def get_investment_machinery_archive(
    response: Response,
    ...
    """
    Get investment machinery archive data with filters and pagination.
    ...
    # Main table columns
    for col in InvestmentMachineryArchive.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InvestmentMachineryArchive, 'id'):
                columns.append(InvestmentMachineryArchive.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InvestmentMachineryArchive)
    
    # Add joins
    query = query.outerjoin(AreaCodes, InvestmentMachineryArchive.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, InvestmentMachineryArchive.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, InvestmentMachineryArchive.element_code_id == Elements.id)
    query = query.outerjoin(Flags, InvestmentMachineryArchive.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(InvestmentMachineryArchive.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InvestmentMachineryArchive.year_code == year_code_exact)
    if year is not None:
        conditions.append(InvestmentMachineryArchive.year == year)
    if year_min is not None:
        conditions.append(InvestmentMachineryArchive.year >= year_min)
    if year_max is not None:
        conditions.append(InvestmentMachineryArchive.year <= year_max)
    if unit is not None:
        conditions.append(InvestmentMachineryArchive.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InvestmentMachineryArchive.unit == unit_exact)
    if value is not None:
        conditions.append(InvestmentMachineryArchive.value == value)
    if value_min is not None:
        conditions.append(InvestmentMachineryArchive.value >= value_min)
    if value_max is not None:
        conditions.append(InvestmentMachineryArchive.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InvestmentMachineryArchive, field_name):
                        order_by_clauses.append(getattr(InvestmentMachineryArchive, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InvestmentMachineryArchive, sort_field):
                        order_by_clauses.append(getattr(InvestmentMachineryArchive, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InvestmentMachineryArchive.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InvestmentMachineryArchive)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InvestmentMachineryArchive.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InvestmentMachineryArchive.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InvestmentMachineryArchive.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InvestmentMachineryArchive.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_investment_machinery_archive'))
...
@router.get("/areas")
@cache_result(prefix="investment_machinery_archive:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="investment_machinery_archive:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InvestmentMachineryArchive.year)
    ...
    if not years_data:
        return {"dataset": "investment_machinery_archive", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="investment_machinery_archive:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InvestmentMachineryArchive.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InvestmentMachineryArchive.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InvestmentMachineryArchive.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InvestmentMachineryArchive.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(InvestmentMachineryArchive.flag_id))).scalar()
