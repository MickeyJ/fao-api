...
from .investment_machinery_archive_config import InvestmentMachineryArchiveConfig
...
router = APIRouter(
    prefix="/investment_machinery_archive",
    responses={404: {"description": "Not found"}},
)


config = InvestmentMachineryArchiveConfig()

@router.get("/", summary="Get investment machinery archive data")
async def get_investment_machinery_archive_data(
    request: Request,
    response: Response,
    ...
    """Get investment machinery archive data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InvestmentMachineryArchive, 
        model_name="InvestmentMachineryArchive",
        table_name="investment_machinery_archive",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated investment machinery archive data")
async def get_investment_machinery_archive_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InvestmentMachineryArchive))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InvestmentMachineryArchive, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InvestmentMachineryArchive, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InvestmentMachineryArchive, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in investment_machinery_archive")
@cache_result(prefix="investment_machinery_archive:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InvestmentMachineryArchive.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InvestmentMachineryArchive, InvestmentMachineryArchive.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'investment_machinery_archive')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="investment_machinery_archive",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in investment_machinery_archive")
@cache_result(prefix="investment_machinery_archive:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InvestmentMachineryArchive, InvestmentMachineryArchive.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'investment_machinery_archive')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in investment_machinery_archive")
@cache_result(prefix="investment_machinery_archive:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InvestmentMachineryArchive, InvestmentMachineryArchive.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'investment_machinery_archive')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in investment_machinery_archive")
@cache_result(prefix="investment_machinery_archive:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InvestmentMachineryArchive, Flags.id == InvestmentMachineryArchive.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InvestmentMachineryArchive.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InvestmentMachineryArchive)
                .where(InvestmentMachineryArchive.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InvestmentMachineryArchive)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in investment_machinery_archive")
@cache_result(prefix="investment_machinery_archive:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InvestmentMachineryArchive.unit,
            func.count(InvestmentMachineryArchive.id).label('record_count')
        )
        .select_from(InvestmentMachineryArchive)
        .group_by(InvestmentMachineryArchive.unit)
        .order_by(InvestmentMachineryArchive.unit)
    )
...
@router.get("/years", summary="Get available years in investment_machinery_archive")
@cache_result(prefix="investment_machinery_archive:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InvestmentMachineryArchive.year,
                InvestmentMachineryArchive.year_code,
                func.count(InvestmentMachineryArchive.id).label('record_count')
            )
            .group_by(InvestmentMachineryArchive.year, InvestmentMachineryArchive.year_code)
            .order_by(InvestmentMachineryArchive.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InvestmentMachineryArchive.year)
            .distinct()
            .order_by(InvestmentMachineryArchive.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of investment_machinery_archive dataset")
@cache_result(prefix="investment_machinery_archive:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentMachineryArchive.area_code_id)))
        .select_from(InvestmentMachineryArchive)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/investment_machinery_archive/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentMachineryArchive.item_code_id)))
        .select_from(InvestmentMachineryArchive)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/investment_machinery_archive/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentMachineryArchive.element_code_id)))
        .select_from(InvestmentMachineryArchive)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/investment_machinery_archive/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InvestmentMachineryArchive.flag_id)))
        .select_from(InvestmentMachineryArchive)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/investment_machinery_archive/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InvestmentMachineryArchive.year).label('min_year'),
            func.max(InvestmentMachineryArchive.year).label('max_year'),
            func.count(func.distinct(InvestmentMachineryArchive.year)).label('year_count')
        )
        .select_from(InvestmentMachineryArchive)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/investment_machinery_archive/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InvestmentMachineryArchive.value).label('min_value'),
            func.max(InvestmentMachineryArchive.value).label('max_value'),
            func.avg(InvestmentMachineryArchive.value).label('avg_value')
        )
        .select_from(InvestmentMachineryArchive)
        .where(and_(InvestmentMachineryArchive.value > 0, InvestmentMachineryArchive.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/investment_machinery_archive",
        "aggregate": f"/investment_machinery_archive/aggregate",
        "summary": f"/investment_machinery_archive/summary",
        "overview": f"/investment_machinery_archive/overview",
        "area_codes": f"/investment_machinery_archive/area_codes",
        "item_codes": f"/investment_machinery_archive/item_codes",
        "elements": f"/investment_machinery_archive/elements",
        "flags": f"/investment_machinery_archive/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the investment_machinery_archive endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InvestmentMachineryArchive)).scalar()
        return {
            "status": "healthy",
            "dataset": "investment_machinery_archive",
            "records": result
