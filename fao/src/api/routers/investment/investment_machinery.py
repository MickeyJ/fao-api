...
@router.get("/")
@cache_result(prefix="investment_machinery", ttl=86400, exclude_params=["response", "db"])
def get_investment_machinery(
    ...
    """
    Get investment machinery data with filters and pagination.
    ...
    # Main table columns
    for col in InvestmentMachinery.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InvestmentMachinery, 'id'):
                columns.append(InvestmentMachinery.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InvestmentMachinery)
    ...
    if year_code is not None:
        conditions.append(InvestmentMachinery.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InvestmentMachinery.year_code == year_code_exact)
    if year is not None:
        conditions.append(InvestmentMachinery.year == year)
    if year_min is not None:
        conditions.append(InvestmentMachinery.year >= year_min)
    if year_max is not None:
        conditions.append(InvestmentMachinery.year <= year_max)
    if unit is not None:
        conditions.append(InvestmentMachinery.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InvestmentMachinery.unit == unit_exact)
    if value is not None:
        conditions.append(InvestmentMachinery.value == value)
    if value_min is not None:
        conditions.append(InvestmentMachinery.value >= value_min)
    if value_max is not None:
        conditions.append(InvestmentMachinery.value <= value_max)
    if note is not None:
        conditions.append(InvestmentMachinery.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(InvestmentMachinery.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InvestmentMachinery, field_name):
                        order_by_clauses.append(getattr(InvestmentMachinery, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InvestmentMachinery, sort_field):
                        order_by_clauses.append(getattr(InvestmentMachinery, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InvestmentMachinery.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InvestmentMachinery)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InvestmentMachinery.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InvestmentMachinery.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InvestmentMachinery.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InvestmentMachinery.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_investment_machinery'))
...
@router.get("/areas")
@cache_result(prefix="investment_machinery:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="investment_machinery:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InvestmentMachinery.year)
    ...
    if not years_data:
        return {"dataset": "investment_machinery", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="investment_machinery:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InvestmentMachinery.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InvestmentMachinery.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InvestmentMachinery.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InvestmentMachinery.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(InvestmentMachinery.flag_id))).scalar()
