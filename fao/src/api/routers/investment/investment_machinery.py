...
from .investment_machinery_config import InvestmentMachineryConfig
...
config = InvestmentMachineryConfig()

@router.get("/", summary="Get investment machinery data")
async def get_investment_machinery_data(
    request: Request,
    response: Response,
    ...
    """Get investment machinery data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InvestmentMachinery, 
        model_name="InvestmentMachinery",
        table_name="investment_machinery",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated investment machinery data")
async def get_investment_machinery_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InvestmentMachinery))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in investment_machinery")
@cache_result(prefix="investment_machinery:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InvestmentMachinery.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InvestmentMachinery, InvestmentMachinery.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'investment_machinery')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in investment_machinery")
@cache_result(prefix="investment_machinery:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InvestmentMachinery, InvestmentMachinery.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'investment_machinery')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in investment_machinery")
@cache_result(prefix="investment_machinery:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InvestmentMachinery, InvestmentMachinery.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'investment_machinery')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in investment_machinery")
@cache_result(prefix="investment_machinery:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InvestmentMachinery, Flags.id == InvestmentMachinery.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InvestmentMachinery.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InvestmentMachinery)
                .where(InvestmentMachinery.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InvestmentMachinery)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in investment_machinery")
@cache_result(prefix="investment_machinery:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InvestmentMachinery.unit,
            func.count(InvestmentMachinery.id).label('record_count')
        )
        .select_from(InvestmentMachinery)
        .group_by(InvestmentMachinery.unit)
        .order_by(InvestmentMachinery.unit)
    )
...
@router.get("/years", summary="Get available years in investment_machinery")
@cache_result(prefix="investment_machinery:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InvestmentMachinery.year,
                InvestmentMachinery.year_code,
                func.count(InvestmentMachinery.id).label('record_count')
            )
            .group_by(InvestmentMachinery.year, InvestmentMachinery.year_code)
            .order_by(InvestmentMachinery.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InvestmentMachinery.year)
            .distinct()
            .order_by(InvestmentMachinery.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of investment_machinery dataset")
@cache_result(prefix="investment_machinery:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentMachinery.area_code_id)))
        .select_from(InvestmentMachinery)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/investment_machinery/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentMachinery.item_code_id)))
        .select_from(InvestmentMachinery)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/investment_machinery/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InvestmentMachinery.element_code_id)))
        .select_from(InvestmentMachinery)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/investment_machinery/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InvestmentMachinery.flag_id)))
        .select_from(InvestmentMachinery)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/investment_machinery/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InvestmentMachinery.year).label('min_year'),
            func.max(InvestmentMachinery.year).label('max_year'),
            func.count(func.distinct(InvestmentMachinery.year)).label('year_count')
        )
        .select_from(InvestmentMachinery)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/investment_machinery/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InvestmentMachinery.value).label('min_value'),
            func.max(InvestmentMachinery.value).label('max_value'),
            func.avg(InvestmentMachinery.value).label('avg_value')
        )
        .select_from(InvestmentMachinery)
        .where(and_(InvestmentMachinery.value > 0, InvestmentMachinery.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/investment_machinery",
        "aggregate": f"/investment_machinery/aggregate",
        "summary": f"/investment_machinery/summary",
        "overview": f"/investment_machinery/overview",
        "area_codes": f"/investment_machinery/area_codes",
        "item_codes": f"/investment_machinery/item_codes",
        "elements": f"/investment_machinery/elements",
        "flags": f"/investment_machinery/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the investment_machinery endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InvestmentMachinery)).scalar()
        return {
            "status": "healthy",
            "dataset": "investment_machinery",
            "records": result
