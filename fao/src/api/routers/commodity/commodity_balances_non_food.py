...
router = APIRouter(
    prefix="/commodity_balances_non_food",
    tags=["commodity_balances_non_food"],
    responses={404: {"description": "Not found"}},
)

@router.get("/")
def get_commodity_balances_non_food(
    ...
    """
    commodity balances non food data with filters.
    Filter options:
    ...
    query = (
        ...
        .select_from(CommodityBalancesNonFood)
        .outerjoin(AreaCodes, CommodityBalancesNonFood.area_code_id == AreaCodes.id)
        .outerjoin(ItemCodes, CommodityBalancesNonFood.item_code_id == ItemCodes.id)
        .outerjoin(Elements, CommodityBalancesNonFood.element_code_id == Elements.id)
        .outerjoin(Flags, CommodityBalancesNonFood.flag_id == Flags.id)
    )
...
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            AreaCodes.area_code,
            AreaCodes.area,
            func.count(CommodityBalancesNonFood.id).label('record_count')
        )
        .join(CommodityBalancesNonFood, AreaCodes.id == CommodityBalancesNonFood.area_code_id)
        .group_by(AreaCodes.area_code, AreaCodes.area)
        .order_by(func.count(CommodityBalancesNonFood.id).desc())
    )
    
    results = db.execute(query).all()
    
    return {
        "dataset": "commodity_balances_non_food",
...
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood, ItemCodes.id == CommodityBalancesNonFood.item_code_id)
...
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood, Elements.id == CommodityBalancesNonFood.element_code_id)
...
def get_data_quality_summary(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood, Flags.id == CommodityBalancesNonFood.flag_id)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            CommodityBalancesNonFood.year,
            func.count(CommodityBalancesNonFood.id).label('record_count')
        )
        .group_by(CommodityBalancesNonFood.year)
        .order_by(CommodityBalancesNonFood.year)
    )
    ...
    if not years_data:
        return {"dataset": "commodity_balances_non_food", "message": "No temporal data available"}
...
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    total_records = db.query(func.count(CommodityBalancesNonFood.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(CommodityBalancesNonFood.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(CommodityBalancesNonFood.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(CommodityBalancesNonFood.element_code_id))).scalar()
