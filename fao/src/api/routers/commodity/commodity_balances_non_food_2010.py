...
@router.get("/")
@cache_result(prefix="commodity_balances_non_food_2010", ttl=86400, exclude_params=["response", "db"])
def get_commodity_balances_non_food_2010(
    ...
    """
    Get commodity balances non food 2010 data with filters and pagination.
    ...
    # Main table columns
    for col in CommodityBalancesNonFood2010.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(CommodityBalancesNonFood2010, 'id'):
                columns.append(CommodityBalancesNonFood2010.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(CommodityBalancesNonFood2010)
    ...
    if year_code is not None:
        conditions.append(CommodityBalancesNonFood2010.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(CommodityBalancesNonFood2010.year_code == year_code_exact)
    if year is not None:
        conditions.append(CommodityBalancesNonFood2010.year == year)
    if year_min is not None:
        conditions.append(CommodityBalancesNonFood2010.year >= year_min)
    if year_max is not None:
        conditions.append(CommodityBalancesNonFood2010.year <= year_max)
    if unit is not None:
        conditions.append(CommodityBalancesNonFood2010.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(CommodityBalancesNonFood2010.unit == unit_exact)
    if value is not None:
        conditions.append(CommodityBalancesNonFood2010.value == value)
    if value_min is not None:
        conditions.append(CommodityBalancesNonFood2010.value >= value_min)
    if value_max is not None:
        conditions.append(CommodityBalancesNonFood2010.value <= value_max)
    if note is not None:
        conditions.append(CommodityBalancesNonFood2010.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(CommodityBalancesNonFood2010.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(CommodityBalancesNonFood2010, field_name):
                        order_by_clauses.append(getattr(CommodityBalancesNonFood2010, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(CommodityBalancesNonFood2010, sort_field):
                        order_by_clauses.append(getattr(CommodityBalancesNonFood2010, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(CommodityBalancesNonFood2010.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(CommodityBalancesNonFood2010)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, CommodityBalancesNonFood2010.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, CommodityBalancesNonFood2010.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, CommodityBalancesNonFood2010.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, CommodityBalancesNonFood2010.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_commodity_balances_non_food_2010'))
...
@router.get("/areas")
@cache_result(prefix="commodity_balances_non_food_2010:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="commodity_balances_non_food_2010:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(CommodityBalancesNonFood2010.year)
    ...
    if not years_data:
        return {"dataset": "commodity_balances_non_food_2010", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="commodity_balances_non_food_2010:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(CommodityBalancesNonFood2010.id)).scalar()
