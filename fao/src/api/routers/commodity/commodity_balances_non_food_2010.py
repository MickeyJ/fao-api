...
from .commodity_balances_non_food_2010_config import CommodityBalancesNonFood2010Config
...
config = CommodityBalancesNonFood2010Config()

@router.get("/", summary="Get commodity balances non food 2010 data")
async def get_commodity_balances_non_food_2010_data(
    request: Request,
    response: Response,
    ...
    """Get commodity balances non food 2010 data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=CommodityBalancesNonFood2010, 
        model_name="CommodityBalancesNonFood2010",
        table_name="commodity_balances_non_food_2010",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated commodity balances non food 2010 data")
async def get_commodity_balances_non_food_2010_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(CommodityBalancesNonFood2010))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(CommodityBalancesNonFood2010, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(CommodityBalancesNonFood2010, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(CommodityBalancesNonFood2010, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in commodity_balances_non_food_2010")
@cache_result(prefix="commodity_balances_non_food_2010:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood2010, CommodityBalancesNonFood2010.area_code_id == AreaCodes.id)
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="commodity_balances_non_food_2010",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in commodity_balances_non_food_2010")
@cache_result(prefix="commodity_balances_non_food_2010:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood2010, CommodityBalancesNonFood2010.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in commodity_balances_non_food_2010")
@cache_result(prefix="commodity_balances_non_food_2010:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood2010, CommodityBalancesNonFood2010.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in commodity_balances_non_food_2010")
@cache_result(prefix="commodity_balances_non_food_2010:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood2010, Flags.id == CommodityBalancesNonFood2010.flag_id)
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(CommodityBalancesNonFood2010)
                .where(CommodityBalancesNonFood2010.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(CommodityBalancesNonFood2010)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in commodity_balances_non_food_2010")
@cache_result(prefix="commodity_balances_non_food_2010:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            CommodityBalancesNonFood2010.unit,
            func.count(CommodityBalancesNonFood2010.id).label('record_count')
        )
        .select_from(CommodityBalancesNonFood2010)
        .group_by(CommodityBalancesNonFood2010.unit)
        .order_by(CommodityBalancesNonFood2010.unit)
    )
...
@router.get("/years", summary="Get available years in commodity_balances_non_food_2010")
...
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                CommodityBalancesNonFood2010.year,
                CommodityBalancesNonFood2010.year_code,
                func.count(CommodityBalancesNonFood2010.id).label('record_count')
            )
            .group_by(CommodityBalancesNonFood2010.year, CommodityBalancesNonFood2010.year_code)
            .order_by(CommodityBalancesNonFood2010.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(CommodityBalancesNonFood2010.year)
            .distinct()
            .order_by(CommodityBalancesNonFood2010.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of commodity_balances_non_food_2010 dataset")
@cache_result(prefix="commodity_balances_non_food_2010:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(CommodityBalancesNonFood2010.area_code_id)))
        .select_from(CommodityBalancesNonFood2010)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/commodity_balances_non_food_2010/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(CommodityBalancesNonFood2010.item_code_id)))
        .select_from(CommodityBalancesNonFood2010)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/commodity_balances_non_food_2010/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(CommodityBalancesNonFood2010.element_code_id)))
        .select_from(CommodityBalancesNonFood2010)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/commodity_balances_non_food_2010/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(CommodityBalancesNonFood2010.flag_id)))
        .select_from(CommodityBalancesNonFood2010)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/commodity_balances_non_food_2010/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(CommodityBalancesNonFood2010.year).label('min_year'),
            func.max(CommodityBalancesNonFood2010.year).label('max_year'),
            func.count(func.distinct(CommodityBalancesNonFood2010.year)).label('year_count')
        )
        .select_from(CommodityBalancesNonFood2010)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/commodity_balances_non_food_2010/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(CommodityBalancesNonFood2010.value).label('min_value'),
            func.max(CommodityBalancesNonFood2010.value).label('max_value'),
            func.avg(CommodityBalancesNonFood2010.value).label('avg_value')
        )
        .select_from(CommodityBalancesNonFood2010)
        .where(and_(CommodityBalancesNonFood2010.value > 0, CommodityBalancesNonFood2010.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/commodity_balances_non_food_2010",
        "aggregate": f"/commodity_balances_non_food_2010/aggregate",
        "summary": f"/commodity_balances_non_food_2010/summary",
        "overview": f"/commodity_balances_non_food_2010/overview",
        "area_codes": f"/commodity_balances_non_food_2010/area_codes",
        "item_codes": f"/commodity_balances_non_food_2010/item_codes",
        "elements": f"/commodity_balances_non_food_2010/elements",
        "flags": f"/commodity_balances_non_food_2010/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the commodity_balances_non_food_2010 endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(CommodityBalancesNonFood2010)).scalar()
        return {
            "status": "healthy",
            "dataset": "commodity_balances_non_food_2010",
            "records": result
