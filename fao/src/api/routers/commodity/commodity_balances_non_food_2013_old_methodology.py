...
from .commodity_balances_non_food_2013_old_methodology_config import CommodityBalancesNonFood2013OldMethodologyConfig
...
router = APIRouter(
    prefix="/commodity_balances_non_food_2013_old_methodology",
    responses={404: {"description": "Not found"}},
)


config = CommodityBalancesNonFood2013OldMethodologyConfig()

@router.get("/", summary="Get commodity balances non food 2013 old methodology data")
async def get_commodity_balances_non_food_2013_old_methodology_data(
    request: Request,
    response: Response,
    ...
    """Get commodity balances non food 2013 old methodology data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=CommodityBalancesNonFood2013OldMethodology, 
        model_name="CommodityBalancesNonFood2013OldMethodology",
...
@router.get("/aggregate", summary="Get aggregated commodity balances non food 2013 old methodology data")
async def get_commodity_balances_non_food_2013_old_methodology_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(CommodityBalancesNonFood2013OldMethodology))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(CommodityBalancesNonFood2013OldMethodology, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(CommodityBalancesNonFood2013OldMethodology, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(CommodityBalancesNonFood2013OldMethodology, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in commodity_balances_non_food_2013_old_methodology")
@cache_result(prefix="commodity_balances_non_food_2013_old_methodology:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(CommodityBalancesNonFood2013OldMethodology.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(CommodityBalancesNonFood2013OldMethodology, CommodityBalancesNonFood2013OldMethodology.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'commodity_balances_non_food_2013_old_methodology')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="commodity_balances_non_food_2013_old_methodology",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in commodity_balances_non_food_2013_old_methodology")
@cache_result(prefix="commodity_balances_non_food_2013_old_methodology:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood2013OldMethodology, CommodityBalancesNonFood2013OldMethodology.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'commodity_balances_non_food_2013_old_methodology')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in commodity_balances_non_food_2013_old_methodology")
@cache_result(prefix="commodity_balances_non_food_2013_old_methodology:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood2013OldMethodology, CommodityBalancesNonFood2013OldMethodology.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'commodity_balances_non_food_2013_old_methodology')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in commodity_balances_non_food_2013_old_methodology")
@cache_result(prefix="commodity_balances_non_food_2013_old_methodology:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(CommodityBalancesNonFood2013OldMethodology, Flags.id == CommodityBalancesNonFood2013OldMethodology.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(CommodityBalancesNonFood2013OldMethodology.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(CommodityBalancesNonFood2013OldMethodology)
                .where(CommodityBalancesNonFood2013OldMethodology.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(CommodityBalancesNonFood2013OldMethodology)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in commodity_balances_non_food_2013_old_methodology")
@cache_result(prefix="commodity_balances_non_food_2013_old_methodology:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            CommodityBalancesNonFood2013OldMethodology.unit,
            func.count(CommodityBalancesNonFood2013OldMethodology.id).label('record_count')
        )
        .select_from(CommodityBalancesNonFood2013OldMethodology)
        .group_by(CommodityBalancesNonFood2013OldMethodology.unit)
        .order_by(CommodityBalancesNonFood2013OldMethodology.unit)
    )
...
@router.get("/years", summary="Get available years in commodity_balances_non_food_2013_old_methodology")
@cache_result(prefix="commodity_balances_non_food_2013_old_methodology:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                CommodityBalancesNonFood2013OldMethodology.year,
                CommodityBalancesNonFood2013OldMethodology.year_code,
                func.count(CommodityBalancesNonFood2013OldMethodology.id).label('record_count')
            )
            .group_by(CommodityBalancesNonFood2013OldMethodology.year, CommodityBalancesNonFood2013OldMethodology.year_code)
            .order_by(CommodityBalancesNonFood2013OldMethodology.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(CommodityBalancesNonFood2013OldMethodology.year)
            .distinct()
            .order_by(CommodityBalancesNonFood2013OldMethodology.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of commodity_balances_non_food_2013_old_methodology dataset")
@cache_result(prefix="commodity_balances_non_food_2013_old_methodology:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(CommodityBalancesNonFood2013OldMethodology.area_code_id)))
        .select_from(CommodityBalancesNonFood2013OldMethodology)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/commodity_balances_non_food_2013_old_methodology/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(CommodityBalancesNonFood2013OldMethodology.item_code_id)))
        .select_from(CommodityBalancesNonFood2013OldMethodology)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/commodity_balances_non_food_2013_old_methodology/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(CommodityBalancesNonFood2013OldMethodology.element_code_id)))
        .select_from(CommodityBalancesNonFood2013OldMethodology)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/commodity_balances_non_food_2013_old_methodology/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(CommodityBalancesNonFood2013OldMethodology.flag_id)))
        .select_from(CommodityBalancesNonFood2013OldMethodology)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/commodity_balances_non_food_2013_old_methodology/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(CommodityBalancesNonFood2013OldMethodology.year).label('min_year'),
            func.max(CommodityBalancesNonFood2013OldMethodology.year).label('max_year'),
            func.count(func.distinct(CommodityBalancesNonFood2013OldMethodology.year)).label('year_count')
        )
        .select_from(CommodityBalancesNonFood2013OldMethodology)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/commodity_balances_non_food_2013_old_methodology/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(CommodityBalancesNonFood2013OldMethodology.value).label('min_value'),
            func.max(CommodityBalancesNonFood2013OldMethodology.value).label('max_value'),
            func.avg(CommodityBalancesNonFood2013OldMethodology.value).label('avg_value')
        )
        .select_from(CommodityBalancesNonFood2013OldMethodology)
        .where(and_(CommodityBalancesNonFood2013OldMethodology.value > 0, CommodityBalancesNonFood2013OldMethodology.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/commodity_balances_non_food_2013_old_methodology",
        "aggregate": f"/commodity_balances_non_food_2013_old_methodology/aggregate",
        "summary": f"/commodity_balances_non_food_2013_old_methodology/summary",
        "overview": f"/commodity_balances_non_food_2013_old_methodology/overview",
        "area_codes": f"/commodity_balances_non_food_2013_old_methodology/area_codes",
        "item_codes": f"/commodity_balances_non_food_2013_old_methodology/item_codes",
        "elements": f"/commodity_balances_non_food_2013_old_methodology/elements",
        "flags": f"/commodity_balances_non_food_2013_old_methodology/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the commodity_balances_non_food_2013_old_methodology endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(CommodityBalancesNonFood2013OldMethodology)).scalar()
        return {
            "status": "healthy",
            "dataset": "commodity_balances_non_food_2013_old_methodology",
            "records": result
