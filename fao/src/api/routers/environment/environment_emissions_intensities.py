...
from .environment_emissions_intensities_config import EnvironmentEmissionsIntensitiesConfig
...
router = APIRouter(
    prefix="/environment_emissions_intensities",
    responses={404: {"description": "Not found"}},
)


config = EnvironmentEmissionsIntensitiesConfig()

@router.get("/", summary="Get environment emissions intensities data")
async def get_environment_emissions_intensities_data(
    request: Request,
    response: Response,
    ...
    """Get environment emissions intensities data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EnvironmentEmissionsIntensities, 
        model_name="EnvironmentEmissionsIntensities",
        table_name="environment_emissions_intensities",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated environment emissions intensities data")
async def get_environment_emissions_intensities_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EnvironmentEmissionsIntensities))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EnvironmentEmissionsIntensities, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EnvironmentEmissionsIntensities, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EnvironmentEmissionsIntensities, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in environment_emissions_intensities")
@cache_result(prefix="environment_emissions_intensities:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(EnvironmentEmissionsIntensities.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(EnvironmentEmissionsIntensities, EnvironmentEmissionsIntensities.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'environment_emissions_intensities')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="environment_emissions_intensities",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in environment_emissions_intensities")
@cache_result(prefix="environment_emissions_intensities:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EnvironmentEmissionsIntensities, EnvironmentEmissionsIntensities.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'environment_emissions_intensities')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in environment_emissions_intensities")
@cache_result(prefix="environment_emissions_intensities:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EnvironmentEmissionsIntensities, EnvironmentEmissionsIntensities.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'environment_emissions_intensities')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in environment_emissions_intensities")
@cache_result(prefix="environment_emissions_intensities:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EnvironmentEmissionsIntensities, Flags.id == EnvironmentEmissionsIntensities.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EnvironmentEmissionsIntensities.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EnvironmentEmissionsIntensities)
                .where(EnvironmentEmissionsIntensities.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EnvironmentEmissionsIntensities)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in environment_emissions_intensities")
@cache_result(prefix="environment_emissions_intensities:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EnvironmentEmissionsIntensities.unit,
            func.count(EnvironmentEmissionsIntensities.id).label('record_count')
        )
        .select_from(EnvironmentEmissionsIntensities)
        .group_by(EnvironmentEmissionsIntensities.unit)
        .order_by(EnvironmentEmissionsIntensities.unit)
    )
...
@router.get("/years", summary="Get available years in environment_emissions_intensities")
@cache_result(prefix="environment_emissions_intensities:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EnvironmentEmissionsIntensities.year,
                EnvironmentEmissionsIntensities.year_code,
                func.count(EnvironmentEmissionsIntensities.id).label('record_count')
            )
            .group_by(EnvironmentEmissionsIntensities.year, EnvironmentEmissionsIntensities.year_code)
            .order_by(EnvironmentEmissionsIntensities.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EnvironmentEmissionsIntensities.year)
            .distinct()
            .order_by(EnvironmentEmissionsIntensities.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of environment_emissions_intensities dataset")
@cache_result(prefix="environment_emissions_intensities:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentEmissionsIntensities.area_code_id)))
        .select_from(EnvironmentEmissionsIntensities)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/environment_emissions_intensities/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentEmissionsIntensities.item_code_id)))
        .select_from(EnvironmentEmissionsIntensities)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/environment_emissions_intensities/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentEmissionsIntensities.element_code_id)))
        .select_from(EnvironmentEmissionsIntensities)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/environment_emissions_intensities/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EnvironmentEmissionsIntensities.flag_id)))
        .select_from(EnvironmentEmissionsIntensities)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/environment_emissions_intensities/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EnvironmentEmissionsIntensities.year).label('min_year'),
            func.max(EnvironmentEmissionsIntensities.year).label('max_year'),
            func.count(func.distinct(EnvironmentEmissionsIntensities.year)).label('year_count')
        )
        .select_from(EnvironmentEmissionsIntensities)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/environment_emissions_intensities/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EnvironmentEmissionsIntensities.value).label('min_value'),
            func.max(EnvironmentEmissionsIntensities.value).label('max_value'),
            func.avg(EnvironmentEmissionsIntensities.value).label('avg_value')
        )
        .select_from(EnvironmentEmissionsIntensities)
        .where(and_(EnvironmentEmissionsIntensities.value > 0, EnvironmentEmissionsIntensities.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/environment_emissions_intensities",
        "aggregate": f"/environment_emissions_intensities/aggregate",
        "summary": f"/environment_emissions_intensities/summary",
        "overview": f"/environment_emissions_intensities/overview",
        "area_codes": f"/environment_emissions_intensities/area_codes",
        "item_codes": f"/environment_emissions_intensities/item_codes",
        "elements": f"/environment_emissions_intensities/elements",
        "flags": f"/environment_emissions_intensities/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the environment_emissions_intensities endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EnvironmentEmissionsIntensities)).scalar()
        return {
            "status": "healthy",
            "dataset": "environment_emissions_intensities",
            "records": result
