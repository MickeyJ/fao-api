...
@router.get("/")
@cache_result(prefix="environment_emissions_intensities", ttl=86400, exclude_params=["response", "db"])
def get_environment_emissions_intensities(
    response: Response,
    ...
    """
    Get environment emissions intensities data with filters and pagination.
    ...
    # Main table columns
    for col in EnvironmentEmissionsIntensities.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EnvironmentEmissionsIntensities, 'id'):
                columns.append(EnvironmentEmissionsIntensities.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EnvironmentEmissionsIntensities)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EnvironmentEmissionsIntensities.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EnvironmentEmissionsIntensities.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EnvironmentEmissionsIntensities.element_code_id == Elements.id)
    query = query.outerjoin(Flags, EnvironmentEmissionsIntensities.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EnvironmentEmissionsIntensities.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EnvironmentEmissionsIntensities.year_code == year_code_exact)
    if year is not None:
        conditions.append(EnvironmentEmissionsIntensities.year == year)
    if year_min is not None:
        conditions.append(EnvironmentEmissionsIntensities.year >= year_min)
    if year_max is not None:
        conditions.append(EnvironmentEmissionsIntensities.year <= year_max)
    if unit is not None:
        conditions.append(EnvironmentEmissionsIntensities.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EnvironmentEmissionsIntensities.unit == unit_exact)
    if value is not None:
        conditions.append(EnvironmentEmissionsIntensities.value == value)
    if value_min is not None:
        conditions.append(EnvironmentEmissionsIntensities.value >= value_min)
    if value_max is not None:
        conditions.append(EnvironmentEmissionsIntensities.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EnvironmentEmissionsIntensities, field_name):
                        order_by_clauses.append(getattr(EnvironmentEmissionsIntensities, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EnvironmentEmissionsIntensities, sort_field):
                        order_by_clauses.append(getattr(EnvironmentEmissionsIntensities, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EnvironmentEmissionsIntensities.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EnvironmentEmissionsIntensities)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EnvironmentEmissionsIntensities.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EnvironmentEmissionsIntensities.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EnvironmentEmissionsIntensities.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EnvironmentEmissionsIntensities.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_environment_emissions_intensities'))
...
@router.get("/areas")
@cache_result(prefix="environment_emissions_intensities:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="environment_emissions_intensities:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EnvironmentEmissionsIntensities.year)
    ...
    if not years_data:
        return {"dataset": "environment_emissions_intensities", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="environment_emissions_intensities:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EnvironmentEmissionsIntensities.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EnvironmentEmissionsIntensities.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EnvironmentEmissionsIntensities.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EnvironmentEmissionsIntensities.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EnvironmentEmissionsIntensities.flag_id))).scalar()
