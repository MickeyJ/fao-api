...
router = APIRouter(
    prefix="/environment_food_waste_disposal",
    tags=["environment_food_waste_disposal"],
    responses={404: {"description": "Not found"}},
)

@router.get("/")
def get_environment_food_waste_disposal(
    ...
    """
    environment food waste disposal data with filters.
    Filter options:
    ...
    query = (
        ...
        .select_from(EnvironmentFoodWasteDisposal)
        .outerjoin(AreaCodes, EnvironmentFoodWasteDisposal.area_code_id == AreaCodes.id)
        .outerjoin(ItemCodes, EnvironmentFoodWasteDisposal.item_code_id == ItemCodes.id)
        .outerjoin(Elements, EnvironmentFoodWasteDisposal.element_code_id == Elements.id)
        .outerjoin(Flags, EnvironmentFoodWasteDisposal.flag_id == Flags.id)
    )
...
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            AreaCodes.area_code,
            AreaCodes.area,
            func.count(EnvironmentFoodWasteDisposal.id).label('record_count')
        )
        .join(EnvironmentFoodWasteDisposal, AreaCodes.id == EnvironmentFoodWasteDisposal.area_code_id)
        .group_by(AreaCodes.area_code, AreaCodes.area)
        .order_by(func.count(EnvironmentFoodWasteDisposal.id).desc())
    )
    
    results = db.execute(query).all()
    
    return {
        "dataset": "environment_food_waste_disposal",
...
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentFoodWasteDisposal, ItemCodes.id == EnvironmentFoodWasteDisposal.item_code_id)
...
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentFoodWasteDisposal, Elements.id == EnvironmentFoodWasteDisposal.element_code_id)
...
def get_data_quality_summary(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentFoodWasteDisposal, Flags.id == EnvironmentFoodWasteDisposal.flag_id)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            EnvironmentFoodWasteDisposal.year,
            func.count(EnvironmentFoodWasteDisposal.id).label('record_count')
        )
        .group_by(EnvironmentFoodWasteDisposal.year)
        .order_by(EnvironmentFoodWasteDisposal.year)
    )
    ...
    if not years_data:
        return {"dataset": "environment_food_waste_disposal", "message": "No temporal data available"}
...
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    total_records = db.query(func.count(EnvironmentFoodWasteDisposal.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EnvironmentFoodWasteDisposal.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EnvironmentFoodWasteDisposal.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EnvironmentFoodWasteDisposal.element_code_id))).scalar()
