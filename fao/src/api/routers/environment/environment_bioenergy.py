...
@router.get("/")
@cache_result(prefix="environment_bioenergy", ttl=86400, exclude_params=["response", "db"])
def get_environment_bioenergy(
    response: Response,
    ...
    """
    Get environment bioenergy data with filters and pagination.
    ...
    # Main table columns
    for col in EnvironmentBioenergy.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EnvironmentBioenergy, 'id'):
                columns.append(EnvironmentBioenergy.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EnvironmentBioenergy)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EnvironmentBioenergy.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EnvironmentBioenergy.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EnvironmentBioenergy.element_code_id == Elements.id)
    query = query.outerjoin(Flags, EnvironmentBioenergy.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EnvironmentBioenergy.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EnvironmentBioenergy.year_code == year_code_exact)
    if year is not None:
        conditions.append(EnvironmentBioenergy.year == year)
    if year_min is not None:
        conditions.append(EnvironmentBioenergy.year >= year_min)
    if year_max is not None:
        conditions.append(EnvironmentBioenergy.year <= year_max)
    if unit is not None:
        conditions.append(EnvironmentBioenergy.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EnvironmentBioenergy.unit == unit_exact)
    if value is not None:
        conditions.append(EnvironmentBioenergy.value == value)
    if value_min is not None:
        conditions.append(EnvironmentBioenergy.value >= value_min)
    if value_max is not None:
        conditions.append(EnvironmentBioenergy.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EnvironmentBioenergy, field_name):
                        order_by_clauses.append(getattr(EnvironmentBioenergy, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EnvironmentBioenergy, sort_field):
                        order_by_clauses.append(getattr(EnvironmentBioenergy, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EnvironmentBioenergy.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EnvironmentBioenergy)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EnvironmentBioenergy.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EnvironmentBioenergy.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EnvironmentBioenergy.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EnvironmentBioenergy.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_environment_bioenergy'))
...
@router.get("/areas")
@cache_result(prefix="environment_bioenergy:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="environment_bioenergy:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EnvironmentBioenergy.year)
    ...
    if not years_data:
        return {"dataset": "environment_bioenergy", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="environment_bioenergy:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EnvironmentBioenergy.id)).scalar()
