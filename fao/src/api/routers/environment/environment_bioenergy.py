...
from .environment_bioenergy_config import EnvironmentBioenergyConfig
...
router = APIRouter(
    prefix="/environment_bioenergy",
    responses={404: {"description": "Not found"}},
)


config = EnvironmentBioenergyConfig()

@router.get("/", summary="Get environment bioenergy data")
async def get_environment_bioenergy_data(
    request: Request,
    response: Response,
    ...
    """Get environment bioenergy data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EnvironmentBioenergy, 
        model_name="EnvironmentBioenergy",
        table_name="environment_bioenergy",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated environment bioenergy data")
async def get_environment_bioenergy_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EnvironmentBioenergy))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EnvironmentBioenergy, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EnvironmentBioenergy, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EnvironmentBioenergy, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in environment_bioenergy")
@cache_result(prefix="environment_bioenergy:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(EnvironmentBioenergy, EnvironmentBioenergy.area_code_id == AreaCodes.id)
...
@router.get("/item_codes", summary="Get item_codes in environment_bioenergy")
@cache_result(prefix="environment_bioenergy:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EnvironmentBioenergy, EnvironmentBioenergy.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in environment_bioenergy")
@cache_result(prefix="environment_bioenergy:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EnvironmentBioenergy, EnvironmentBioenergy.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in environment_bioenergy")
@cache_result(prefix="environment_bioenergy:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EnvironmentBioenergy, Flags.id == EnvironmentBioenergy.flag_id)
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EnvironmentBioenergy)
                .where(EnvironmentBioenergy.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EnvironmentBioenergy)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in environment_bioenergy")
@cache_result(prefix="environment_bioenergy:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EnvironmentBioenergy.unit,
            func.count(EnvironmentBioenergy.id).label('record_count')
        )
        .select_from(EnvironmentBioenergy)
        .group_by(EnvironmentBioenergy.unit)
        .order_by(EnvironmentBioenergy.unit)
    )
...
@router.get("/years", summary="Get available years in environment_bioenergy")
...
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EnvironmentBioenergy.year,
                EnvironmentBioenergy.year_code,
                func.count(EnvironmentBioenergy.id).label('record_count')
            )
            .group_by(EnvironmentBioenergy.year, EnvironmentBioenergy.year_code)
            .order_by(EnvironmentBioenergy.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EnvironmentBioenergy.year)
            .distinct()
            .order_by(EnvironmentBioenergy.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of environment_bioenergy dataset")
@cache_result(prefix="environment_bioenergy:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentBioenergy.area_code_id)))
        .select_from(EnvironmentBioenergy)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/environment_bioenergy/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentBioenergy.item_code_id)))
        .select_from(EnvironmentBioenergy)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/environment_bioenergy/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentBioenergy.element_code_id)))
        .select_from(EnvironmentBioenergy)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/environment_bioenergy/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EnvironmentBioenergy.flag_id)))
        .select_from(EnvironmentBioenergy)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/environment_bioenergy/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EnvironmentBioenergy.year).label('min_year'),
            func.max(EnvironmentBioenergy.year).label('max_year'),
            func.count(func.distinct(EnvironmentBioenergy.year)).label('year_count')
        )
        .select_from(EnvironmentBioenergy)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/environment_bioenergy/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EnvironmentBioenergy.value).label('min_value'),
            func.max(EnvironmentBioenergy.value).label('max_value'),
            func.avg(EnvironmentBioenergy.value).label('avg_value')
        )
        .select_from(EnvironmentBioenergy)
        .where(and_(EnvironmentBioenergy.value > 0, EnvironmentBioenergy.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/environment_bioenergy",
        "aggregate": f"/environment_bioenergy/aggregate",
        "summary": f"/environment_bioenergy/summary",
        "overview": f"/environment_bioenergy/overview",
        "area_codes": f"/environment_bioenergy/area_codes",
        "item_codes": f"/environment_bioenergy/item_codes",
        "elements": f"/environment_bioenergy/elements",
        "flags": f"/environment_bioenergy/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the environment_bioenergy endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EnvironmentBioenergy)).scalar()
        return {
            "status": "healthy",
            "dataset": "environment_bioenergy",
            "records": result
