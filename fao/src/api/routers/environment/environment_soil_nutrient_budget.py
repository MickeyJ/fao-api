...
def get_environment_soil_nutrient_budget(
    ...
    """
    environment soil nutrient budget data with filters.
    Filter options:
    ...
    query = (
        ...
        .select_from(EnvironmentSoilNutrientBudget)
        .outerjoin(AreaCodes, EnvironmentSoilNutrientBudget.area_code_id == AreaCodes.id)
        .outerjoin(ItemCodes, EnvironmentSoilNutrientBudget.item_code_id == ItemCodes.id)
        .outerjoin(Elements, EnvironmentSoilNutrientBudget.element_code_id == Elements.id)
        .outerjoin(Flags, EnvironmentSoilNutrientBudget.flag_id == Flags.id)
    )
...
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            AreaCodes.area_code,
            AreaCodes.area,
            func.count(EnvironmentSoilNutrientBudget.id).label('record_count')
        )
        .join(EnvironmentSoilNutrientBudget, AreaCodes.id == EnvironmentSoilNutrientBudget.area_code_id)
        .group_by(AreaCodes.area_code, AreaCodes.area)
        .order_by(func.count(EnvironmentSoilNutrientBudget.id).desc())
    )
    
    results = db.execute(query).all()
    
    return {
        "dataset": "environment_soil_nutrient_budget",
...
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentSoilNutrientBudget, ItemCodes.id == EnvironmentSoilNutrientBudget.item_code_id)
...
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentSoilNutrientBudget, Elements.id == EnvironmentSoilNutrientBudget.element_code_id)
...
def get_data_quality_summary(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentSoilNutrientBudget, Flags.id == EnvironmentSoilNutrientBudget.flag_id)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            EnvironmentSoilNutrientBudget.year,
            func.count(EnvironmentSoilNutrientBudget.id).label('record_count')
        )
        .group_by(EnvironmentSoilNutrientBudget.year)
        .order_by(EnvironmentSoilNutrientBudget.year)
    )
    ...
    if not years_data:
        return {"dataset": "environment_soil_nutrient_budget", "message": "No temporal data available"}
...
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    total_records = db.query(func.count(EnvironmentSoilNutrientBudget.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EnvironmentSoilNutrientBudget.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EnvironmentSoilNutrientBudget.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EnvironmentSoilNutrientBudget.element_code_id))).scalar()
