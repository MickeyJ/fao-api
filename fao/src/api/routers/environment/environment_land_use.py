...
router = APIRouter(
    prefix="/environment_land_use",
    tags=["environment_land_use"],
    responses={404: {"description": "Not found"}},
)

@router.get("/")
def get_environment_land_use(
    ...
    """
    environment land use data with filters.
    Filter options:
    ...
    query = (
        ...
        .select_from(EnvironmentLandUse)
        .outerjoin(AreaCodes, EnvironmentLandUse.area_code_id == AreaCodes.id)
        .outerjoin(ItemCodes, EnvironmentLandUse.item_code_id == ItemCodes.id)
        .outerjoin(Elements, EnvironmentLandUse.element_code_id == Elements.id)
        .outerjoin(Flags, EnvironmentLandUse.flag_id == Flags.id)
    )
...
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            AreaCodes.area_code,
            AreaCodes.area,
            func.count(EnvironmentLandUse.id).label('record_count')
        )
        .join(EnvironmentLandUse, AreaCodes.id == EnvironmentLandUse.area_code_id)
        .group_by(AreaCodes.area_code, AreaCodes.area)
        .order_by(func.count(EnvironmentLandUse.id).desc())
    )
    
    results = db.execute(query).all()
    
    return {
        "dataset": "environment_land_use",
...
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentLandUse, ItemCodes.id == EnvironmentLandUse.item_code_id)
...
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentLandUse, Elements.id == EnvironmentLandUse.element_code_id)
...
def get_data_quality_summary(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentLandUse, Flags.id == EnvironmentLandUse.flag_id)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            EnvironmentLandUse.year,
            func.count(EnvironmentLandUse.id).label('record_count')
        )
        .group_by(EnvironmentLandUse.year)
        .order_by(EnvironmentLandUse.year)
    )
    ...
    if not years_data:
        return {"dataset": "environment_land_use", "message": "No temporal data available"}
...
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    total_records = db.query(func.count(EnvironmentLandUse.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EnvironmentLandUse.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EnvironmentLandUse.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EnvironmentLandUse.element_code_id))).scalar()
