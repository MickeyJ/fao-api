...
from .environment_livestock_manure_config import EnvironmentLivestockManureConfig
...
router = APIRouter(
    prefix="/environment_livestock_manure",
    responses={404: {"description": "Not found"}},
)


config = EnvironmentLivestockManureConfig()

@router.get("/", summary="Get environment livestock manure data")
async def get_environment_livestock_manure_data(
    request: Request,
    response: Response,
    ...
    """Get environment livestock manure data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EnvironmentLivestockManure, 
        model_name="EnvironmentLivestockManure",
        table_name="environment_livestock_manure",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated environment livestock manure data")
async def get_environment_livestock_manure_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EnvironmentLivestockManure))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EnvironmentLivestockManure, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EnvironmentLivestockManure, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EnvironmentLivestockManure, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in environment_livestock_manure")
@cache_result(prefix="environment_livestock_manure:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(EnvironmentLivestockManure.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(EnvironmentLivestockManure, EnvironmentLivestockManure.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'environment_livestock_manure')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="environment_livestock_manure",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in environment_livestock_manure")
@cache_result(prefix="environment_livestock_manure:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EnvironmentLivestockManure, EnvironmentLivestockManure.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'environment_livestock_manure')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in environment_livestock_manure")
@cache_result(prefix="environment_livestock_manure:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EnvironmentLivestockManure, EnvironmentLivestockManure.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'environment_livestock_manure')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in environment_livestock_manure")
@cache_result(prefix="environment_livestock_manure:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EnvironmentLivestockManure, Flags.id == EnvironmentLivestockManure.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EnvironmentLivestockManure.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EnvironmentLivestockManure)
                .where(EnvironmentLivestockManure.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EnvironmentLivestockManure)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in environment_livestock_manure")
@cache_result(prefix="environment_livestock_manure:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EnvironmentLivestockManure.unit,
            func.count(EnvironmentLivestockManure.id).label('record_count')
        )
        .select_from(EnvironmentLivestockManure)
        .group_by(EnvironmentLivestockManure.unit)
        .order_by(EnvironmentLivestockManure.unit)
    )
...
@router.get("/years", summary="Get available years in environment_livestock_manure")
@cache_result(prefix="environment_livestock_manure:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EnvironmentLivestockManure.year,
                EnvironmentLivestockManure.year_code,
                func.count(EnvironmentLivestockManure.id).label('record_count')
            )
            .group_by(EnvironmentLivestockManure.year, EnvironmentLivestockManure.year_code)
            .order_by(EnvironmentLivestockManure.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EnvironmentLivestockManure.year)
            .distinct()
            .order_by(EnvironmentLivestockManure.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of environment_livestock_manure dataset")
@cache_result(prefix="environment_livestock_manure:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLivestockManure.area_code_id)))
        .select_from(EnvironmentLivestockManure)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/environment_livestock_manure/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLivestockManure.item_code_id)))
        .select_from(EnvironmentLivestockManure)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/environment_livestock_manure/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLivestockManure.element_code_id)))
        .select_from(EnvironmentLivestockManure)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/environment_livestock_manure/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLivestockManure.flag_id)))
        .select_from(EnvironmentLivestockManure)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/environment_livestock_manure/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EnvironmentLivestockManure.year).label('min_year'),
            func.max(EnvironmentLivestockManure.year).label('max_year'),
            func.count(func.distinct(EnvironmentLivestockManure.year)).label('year_count')
        )
        .select_from(EnvironmentLivestockManure)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/environment_livestock_manure/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EnvironmentLivestockManure.value).label('min_value'),
            func.max(EnvironmentLivestockManure.value).label('max_value'),
            func.avg(EnvironmentLivestockManure.value).label('avg_value')
        )
        .select_from(EnvironmentLivestockManure)
        .where(and_(EnvironmentLivestockManure.value > 0, EnvironmentLivestockManure.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/environment_livestock_manure",
        "aggregate": f"/environment_livestock_manure/aggregate",
        "summary": f"/environment_livestock_manure/summary",
        "overview": f"/environment_livestock_manure/overview",
        "area_codes": f"/environment_livestock_manure/area_codes",
        "item_codes": f"/environment_livestock_manure/item_codes",
        "elements": f"/environment_livestock_manure/elements",
        "flags": f"/environment_livestock_manure/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the environment_livestock_manure endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EnvironmentLivestockManure)).scalar()
        return {
            "status": "healthy",
            "dataset": "environment_livestock_manure",
            "records": result
