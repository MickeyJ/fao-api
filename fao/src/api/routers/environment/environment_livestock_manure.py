...
@router.get("/")
@cache_result(prefix="environment_livestock_manure", ttl=86400, exclude_params=["response", "db"])
def get_environment_livestock_manure(
    response: Response,
    ...
    """
    Get environment livestock manure data with filters and pagination.
    ...
    # Main table columns
    for col in EnvironmentLivestockManure.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EnvironmentLivestockManure, 'id'):
                columns.append(EnvironmentLivestockManure.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EnvironmentLivestockManure)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EnvironmentLivestockManure.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EnvironmentLivestockManure.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EnvironmentLivestockManure.element_code_id == Elements.id)
    query = query.outerjoin(Flags, EnvironmentLivestockManure.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EnvironmentLivestockManure.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EnvironmentLivestockManure.year_code == year_code_exact)
    if year is not None:
        conditions.append(EnvironmentLivestockManure.year == year)
    if year_min is not None:
        conditions.append(EnvironmentLivestockManure.year >= year_min)
    if year_max is not None:
        conditions.append(EnvironmentLivestockManure.year <= year_max)
    if unit is not None:
        conditions.append(EnvironmentLivestockManure.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EnvironmentLivestockManure.unit == unit_exact)
    if value is not None:
        conditions.append(EnvironmentLivestockManure.value == value)
    if value_min is not None:
        conditions.append(EnvironmentLivestockManure.value >= value_min)
    if value_max is not None:
        conditions.append(EnvironmentLivestockManure.value <= value_max)
    if note is not None:
        conditions.append(EnvironmentLivestockManure.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EnvironmentLivestockManure.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EnvironmentLivestockManure, field_name):
                        order_by_clauses.append(getattr(EnvironmentLivestockManure, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EnvironmentLivestockManure, sort_field):
                        order_by_clauses.append(getattr(EnvironmentLivestockManure, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EnvironmentLivestockManure.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EnvironmentLivestockManure)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EnvironmentLivestockManure.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EnvironmentLivestockManure.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EnvironmentLivestockManure.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EnvironmentLivestockManure.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_environment_livestock_manure'))
...
@router.get("/areas")
@cache_result(prefix="environment_livestock_manure:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="environment_livestock_manure:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EnvironmentLivestockManure.year)
    ...
    if not years_data:
        return {"dataset": "environment_livestock_manure", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="environment_livestock_manure:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EnvironmentLivestockManure.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EnvironmentLivestockManure.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EnvironmentLivestockManure.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EnvironmentLivestockManure.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EnvironmentLivestockManure.flag_id))).scalar()
