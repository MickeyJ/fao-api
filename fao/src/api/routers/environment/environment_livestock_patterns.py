...
from .environment_livestock_patterns_config import EnvironmentLivestockPatternsConfig
...
router = APIRouter(
    prefix="/environment_livestock_patterns",
    responses={404: {"description": "Not found"}},
)


config = EnvironmentLivestockPatternsConfig()

@router.get("/", summary="Get environment livestock patterns data")
async def get_environment_livestock_patterns_data(
    request: Request,
    response: Response,
    ...
    """Get environment livestock patterns data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EnvironmentLivestockPatterns, 
        model_name="EnvironmentLivestockPatterns",
        table_name="environment_livestock_patterns",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated environment livestock patterns data")
async def get_environment_livestock_patterns_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EnvironmentLivestockPatterns))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EnvironmentLivestockPatterns, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EnvironmentLivestockPatterns, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EnvironmentLivestockPatterns, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in environment_livestock_patterns")
@cache_result(prefix="environment_livestock_patterns:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(EnvironmentLivestockPatterns.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(EnvironmentLivestockPatterns, EnvironmentLivestockPatterns.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'environment_livestock_patterns')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="environment_livestock_patterns",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in environment_livestock_patterns")
@cache_result(prefix="environment_livestock_patterns:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EnvironmentLivestockPatterns, EnvironmentLivestockPatterns.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'environment_livestock_patterns')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in environment_livestock_patterns")
@cache_result(prefix="environment_livestock_patterns:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EnvironmentLivestockPatterns, EnvironmentLivestockPatterns.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'environment_livestock_patterns')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in environment_livestock_patterns")
@cache_result(prefix="environment_livestock_patterns:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EnvironmentLivestockPatterns, Flags.id == EnvironmentLivestockPatterns.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EnvironmentLivestockPatterns.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EnvironmentLivestockPatterns)
                .where(EnvironmentLivestockPatterns.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EnvironmentLivestockPatterns)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in environment_livestock_patterns")
@cache_result(prefix="environment_livestock_patterns:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EnvironmentLivestockPatterns.unit,
            func.count(EnvironmentLivestockPatterns.id).label('record_count')
        )
        .select_from(EnvironmentLivestockPatterns)
        .group_by(EnvironmentLivestockPatterns.unit)
        .order_by(EnvironmentLivestockPatterns.unit)
    )
...
@router.get("/years", summary="Get available years in environment_livestock_patterns")
@cache_result(prefix="environment_livestock_patterns:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EnvironmentLivestockPatterns.year,
                EnvironmentLivestockPatterns.year_code,
                func.count(EnvironmentLivestockPatterns.id).label('record_count')
            )
            .group_by(EnvironmentLivestockPatterns.year, EnvironmentLivestockPatterns.year_code)
            .order_by(EnvironmentLivestockPatterns.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EnvironmentLivestockPatterns.year)
            .distinct()
            .order_by(EnvironmentLivestockPatterns.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of environment_livestock_patterns dataset")
@cache_result(prefix="environment_livestock_patterns:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLivestockPatterns.area_code_id)))
        .select_from(EnvironmentLivestockPatterns)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/environment_livestock_patterns/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLivestockPatterns.item_code_id)))
        .select_from(EnvironmentLivestockPatterns)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/environment_livestock_patterns/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLivestockPatterns.element_code_id)))
        .select_from(EnvironmentLivestockPatterns)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/environment_livestock_patterns/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLivestockPatterns.flag_id)))
        .select_from(EnvironmentLivestockPatterns)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/environment_livestock_patterns/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EnvironmentLivestockPatterns.year).label('min_year'),
            func.max(EnvironmentLivestockPatterns.year).label('max_year'),
            func.count(func.distinct(EnvironmentLivestockPatterns.year)).label('year_count')
        )
        .select_from(EnvironmentLivestockPatterns)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/environment_livestock_patterns/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EnvironmentLivestockPatterns.value).label('min_value'),
            func.max(EnvironmentLivestockPatterns.value).label('max_value'),
            func.avg(EnvironmentLivestockPatterns.value).label('avg_value')
        )
        .select_from(EnvironmentLivestockPatterns)
        .where(and_(EnvironmentLivestockPatterns.value > 0, EnvironmentLivestockPatterns.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/environment_livestock_patterns",
        "aggregate": f"/environment_livestock_patterns/aggregate",
        "summary": f"/environment_livestock_patterns/summary",
        "overview": f"/environment_livestock_patterns/overview",
        "area_codes": f"/environment_livestock_patterns/area_codes",
        "item_codes": f"/environment_livestock_patterns/item_codes",
        "elements": f"/environment_livestock_patterns/elements",
        "flags": f"/environment_livestock_patterns/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the environment_livestock_patterns endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EnvironmentLivestockPatterns)).scalar()
        return {
            "status": "healthy",
            "dataset": "environment_livestock_patterns",
            "records": result
