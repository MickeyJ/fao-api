...
@router.get("/")
@cache_result(prefix="environment_temperature_change", ttl=86400, exclude_params=["response", "db"])
def get_environment_temperature_change(
    ...
    """
    Get environment temperature change data with filters and pagination.
    ...
    # Main table columns
    for col in EnvironmentTemperatureChange.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EnvironmentTemperatureChange, 'id'):
                columns.append(EnvironmentTemperatureChange.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EnvironmentTemperatureChange)
    ...
    if months_code is not None:
        conditions.append(EnvironmentTemperatureChange.months_code.ilike("%" + months_code + "%"))
    if months_code_exact is not None:
        conditions.append(EnvironmentTemperatureChange.months_code == months_code_exact)
    if months is not None:
        conditions.append(EnvironmentTemperatureChange.months.ilike("%" + months + "%"))
    if months_exact is not None:
        conditions.append(EnvironmentTemperatureChange.months == months_exact)
    if year_code is not None:
        conditions.append(EnvironmentTemperatureChange.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EnvironmentTemperatureChange.year_code == year_code_exact)
    if year is not None:
        conditions.append(EnvironmentTemperatureChange.year == year)
    if year_min is not None:
        conditions.append(EnvironmentTemperatureChange.year >= year_min)
    if year_max is not None:
        conditions.append(EnvironmentTemperatureChange.year <= year_max)
    if unit is not None:
        conditions.append(EnvironmentTemperatureChange.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EnvironmentTemperatureChange.unit == unit_exact)
    if value is not None:
        conditions.append(EnvironmentTemperatureChange.value == value)
    if value_min is not None:
        conditions.append(EnvironmentTemperatureChange.value >= value_min)
    if value_max is not None:
        conditions.append(EnvironmentTemperatureChange.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EnvironmentTemperatureChange, field_name):
                        order_by_clauses.append(getattr(EnvironmentTemperatureChange, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EnvironmentTemperatureChange, sort_field):
                        order_by_clauses.append(getattr(EnvironmentTemperatureChange, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EnvironmentTemperatureChange.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EnvironmentTemperatureChange)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EnvironmentTemperatureChange.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(Elements, EnvironmentTemperatureChange.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EnvironmentTemperatureChange.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_environment_temperature_change'))
...
@router.get("/areas")
@cache_result(prefix="environment_temperature_change:areas", ttl=604800)
...
@router.get("/summary")
@cache_result(prefix="environment_temperature_change:summary", ttl=604800)
