...
from .environment_temperature_change_config import EnvironmentTemperatureChangeConfig
...
config = EnvironmentTemperatureChangeConfig()

@router.get("/", summary="Get environment temperature change data")
async def get_environment_temperature_change_data(
    request: Request,
    response: Response,
    ...
    """Get environment temperature change data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EnvironmentTemperatureChange, 
        model_name="EnvironmentTemperatureChange",
        table_name="environment_temperature_change",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated environment temperature change data")
async def get_environment_temperature_change_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EnvironmentTemperatureChange))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EnvironmentTemperatureChange, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EnvironmentTemperatureChange, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EnvironmentTemperatureChange, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in environment_temperature_change")
@cache_result(prefix="environment_temperature_change:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(EnvironmentTemperatureChange, EnvironmentTemperatureChange.area_code_id == AreaCodes.id)
...
@router.get("/elements", summary="Get elements in environment_temperature_change")
@cache_result(prefix="environment_temperature_change:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EnvironmentTemperatureChange, EnvironmentTemperatureChange.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in environment_temperature_change")
@cache_result(prefix="environment_temperature_change:flags", ttl=604800)
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(EnvironmentTemperatureChange.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EnvironmentTemperatureChange)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in environment_temperature_change")
@cache_result(prefix="environment_temperature_change:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EnvironmentTemperatureChange.unit,
            func.count(EnvironmentTemperatureChange.id).label('record_count')
        )
        .select_from(EnvironmentTemperatureChange)
        .group_by(EnvironmentTemperatureChange.unit)
        .order_by(EnvironmentTemperatureChange.unit)
    )
...
@router.get("/years", summary="Get available years in environment_temperature_change")
    ...
    if include_counts:
        query = (
            select(
                ...
                EnvironmentTemperatureChange.year_code,
                func.count(EnvironmentTemperatureChange.id).label('record_count')
            )
            .group_by(EnvironmentTemperatureChange.year, EnvironmentTemperatureChange.year_code)
            .order_by(EnvironmentTemperatureChange.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EnvironmentTemperatureChange.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of environment_temperature_change dataset")
@cache_result(prefix="environment_temperature_change:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentTemperatureChange.area_code_id)))
        .select_from(EnvironmentTemperatureChange)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/environment_temperature_change/area_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentTemperatureChange.element_code_id)))
        .select_from(EnvironmentTemperatureChange)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/environment_temperature_change/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EnvironmentTemperatureChange.flag_id)))
        .select_from(EnvironmentTemperatureChange)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/environment_temperature_change/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EnvironmentTemperatureChange.year).label('min_year'),
            func.max(EnvironmentTemperatureChange.year).label('max_year'),
            func.count(func.distinct(EnvironmentTemperatureChange.year)).label('year_count')
        )
        .select_from(EnvironmentTemperatureChange)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/environment_temperature_change/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EnvironmentTemperatureChange.value).label('min_value'),
            func.max(EnvironmentTemperatureChange.value).label('max_value'),
            func.avg(EnvironmentTemperatureChange.value).label('avg_value')
        )
        .select_from(EnvironmentTemperatureChange)
        .where(and_(EnvironmentTemperatureChange.value > 0, EnvironmentTemperatureChange.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/environment_temperature_change",
        "aggregate": f"/environment_temperature_change/aggregate",
        "summary": f"/environment_temperature_change/summary",
        "overview": f"/environment_temperature_change/overview",
        "area_codes": f"/environment_temperature_change/area_codes",
        "elements": f"/environment_temperature_change/elements",
        "flags": f"/environment_temperature_change/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the environment_temperature_change endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EnvironmentTemperatureChange)).scalar()
        return {
            "status": "healthy",
            "dataset": "environment_temperature_change",
            "records": result
