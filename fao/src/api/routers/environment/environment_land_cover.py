...
from .environment_land_cover_config import EnvironmentLandCoverConfig
...
config = EnvironmentLandCoverConfig()

@router.get("/", summary="Get environment land cover data")
async def get_environment_land_cover_data(
    request: Request,
    response: Response,
    ...
    """Get environment land cover data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EnvironmentLandCover, 
        model_name="EnvironmentLandCover",
        table_name="environment_land_cover",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated environment land cover data")
async def get_environment_land_cover_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EnvironmentLandCover))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EnvironmentLandCover, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EnvironmentLandCover, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EnvironmentLandCover, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in environment_land_cover")
@cache_result(prefix="environment_land_cover:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(EnvironmentLandCover, EnvironmentLandCover.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'environment_land_cover')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in environment_land_cover")
@cache_result(prefix="environment_land_cover:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EnvironmentLandCover, EnvironmentLandCover.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'environment_land_cover')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in environment_land_cover")
@cache_result(prefix="environment_land_cover:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EnvironmentLandCover, EnvironmentLandCover.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'environment_land_cover')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in environment_land_cover")
@cache_result(prefix="environment_land_cover:flags", ttl=604800)
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(EnvironmentLandCover.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EnvironmentLandCover)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in environment_land_cover")
@cache_result(prefix="environment_land_cover:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EnvironmentLandCover.unit,
            func.count(EnvironmentLandCover.id).label('record_count')
        )
        .select_from(EnvironmentLandCover)
        .group_by(EnvironmentLandCover.unit)
        .order_by(EnvironmentLandCover.unit)
    )
...
@router.get("/years", summary="Get available years in environment_land_cover")
@cache_result(prefix="environment_land_cover:years", ttl=604800)
    ...
    if include_counts:
        query = (
            select(
                ...
                EnvironmentLandCover.year_code,
                func.count(EnvironmentLandCover.id).label('record_count')
            )
            .group_by(EnvironmentLandCover.year, EnvironmentLandCover.year_code)
            .order_by(EnvironmentLandCover.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EnvironmentLandCover.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of environment_land_cover dataset")
@cache_result(prefix="environment_land_cover:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLandCover.area_code_id)))
        .select_from(EnvironmentLandCover)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/environment_land_cover/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLandCover.item_code_id)))
        .select_from(EnvironmentLandCover)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/environment_land_cover/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLandCover.element_code_id)))
        .select_from(EnvironmentLandCover)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/environment_land_cover/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EnvironmentLandCover.flag_id)))
        .select_from(EnvironmentLandCover)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/environment_land_cover/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EnvironmentLandCover.year).label('min_year'),
            func.max(EnvironmentLandCover.year).label('max_year'),
            func.count(func.distinct(EnvironmentLandCover.year)).label('year_count')
        )
        .select_from(EnvironmentLandCover)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/environment_land_cover/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EnvironmentLandCover.value).label('min_value'),
            func.max(EnvironmentLandCover.value).label('max_value'),
            func.avg(EnvironmentLandCover.value).label('avg_value')
        )
        .select_from(EnvironmentLandCover)
        .where(and_(EnvironmentLandCover.value > 0, EnvironmentLandCover.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/environment_land_cover",
        "aggregate": f"/environment_land_cover/aggregate",
        "summary": f"/environment_land_cover/summary",
        "overview": f"/environment_land_cover/overview",
        "area_codes": f"/environment_land_cover/area_codes",
        "item_codes": f"/environment_land_cover/item_codes",
        "elements": f"/environment_land_cover/elements",
        "flags": f"/environment_land_cover/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the environment_land_cover endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EnvironmentLandCover)).scalar()
        return {
            "status": "healthy",
            "dataset": "environment_land_cover",
            "records": result
