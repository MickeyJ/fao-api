...
@router.get("/")
@cache_result(prefix="environment_land_cover", ttl=86400, exclude_params=["response", "db"])
def get_environment_land_cover(
    ...
    """
    Get environment land cover data with filters and pagination.
    ...
    # Main table columns
    for col in EnvironmentLandCover.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EnvironmentLandCover, 'id'):
                columns.append(EnvironmentLandCover.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EnvironmentLandCover)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EnvironmentLandCover.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EnvironmentLandCover.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EnvironmentLandCover.element_code_id == Elements.id)
    query = query.outerjoin(Flags, EnvironmentLandCover.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EnvironmentLandCover.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EnvironmentLandCover.year_code == year_code_exact)
    if year is not None:
        conditions.append(EnvironmentLandCover.year == year)
    if year_min is not None:
        conditions.append(EnvironmentLandCover.year >= year_min)
    if year_max is not None:
        conditions.append(EnvironmentLandCover.year <= year_max)
    if unit is not None:
        conditions.append(EnvironmentLandCover.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EnvironmentLandCover.unit == unit_exact)
    if value is not None:
        conditions.append(EnvironmentLandCover.value == value)
    if value_min is not None:
        conditions.append(EnvironmentLandCover.value >= value_min)
    if value_max is not None:
        conditions.append(EnvironmentLandCover.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EnvironmentLandCover, field_name):
                        order_by_clauses.append(getattr(EnvironmentLandCover, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EnvironmentLandCover, sort_field):
                        order_by_clauses.append(getattr(EnvironmentLandCover, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EnvironmentLandCover.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EnvironmentLandCover)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EnvironmentLandCover.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EnvironmentLandCover.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EnvironmentLandCover.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EnvironmentLandCover.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_environment_land_cover'))
...
@router.get("/areas")
@cache_result(prefix="environment_land_cover:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="environment_land_cover:items", ttl=604800)
...
@router.get("/summary")
@cache_result(prefix="environment_land_cover:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    summary["unique_flags"] = db.query(func.count(func.distinct(EnvironmentLandCover.flag_id))).scalar()
