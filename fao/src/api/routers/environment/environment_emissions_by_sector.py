...
router = APIRouter(
    prefix="/environment_emissions_by_sector",
    tags=["environment_emissions_by_sector"],
    responses={404: {"description": "Not found"}},
)

@router.get("/")
def get_environment_emissions_by_sector(
    ...
    """
    environment emissions by sector data with filters.
    Filter options:
    ...
    query = (
        ...
        .select_from(EnvironmentEmissionsBySector)
        .outerjoin(AreaCodes, EnvironmentEmissionsBySector.area_code_id == AreaCodes.id)
        .outerjoin(ItemCodes, EnvironmentEmissionsBySector.item_code_id == ItemCodes.id)
        .outerjoin(Elements, EnvironmentEmissionsBySector.element_code_id == Elements.id)
        .outerjoin(Flags, EnvironmentEmissionsBySector.flag_id == Flags.id)
    )
...
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            AreaCodes.area_code,
            AreaCodes.area,
            func.count(EnvironmentEmissionsBySector.id).label('record_count')
        )
        .join(EnvironmentEmissionsBySector, AreaCodes.id == EnvironmentEmissionsBySector.area_code_id)
        .group_by(AreaCodes.area_code, AreaCodes.area)
        .order_by(func.count(EnvironmentEmissionsBySector.id).desc())
    )
    
    results = db.execute(query).all()
    
    return {
        "dataset": "environment_emissions_by_sector",
...
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentEmissionsBySector, ItemCodes.id == EnvironmentEmissionsBySector.item_code_id)
...
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentEmissionsBySector, Elements.id == EnvironmentEmissionsBySector.element_code_id)
...
def get_data_quality_summary(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(EnvironmentEmissionsBySector, Flags.id == EnvironmentEmissionsBySector.flag_id)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            EnvironmentEmissionsBySector.year,
            func.count(EnvironmentEmissionsBySector.id).label('record_count')
        )
        .group_by(EnvironmentEmissionsBySector.year)
        .order_by(EnvironmentEmissionsBySector.year)
    )
    ...
    if not years_data:
        return {"dataset": "environment_emissions_by_sector", "message": "No temporal data available"}
...
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    total_records = db.query(func.count(EnvironmentEmissionsBySector.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EnvironmentEmissionsBySector.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EnvironmentEmissionsBySector.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EnvironmentEmissionsBySector.element_code_id))).scalar()
