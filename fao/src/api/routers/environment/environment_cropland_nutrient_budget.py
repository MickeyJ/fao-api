...
@router.get("/")
@cache_result(prefix="environment_cropland_nutrient_budget", ttl=86400, exclude_params=["response", "db"])
def get_environment_cropland_nutrient_budget(
    response: Response,
    ...
    """
    Get environment cropland nutrient budget data with filters and pagination.
    ...
    # Main table columns
    for col in EnvironmentCroplandNutrientBudget.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EnvironmentCroplandNutrientBudget, 'id'):
                columns.append(EnvironmentCroplandNutrientBudget.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EnvironmentCroplandNutrientBudget)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EnvironmentCroplandNutrientBudget.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EnvironmentCroplandNutrientBudget.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EnvironmentCroplandNutrientBudget.element_code_id == Elements.id)
    query = query.outerjoin(Flags, EnvironmentCroplandNutrientBudget.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.year_code == year_code_exact)
    if year is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.year == year)
    if year_min is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.year >= year_min)
    if year_max is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.year <= year_max)
    if unit is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.unit == unit_exact)
    if value is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.value == value)
    if value_min is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.value >= value_min)
    if value_max is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.value <= value_max)
    if note is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EnvironmentCroplandNutrientBudget.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EnvironmentCroplandNutrientBudget, field_name):
                        order_by_clauses.append(getattr(EnvironmentCroplandNutrientBudget, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EnvironmentCroplandNutrientBudget, sort_field):
                        order_by_clauses.append(getattr(EnvironmentCroplandNutrientBudget, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EnvironmentCroplandNutrientBudget.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EnvironmentCroplandNutrientBudget)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EnvironmentCroplandNutrientBudget.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EnvironmentCroplandNutrientBudget.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EnvironmentCroplandNutrientBudget.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EnvironmentCroplandNutrientBudget.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_environment_cropland_nutrient_budget'))
...
@router.get("/areas")
@cache_result(prefix="environment_cropland_nutrient_budget:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="environment_cropland_nutrient_budget:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EnvironmentCroplandNutrientBudget.year)
    ...
    if not years_data:
        return {"dataset": "environment_cropland_nutrient_budget", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="environment_cropland_nutrient_budget:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EnvironmentCroplandNutrientBudget.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EnvironmentCroplandNutrientBudget.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EnvironmentCroplandNutrientBudget.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EnvironmentCroplandNutrientBudget.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EnvironmentCroplandNutrientBudget.flag_id))).scalar()
