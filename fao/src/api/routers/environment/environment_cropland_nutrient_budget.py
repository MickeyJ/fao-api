...
from .environment_cropland_nutrient_budget_config import EnvironmentCroplandNutrientBudgetConfig
...
router = APIRouter(
    prefix="/environment_cropland_nutrient_budget",
    responses={404: {"description": "Not found"}},
)


config = EnvironmentCroplandNutrientBudgetConfig()

@router.get("/", summary="Get environment cropland nutrient budget data")
async def get_environment_cropland_nutrient_budget_data(
    request: Request,
    response: Response,
    ...
    """Get environment cropland nutrient budget data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EnvironmentCroplandNutrientBudget, 
        model_name="EnvironmentCroplandNutrientBudget",
        table_name="environment_cropland_nutrient_budget",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated environment cropland nutrient budget data")
async def get_environment_cropland_nutrient_budget_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EnvironmentCroplandNutrientBudget))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EnvironmentCroplandNutrientBudget, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EnvironmentCroplandNutrientBudget, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EnvironmentCroplandNutrientBudget, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in environment_cropland_nutrient_budget")
@cache_result(prefix="environment_cropland_nutrient_budget:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(EnvironmentCroplandNutrientBudget.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(EnvironmentCroplandNutrientBudget, EnvironmentCroplandNutrientBudget.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'environment_cropland_nutrient_budget')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="environment_cropland_nutrient_budget",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in environment_cropland_nutrient_budget")
@cache_result(prefix="environment_cropland_nutrient_budget:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EnvironmentCroplandNutrientBudget, EnvironmentCroplandNutrientBudget.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'environment_cropland_nutrient_budget')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in environment_cropland_nutrient_budget")
@cache_result(prefix="environment_cropland_nutrient_budget:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EnvironmentCroplandNutrientBudget, EnvironmentCroplandNutrientBudget.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'environment_cropland_nutrient_budget')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in environment_cropland_nutrient_budget")
@cache_result(prefix="environment_cropland_nutrient_budget:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EnvironmentCroplandNutrientBudget, Flags.id == EnvironmentCroplandNutrientBudget.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EnvironmentCroplandNutrientBudget.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EnvironmentCroplandNutrientBudget)
                .where(EnvironmentCroplandNutrientBudget.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EnvironmentCroplandNutrientBudget)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in environment_cropland_nutrient_budget")
@cache_result(prefix="environment_cropland_nutrient_budget:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EnvironmentCroplandNutrientBudget.unit,
            func.count(EnvironmentCroplandNutrientBudget.id).label('record_count')
        )
        .select_from(EnvironmentCroplandNutrientBudget)
        .group_by(EnvironmentCroplandNutrientBudget.unit)
        .order_by(EnvironmentCroplandNutrientBudget.unit)
    )
...
@router.get("/years", summary="Get available years in environment_cropland_nutrient_budget")
@cache_result(prefix="environment_cropland_nutrient_budget:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EnvironmentCroplandNutrientBudget.year,
                EnvironmentCroplandNutrientBudget.year_code,
                func.count(EnvironmentCroplandNutrientBudget.id).label('record_count')
            )
            .group_by(EnvironmentCroplandNutrientBudget.year, EnvironmentCroplandNutrientBudget.year_code)
            .order_by(EnvironmentCroplandNutrientBudget.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EnvironmentCroplandNutrientBudget.year)
            .distinct()
            .order_by(EnvironmentCroplandNutrientBudget.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of environment_cropland_nutrient_budget dataset")
@cache_result(prefix="environment_cropland_nutrient_budget:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentCroplandNutrientBudget.area_code_id)))
        .select_from(EnvironmentCroplandNutrientBudget)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/environment_cropland_nutrient_budget/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentCroplandNutrientBudget.item_code_id)))
        .select_from(EnvironmentCroplandNutrientBudget)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/environment_cropland_nutrient_budget/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EnvironmentCroplandNutrientBudget.element_code_id)))
        .select_from(EnvironmentCroplandNutrientBudget)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/environment_cropland_nutrient_budget/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EnvironmentCroplandNutrientBudget.flag_id)))
        .select_from(EnvironmentCroplandNutrientBudget)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/environment_cropland_nutrient_budget/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EnvironmentCroplandNutrientBudget.year).label('min_year'),
            func.max(EnvironmentCroplandNutrientBudget.year).label('max_year'),
            func.count(func.distinct(EnvironmentCroplandNutrientBudget.year)).label('year_count')
        )
        .select_from(EnvironmentCroplandNutrientBudget)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/environment_cropland_nutrient_budget/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EnvironmentCroplandNutrientBudget.value).label('min_value'),
            func.max(EnvironmentCroplandNutrientBudget.value).label('max_value'),
            func.avg(EnvironmentCroplandNutrientBudget.value).label('avg_value')
        )
        .select_from(EnvironmentCroplandNutrientBudget)
        .where(and_(EnvironmentCroplandNutrientBudget.value > 0, EnvironmentCroplandNutrientBudget.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/environment_cropland_nutrient_budget",
        "aggregate": f"/environment_cropland_nutrient_budget/aggregate",
        "summary": f"/environment_cropland_nutrient_budget/summary",
        "overview": f"/environment_cropland_nutrient_budget/overview",
        "area_codes": f"/environment_cropland_nutrient_budget/area_codes",
        "item_codes": f"/environment_cropland_nutrient_budget/item_codes",
        "elements": f"/environment_cropland_nutrient_budget/elements",
        "flags": f"/environment_cropland_nutrient_budget/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the environment_cropland_nutrient_budget endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EnvironmentCroplandNutrientBudget)).scalar()
        return {
            "status": "healthy",
            "dataset": "environment_cropland_nutrient_budget",
            "records": result
