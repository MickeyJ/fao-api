...
from .asti_researchers_config import AstiResearchersConfig
...
router = APIRouter(
    prefix="/asti_researchers",
    responses={404: {"description": "Not found"}},
)


config = AstiResearchersConfig()

@router.get("/", summary="Get asti researchers data")
async def get_asti_researchers_data(
    request: Request,
    response: Response,
    ...
    """Get asti researchers data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=AstiResearchers, 
        model_name="AstiResearchers",
        table_name="asti_researchers",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated asti researchers data")
async def get_asti_researchers_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(AstiResearchers))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in asti_researchers")
@cache_result(prefix="asti_researchers:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(AstiResearchers.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(AstiResearchers, AstiResearchers.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'asti_researchers')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in asti_researchers")
@cache_result(prefix="asti_researchers:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(AstiResearchers, AstiResearchers.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'asti_researchers')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in asti_researchers")
@cache_result(prefix="asti_researchers:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(AstiResearchers, AstiResearchers.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'asti_researchers')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in asti_researchers")
@cache_result(prefix="asti_researchers:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(AstiResearchers, Flags.id == AstiResearchers.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(AstiResearchers.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(AstiResearchers)
                .where(AstiResearchers.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(AstiResearchers)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in asti_researchers")
@cache_result(prefix="asti_researchers:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            AstiResearchers.unit,
            func.count(AstiResearchers.id).label('record_count')
        )
        .select_from(AstiResearchers)
        .group_by(AstiResearchers.unit)
        .order_by(AstiResearchers.unit)
    )
...
@router.get("/years", summary="Get available years in asti_researchers")
@cache_result(prefix="asti_researchers:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                AstiResearchers.year,
                AstiResearchers.year_code,
                func.count(AstiResearchers.id).label('record_count')
            )
            .group_by(AstiResearchers.year, AstiResearchers.year_code)
            .order_by(AstiResearchers.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(AstiResearchers.year)
            .distinct()
            .order_by(AstiResearchers.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of asti_researchers dataset")
@cache_result(prefix="asti_researchers:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(AstiResearchers.area_code_id)))
        .select_from(AstiResearchers)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/asti_researchers/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(AstiResearchers.item_code_id)))
        .select_from(AstiResearchers)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/asti_researchers/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(AstiResearchers.element_code_id)))
        .select_from(AstiResearchers)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/asti_researchers/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(AstiResearchers.flag_id)))
        .select_from(AstiResearchers)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/asti_researchers/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(AstiResearchers.year).label('min_year'),
            func.max(AstiResearchers.year).label('max_year'),
            func.count(func.distinct(AstiResearchers.year)).label('year_count')
        )
        .select_from(AstiResearchers)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/asti_researchers/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(AstiResearchers.value).label('min_value'),
            func.max(AstiResearchers.value).label('max_value'),
            func.avg(AstiResearchers.value).label('avg_value')
        )
        .select_from(AstiResearchers)
        .where(and_(AstiResearchers.value > 0, AstiResearchers.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/asti_researchers",
        "aggregate": f"/asti_researchers/aggregate",
        "summary": f"/asti_researchers/summary",
        "overview": f"/asti_researchers/overview",
        "area_codes": f"/asti_researchers/area_codes",
        "item_codes": f"/asti_researchers/item_codes",
        "elements": f"/asti_researchers/elements",
        "flags": f"/asti_researchers/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the asti_researchers endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(AstiResearchers)).scalar()
        return {
            "status": "healthy",
            "dataset": "asti_researchers",
            "records": result
