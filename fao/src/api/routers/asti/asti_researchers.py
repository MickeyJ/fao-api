...
@router.get("/")
@cache_result(prefix="asti_researchers", ttl=86400, exclude_params=["response", "db"])
def get_asti_researchers(
    response: Response,
    ...
    """
    Get asti researchers data with filters and pagination.
    ...
    # Main table columns
    for col in AstiResearchers.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(AstiResearchers, 'id'):
                columns.append(AstiResearchers.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(AstiResearchers)
    
    # Add joins
    query = query.outerjoin(AreaCodes, AstiResearchers.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, AstiResearchers.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, AstiResearchers.element_code_id == Elements.id)
    query = query.outerjoin(Flags, AstiResearchers.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(AstiResearchers.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(AstiResearchers.year_code == year_code_exact)
    if year is not None:
        conditions.append(AstiResearchers.year == year)
    if year_min is not None:
        conditions.append(AstiResearchers.year >= year_min)
    if year_max is not None:
        conditions.append(AstiResearchers.year <= year_max)
    if unit is not None:
        conditions.append(AstiResearchers.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(AstiResearchers.unit == unit_exact)
    if value is not None:
        conditions.append(AstiResearchers.value == value)
    if value_min is not None:
        conditions.append(AstiResearchers.value >= value_min)
    if value_max is not None:
        conditions.append(AstiResearchers.value <= value_max)
    if note is not None:
        conditions.append(AstiResearchers.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(AstiResearchers.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(AstiResearchers, field_name):
                        order_by_clauses.append(getattr(AstiResearchers, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(AstiResearchers, sort_field):
                        order_by_clauses.append(getattr(AstiResearchers, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(AstiResearchers.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(AstiResearchers)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, AstiResearchers.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, AstiResearchers.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, AstiResearchers.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, AstiResearchers.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_asti_researchers'))
...
@router.get("/areas")
@cache_result(prefix="asti_researchers:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="asti_researchers:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    if not years_data:
        return {"dataset": "asti_researchers", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="asti_researchers:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(AstiResearchers.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(AstiResearchers.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(AstiResearchers.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(AstiResearchers.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(AstiResearchers.flag_id))).scalar()
