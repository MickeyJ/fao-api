...
@router.get("/")
@cache_result(prefix="asti_expenditures", ttl=86400, exclude_params=["response", "db"])
def get_asti_expenditures(
    response: Response,
    ...
    """
    Get asti expenditures data with filters and pagination.
    ...
    # Main table columns
    for col in AstiExpenditures.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(AstiExpenditures, 'id'):
                columns.append(AstiExpenditures.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(AstiExpenditures)
    
    # Add joins
    query = query.outerjoin(AreaCodes, AstiExpenditures.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, AstiExpenditures.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, AstiExpenditures.element_code_id == Elements.id)
    query = query.outerjoin(Flags, AstiExpenditures.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(AstiExpenditures.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(AstiExpenditures.year_code == year_code_exact)
    if year is not None:
        conditions.append(AstiExpenditures.year == year)
    if year_min is not None:
        conditions.append(AstiExpenditures.year >= year_min)
    if year_max is not None:
        conditions.append(AstiExpenditures.year <= year_max)
    if unit is not None:
        conditions.append(AstiExpenditures.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(AstiExpenditures.unit == unit_exact)
    if value is not None:
        conditions.append(AstiExpenditures.value == value)
    if value_min is not None:
        conditions.append(AstiExpenditures.value >= value_min)
    if value_max is not None:
        conditions.append(AstiExpenditures.value <= value_max)
    if note is not None:
        conditions.append(AstiExpenditures.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(AstiExpenditures.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(AstiExpenditures, field_name):
                        order_by_clauses.append(getattr(AstiExpenditures, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(AstiExpenditures, sort_field):
                        order_by_clauses.append(getattr(AstiExpenditures, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(AstiExpenditures.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(AstiExpenditures)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, AstiExpenditures.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, AstiExpenditures.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, AstiExpenditures.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, AstiExpenditures.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_asti_expenditures'))
...
@router.get("/areas")
@cache_result(prefix="asti_expenditures:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="asti_expenditures:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(AstiExpenditures.year)
    ...
    if not years_data:
        return {"dataset": "asti_expenditures", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="asti_expenditures:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(AstiExpenditures.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(AstiExpenditures.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(AstiExpenditures.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(AstiExpenditures.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(AstiExpenditures.flag_id))).scalar()
