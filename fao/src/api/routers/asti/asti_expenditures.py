...
from .asti_expenditures_config import AstiExpendituresConfig
...
router = APIRouter(
    prefix="/asti_expenditures",
    responses={404: {"description": "Not found"}},
)


config = AstiExpendituresConfig()

@router.get("/", summary="Get asti expenditures data")
async def get_asti_expenditures_data(
    request: Request,
    response: Response,
    ...
    """Get asti expenditures data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=AstiExpenditures, 
        model_name="AstiExpenditures",
        table_name="asti_expenditures",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated asti expenditures data")
async def get_asti_expenditures_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(AstiExpenditures))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in asti_expenditures")
@cache_result(prefix="asti_expenditures:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(AstiExpenditures.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(AstiExpenditures, AstiExpenditures.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'asti_expenditures')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in asti_expenditures")
@cache_result(prefix="asti_expenditures:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(AstiExpenditures, AstiExpenditures.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'asti_expenditures')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in asti_expenditures")
@cache_result(prefix="asti_expenditures:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(AstiExpenditures, AstiExpenditures.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'asti_expenditures')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in asti_expenditures")
@cache_result(prefix="asti_expenditures:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(AstiExpenditures, Flags.id == AstiExpenditures.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(AstiExpenditures.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(AstiExpenditures)
                .where(AstiExpenditures.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(AstiExpenditures)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in asti_expenditures")
@cache_result(prefix="asti_expenditures:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            AstiExpenditures.unit,
            func.count(AstiExpenditures.id).label('record_count')
        )
        .select_from(AstiExpenditures)
        .group_by(AstiExpenditures.unit)
        .order_by(AstiExpenditures.unit)
    )
...
@router.get("/years", summary="Get available years in asti_expenditures")
@cache_result(prefix="asti_expenditures:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                AstiExpenditures.year,
                AstiExpenditures.year_code,
                func.count(AstiExpenditures.id).label('record_count')
            )
            .group_by(AstiExpenditures.year, AstiExpenditures.year_code)
            .order_by(AstiExpenditures.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(AstiExpenditures.year)
            .distinct()
            .order_by(AstiExpenditures.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of asti_expenditures dataset")
@cache_result(prefix="asti_expenditures:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(AstiExpenditures.area_code_id)))
        .select_from(AstiExpenditures)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/asti_expenditures/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(AstiExpenditures.item_code_id)))
        .select_from(AstiExpenditures)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/asti_expenditures/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(AstiExpenditures.element_code_id)))
        .select_from(AstiExpenditures)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/asti_expenditures/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(AstiExpenditures.flag_id)))
        .select_from(AstiExpenditures)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/asti_expenditures/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(AstiExpenditures.year).label('min_year'),
            func.max(AstiExpenditures.year).label('max_year'),
            func.count(func.distinct(AstiExpenditures.year)).label('year_count')
        )
        .select_from(AstiExpenditures)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/asti_expenditures/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(AstiExpenditures.value).label('min_value'),
            func.max(AstiExpenditures.value).label('max_value'),
            func.avg(AstiExpenditures.value).label('avg_value')
        )
        .select_from(AstiExpenditures)
        .where(and_(AstiExpenditures.value > 0, AstiExpenditures.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/asti_expenditures",
        "aggregate": f"/asti_expenditures/aggregate",
        "summary": f"/asti_expenditures/summary",
        "overview": f"/asti_expenditures/overview",
        "area_codes": f"/asti_expenditures/area_codes",
        "item_codes": f"/asti_expenditures/item_codes",
        "elements": f"/asti_expenditures/elements",
        "flags": f"/asti_expenditures/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the asti_expenditures endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(AstiExpenditures)).scalar()
        return {
            "status": "healthy",
            "dataset": "asti_expenditures",
            "records": result
