...
@router.get("/")
@cache_result(prefix="trade_crops_livestock", ttl=86400, exclude_params=["response", "db"])
def get_trade_crops_livestock(
    response: Response,
    ...
    """
    Get trade crops livestock data with filters and pagination.
    ...
    # Main table columns
    for col in TradeCropsLivestock.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(TradeCropsLivestock, 'id'):
                columns.append(TradeCropsLivestock.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(TradeCropsLivestock)
    
    # Add joins
    query = query.outerjoin(AreaCodes, TradeCropsLivestock.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, TradeCropsLivestock.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, TradeCropsLivestock.element_code_id == Elements.id)
    query = query.outerjoin(Flags, TradeCropsLivestock.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(TradeCropsLivestock.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(TradeCropsLivestock.year_code == year_code_exact)
    if year is not None:
        conditions.append(TradeCropsLivestock.year == year)
    if year_min is not None:
        conditions.append(TradeCropsLivestock.year >= year_min)
    if year_max is not None:
        conditions.append(TradeCropsLivestock.year <= year_max)
    if unit is not None:
        conditions.append(TradeCropsLivestock.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(TradeCropsLivestock.unit == unit_exact)
    if value is not None:
        conditions.append(TradeCropsLivestock.value == value)
    if value_min is not None:
        conditions.append(TradeCropsLivestock.value >= value_min)
    if value_max is not None:
        conditions.append(TradeCropsLivestock.value <= value_max)
    if note is not None:
        conditions.append(TradeCropsLivestock.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(TradeCropsLivestock.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(TradeCropsLivestock, field_name):
                        order_by_clauses.append(getattr(TradeCropsLivestock, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(TradeCropsLivestock, sort_field):
                        order_by_clauses.append(getattr(TradeCropsLivestock, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(TradeCropsLivestock.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(TradeCropsLivestock)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, TradeCropsLivestock.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, TradeCropsLivestock.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, TradeCropsLivestock.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, TradeCropsLivestock.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_trade_crops_livestock'))
...
@router.get("/areas")
@cache_result(prefix="trade_crops_livestock:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="trade_crops_livestock:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(TradeCropsLivestock.year)
    ...
    if not years_data:
        return {"dataset": "trade_crops_livestock", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="trade_crops_livestock:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(TradeCropsLivestock.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(TradeCropsLivestock.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(TradeCropsLivestock.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(TradeCropsLivestock.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(TradeCropsLivestock.flag_id))).scalar()
