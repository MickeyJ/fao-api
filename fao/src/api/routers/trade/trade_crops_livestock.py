...
from fao.src.db.pipelines.trade_crops_livestock.trade_crops_livestock_model import TradeCropsLivestock
...
from .trade_crops_livestock_config import TradeCropsLivestockConfig
...
router = APIRouter(
    prefix="/trade_crops_livestock",
    responses={404: {"description": "Not found"}},
)


config = TradeCropsLivestockConfig()

@router.get("/", summary="Get trade crops livestock data")
async def get_trade_crops_livestock_data(
    request: Request,
    response: Response,
    ...
    """Get trade crops livestock data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=TradeCropsLivestock, 
        model_name="TradeCropsLivestock",
        table_name="trade_crops_livestock",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated trade crops livestock data")
async def get_trade_crops_livestock_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(TradeCropsLivestock))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in trade_crops_livestock")
@cache_result(prefix="trade_crops_livestock:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(TradeCropsLivestock.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(TradeCropsLivestock, TradeCropsLivestock.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'trade_crops_livestock')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in trade_crops_livestock")
@cache_result(prefix="trade_crops_livestock:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(TradeCropsLivestock, TradeCropsLivestock.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'trade_crops_livestock')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in trade_crops_livestock")
@cache_result(prefix="trade_crops_livestock:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(TradeCropsLivestock, TradeCropsLivestock.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'trade_crops_livestock')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in trade_crops_livestock")
@cache_result(prefix="trade_crops_livestock:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(TradeCropsLivestock, Flags.id == TradeCropsLivestock.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(TradeCropsLivestock.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(TradeCropsLivestock)
                .where(TradeCropsLivestock.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(TradeCropsLivestock)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in trade_crops_livestock")
@cache_result(prefix="trade_crops_livestock:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            TradeCropsLivestock.unit,
            func.count(TradeCropsLivestock.id).label('record_count')
        )
        .select_from(TradeCropsLivestock)
        .group_by(TradeCropsLivestock.unit)
        .order_by(TradeCropsLivestock.unit)
    )
...
@router.get("/years", summary="Get available years in trade_crops_livestock")
@cache_result(prefix="trade_crops_livestock:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                TradeCropsLivestock.year,
                TradeCropsLivestock.year_code,
                func.count(TradeCropsLivestock.id).label('record_count')
            )
            .group_by(TradeCropsLivestock.year, TradeCropsLivestock.year_code)
            .order_by(TradeCropsLivestock.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(TradeCropsLivestock.year)
            .distinct()
            .order_by(TradeCropsLivestock.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of trade_crops_livestock dataset")
@cache_result(prefix="trade_crops_livestock:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(TradeCropsLivestock.area_code_id)))
        .select_from(TradeCropsLivestock)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/trade_crops_livestock/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(TradeCropsLivestock.item_code_id)))
        .select_from(TradeCropsLivestock)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/trade_crops_livestock/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(TradeCropsLivestock.element_code_id)))
        .select_from(TradeCropsLivestock)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/trade_crops_livestock/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(TradeCropsLivestock.flag_id)))
        .select_from(TradeCropsLivestock)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/trade_crops_livestock/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(TradeCropsLivestock.year).label('min_year'),
            func.max(TradeCropsLivestock.year).label('max_year'),
            func.count(func.distinct(TradeCropsLivestock.year)).label('year_count')
        )
        .select_from(TradeCropsLivestock)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/trade_crops_livestock/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(TradeCropsLivestock.value).label('min_value'),
            func.max(TradeCropsLivestock.value).label('max_value'),
            func.avg(TradeCropsLivestock.value).label('avg_value')
        )
        .select_from(TradeCropsLivestock)
        .where(and_(TradeCropsLivestock.value > 0, TradeCropsLivestock.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/trade_crops_livestock",
        "aggregate": f"/trade_crops_livestock/aggregate",
        "summary": f"/trade_crops_livestock/summary",
        "overview": f"/trade_crops_livestock/overview",
        "area_codes": f"/trade_crops_livestock/area_codes",
        "item_codes": f"/trade_crops_livestock/item_codes",
        "elements": f"/trade_crops_livestock/elements",
        "flags": f"/trade_crops_livestock/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the trade_crops_livestock endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(TradeCropsLivestock)).scalar()
        return {
            "status": "healthy",
            "dataset": "trade_crops_livestock",
            "records": result
