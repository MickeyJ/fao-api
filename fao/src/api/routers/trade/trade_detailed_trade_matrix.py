...
@router.get("/")
@cache_result(prefix="trade_detailed_trade_matrix", ttl=86400, exclude_params=["response", "db"])
def get_trade_detailed_trade_matrix(
    response: Response,
    ...
    """
    Get trade detailed trade matrix data with filters and pagination.
    ...
    # Main table columns
    for col in TradeDetailedTradeMatrix.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(TradeDetailedTradeMatrix, 'id'):
                columns.append(TradeDetailedTradeMatrix.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(TradeDetailedTradeMatrix)
    
    # Add joins
    query = query.outerjoin(ReporterCountryCodes, TradeDetailedTradeMatrix.reporter_country_code_id == ReporterCountryCodes.id)
    query = query.outerjoin(PartnerCountryCodes, TradeDetailedTradeMatrix.partner_country_code_id == PartnerCountryCodes.id)
    query = query.outerjoin(ItemCodes, TradeDetailedTradeMatrix.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, TradeDetailedTradeMatrix.element_code_id == Elements.id)
    query = query.outerjoin(Flags, TradeDetailedTradeMatrix.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(TradeDetailedTradeMatrix.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(TradeDetailedTradeMatrix.year_code == year_code_exact)
    if year is not None:
        conditions.append(TradeDetailedTradeMatrix.year == year)
    if year_min is not None:
        conditions.append(TradeDetailedTradeMatrix.year >= year_min)
    if year_max is not None:
        conditions.append(TradeDetailedTradeMatrix.year <= year_max)
    if unit is not None:
        conditions.append(TradeDetailedTradeMatrix.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(TradeDetailedTradeMatrix.unit == unit_exact)
    if value is not None:
        conditions.append(TradeDetailedTradeMatrix.value == value)
    if value_min is not None:
        conditions.append(TradeDetailedTradeMatrix.value >= value_min)
    if value_max is not None:
        conditions.append(TradeDetailedTradeMatrix.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(TradeDetailedTradeMatrix, field_name):
                        order_by_clauses.append(getattr(TradeDetailedTradeMatrix, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(TradeDetailedTradeMatrix, sort_field):
                        order_by_clauses.append(getattr(TradeDetailedTradeMatrix, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(TradeDetailedTradeMatrix.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(TradeDetailedTradeMatrix)
    
    # Add joins to count query
    count_query = count_query.outerjoin(ReporterCountryCodes, TradeDetailedTradeMatrix.reporter_country_code_id == ReporterCountryCodes.id)
    count_query = count_query.outerjoin(PartnerCountryCodes, TradeDetailedTradeMatrix.partner_country_code_id == PartnerCountryCodes.id)
    count_query = count_query.outerjoin(ItemCodes, TradeDetailedTradeMatrix.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, TradeDetailedTradeMatrix.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, TradeDetailedTradeMatrix.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_trade_detailed_trade_matrix'))
...
@cache_result(prefix="trade_detailed_trade_matrix:reporter_countries", ttl=604800)
...
@cache_result(prefix="trade_detailed_trade_matrix:partner_countries", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="trade_detailed_trade_matrix:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(TradeDetailedTradeMatrix.year)
    ...
    if not years_data:
        return {"dataset": "trade_detailed_trade_matrix", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="trade_detailed_trade_matrix:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(TradeDetailedTradeMatrix.id)).scalar()
    ...
    summary["unique_reporter_countries"] = db.query(func.count(func.distinct(TradeDetailedTradeMatrix.reporter_country_code_id))).scalar()
    summary["unique_partner_countries"] = db.query(func.count(func.distinct(TradeDetailedTradeMatrix.partner_country_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(TradeDetailedTradeMatrix.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(TradeDetailedTradeMatrix.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(TradeDetailedTradeMatrix.flag_id))).scalar()
