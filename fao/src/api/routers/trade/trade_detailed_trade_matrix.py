...
from fao.src.db.pipelines.trade_detailed_trade_matrix.trade_detailed_trade_matrix_model import TradeDetailedTradeMatrix
...
from .trade_detailed_trade_matrix_config import TradeDetailedTradeMatrixConfig
...
router = APIRouter(
    prefix="/trade_detailed_trade_matrix",
    responses={404: {"description": "Not found"}},
)


config = TradeDetailedTradeMatrixConfig()

@router.get("/", summary="Get trade detailed trade matrix data")
async def get_trade_detailed_trade_matrix_data(
    request: Request,
    response: Response,
    ...
    """Get trade detailed trade matrix data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=TradeDetailedTradeMatrix, 
        model_name="TradeDetailedTradeMatrix",
        table_name="trade_detailed_trade_matrix",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated trade detailed trade matrix data")
async def get_trade_detailed_trade_matrix_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(TradeDetailedTradeMatrix))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(TradeDetailedTradeMatrix, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(TradeDetailedTradeMatrix, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(TradeDetailedTradeMatrix, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/reporter_country_codes", summary="Get reporter_country_codes in trade_detailed_trade_matrix")
@cache_result(prefix="trade_detailed_trade_matrix:reporter_country_codes", ttl=604800)
async def get_available_reporter_country_codes(
    ...
    query = (
        select(
            ...
            func.count(TradeDetailedTradeMatrix.id).label('record_count')
        )
        .select_from(ReporterCountryCodes)
        .join(TradeDetailedTradeMatrix, 
              TradeDetailedTradeMatrix.reporter_country_code == ReporterCountryCodes.reporter_country_code)
        .where(ReporterCountryCodes.source_dataset == 'trade_detailed_trade_matrix')
        .group_by(
            ReporterCountryCodes.reporter_country_code,
            ReporterCountryCodes.reporter_countries
        )
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="trade_detailed_trade_matrix",
...
@router.get("/partner_country_codes", summary="Get partner_country_codes in trade_detailed_trade_matrix")
@cache_result(prefix="trade_detailed_trade_matrix:partner_country_codes", ttl=604800)
async def get_available_partner_country_codes(
    ...
    query = (
        ...
        .join(TradeDetailedTradeMatrix, 
              TradeDetailedTradeMatrix.partner_country_code_id == PartnerCountryCodes.id)
        .where(PartnerCountryCodes.source_dataset == 'trade_detailed_trade_matrix')
    )
    ...
    if reporter_country_code:
        query = query.join(
            ...
            TradeDetailedTradeMatrix.reporter_country_code_id == ReporterCountryCodes.id
        ).where(
...
@router.get("/item_codes", summary="Get item_codes in trade_detailed_trade_matrix")
@cache_result(prefix="trade_detailed_trade_matrix:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(TradeDetailedTradeMatrix, TradeDetailedTradeMatrix.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'trade_detailed_trade_matrix')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in trade_detailed_trade_matrix")
@cache_result(prefix="trade_detailed_trade_matrix:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(TradeDetailedTradeMatrix, TradeDetailedTradeMatrix.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'trade_detailed_trade_matrix')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in trade_detailed_trade_matrix")
@cache_result(prefix="trade_detailed_trade_matrix:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(TradeDetailedTradeMatrix, Flags.id == TradeDetailedTradeMatrix.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(TradeDetailedTradeMatrix.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(TradeDetailedTradeMatrix)
                .where(TradeDetailedTradeMatrix.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(TradeDetailedTradeMatrix)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in trade_detailed_trade_matrix")
@cache_result(prefix="trade_detailed_trade_matrix:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            TradeDetailedTradeMatrix.unit,
            func.count(TradeDetailedTradeMatrix.id).label('record_count')
        )
        .select_from(TradeDetailedTradeMatrix)
        .group_by(TradeDetailedTradeMatrix.unit)
        .order_by(TradeDetailedTradeMatrix.unit)
    )
...
@router.get("/years", summary="Get available years in trade_detailed_trade_matrix")
@cache_result(prefix="trade_detailed_trade_matrix:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                TradeDetailedTradeMatrix.year,
                TradeDetailedTradeMatrix.year_code,
                func.count(TradeDetailedTradeMatrix.id).label('record_count')
            )
            .group_by(TradeDetailedTradeMatrix.year, TradeDetailedTradeMatrix.year_code)
            .order_by(TradeDetailedTradeMatrix.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(TradeDetailedTradeMatrix.year)
            .distinct()
            .order_by(TradeDetailedTradeMatrix.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of trade_detailed_trade_matrix dataset")
@cache_result(prefix="trade_detailed_trade_matrix:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    reporter_country_code_ids = db.execute(
        select(func.count(func.distinct(TradeDetailedTradeMatrix.reporter_country_code_id)))
        .select_from(TradeDetailedTradeMatrix)
    ).scalar() or 0

    overview["dimensions"]["reporter_country_codes"] = {
        "count": reporter_country_code_ids,
        "endpoint": f"/trade_detailed_trade_matrix/reporter_country_codes"
    }

    partner_country_code_ids = db.execute(
        select(func.count(func.distinct(TradeDetailedTradeMatrix.partner_country_code_id)))
        .select_from(TradeDetailedTradeMatrix)
    ).scalar() or 0

    overview["dimensions"]["partner_country_codes"] = {
        "count": partner_country_code_ids,
        "endpoint": f"/trade_detailed_trade_matrix/partner_country_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(TradeDetailedTradeMatrix.item_code_id)))
        .select_from(TradeDetailedTradeMatrix)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/trade_detailed_trade_matrix/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(TradeDetailedTradeMatrix.element_code_id)))
        .select_from(TradeDetailedTradeMatrix)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/trade_detailed_trade_matrix/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(TradeDetailedTradeMatrix.flag_id)))
        .select_from(TradeDetailedTradeMatrix)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/trade_detailed_trade_matrix/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(TradeDetailedTradeMatrix.year).label('min_year'),
            func.max(TradeDetailedTradeMatrix.year).label('max_year'),
            func.count(func.distinct(TradeDetailedTradeMatrix.year)).label('year_count')
        )
        .select_from(TradeDetailedTradeMatrix)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/trade_detailed_trade_matrix/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(TradeDetailedTradeMatrix.value).label('min_value'),
            func.max(TradeDetailedTradeMatrix.value).label('max_value'),
            func.avg(TradeDetailedTradeMatrix.value).label('avg_value')
        )
        .select_from(TradeDetailedTradeMatrix)
        .where(and_(TradeDetailedTradeMatrix.value > 0, TradeDetailedTradeMatrix.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/trade_detailed_trade_matrix",
        "aggregate": f"/trade_detailed_trade_matrix/aggregate",
        "summary": f"/trade_detailed_trade_matrix/summary",
        "overview": f"/trade_detailed_trade_matrix/overview",
        "item_codes": f"/trade_detailed_trade_matrix/item_codes",
        "elements": f"/trade_detailed_trade_matrix/elements",
        "flags": f"/trade_detailed_trade_matrix/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the trade_detailed_trade_matrix endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(TradeDetailedTradeMatrix)).scalar()
        return {
            "status": "healthy",
            "dataset": "trade_detailed_trade_matrix",
            "records": result
