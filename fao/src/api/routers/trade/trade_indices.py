...
from fao.src.db.pipelines.trade_indices.trade_indices_model import TradeIndices
...
from .trade_indices_config import TradeIndicesConfig
...
router = APIRouter(
    prefix="/trade_indices",
    responses={404: {"description": "Not found"}},
)


config = TradeIndicesConfig()

@router.get("/", summary="Get trade indices data")
async def get_trade_indices_data(
    request: Request,
    response: Response,
    ...
    """Get trade indices data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=TradeIndices, 
        model_name="TradeIndices",
        table_name="trade_indices",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated trade indices data")
async def get_trade_indices_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(TradeIndices))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in trade_indices")
@cache_result(prefix="trade_indices:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(TradeIndices.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(TradeIndices, TradeIndices.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'trade_indices')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in trade_indices")
@cache_result(prefix="trade_indices:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(TradeIndices, TradeIndices.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'trade_indices')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in trade_indices")
@cache_result(prefix="trade_indices:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(TradeIndices, TradeIndices.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'trade_indices')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in trade_indices")
@cache_result(prefix="trade_indices:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(TradeIndices, Flags.id == TradeIndices.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(TradeIndices.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(TradeIndices)
                .where(TradeIndices.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(TradeIndices)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in trade_indices")
@cache_result(prefix="trade_indices:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(TradeIndices)
        .group_by(TradeIndices.unit)
        .order_by(TradeIndices.unit)
    )
...
@router.get("/years", summary="Get available years in trade_indices")
@cache_result(prefix="trade_indices:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                TradeIndices.year,
                TradeIndices.year_code,
                func.count(TradeIndices.id).label('record_count')
            )
            .group_by(TradeIndices.year, TradeIndices.year_code)
            .order_by(TradeIndices.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(TradeIndices.year)
            .distinct()
            .order_by(TradeIndices.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of trade_indices dataset")
@cache_result(prefix="trade_indices:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(TradeIndices.area_code_id)))
        .select_from(TradeIndices)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/trade_indices/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(TradeIndices.item_code_id)))
        .select_from(TradeIndices)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/trade_indices/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(TradeIndices.element_code_id)))
        .select_from(TradeIndices)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/trade_indices/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(TradeIndices.flag_id)))
        .select_from(TradeIndices)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/trade_indices/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(TradeIndices.year).label('min_year'),
            func.max(TradeIndices.year).label('max_year'),
            func.count(func.distinct(TradeIndices.year)).label('year_count')
        )
        .select_from(TradeIndices)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/trade_indices/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(TradeIndices.value).label('min_value'),
            func.max(TradeIndices.value).label('max_value'),
            func.avg(TradeIndices.value).label('avg_value')
        )
        .select_from(TradeIndices)
        .where(and_(TradeIndices.value > 0, TradeIndices.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/trade_indices",
        "aggregate": f"/trade_indices/aggregate",
        "summary": f"/trade_indices/summary",
        "overview": f"/trade_indices/overview",
        "area_codes": f"/trade_indices/area_codes",
        "item_codes": f"/trade_indices/item_codes",
        "elements": f"/trade_indices/elements",
        "flags": f"/trade_indices/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the trade_indices endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(TradeIndices)).scalar()
        return {
            "status": "healthy",
            "dataset": "trade_indices",
            "records": result
