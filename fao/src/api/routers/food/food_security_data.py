...
from .food_security_data_config import FoodSecurityDataConfig
...
config = FoodSecurityDataConfig()

@router.get("/", summary="Get food security data data")
async def get_food_security_data_data(
    request: Request,
    response: Response,
    ...
    """Get food security data data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=FoodSecurityData, 
        model_name="FoodSecurityData",
        table_name="food_security_data",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated food security data data")
async def get_food_security_data_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(FoodSecurityData))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in food_security_data")
@cache_result(prefix="food_security_data:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(FoodSecurityData, FoodSecurityData.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'food_security_data')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in food_security_data")
@cache_result(prefix="food_security_data:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(FoodSecurityData, FoodSecurityData.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'food_security_data')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in food_security_data")
@cache_result(prefix="food_security_data:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(FoodSecurityData, FoodSecurityData.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'food_security_data')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in food_security_data")
@cache_result(prefix="food_security_data:flags", ttl=604800)
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(FoodSecurityData.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(FoodSecurityData)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in food_security_data")
@cache_result(prefix="food_security_data:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            FoodSecurityData.unit,
            func.count(FoodSecurityData.id).label('record_count')
        )
        .select_from(FoodSecurityData)
        .group_by(FoodSecurityData.unit)
        .order_by(FoodSecurityData.unit)
    )
...
@router.get("/years", summary="Get available years in food_security_data")
@cache_result(prefix="food_security_data:years", ttl=604800)
    ...
    if include_counts:
        query = (
            select(
                ...
                FoodSecurityData.year_code,
                func.count(FoodSecurityData.id).label('record_count')
            )
            .group_by(FoodSecurityData.year, FoodSecurityData.year_code)
            .order_by(FoodSecurityData.year_code)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of food_security_data dataset")
@cache_result(prefix="food_security_data:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(FoodSecurityData.area_code_id)))
        .select_from(FoodSecurityData)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/food_security_data/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(FoodSecurityData.item_code_id)))
        .select_from(FoodSecurityData)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/food_security_data/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(FoodSecurityData.element_code_id)))
        .select_from(FoodSecurityData)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/food_security_data/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(FoodSecurityData.flag_id)))
        .select_from(FoodSecurityData)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/food_security_data/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(FoodSecurityData.year).label('min_year'),
            func.max(FoodSecurityData.year).label('max_year'),
            func.count(func.distinct(FoodSecurityData.year)).label('year_count')
        )
        .select_from(FoodSecurityData)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/food_security_data/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(FoodSecurityData.value).label('min_value'),
            func.max(FoodSecurityData.value).label('max_value'),
            func.avg(FoodSecurityData.value).label('avg_value')
        )
        .select_from(FoodSecurityData)
        .where(and_(FoodSecurityData.value > 0, FoodSecurityData.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/food_security_data",
        "aggregate": f"/food_security_data/aggregate",
        "summary": f"/food_security_data/summary",
        "overview": f"/food_security_data/overview",
        "area_codes": f"/food_security_data/area_codes",
        "item_codes": f"/food_security_data/item_codes",
        "elements": f"/food_security_data/elements",
        "flags": f"/food_security_data/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the food_security_data endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(FoodSecurityData)).scalar()
        return {
            "status": "healthy",
            "dataset": "food_security_data",
            "records": result
