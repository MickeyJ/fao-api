...
@router.get("/")
@cache_result(prefix="food_security_data", ttl=86400, exclude_params=["response", "db"])
def get_food_security_data(
    ...
    """
    Get food security data data with filters and pagination.
    ...
    # Main table columns
    for col in FoodSecurityData.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(FoodSecurityData, 'id'):
                columns.append(FoodSecurityData.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(FoodSecurityData)
    ...
    if year_code is not None:
        conditions.append(FoodSecurityData.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(FoodSecurityData.year_code == year_code_exact)
    if year is not None:
        conditions.append(FoodSecurityData.year == year)
    if year_min is not None:
        conditions.append(FoodSecurityData.year >= year_min)
    if year_max is not None:
        conditions.append(FoodSecurityData.year <= year_max)
    if unit is not None:
        conditions.append(FoodSecurityData.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(FoodSecurityData.unit == unit_exact)
    if value is not None:
        conditions.append(FoodSecurityData.value == value)
    if value_min is not None:
        conditions.append(FoodSecurityData.value >= value_min)
    if value_max is not None:
        conditions.append(FoodSecurityData.value <= value_max)
    if note is not None:
        conditions.append(FoodSecurityData.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(FoodSecurityData.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(FoodSecurityData, field_name):
                        order_by_clauses.append(getattr(FoodSecurityData, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(FoodSecurityData, sort_field):
                        order_by_clauses.append(getattr(FoodSecurityData, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(FoodSecurityData.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(FoodSecurityData)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, FoodSecurityData.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, FoodSecurityData.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, FoodSecurityData.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, FoodSecurityData.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_food_security_data'))
...
@router.get("/areas")
@cache_result(prefix="food_security_data:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="food_security_data:items", ttl=604800)
...
@router.get("/summary")
@cache_result(prefix="food_security_data:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    summary["unique_flags"] = db.query(func.count(func.distinct(FoodSecurityData.flag_id))).scalar()
