...
from .food_balance_sheets_historic_config import FoodBalanceSheetsHistoricConfig
...
config = FoodBalanceSheetsHistoricConfig()

@router.get("/", summary="Get food balance sheets historic data")
async def get_food_balance_sheets_historic_data(
    request: Request,
    response: Response,
    ...
    """Get food balance sheets historic data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=FoodBalanceSheetsHistoric, 
        model_name="FoodBalanceSheetsHistoric",
        table_name="food_balance_sheets_historic",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated food balance sheets historic data")
async def get_food_balance_sheets_historic_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(FoodBalanceSheetsHistoric))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(FoodBalanceSheetsHistoric, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(FoodBalanceSheetsHistoric, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(FoodBalanceSheetsHistoric, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in food_balance_sheets_historic")
@cache_result(prefix="food_balance_sheets_historic:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(FoodBalanceSheetsHistoric.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(FoodBalanceSheetsHistoric, FoodBalanceSheetsHistoric.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'food_balance_sheets_historic')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="food_balance_sheets_historic",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in food_balance_sheets_historic")
@cache_result(prefix="food_balance_sheets_historic:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(FoodBalanceSheetsHistoric, FoodBalanceSheetsHistoric.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'food_balance_sheets_historic')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in food_balance_sheets_historic")
@cache_result(prefix="food_balance_sheets_historic:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(FoodBalanceSheetsHistoric, FoodBalanceSheetsHistoric.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'food_balance_sheets_historic')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in food_balance_sheets_historic")
@cache_result(prefix="food_balance_sheets_historic:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(FoodBalanceSheetsHistoric, Flags.id == FoodBalanceSheetsHistoric.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(FoodBalanceSheetsHistoric.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(FoodBalanceSheetsHistoric)
                .where(FoodBalanceSheetsHistoric.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(FoodBalanceSheetsHistoric)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in food_balance_sheets_historic")
@cache_result(prefix="food_balance_sheets_historic:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            FoodBalanceSheetsHistoric.unit,
            func.count(FoodBalanceSheetsHistoric.id).label('record_count')
        )
        .select_from(FoodBalanceSheetsHistoric)
        .group_by(FoodBalanceSheetsHistoric.unit)
        .order_by(FoodBalanceSheetsHistoric.unit)
    )
...
@router.get("/years", summary="Get available years in food_balance_sheets_historic")
@cache_result(prefix="food_balance_sheets_historic:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                FoodBalanceSheetsHistoric.year,
                FoodBalanceSheetsHistoric.year_code,
                func.count(FoodBalanceSheetsHistoric.id).label('record_count')
            )
            .group_by(FoodBalanceSheetsHistoric.year, FoodBalanceSheetsHistoric.year_code)
            .order_by(FoodBalanceSheetsHistoric.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(FoodBalanceSheetsHistoric.year)
            .distinct()
            .order_by(FoodBalanceSheetsHistoric.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of food_balance_sheets_historic dataset")
@cache_result(prefix="food_balance_sheets_historic:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(FoodBalanceSheetsHistoric.area_code_id)))
        .select_from(FoodBalanceSheetsHistoric)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/food_balance_sheets_historic/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(FoodBalanceSheetsHistoric.item_code_id)))
        .select_from(FoodBalanceSheetsHistoric)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/food_balance_sheets_historic/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(FoodBalanceSheetsHistoric.element_code_id)))
        .select_from(FoodBalanceSheetsHistoric)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/food_balance_sheets_historic/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(FoodBalanceSheetsHistoric.flag_id)))
        .select_from(FoodBalanceSheetsHistoric)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/food_balance_sheets_historic/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(FoodBalanceSheetsHistoric.year).label('min_year'),
            func.max(FoodBalanceSheetsHistoric.year).label('max_year'),
            func.count(func.distinct(FoodBalanceSheetsHistoric.year)).label('year_count')
        )
        .select_from(FoodBalanceSheetsHistoric)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/food_balance_sheets_historic/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(FoodBalanceSheetsHistoric.value).label('min_value'),
            func.max(FoodBalanceSheetsHistoric.value).label('max_value'),
            func.avg(FoodBalanceSheetsHistoric.value).label('avg_value')
        )
        .select_from(FoodBalanceSheetsHistoric)
        .where(and_(FoodBalanceSheetsHistoric.value > 0, FoodBalanceSheetsHistoric.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/food_balance_sheets_historic",
        "aggregate": f"/food_balance_sheets_historic/aggregate",
        "summary": f"/food_balance_sheets_historic/summary",
        "overview": f"/food_balance_sheets_historic/overview",
        "area_codes": f"/food_balance_sheets_historic/area_codes",
        "item_codes": f"/food_balance_sheets_historic/item_codes",
        "elements": f"/food_balance_sheets_historic/elements",
        "flags": f"/food_balance_sheets_historic/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the food_balance_sheets_historic endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(FoodBalanceSheetsHistoric)).scalar()
        return {
            "status": "healthy",
            "dataset": "food_balance_sheets_historic",
            "records": result
