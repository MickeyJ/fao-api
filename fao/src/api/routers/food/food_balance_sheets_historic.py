...
@router.get("/")
@cache_result(prefix="food_balance_sheets_historic", ttl=86400, exclude_params=["response", "db"])
def get_food_balance_sheets_historic(
    ...
    """
    Get food balance sheets historic data with filters and pagination.
    ...
    # Main table columns
    for col in FoodBalanceSheetsHistoric.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(FoodBalanceSheetsHistoric, 'id'):
                columns.append(FoodBalanceSheetsHistoric.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(FoodBalanceSheetsHistoric)
    ...
    if year_code is not None:
        conditions.append(FoodBalanceSheetsHistoric.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(FoodBalanceSheetsHistoric.year_code == year_code_exact)
    if year is not None:
        conditions.append(FoodBalanceSheetsHistoric.year == year)
    if year_min is not None:
        conditions.append(FoodBalanceSheetsHistoric.year >= year_min)
    if year_max is not None:
        conditions.append(FoodBalanceSheetsHistoric.year <= year_max)
    if unit is not None:
        conditions.append(FoodBalanceSheetsHistoric.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(FoodBalanceSheetsHistoric.unit == unit_exact)
    if value is not None:
        conditions.append(FoodBalanceSheetsHistoric.value == value)
    if value_min is not None:
        conditions.append(FoodBalanceSheetsHistoric.value >= value_min)
    if value_max is not None:
        conditions.append(FoodBalanceSheetsHistoric.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(FoodBalanceSheetsHistoric, field_name):
                        order_by_clauses.append(getattr(FoodBalanceSheetsHistoric, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(FoodBalanceSheetsHistoric, sort_field):
                        order_by_clauses.append(getattr(FoodBalanceSheetsHistoric, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(FoodBalanceSheetsHistoric.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(FoodBalanceSheetsHistoric)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, FoodBalanceSheetsHistoric.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, FoodBalanceSheetsHistoric.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, FoodBalanceSheetsHistoric.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, FoodBalanceSheetsHistoric.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_food_balance_sheets_historic'))
...
@router.get("/areas")
@cache_result(prefix="food_balance_sheets_historic:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="food_balance_sheets_historic:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(FoodBalanceSheetsHistoric.year)
    ...
    if not years_data:
        return {"dataset": "food_balance_sheets_historic", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="food_balance_sheets_historic:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(FoodBalanceSheetsHistoric.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(FoodBalanceSheetsHistoric.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(FoodBalanceSheetsHistoric.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(FoodBalanceSheetsHistoric.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(FoodBalanceSheetsHistoric.flag_id))).scalar()
