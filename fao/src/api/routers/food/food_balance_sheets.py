...
@router.get("/")
@cache_result(prefix="food_balance_sheets", ttl=86400, exclude_params=["response", "db"])
def get_food_balance_sheets(
    response: Response,
    ...
    """
    Get food balance sheets data with filters and pagination.
    ...
    # Main table columns
    for col in FoodBalanceSheets.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(FoodBalanceSheets, 'id'):
                columns.append(FoodBalanceSheets.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(FoodBalanceSheets)
    
    # Add joins
    query = query.outerjoin(AreaCodes, FoodBalanceSheets.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, FoodBalanceSheets.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, FoodBalanceSheets.element_code_id == Elements.id)
    query = query.outerjoin(Flags, FoodBalanceSheets.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(FoodBalanceSheets.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(FoodBalanceSheets.year_code == year_code_exact)
    if year is not None:
        conditions.append(FoodBalanceSheets.year == year)
    if year_min is not None:
        conditions.append(FoodBalanceSheets.year >= year_min)
    if year_max is not None:
        conditions.append(FoodBalanceSheets.year <= year_max)
    if unit is not None:
        conditions.append(FoodBalanceSheets.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(FoodBalanceSheets.unit == unit_exact)
    if value is not None:
        conditions.append(FoodBalanceSheets.value == value)
    if value_min is not None:
        conditions.append(FoodBalanceSheets.value >= value_min)
    if value_max is not None:
        conditions.append(FoodBalanceSheets.value <= value_max)
    if note is not None:
        conditions.append(FoodBalanceSheets.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(FoodBalanceSheets.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(FoodBalanceSheets, field_name):
                        order_by_clauses.append(getattr(FoodBalanceSheets, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(FoodBalanceSheets, sort_field):
                        order_by_clauses.append(getattr(FoodBalanceSheets, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(FoodBalanceSheets.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(FoodBalanceSheets)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, FoodBalanceSheets.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, FoodBalanceSheets.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, FoodBalanceSheets.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, FoodBalanceSheets.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_food_balance_sheets'))
...
@router.get("/areas")
@cache_result(prefix="food_balance_sheets:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="food_balance_sheets:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(FoodBalanceSheets.year)
    ...
    if not years_data:
        return {"dataset": "food_balance_sheets", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="food_balance_sheets:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(FoodBalanceSheets.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(FoodBalanceSheets.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(FoodBalanceSheets.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(FoodBalanceSheets.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(FoodBalanceSheets.flag_id))).scalar()
