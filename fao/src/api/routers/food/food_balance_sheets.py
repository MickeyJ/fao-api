...
from .food_balance_sheets_config import FoodBalanceSheetsConfig
...
router = APIRouter(
    prefix="/food_balance_sheets",
    responses={404: {"description": "Not found"}},
)


config = FoodBalanceSheetsConfig()

@router.get("/", summary="Get food balance sheets data")
async def get_food_balance_sheets_data(
    request: Request,
    response: Response,
    ...
    """Get food balance sheets data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=FoodBalanceSheets, 
        model_name="FoodBalanceSheets",
        table_name="food_balance_sheets",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated food balance sheets data")
async def get_food_balance_sheets_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(FoodBalanceSheets))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in food_balance_sheets")
@cache_result(prefix="food_balance_sheets:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(FoodBalanceSheets.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(FoodBalanceSheets, FoodBalanceSheets.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'food_balance_sheets')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in food_balance_sheets")
@cache_result(prefix="food_balance_sheets:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(FoodBalanceSheets, FoodBalanceSheets.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'food_balance_sheets')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in food_balance_sheets")
@cache_result(prefix="food_balance_sheets:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(FoodBalanceSheets, FoodBalanceSheets.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'food_balance_sheets')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in food_balance_sheets")
@cache_result(prefix="food_balance_sheets:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(FoodBalanceSheets, Flags.id == FoodBalanceSheets.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(FoodBalanceSheets.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(FoodBalanceSheets)
                .where(FoodBalanceSheets.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(FoodBalanceSheets)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in food_balance_sheets")
@cache_result(prefix="food_balance_sheets:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            FoodBalanceSheets.unit,
            func.count(FoodBalanceSheets.id).label('record_count')
        )
        .select_from(FoodBalanceSheets)
        .group_by(FoodBalanceSheets.unit)
        .order_by(FoodBalanceSheets.unit)
    )
...
@router.get("/years", summary="Get available years in food_balance_sheets")
@cache_result(prefix="food_balance_sheets:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                FoodBalanceSheets.year,
                FoodBalanceSheets.year_code,
                func.count(FoodBalanceSheets.id).label('record_count')
            )
            .group_by(FoodBalanceSheets.year, FoodBalanceSheets.year_code)
            .order_by(FoodBalanceSheets.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(FoodBalanceSheets.year)
            .distinct()
            .order_by(FoodBalanceSheets.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of food_balance_sheets dataset")
@cache_result(prefix="food_balance_sheets:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(FoodBalanceSheets.area_code_id)))
        .select_from(FoodBalanceSheets)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/food_balance_sheets/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(FoodBalanceSheets.item_code_id)))
        .select_from(FoodBalanceSheets)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/food_balance_sheets/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(FoodBalanceSheets.element_code_id)))
        .select_from(FoodBalanceSheets)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/food_balance_sheets/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(FoodBalanceSheets.flag_id)))
        .select_from(FoodBalanceSheets)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/food_balance_sheets/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(FoodBalanceSheets.year).label('min_year'),
            func.max(FoodBalanceSheets.year).label('max_year'),
            func.count(func.distinct(FoodBalanceSheets.year)).label('year_count')
        )
        .select_from(FoodBalanceSheets)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/food_balance_sheets/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(FoodBalanceSheets.value).label('min_value'),
            func.max(FoodBalanceSheets.value).label('max_value'),
            func.avg(FoodBalanceSheets.value).label('avg_value')
        )
        .select_from(FoodBalanceSheets)
        .where(and_(FoodBalanceSheets.value > 0, FoodBalanceSheets.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/food_balance_sheets",
        "aggregate": f"/food_balance_sheets/aggregate",
        "summary": f"/food_balance_sheets/summary",
        "overview": f"/food_balance_sheets/overview",
        "area_codes": f"/food_balance_sheets/area_codes",
        "item_codes": f"/food_balance_sheets/item_codes",
        "elements": f"/food_balance_sheets/elements",
        "flags": f"/food_balance_sheets/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the food_balance_sheets endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(FoodBalanceSheets)).scalar()
        return {
            "status": "healthy",
            "dataset": "food_balance_sheets",
            "records": result
