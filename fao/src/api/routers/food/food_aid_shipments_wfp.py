...
@router.get("/")
@cache_result(prefix="food_aid_shipments_wfp", ttl=86400, exclude_params=["response", "db"])
def get_food_aid_shipments_wfp(
    ...
    """
    Get food aid shipments wfp data with filters and pagination.
    ...
    # Main table columns
    for col in FoodAidShipmentsWfp.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(FoodAidShipmentsWfp, 'id'):
                columns.append(FoodAidShipmentsWfp.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(FoodAidShipmentsWfp)
    
    # Add joins
    query = query.outerjoin(RecipientCountryCodes, FoodAidShipmentsWfp.recipient_country_code_id == RecipientCountryCodes.id)
    query = query.outerjoin(ItemCodes, FoodAidShipmentsWfp.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, FoodAidShipmentsWfp.element_code_id == Elements.id)
    query = query.outerjoin(Flags, FoodAidShipmentsWfp.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(FoodAidShipmentsWfp.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(FoodAidShipmentsWfp.year_code == year_code_exact)
    if year is not None:
        conditions.append(FoodAidShipmentsWfp.year == year)
    if year_min is not None:
        conditions.append(FoodAidShipmentsWfp.year >= year_min)
    if year_max is not None:
        conditions.append(FoodAidShipmentsWfp.year <= year_max)
    if unit is not None:
        conditions.append(FoodAidShipmentsWfp.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(FoodAidShipmentsWfp.unit == unit_exact)
    if value is not None:
        conditions.append(FoodAidShipmentsWfp.value == value)
    if value_min is not None:
        conditions.append(FoodAidShipmentsWfp.value >= value_min)
    if value_max is not None:
        conditions.append(FoodAidShipmentsWfp.value <= value_max)
    if note is not None:
        conditions.append(FoodAidShipmentsWfp.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(FoodAidShipmentsWfp.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(FoodAidShipmentsWfp, field_name):
                        order_by_clauses.append(getattr(FoodAidShipmentsWfp, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(FoodAidShipmentsWfp, sort_field):
                        order_by_clauses.append(getattr(FoodAidShipmentsWfp, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(FoodAidShipmentsWfp.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(FoodAidShipmentsWfp)
    
    # Add joins to count query
    count_query = count_query.outerjoin(RecipientCountryCodes, FoodAidShipmentsWfp.recipient_country_code_id == RecipientCountryCodes.id)
    count_query = count_query.outerjoin(ItemCodes, FoodAidShipmentsWfp.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, FoodAidShipmentsWfp.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, FoodAidShipmentsWfp.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_food_aid_shipments_wfp'))
...
@router.get("/summary")
@cache_result(prefix="food_aid_shipments_wfp:summary", ttl=604800)
