...
from .food_aid_shipments_wfp_config import FoodAidShipmentsWfpConfig
...
config = FoodAidShipmentsWfpConfig()

@router.get("/", summary="Get food aid shipments wfp data")
async def get_food_aid_shipments_wfp_data(
    request: Request,
    response: Response,
    ...
    """Get food aid shipments wfp data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=FoodAidShipmentsWfp, 
        model_name="FoodAidShipmentsWfp",
...
@router.get("/aggregate", summary="Get aggregated food aid shipments wfp data")
async def get_food_aid_shipments_wfp_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(FoodAidShipmentsWfp))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/recipient_country_codes", summary="Get recipient_country_codes in food_aid_shipments_wfp")
@cache_result(prefix="food_aid_shipments_wfp:recipient_country_codes", ttl=604800)
async def get_available_recipient_country_codes(
    ...
    query = (
        ...
        .join(FoodAidShipmentsWfp, 
              FoodAidShipmentsWfp.partner_country_code_id == PartnerCountryCodes.id)
        .where(PartnerCountryCodes.source_dataset == 'food_aid_shipments_wfp')
    )
...
@router.get("/item_codes", summary="Get item_codes in food_aid_shipments_wfp")
@cache_result(prefix="food_aid_shipments_wfp:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(FoodAidShipmentsWfp, FoodAidShipmentsWfp.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in food_aid_shipments_wfp")
@cache_result(prefix="food_aid_shipments_wfp:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(FoodAidShipmentsWfp, FoodAidShipmentsWfp.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in food_aid_shipments_wfp")
@cache_result(prefix="food_aid_shipments_wfp:flags", ttl=604800)
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(FoodAidShipmentsWfp.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(FoodAidShipmentsWfp)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in food_aid_shipments_wfp")
@cache_result(prefix="food_aid_shipments_wfp:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            FoodAidShipmentsWfp.unit,
            func.count(FoodAidShipmentsWfp.id).label('record_count')
        )
        .select_from(FoodAidShipmentsWfp)
        .group_by(FoodAidShipmentsWfp.unit)
        .order_by(FoodAidShipmentsWfp.unit)
    )
...
@router.get("/years", summary="Get available years in food_aid_shipments_wfp")
    ...
    if include_counts:
        query = (
            select(
                ...
                FoodAidShipmentsWfp.year_code,
                func.count(FoodAidShipmentsWfp.id).label('record_count')
            )
            .group_by(FoodAidShipmentsWfp.year, FoodAidShipmentsWfp.year_code)
            .order_by(FoodAidShipmentsWfp.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(FoodAidShipmentsWfp.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of food_aid_shipments_wfp dataset")
@cache_result(prefix="food_aid_shipments_wfp:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    recipient_country_code_ids = db.execute(
        select(func.count(func.distinct(FoodAidShipmentsWfp.recipient_country_code_id)))
        .select_from(FoodAidShipmentsWfp)
    ).scalar() or 0

    overview["dimensions"]["recipient_country_codes"] = {
        "count": recipient_country_code_ids,
        "endpoint": f"/food_aid_shipments_wfp/recipient_country_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(FoodAidShipmentsWfp.item_code_id)))
        .select_from(FoodAidShipmentsWfp)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/food_aid_shipments_wfp/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(FoodAidShipmentsWfp.element_code_id)))
        .select_from(FoodAidShipmentsWfp)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/food_aid_shipments_wfp/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(FoodAidShipmentsWfp.flag_id)))
        .select_from(FoodAidShipmentsWfp)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/food_aid_shipments_wfp/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(FoodAidShipmentsWfp.year).label('min_year'),
            func.max(FoodAidShipmentsWfp.year).label('max_year'),
            func.count(func.distinct(FoodAidShipmentsWfp.year)).label('year_count')
        )
        .select_from(FoodAidShipmentsWfp)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/food_aid_shipments_wfp/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(FoodAidShipmentsWfp.value).label('min_value'),
            func.max(FoodAidShipmentsWfp.value).label('max_value'),
            func.avg(FoodAidShipmentsWfp.value).label('avg_value')
        )
        .select_from(FoodAidShipmentsWfp)
        .where(and_(FoodAidShipmentsWfp.value > 0, FoodAidShipmentsWfp.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/food_aid_shipments_wfp",
        "aggregate": f"/food_aid_shipments_wfp/aggregate",
        "summary": f"/food_aid_shipments_wfp/summary",
        "overview": f"/food_aid_shipments_wfp/overview",
        "item_codes": f"/food_aid_shipments_wfp/item_codes",
        "elements": f"/food_aid_shipments_wfp/elements",
        "flags": f"/food_aid_shipments_wfp/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the food_aid_shipments_wfp endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(FoodAidShipmentsWfp)).scalar()
        return {
            "status": "healthy",
            "dataset": "food_aid_shipments_wfp",
            "records": result
