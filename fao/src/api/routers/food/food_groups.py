...
@router.get("/")
@cache_result(prefix="food_groups", ttl=86400, exclude_params=["response", "db"])
def get_food_groups(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    food_group_code: Optional[str] = Query(None, description="Filter by food group code (partial match)"),
    food_group_code_exact: Optional[str] = Query(None, description="Filter by exact food group code"),
    food_group: Optional[str] = Query(None, description="Filter by food group (partial match)"),
    food_group_exact: Optional[str] = Query(None, description="Filter by exact food group"),
    ...
    """
    Get food groups data with filters and pagination.
    ...
    - food_group_code: Partial match (case-insensitive)
    - food_group_code_exact: Exact match
    - food_group: Partial match (case-insensitive)
    - food_group_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(FoodGroups, 'id'):
                columns.append(FoodGroups.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(FoodGroups)
    ...
    if food_group_code is not None:
        conditions.append(FoodGroups.food_group_code.ilike("%" + food_group_code + "%"))
    if food_group_code_exact is not None:
        conditions.append(FoodGroups.food_group_code == food_group_code_exact)
    if food_group is not None:
        conditions.append(FoodGroups.food_group.ilike("%" + food_group + "%"))
    if food_group_exact is not None:
        conditions.append(FoodGroups.food_group == food_group_exact)
    if source_dataset is not None:
        conditions.append(FoodGroups.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(FoodGroups.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(FoodGroups, field_name):
                        order_by_clauses.append(getattr(FoodGroups, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(FoodGroups, sort_field):
                        order_by_clauses.append(getattr(FoodGroups, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(FoodGroups.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(FoodGroups)
    ...
    base_url = str(router.url_path_for('get_food_groups'))
    ...
    all_params = {
        ...
        'food_group_code_exact': food_group_code_exact,
        'food_group': food_group,
        'food_group_exact': food_group_exact,
