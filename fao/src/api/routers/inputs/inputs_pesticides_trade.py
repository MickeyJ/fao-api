...
from .inputs_pesticides_trade_config import InputsPesticidesTradeConfig
...
config = InputsPesticidesTradeConfig()

@router.get("/", summary="Get inputs pesticides trade data")
async def get_inputs_pesticides_trade_data(
    request: Request,
    response: Response,
    ...
    """Get inputs pesticides trade data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InputsPesticidesTrade, 
        model_name="InputsPesticidesTrade",
        table_name="inputs_pesticides_trade",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated inputs pesticides trade data")
async def get_inputs_pesticides_trade_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InputsPesticidesTrade))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InputsPesticidesTrade, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InputsPesticidesTrade, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InputsPesticidesTrade, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in inputs_pesticides_trade")
@cache_result(prefix="inputs_pesticides_trade:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(InputsPesticidesTrade, InputsPesticidesTrade.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'inputs_pesticides_trade')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in inputs_pesticides_trade")
@cache_result(prefix="inputs_pesticides_trade:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InputsPesticidesTrade, InputsPesticidesTrade.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'inputs_pesticides_trade')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in inputs_pesticides_trade")
@cache_result(prefix="inputs_pesticides_trade:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InputsPesticidesTrade, InputsPesticidesTrade.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'inputs_pesticides_trade')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in inputs_pesticides_trade")
@cache_result(prefix="inputs_pesticides_trade:flags", ttl=604800)
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(InputsPesticidesTrade.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InputsPesticidesTrade)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in inputs_pesticides_trade")
@cache_result(prefix="inputs_pesticides_trade:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InputsPesticidesTrade.unit,
            func.count(InputsPesticidesTrade.id).label('record_count')
        )
        .select_from(InputsPesticidesTrade)
        .group_by(InputsPesticidesTrade.unit)
        .order_by(InputsPesticidesTrade.unit)
    )
...
@router.get("/years", summary="Get available years in inputs_pesticides_trade")
@cache_result(prefix="inputs_pesticides_trade:years", ttl=604800)
    ...
    if include_counts:
        query = (
            select(
                ...
                InputsPesticidesTrade.year_code,
                func.count(InputsPesticidesTrade.id).label('record_count')
            )
            .group_by(InputsPesticidesTrade.year, InputsPesticidesTrade.year_code)
            .order_by(InputsPesticidesTrade.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InputsPesticidesTrade.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of inputs_pesticides_trade dataset")
@cache_result(prefix="inputs_pesticides_trade:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InputsPesticidesTrade.area_code_id)))
        .select_from(InputsPesticidesTrade)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/inputs_pesticides_trade/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InputsPesticidesTrade.item_code_id)))
        .select_from(InputsPesticidesTrade)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/inputs_pesticides_trade/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InputsPesticidesTrade.element_code_id)))
        .select_from(InputsPesticidesTrade)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/inputs_pesticides_trade/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InputsPesticidesTrade.flag_id)))
        .select_from(InputsPesticidesTrade)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/inputs_pesticides_trade/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InputsPesticidesTrade.year).label('min_year'),
            func.max(InputsPesticidesTrade.year).label('max_year'),
            func.count(func.distinct(InputsPesticidesTrade.year)).label('year_count')
        )
        .select_from(InputsPesticidesTrade)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/inputs_pesticides_trade/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InputsPesticidesTrade.value).label('min_value'),
            func.max(InputsPesticidesTrade.value).label('max_value'),
            func.avg(InputsPesticidesTrade.value).label('avg_value')
        )
        .select_from(InputsPesticidesTrade)
        .where(and_(InputsPesticidesTrade.value > 0, InputsPesticidesTrade.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/inputs_pesticides_trade",
        "aggregate": f"/inputs_pesticides_trade/aggregate",
        "summary": f"/inputs_pesticides_trade/summary",
        "overview": f"/inputs_pesticides_trade/overview",
        "area_codes": f"/inputs_pesticides_trade/area_codes",
        "item_codes": f"/inputs_pesticides_trade/item_codes",
        "elements": f"/inputs_pesticides_trade/elements",
        "flags": f"/inputs_pesticides_trade/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the inputs_pesticides_trade endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InputsPesticidesTrade)).scalar()
        return {
            "status": "healthy",
            "dataset": "inputs_pesticides_trade",
            "records": result
