...
from .inputs_land_use_config import InputsLandUseConfig
...
router = APIRouter(
    prefix="/inputs_land_use",
    responses={404: {"description": "Not found"}},
)


config = InputsLandUseConfig()

@router.get("/", summary="Get inputs land use data")
async def get_inputs_land_use_data(
    request: Request,
    response: Response,
    ...
    """Get inputs land use data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InputsLandUse, 
        model_name="InputsLandUse",
        table_name="inputs_land_use",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated inputs land use data")
async def get_inputs_land_use_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InputsLandUse))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in inputs_land_use")
@cache_result(prefix="inputs_land_use:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InputsLandUse.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InputsLandUse, InputsLandUse.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'inputs_land_use')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in inputs_land_use")
@cache_result(prefix="inputs_land_use:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InputsLandUse, InputsLandUse.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'inputs_land_use')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in inputs_land_use")
@cache_result(prefix="inputs_land_use:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InputsLandUse, InputsLandUse.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'inputs_land_use')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in inputs_land_use")
@cache_result(prefix="inputs_land_use:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InputsLandUse, Flags.id == InputsLandUse.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InputsLandUse.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InputsLandUse)
                .where(InputsLandUse.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InputsLandUse)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in inputs_land_use")
@cache_result(prefix="inputs_land_use:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(InputsLandUse)
        .group_by(InputsLandUse.unit)
        .order_by(InputsLandUse.unit)
    )
...
@router.get("/years", summary="Get available years in inputs_land_use")
@cache_result(prefix="inputs_land_use:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InputsLandUse.year,
                InputsLandUse.year_code,
                func.count(InputsLandUse.id).label('record_count')
            )
            .group_by(InputsLandUse.year, InputsLandUse.year_code)
            .order_by(InputsLandUse.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InputsLandUse.year)
            .distinct()
            .order_by(InputsLandUse.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of inputs_land_use dataset")
@cache_result(prefix="inputs_land_use:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InputsLandUse.area_code_id)))
        .select_from(InputsLandUse)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/inputs_land_use/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InputsLandUse.item_code_id)))
        .select_from(InputsLandUse)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/inputs_land_use/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InputsLandUse.element_code_id)))
        .select_from(InputsLandUse)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/inputs_land_use/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InputsLandUse.flag_id)))
        .select_from(InputsLandUse)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/inputs_land_use/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InputsLandUse.year).label('min_year'),
            func.max(InputsLandUse.year).label('max_year'),
            func.count(func.distinct(InputsLandUse.year)).label('year_count')
        )
        .select_from(InputsLandUse)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/inputs_land_use/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InputsLandUse.value).label('min_value'),
            func.max(InputsLandUse.value).label('max_value'),
            func.avg(InputsLandUse.value).label('avg_value')
        )
        .select_from(InputsLandUse)
        .where(and_(InputsLandUse.value > 0, InputsLandUse.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/inputs_land_use",
        "aggregate": f"/inputs_land_use/aggregate",
        "summary": f"/inputs_land_use/summary",
        "overview": f"/inputs_land_use/overview",
        "area_codes": f"/inputs_land_use/area_codes",
        "item_codes": f"/inputs_land_use/item_codes",
        "elements": f"/inputs_land_use/elements",
        "flags": f"/inputs_land_use/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the inputs_land_use endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InputsLandUse)).scalar()
        return {
            "status": "healthy",
            "dataset": "inputs_land_use",
            "records": result
