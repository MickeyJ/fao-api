...
from .inputs_pesticides_use_config import InputsPesticidesUseConfig
...
router = APIRouter(
    prefix="/inputs_pesticides_use",
    responses={404: {"description": "Not found"}},
)


config = InputsPesticidesUseConfig()

@router.get("/", summary="Get inputs pesticides use data")
async def get_inputs_pesticides_use_data(
    request: Request,
    response: Response,
    ...
    """Get inputs pesticides use data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InputsPesticidesUse, 
        model_name="InputsPesticidesUse",
        table_name="inputs_pesticides_use",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated inputs pesticides use data")
async def get_inputs_pesticides_use_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InputsPesticidesUse))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in inputs_pesticides_use")
@cache_result(prefix="inputs_pesticides_use:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InputsPesticidesUse.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InputsPesticidesUse, InputsPesticidesUse.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'inputs_pesticides_use')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in inputs_pesticides_use")
@cache_result(prefix="inputs_pesticides_use:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InputsPesticidesUse, InputsPesticidesUse.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'inputs_pesticides_use')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in inputs_pesticides_use")
@cache_result(prefix="inputs_pesticides_use:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InputsPesticidesUse, InputsPesticidesUse.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'inputs_pesticides_use')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in inputs_pesticides_use")
@cache_result(prefix="inputs_pesticides_use:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InputsPesticidesUse, Flags.id == InputsPesticidesUse.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InputsPesticidesUse.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InputsPesticidesUse)
                .where(InputsPesticidesUse.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InputsPesticidesUse)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in inputs_pesticides_use")
@cache_result(prefix="inputs_pesticides_use:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InputsPesticidesUse.unit,
            func.count(InputsPesticidesUse.id).label('record_count')
        )
        .select_from(InputsPesticidesUse)
        .group_by(InputsPesticidesUse.unit)
        .order_by(InputsPesticidesUse.unit)
    )
...
@router.get("/years", summary="Get available years in inputs_pesticides_use")
@cache_result(prefix="inputs_pesticides_use:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InputsPesticidesUse.year,
                InputsPesticidesUse.year_code,
                func.count(InputsPesticidesUse.id).label('record_count')
            )
            .group_by(InputsPesticidesUse.year, InputsPesticidesUse.year_code)
            .order_by(InputsPesticidesUse.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InputsPesticidesUse.year)
            .distinct()
            .order_by(InputsPesticidesUse.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of inputs_pesticides_use dataset")
@cache_result(prefix="inputs_pesticides_use:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InputsPesticidesUse.area_code_id)))
        .select_from(InputsPesticidesUse)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/inputs_pesticides_use/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InputsPesticidesUse.item_code_id)))
        .select_from(InputsPesticidesUse)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/inputs_pesticides_use/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InputsPesticidesUse.element_code_id)))
        .select_from(InputsPesticidesUse)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/inputs_pesticides_use/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InputsPesticidesUse.flag_id)))
        .select_from(InputsPesticidesUse)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/inputs_pesticides_use/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InputsPesticidesUse.year).label('min_year'),
            func.max(InputsPesticidesUse.year).label('max_year'),
            func.count(func.distinct(InputsPesticidesUse.year)).label('year_count')
        )
        .select_from(InputsPesticidesUse)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/inputs_pesticides_use/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InputsPesticidesUse.value).label('min_value'),
            func.max(InputsPesticidesUse.value).label('max_value'),
            func.avg(InputsPesticidesUse.value).label('avg_value')
        )
        .select_from(InputsPesticidesUse)
        .where(and_(InputsPesticidesUse.value > 0, InputsPesticidesUse.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/inputs_pesticides_use",
        "aggregate": f"/inputs_pesticides_use/aggregate",
        "summary": f"/inputs_pesticides_use/summary",
        "overview": f"/inputs_pesticides_use/overview",
        "area_codes": f"/inputs_pesticides_use/area_codes",
        "item_codes": f"/inputs_pesticides_use/item_codes",
        "elements": f"/inputs_pesticides_use/elements",
        "flags": f"/inputs_pesticides_use/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the inputs_pesticides_use endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InputsPesticidesUse)).scalar()
        return {
            "status": "healthy",
            "dataset": "inputs_pesticides_use",
            "records": result
