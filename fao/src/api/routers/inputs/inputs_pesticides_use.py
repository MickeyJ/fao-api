...
@router.get("/")
@cache_result(prefix="inputs_pesticides_use", ttl=86400, exclude_params=["response", "db"])
def get_inputs_pesticides_use(
    response: Response,
    ...
    """
    Get inputs pesticides use data with filters and pagination.
    ...
    # Main table columns
    for col in InputsPesticidesUse.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InputsPesticidesUse, 'id'):
                columns.append(InputsPesticidesUse.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InputsPesticidesUse)
    
    # Add joins
    query = query.outerjoin(AreaCodes, InputsPesticidesUse.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, InputsPesticidesUse.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, InputsPesticidesUse.element_code_id == Elements.id)
    query = query.outerjoin(Flags, InputsPesticidesUse.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(InputsPesticidesUse.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InputsPesticidesUse.year_code == year_code_exact)
    if year is not None:
        conditions.append(InputsPesticidesUse.year == year)
    if year_min is not None:
        conditions.append(InputsPesticidesUse.year >= year_min)
    if year_max is not None:
        conditions.append(InputsPesticidesUse.year <= year_max)
    if unit is not None:
        conditions.append(InputsPesticidesUse.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InputsPesticidesUse.unit == unit_exact)
    if value is not None:
        conditions.append(InputsPesticidesUse.value == value)
    if value_min is not None:
        conditions.append(InputsPesticidesUse.value >= value_min)
    if value_max is not None:
        conditions.append(InputsPesticidesUse.value <= value_max)
    if note is not None:
        conditions.append(InputsPesticidesUse.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(InputsPesticidesUse.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InputsPesticidesUse, field_name):
                        order_by_clauses.append(getattr(InputsPesticidesUse, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InputsPesticidesUse, sort_field):
                        order_by_clauses.append(getattr(InputsPesticidesUse, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InputsPesticidesUse.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InputsPesticidesUse)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InputsPesticidesUse.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InputsPesticidesUse.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InputsPesticidesUse.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InputsPesticidesUse.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_inputs_pesticides_use'))
...
@router.get("/areas")
@cache_result(prefix="inputs_pesticides_use:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="inputs_pesticides_use:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InputsPesticidesUse.year)
    ...
    if not years_data:
        return {"dataset": "inputs_pesticides_use", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="inputs_pesticides_use:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InputsPesticidesUse.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InputsPesticidesUse.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InputsPesticidesUse.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InputsPesticidesUse.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(InputsPesticidesUse.flag_id))).scalar()
