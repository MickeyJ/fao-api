...
from .inputs_fertilizers_product_config import InputsFertilizersProductConfig
...
router = APIRouter(
    prefix="/inputs_fertilizers_product",
    responses={404: {"description": "Not found"}},
)


config = InputsFertilizersProductConfig()

@router.get("/", summary="Get inputs fertilizers product data")
async def get_inputs_fertilizers_product_data(
    request: Request,
    response: Response,
    ...
    """Get inputs fertilizers product data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InputsFertilizersProduct, 
        model_name="InputsFertilizersProduct",
        table_name="inputs_fertilizers_product",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated inputs fertilizers product data")
async def get_inputs_fertilizers_product_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InputsFertilizersProduct))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InputsFertilizersProduct, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InputsFertilizersProduct, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InputsFertilizersProduct, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in inputs_fertilizers_product")
@cache_result(prefix="inputs_fertilizers_product:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InputsFertilizersProduct.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InputsFertilizersProduct, InputsFertilizersProduct.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'inputs_fertilizers_product')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="inputs_fertilizers_product",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in inputs_fertilizers_product")
@cache_result(prefix="inputs_fertilizers_product:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InputsFertilizersProduct, InputsFertilizersProduct.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'inputs_fertilizers_product')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in inputs_fertilizers_product")
@cache_result(prefix="inputs_fertilizers_product:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InputsFertilizersProduct, InputsFertilizersProduct.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'inputs_fertilizers_product')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in inputs_fertilizers_product")
@cache_result(prefix="inputs_fertilizers_product:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InputsFertilizersProduct, Flags.id == InputsFertilizersProduct.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InputsFertilizersProduct.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InputsFertilizersProduct)
                .where(InputsFertilizersProduct.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InputsFertilizersProduct)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in inputs_fertilizers_product")
@cache_result(prefix="inputs_fertilizers_product:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InputsFertilizersProduct.unit,
            func.count(InputsFertilizersProduct.id).label('record_count')
        )
        .select_from(InputsFertilizersProduct)
        .group_by(InputsFertilizersProduct.unit)
        .order_by(InputsFertilizersProduct.unit)
    )
...
@router.get("/years", summary="Get available years in inputs_fertilizers_product")
@cache_result(prefix="inputs_fertilizers_product:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InputsFertilizersProduct.year,
                InputsFertilizersProduct.year_code,
                func.count(InputsFertilizersProduct.id).label('record_count')
            )
            .group_by(InputsFertilizersProduct.year, InputsFertilizersProduct.year_code)
            .order_by(InputsFertilizersProduct.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InputsFertilizersProduct.year)
            .distinct()
            .order_by(InputsFertilizersProduct.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of inputs_fertilizers_product dataset")
@cache_result(prefix="inputs_fertilizers_product:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersProduct.area_code_id)))
        .select_from(InputsFertilizersProduct)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/inputs_fertilizers_product/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersProduct.item_code_id)))
        .select_from(InputsFertilizersProduct)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/inputs_fertilizers_product/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersProduct.element_code_id)))
        .select_from(InputsFertilizersProduct)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/inputs_fertilizers_product/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersProduct.flag_id)))
        .select_from(InputsFertilizersProduct)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/inputs_fertilizers_product/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InputsFertilizersProduct.year).label('min_year'),
            func.max(InputsFertilizersProduct.year).label('max_year'),
            func.count(func.distinct(InputsFertilizersProduct.year)).label('year_count')
        )
        .select_from(InputsFertilizersProduct)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/inputs_fertilizers_product/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InputsFertilizersProduct.value).label('min_value'),
            func.max(InputsFertilizersProduct.value).label('max_value'),
            func.avg(InputsFertilizersProduct.value).label('avg_value')
        )
        .select_from(InputsFertilizersProduct)
        .where(and_(InputsFertilizersProduct.value > 0, InputsFertilizersProduct.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/inputs_fertilizers_product",
        "aggregate": f"/inputs_fertilizers_product/aggregate",
        "summary": f"/inputs_fertilizers_product/summary",
        "overview": f"/inputs_fertilizers_product/overview",
        "area_codes": f"/inputs_fertilizers_product/area_codes",
        "item_codes": f"/inputs_fertilizers_product/item_codes",
        "elements": f"/inputs_fertilizers_product/elements",
        "flags": f"/inputs_fertilizers_product/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the inputs_fertilizers_product endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InputsFertilizersProduct)).scalar()
        return {
            "status": "healthy",
            "dataset": "inputs_fertilizers_product",
            "records": result
