...
@router.get("/")
@cache_result(prefix="inputs_fertilizers_product", ttl=86400, exclude_params=["response", "db"])
def get_inputs_fertilizers_product(
    response: Response,
    ...
    """
    Get inputs fertilizers product data with filters and pagination.
    ...
    # Main table columns
    for col in InputsFertilizersProduct.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InputsFertilizersProduct, 'id'):
                columns.append(InputsFertilizersProduct.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InputsFertilizersProduct)
    
    # Add joins
    query = query.outerjoin(AreaCodes, InputsFertilizersProduct.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, InputsFertilizersProduct.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, InputsFertilizersProduct.element_code_id == Elements.id)
    query = query.outerjoin(Flags, InputsFertilizersProduct.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(InputsFertilizersProduct.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InputsFertilizersProduct.year_code == year_code_exact)
    if year is not None:
        conditions.append(InputsFertilizersProduct.year == year)
    if year_min is not None:
        conditions.append(InputsFertilizersProduct.year >= year_min)
    if year_max is not None:
        conditions.append(InputsFertilizersProduct.year <= year_max)
    if unit is not None:
        conditions.append(InputsFertilizersProduct.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InputsFertilizersProduct.unit == unit_exact)
    if value is not None:
        conditions.append(InputsFertilizersProduct.value == value)
    if value_min is not None:
        conditions.append(InputsFertilizersProduct.value >= value_min)
    if value_max is not None:
        conditions.append(InputsFertilizersProduct.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InputsFertilizersProduct, field_name):
                        order_by_clauses.append(getattr(InputsFertilizersProduct, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InputsFertilizersProduct, sort_field):
                        order_by_clauses.append(getattr(InputsFertilizersProduct, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InputsFertilizersProduct.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InputsFertilizersProduct)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InputsFertilizersProduct.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InputsFertilizersProduct.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InputsFertilizersProduct.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InputsFertilizersProduct.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_inputs_fertilizers_product'))
...
@router.get("/areas")
@cache_result(prefix="inputs_fertilizers_product:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="inputs_fertilizers_product:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InputsFertilizersProduct.year)
    ...
    if not years_data:
        return {"dataset": "inputs_fertilizers_product", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="inputs_fertilizers_product:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InputsFertilizersProduct.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InputsFertilizersProduct.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InputsFertilizersProduct.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InputsFertilizersProduct.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(InputsFertilizersProduct.flag_id))).scalar()
