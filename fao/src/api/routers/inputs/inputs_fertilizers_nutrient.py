...
@router.get("/")
@cache_result(prefix="inputs_fertilizers_nutrient", ttl=86400, exclude_params=["response", "db"])
def get_inputs_fertilizers_nutrient(
    response: Response,
    ...
    """
    Get inputs fertilizers nutrient data with filters and pagination.
    ...
    # Main table columns
    for col in InputsFertilizersNutrient.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InputsFertilizersNutrient, 'id'):
                columns.append(InputsFertilizersNutrient.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InputsFertilizersNutrient)
    
    # Add joins
    query = query.outerjoin(AreaCodes, InputsFertilizersNutrient.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, InputsFertilizersNutrient.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, InputsFertilizersNutrient.element_code_id == Elements.id)
    query = query.outerjoin(Flags, InputsFertilizersNutrient.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(InputsFertilizersNutrient.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InputsFertilizersNutrient.year_code == year_code_exact)
    if year is not None:
        conditions.append(InputsFertilizersNutrient.year == year)
    if year_min is not None:
        conditions.append(InputsFertilizersNutrient.year >= year_min)
    if year_max is not None:
        conditions.append(InputsFertilizersNutrient.year <= year_max)
    if unit is not None:
        conditions.append(InputsFertilizersNutrient.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InputsFertilizersNutrient.unit == unit_exact)
    if value is not None:
        conditions.append(InputsFertilizersNutrient.value == value)
    if value_min is not None:
        conditions.append(InputsFertilizersNutrient.value >= value_min)
    if value_max is not None:
        conditions.append(InputsFertilizersNutrient.value <= value_max)
    if note is not None:
        conditions.append(InputsFertilizersNutrient.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(InputsFertilizersNutrient.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InputsFertilizersNutrient, field_name):
                        order_by_clauses.append(getattr(InputsFertilizersNutrient, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InputsFertilizersNutrient, sort_field):
                        order_by_clauses.append(getattr(InputsFertilizersNutrient, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InputsFertilizersNutrient.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InputsFertilizersNutrient)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InputsFertilizersNutrient.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InputsFertilizersNutrient.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InputsFertilizersNutrient.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InputsFertilizersNutrient.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_inputs_fertilizers_nutrient'))
...
@router.get("/areas")
@cache_result(prefix="inputs_fertilizers_nutrient:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="inputs_fertilizers_nutrient:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(InputsFertilizersNutrient.year)
    ...
    if not years_data:
        return {"dataset": "inputs_fertilizers_nutrient", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="inputs_fertilizers_nutrient:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(InputsFertilizersNutrient.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(InputsFertilizersNutrient.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(InputsFertilizersNutrient.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(InputsFertilizersNutrient.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(InputsFertilizersNutrient.flag_id))).scalar()
