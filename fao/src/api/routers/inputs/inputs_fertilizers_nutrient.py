...
from .inputs_fertilizers_nutrient_config import InputsFertilizersNutrientConfig
...
router = APIRouter(
    prefix="/inputs_fertilizers_nutrient",
    responses={404: {"description": "Not found"}},
)


config = InputsFertilizersNutrientConfig()

@router.get("/", summary="Get inputs fertilizers nutrient data")
async def get_inputs_fertilizers_nutrient_data(
    request: Request,
    response: Response,
    ...
    """Get inputs fertilizers nutrient data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InputsFertilizersNutrient, 
        model_name="InputsFertilizersNutrient",
        table_name="inputs_fertilizers_nutrient",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated inputs fertilizers nutrient data")
async def get_inputs_fertilizers_nutrient_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InputsFertilizersNutrient))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InputsFertilizersNutrient, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InputsFertilizersNutrient, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InputsFertilizersNutrient, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in inputs_fertilizers_nutrient")
@cache_result(prefix="inputs_fertilizers_nutrient:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(InputsFertilizersNutrient.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(InputsFertilizersNutrient, InputsFertilizersNutrient.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'inputs_fertilizers_nutrient')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="inputs_fertilizers_nutrient",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in inputs_fertilizers_nutrient")
@cache_result(prefix="inputs_fertilizers_nutrient:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InputsFertilizersNutrient, InputsFertilizersNutrient.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'inputs_fertilizers_nutrient')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in inputs_fertilizers_nutrient")
@cache_result(prefix="inputs_fertilizers_nutrient:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InputsFertilizersNutrient, InputsFertilizersNutrient.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'inputs_fertilizers_nutrient')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in inputs_fertilizers_nutrient")
@cache_result(prefix="inputs_fertilizers_nutrient:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(InputsFertilizersNutrient, Flags.id == InputsFertilizersNutrient.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(InputsFertilizersNutrient.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(InputsFertilizersNutrient)
                .where(InputsFertilizersNutrient.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InputsFertilizersNutrient)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in inputs_fertilizers_nutrient")
@cache_result(prefix="inputs_fertilizers_nutrient:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InputsFertilizersNutrient.unit,
            func.count(InputsFertilizersNutrient.id).label('record_count')
        )
        .select_from(InputsFertilizersNutrient)
        .group_by(InputsFertilizersNutrient.unit)
        .order_by(InputsFertilizersNutrient.unit)
    )
...
@router.get("/years", summary="Get available years in inputs_fertilizers_nutrient")
@cache_result(prefix="inputs_fertilizers_nutrient:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                InputsFertilizersNutrient.year,
                InputsFertilizersNutrient.year_code,
                func.count(InputsFertilizersNutrient.id).label('record_count')
            )
            .group_by(InputsFertilizersNutrient.year, InputsFertilizersNutrient.year_code)
            .order_by(InputsFertilizersNutrient.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InputsFertilizersNutrient.year)
            .distinct()
            .order_by(InputsFertilizersNutrient.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of inputs_fertilizers_nutrient dataset")
@cache_result(prefix="inputs_fertilizers_nutrient:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersNutrient.area_code_id)))
        .select_from(InputsFertilizersNutrient)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/inputs_fertilizers_nutrient/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersNutrient.item_code_id)))
        .select_from(InputsFertilizersNutrient)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/inputs_fertilizers_nutrient/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersNutrient.element_code_id)))
        .select_from(InputsFertilizersNutrient)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/inputs_fertilizers_nutrient/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersNutrient.flag_id)))
        .select_from(InputsFertilizersNutrient)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/inputs_fertilizers_nutrient/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InputsFertilizersNutrient.year).label('min_year'),
            func.max(InputsFertilizersNutrient.year).label('max_year'),
            func.count(func.distinct(InputsFertilizersNutrient.year)).label('year_count')
        )
        .select_from(InputsFertilizersNutrient)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/inputs_fertilizers_nutrient/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InputsFertilizersNutrient.value).label('min_value'),
            func.max(InputsFertilizersNutrient.value).label('max_value'),
            func.avg(InputsFertilizersNutrient.value).label('avg_value')
        )
        .select_from(InputsFertilizersNutrient)
        .where(and_(InputsFertilizersNutrient.value > 0, InputsFertilizersNutrient.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/inputs_fertilizers_nutrient",
        "aggregate": f"/inputs_fertilizers_nutrient/aggregate",
        "summary": f"/inputs_fertilizers_nutrient/summary",
        "overview": f"/inputs_fertilizers_nutrient/overview",
        "area_codes": f"/inputs_fertilizers_nutrient/area_codes",
        "item_codes": f"/inputs_fertilizers_nutrient/item_codes",
        "elements": f"/inputs_fertilizers_nutrient/elements",
        "flags": f"/inputs_fertilizers_nutrient/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the inputs_fertilizers_nutrient endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InputsFertilizersNutrient)).scalar()
        return {
            "status": "healthy",
            "dataset": "inputs_fertilizers_nutrient",
            "records": result
