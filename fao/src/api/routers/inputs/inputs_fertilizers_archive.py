...
from .inputs_fertilizers_archive_config import InputsFertilizersArchiveConfig
...
config = InputsFertilizersArchiveConfig()

@router.get("/", summary="Get inputs fertilizers archive data")
async def get_inputs_fertilizers_archive_data(
    request: Request,
    response: Response,
    ...
    """Get inputs fertilizers archive data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=InputsFertilizersArchive, 
        model_name="InputsFertilizersArchive",
        table_name="inputs_fertilizers_archive",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated inputs fertilizers archive data")
async def get_inputs_fertilizers_archive_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(InputsFertilizersArchive))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(InputsFertilizersArchive, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(InputsFertilizersArchive, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(InputsFertilizersArchive, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in inputs_fertilizers_archive")
@cache_result(prefix="inputs_fertilizers_archive:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(InputsFertilizersArchive, InputsFertilizersArchive.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'inputs_fertilizers_archive')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in inputs_fertilizers_archive")
@cache_result(prefix="inputs_fertilizers_archive:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(InputsFertilizersArchive, InputsFertilizersArchive.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'inputs_fertilizers_archive')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in inputs_fertilizers_archive")
@cache_result(prefix="inputs_fertilizers_archive:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(InputsFertilizersArchive, InputsFertilizersArchive.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'inputs_fertilizers_archive')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in inputs_fertilizers_archive")
@cache_result(prefix="inputs_fertilizers_archive:flags", ttl=604800)
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(InputsFertilizersArchive.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(InputsFertilizersArchive)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in inputs_fertilizers_archive")
@cache_result(prefix="inputs_fertilizers_archive:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            InputsFertilizersArchive.unit,
            func.count(InputsFertilizersArchive.id).label('record_count')
        )
        .select_from(InputsFertilizersArchive)
        .group_by(InputsFertilizersArchive.unit)
        .order_by(InputsFertilizersArchive.unit)
    )
...
@router.get("/years", summary="Get available years in inputs_fertilizers_archive")
@cache_result(prefix="inputs_fertilizers_archive:years", ttl=604800)
    ...
    if include_counts:
        query = (
            select(
                ...
                InputsFertilizersArchive.year_code,
                func.count(InputsFertilizersArchive.id).label('record_count')
            )
            .group_by(InputsFertilizersArchive.year, InputsFertilizersArchive.year_code)
            .order_by(InputsFertilizersArchive.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(InputsFertilizersArchive.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of inputs_fertilizers_archive dataset")
@cache_result(prefix="inputs_fertilizers_archive:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersArchive.area_code_id)))
        .select_from(InputsFertilizersArchive)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/inputs_fertilizers_archive/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersArchive.item_code_id)))
        .select_from(InputsFertilizersArchive)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/inputs_fertilizers_archive/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersArchive.element_code_id)))
        .select_from(InputsFertilizersArchive)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/inputs_fertilizers_archive/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(InputsFertilizersArchive.flag_id)))
        .select_from(InputsFertilizersArchive)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/inputs_fertilizers_archive/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(InputsFertilizersArchive.year).label('min_year'),
            func.max(InputsFertilizersArchive.year).label('max_year'),
            func.count(func.distinct(InputsFertilizersArchive.year)).label('year_count')
        )
        .select_from(InputsFertilizersArchive)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/inputs_fertilizers_archive/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(InputsFertilizersArchive.value).label('min_value'),
            func.max(InputsFertilizersArchive.value).label('max_value'),
            func.avg(InputsFertilizersArchive.value).label('avg_value')
        )
        .select_from(InputsFertilizersArchive)
        .where(and_(InputsFertilizersArchive.value > 0, InputsFertilizersArchive.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/inputs_fertilizers_archive",
        "aggregate": f"/inputs_fertilizers_archive/aggregate",
        "summary": f"/inputs_fertilizers_archive/summary",
        "overview": f"/inputs_fertilizers_archive/overview",
        "area_codes": f"/inputs_fertilizers_archive/area_codes",
        "item_codes": f"/inputs_fertilizers_archive/item_codes",
        "elements": f"/inputs_fertilizers_archive/elements",
        "flags": f"/inputs_fertilizers_archive/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the inputs_fertilizers_archive endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(InputsFertilizersArchive)).scalar()
        return {
            "status": "healthy",
            "dataset": "inputs_fertilizers_archive",
            "records": result
