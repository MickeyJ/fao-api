...
@router.get("/")
@cache_result(prefix="inputs_fertilizers_archive", ttl=86400, exclude_params=["response", "db"])
def get_inputs_fertilizers_archive(
    ...
    """
    Get inputs fertilizers archive data with filters and pagination.
    ...
    # Main table columns
    for col in InputsFertilizersArchive.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(InputsFertilizersArchive, 'id'):
                columns.append(InputsFertilizersArchive.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(InputsFertilizersArchive)
    
    # Add joins
    query = query.outerjoin(AreaCodes, InputsFertilizersArchive.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, InputsFertilizersArchive.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, InputsFertilizersArchive.element_code_id == Elements.id)
    query = query.outerjoin(Flags, InputsFertilizersArchive.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(InputsFertilizersArchive.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(InputsFertilizersArchive.year_code == year_code_exact)
    if year is not None:
        conditions.append(InputsFertilizersArchive.year == year)
    if year_min is not None:
        conditions.append(InputsFertilizersArchive.year >= year_min)
    if year_max is not None:
        conditions.append(InputsFertilizersArchive.year <= year_max)
    if unit is not None:
        conditions.append(InputsFertilizersArchive.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(InputsFertilizersArchive.unit == unit_exact)
    if value is not None:
        conditions.append(InputsFertilizersArchive.value == value)
    if value_min is not None:
        conditions.append(InputsFertilizersArchive.value >= value_min)
    if value_max is not None:
        conditions.append(InputsFertilizersArchive.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(InputsFertilizersArchive, field_name):
                        order_by_clauses.append(getattr(InputsFertilizersArchive, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(InputsFertilizersArchive, sort_field):
                        order_by_clauses.append(getattr(InputsFertilizersArchive, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(InputsFertilizersArchive.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(InputsFertilizersArchive)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, InputsFertilizersArchive.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, InputsFertilizersArchive.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, InputsFertilizersArchive.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, InputsFertilizersArchive.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_inputs_fertilizers_archive'))
...
@router.get("/areas")
@cache_result(prefix="inputs_fertilizers_archive:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="inputs_fertilizers_archive:items", ttl=604800)
...
@router.get("/summary")
@cache_result(prefix="inputs_fertilizers_archive:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    summary["unique_flags"] = db.query(func.count(func.distinct(InputsFertilizersArchive.flag_id))).scalar()
