...
from .production_indices_config import ProductionIndicesConfig
...
router = APIRouter(
    prefix="/production_indices",
    responses={404: {"description": "Not found"}},
)


config = ProductionIndicesConfig()

@router.get("/", summary="Get production indices data")
async def get_production_indices_data(
    request: Request,
    response: Response,
    ...
    """Get production indices data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=ProductionIndices, 
        model_name="ProductionIndices",
        table_name="production_indices",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated production indices data")
async def get_production_indices_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(ProductionIndices))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in production_indices")
@cache_result(prefix="production_indices:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(ProductionIndices.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(ProductionIndices, ProductionIndices.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'production_indices')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in production_indices")
@cache_result(prefix="production_indices:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(ProductionIndices, ProductionIndices.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'production_indices')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in production_indices")
@cache_result(prefix="production_indices:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(ProductionIndices, ProductionIndices.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'production_indices')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in production_indices")
@cache_result(prefix="production_indices:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(ProductionIndices, Flags.id == ProductionIndices.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(ProductionIndices.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(ProductionIndices)
                .where(ProductionIndices.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(ProductionIndices)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in production_indices")
@cache_result(prefix="production_indices:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            ProductionIndices.unit,
            func.count(ProductionIndices.id).label('record_count')
        )
        .select_from(ProductionIndices)
        .group_by(ProductionIndices.unit)
        .order_by(ProductionIndices.unit)
    )
...
@router.get("/years", summary="Get available years in production_indices")
@cache_result(prefix="production_indices:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                ProductionIndices.year,
                ProductionIndices.year_code,
                func.count(ProductionIndices.id).label('record_count')
            )
            .group_by(ProductionIndices.year, ProductionIndices.year_code)
            .order_by(ProductionIndices.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(ProductionIndices.year)
            .distinct()
            .order_by(ProductionIndices.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of production_indices dataset")
@cache_result(prefix="production_indices:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(ProductionIndices.area_code_id)))
        .select_from(ProductionIndices)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/production_indices/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(ProductionIndices.item_code_id)))
        .select_from(ProductionIndices)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/production_indices/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(ProductionIndices.element_code_id)))
        .select_from(ProductionIndices)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/production_indices/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(ProductionIndices.flag_id)))
        .select_from(ProductionIndices)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/production_indices/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(ProductionIndices.year).label('min_year'),
            func.max(ProductionIndices.year).label('max_year'),
            func.count(func.distinct(ProductionIndices.year)).label('year_count')
        )
        .select_from(ProductionIndices)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/production_indices/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(ProductionIndices.value).label('min_value'),
            func.max(ProductionIndices.value).label('max_value'),
            func.avg(ProductionIndices.value).label('avg_value')
        )
        .select_from(ProductionIndices)
        .where(and_(ProductionIndices.value > 0, ProductionIndices.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/production_indices",
        "aggregate": f"/production_indices/aggregate",
        "summary": f"/production_indices/summary",
        "overview": f"/production_indices/overview",
        "area_codes": f"/production_indices/area_codes",
        "item_codes": f"/production_indices/item_codes",
        "elements": f"/production_indices/elements",
        "flags": f"/production_indices/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the production_indices endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(ProductionIndices)).scalar()
        return {
            "status": "healthy",
            "dataset": "production_indices",
            "records": result
