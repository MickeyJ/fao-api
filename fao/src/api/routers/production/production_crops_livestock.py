...
async def get_production_crops_livestock_aggregated(
    ...
    # Build query
    query_builder = QueryBuilder(select(ProductionCropsLivestock))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(ProductionCropsLivestock, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(ProductionCropsLivestock, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(ProductionCropsLivestock, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in production_crops_livestock")
...
@router.get("/item_codes", summary="Get item_codes in production_crops_livestock")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(ProductionCropsLivestock, ProductionCropsLivestock.element_code_id == Elements.id)
