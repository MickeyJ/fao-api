...
@router.get("/")
@cache_result(prefix="production_crops_livestock", ttl=86400, exclude_params=["response", "db"])
def get_production_crops_livestock(
    response: Response,
    ...
    """
    Get production crops livestock data with filters and pagination.
    ...
    # Main table columns
    for col in ProductionCropsLivestock.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(ProductionCropsLivestock, 'id'):
                columns.append(ProductionCropsLivestock.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(ProductionCropsLivestock)
    
    # Add joins
    query = query.outerjoin(AreaCodes, ProductionCropsLivestock.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, ProductionCropsLivestock.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, ProductionCropsLivestock.element_code_id == Elements.id)
    query = query.outerjoin(Flags, ProductionCropsLivestock.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(ProductionCropsLivestock.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(ProductionCropsLivestock.year_code == year_code_exact)
    if year is not None:
        conditions.append(ProductionCropsLivestock.year == year)
    if year_min is not None:
        conditions.append(ProductionCropsLivestock.year >= year_min)
    if year_max is not None:
        conditions.append(ProductionCropsLivestock.year <= year_max)
    if unit is not None:
        conditions.append(ProductionCropsLivestock.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(ProductionCropsLivestock.unit == unit_exact)
    if value is not None:
        conditions.append(ProductionCropsLivestock.value == value)
    if value_min is not None:
        conditions.append(ProductionCropsLivestock.value >= value_min)
    if value_max is not None:
        conditions.append(ProductionCropsLivestock.value <= value_max)
    if note is not None:
        conditions.append(ProductionCropsLivestock.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(ProductionCropsLivestock.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(ProductionCropsLivestock, field_name):
                        order_by_clauses.append(getattr(ProductionCropsLivestock, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(ProductionCropsLivestock, sort_field):
                        order_by_clauses.append(getattr(ProductionCropsLivestock, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(ProductionCropsLivestock.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(ProductionCropsLivestock)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, ProductionCropsLivestock.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, ProductionCropsLivestock.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, ProductionCropsLivestock.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, ProductionCropsLivestock.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_production_crops_livestock'))
...
@router.get("/areas")
@cache_result(prefix="production_crops_livestock:areas", ttl=604800)
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(ProductionCropsLivestock, AreaCodes.id == ProductionCropsLivestock.area_code_id)
...
@router.get("/items")
@cache_result(prefix="production_crops_livestock:items", ttl=604800)
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(ProductionCropsLivestock, ItemCodes.id == ProductionCropsLivestock.item_code_id)
...
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(ProductionCropsLivestock, Elements.id == ProductionCropsLivestock.element_code_id)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(ProductionCropsLivestock.year)
    ...
    if not years_data:
        return {"dataset": "production_crops_livestock", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="production_crops_livestock:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(ProductionCropsLivestock.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(ProductionCropsLivestock.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(ProductionCropsLivestock.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(ProductionCropsLivestock.element_code_id))).scalar()
