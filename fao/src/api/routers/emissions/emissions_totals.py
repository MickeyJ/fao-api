...
from .emissions_totals_config import EmissionsTotalsConfig
...
config = EmissionsTotalsConfig()

@router.get("/", summary="Get emissions totals data")
async def get_emissions_totals_data(
    request: Request,
    response: Response,
    ...
    """Get emissions totals data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EmissionsTotals, 
        model_name="EmissionsTotals",
        table_name="emissions_totals",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated emissions totals data")
async def get_emissions_totals_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EmissionsTotals))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in emissions_totals")
@cache_result(prefix="emissions_totals:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(EmissionsTotals, EmissionsTotals.area_code_id == AreaCodes.id)
...
@router.get("/item_codes", summary="Get item_codes in emissions_totals")
@cache_result(prefix="emissions_totals:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EmissionsTotals, EmissionsTotals.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in emissions_totals")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmissionsTotals, EmissionsTotals.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in emissions_totals")
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EmissionsTotals)
                .where(EmissionsTotals.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmissionsTotals)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in emissions_totals")
@cache_result(prefix="emissions_totals:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EmissionsTotals.unit,
            func.count(EmissionsTotals.id).label('record_count')
        )
        .select_from(EmissionsTotals)
        .group_by(EmissionsTotals.unit)
        .order_by(EmissionsTotals.unit)
    )
...
@router.get("/years", summary="Get available years in emissions_totals")
    ...
    if include_counts:
        query = (
            select(
                ...
                EmissionsTotals.year_code,
                func.count(EmissionsTotals.id).label('record_count')
            )
            .group_by(EmissionsTotals.year, EmissionsTotals.year_code)
            .order_by(EmissionsTotals.year_code)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of emissions_totals dataset")
@cache_result(prefix="emissions_totals:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsTotals.area_code_id)))
        .select_from(EmissionsTotals)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/emissions_totals/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsTotals.item_code_id)))
        .select_from(EmissionsTotals)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/emissions_totals/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsTotals.element_code_id)))
        .select_from(EmissionsTotals)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/emissions_totals/elements"
    }

    source_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsTotals.source_code_id)))
        .select_from(EmissionsTotals)
    ).scalar() or 0

    overview["dimensions"]["sources"] = {
        "count": source_code_ids,
        "endpoint": f"/emissions_totals/sources"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmissionsTotals.flag_id)))
        .select_from(EmissionsTotals)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/emissions_totals/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmissionsTotals.year).label('min_year'),
            func.max(EmissionsTotals.year).label('max_year'),
            func.count(func.distinct(EmissionsTotals.year)).label('year_count')
        )
        .select_from(EmissionsTotals)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/emissions_totals/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmissionsTotals.value).label('min_value'),
            func.max(EmissionsTotals.value).label('max_value'),
            func.avg(EmissionsTotals.value).label('avg_value')
        )
        .select_from(EmissionsTotals)
        .where(and_(EmissionsTotals.value > 0, EmissionsTotals.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/emissions_totals",
        "aggregate": f"/emissions_totals/aggregate",
        "summary": f"/emissions_totals/summary",
        "overview": f"/emissions_totals/overview",
        "area_codes": f"/emissions_totals/area_codes",
        "item_codes": f"/emissions_totals/item_codes",
        "elements": f"/emissions_totals/elements",
        "flags": f"/emissions_totals/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the emissions_totals endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmissionsTotals)).scalar()
        return {
            "status": "healthy",
            "dataset": "emissions_totals",
            "records": result
