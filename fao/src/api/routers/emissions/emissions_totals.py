...
@router.get("/")
@cache_result(prefix="emissions_totals", ttl=86400, exclude_params=["response", "db"])
def get_emissions_totals(
    response: Response,
    ...
    """
    Get emissions totals data with filters and pagination.
    ...
    # Main table columns
    for col in EmissionsTotals.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmissionsTotals, 'id'):
                columns.append(EmissionsTotals.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmissionsTotals)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EmissionsTotals.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EmissionsTotals.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EmissionsTotals.element_code_id == Elements.id)
    query = query.outerjoin(Sources, EmissionsTotals.source_code_id == Sources.id)
    query = query.outerjoin(Flags, EmissionsTotals.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EmissionsTotals.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmissionsTotals.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmissionsTotals.year == year)
    if year_min is not None:
        conditions.append(EmissionsTotals.year >= year_min)
    if year_max is not None:
        conditions.append(EmissionsTotals.year <= year_max)
    if unit is not None:
        conditions.append(EmissionsTotals.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmissionsTotals.unit == unit_exact)
    if value is not None:
        conditions.append(EmissionsTotals.value == value)
    if value_min is not None:
        conditions.append(EmissionsTotals.value >= value_min)
    if value_max is not None:
        conditions.append(EmissionsTotals.value <= value_max)
    if note is not None:
        conditions.append(EmissionsTotals.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EmissionsTotals.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmissionsTotals, field_name):
                        order_by_clauses.append(getattr(EmissionsTotals, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmissionsTotals, sort_field):
                        order_by_clauses.append(getattr(EmissionsTotals, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmissionsTotals.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmissionsTotals)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmissionsTotals.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EmissionsTotals.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EmissionsTotals.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Sources, EmissionsTotals.source_code_id == Sources.id)
    count_query = count_query.outerjoin(Flags, EmissionsTotals.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_emissions_totals'))
...
@router.get("/areas")
@cache_result(prefix="emissions_totals:areas", ttl=604800)
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(AreaCodes.source_dataset == 'emissions_totals')
...
@router.get("/items")
@cache_result(prefix="emissions_totals:items", ttl=604800)
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(ItemCodes.source_dataset == 'emissions_totals')
...
@router.get("/elements")
@cache_result(prefix="emissions_totals:elements", ttl=604800)
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Elements.source_dataset == 'emissions_totals')
...
@router.get("/sources")
@cache_result(prefix="emissions_totals:sources", ttl=604800)
def get_available_sources(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Sources.source_dataset == 'emissions_totals')
...
@router.get("/flags")
@cache_result(prefix="emissions_totals:flags", ttl=604800)
def get_data_quality_summary(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            Flags.flag,
            Flags.description,
            func.count(EmissionsTotals.id).label('record_count')
        )
        .join(EmissionsTotals, Flags.id == EmissionsTotals.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EmissionsTotals.id).desc())
    )
...
@router.get("/years")
@cache_result(prefix="emissions_totals:years", ttl=604800)
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            EmissionsTotals.year,
            func.count(EmissionsTotals.id).label('record_count')
        )
        .group_by(EmissionsTotals.year)
        .order_by(EmissionsTotals.year)
    )
    ...
    if not years_data:
        return {"dataset": "emissions_totals", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="emissions_totals:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmissionsTotals.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EmissionsTotals.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EmissionsTotals.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EmissionsTotals.element_code_id))).scalar()
    summary["unique_sources"] = db.query(func.count(func.distinct(EmissionsTotals.source_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EmissionsTotals.flag_id))).scalar()
