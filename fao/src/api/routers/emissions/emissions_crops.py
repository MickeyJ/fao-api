...
@router.get("/")
@cache_result(prefix="emissions_crops", ttl=86400, exclude_params=["response", "db"])
def get_emissions_crops(
    response: Response,
    ...
    """
    Get emissions crops data with filters and pagination.
    ...
    # Main table columns
    for col in EmissionsCrops.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmissionsCrops, 'id'):
                columns.append(EmissionsCrops.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmissionsCrops)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EmissionsCrops.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EmissionsCrops.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EmissionsCrops.element_code_id == Elements.id)
    query = query.outerjoin(Sources, EmissionsCrops.source_code_id == Sources.id)
    query = query.outerjoin(Flags, EmissionsCrops.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EmissionsCrops.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmissionsCrops.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmissionsCrops.year == year)
    if year_min is not None:
        conditions.append(EmissionsCrops.year >= year_min)
    if year_max is not None:
        conditions.append(EmissionsCrops.year <= year_max)
    if unit is not None:
        conditions.append(EmissionsCrops.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmissionsCrops.unit == unit_exact)
    if value is not None:
        conditions.append(EmissionsCrops.value == value)
    if value_min is not None:
        conditions.append(EmissionsCrops.value >= value_min)
    if value_max is not None:
        conditions.append(EmissionsCrops.value <= value_max)
    if note is not None:
        conditions.append(EmissionsCrops.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EmissionsCrops.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmissionsCrops, field_name):
                        order_by_clauses.append(getattr(EmissionsCrops, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmissionsCrops, sort_field):
                        order_by_clauses.append(getattr(EmissionsCrops, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmissionsCrops.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmissionsCrops)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmissionsCrops.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EmissionsCrops.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EmissionsCrops.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Sources, EmissionsCrops.source_code_id == Sources.id)
    count_query = count_query.outerjoin(Flags, EmissionsCrops.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_emissions_crops'))
...
@router.get("/areas")
@cache_result(prefix="emissions_crops:areas", ttl=604800)
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(AreaCodes.source_dataset == 'emissions_crops')
...
@router.get("/items")
@cache_result(prefix="emissions_crops:items", ttl=604800)
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(ItemCodes.source_dataset == 'emissions_crops')
...
@router.get("/elements")
@cache_result(prefix="emissions_crops:elements", ttl=604800)
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Elements.source_dataset == 'emissions_crops')
...
@router.get("/sources")
@cache_result(prefix="emissions_crops:sources", ttl=604800)
def get_available_sources(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Sources.source_dataset == 'emissions_crops')
...
@router.get("/flags")
@cache_result(prefix="emissions_crops:flags", ttl=604800)
def get_data_quality_summary(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            Flags.flag,
            Flags.description,
            func.count(EmissionsCrops.id).label('record_count')
        )
        .join(EmissionsCrops, Flags.id == EmissionsCrops.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EmissionsCrops.id).desc())
    )
...
@router.get("/years")
@cache_result(prefix="emissions_crops:years", ttl=604800)
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EmissionsCrops.year)
        .order_by(EmissionsCrops.year)
    )
    ...
    if not years_data:
        return {"dataset": "emissions_crops", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="emissions_crops:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmissionsCrops.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EmissionsCrops.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EmissionsCrops.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EmissionsCrops.element_code_id))).scalar()
    summary["unique_sources"] = db.query(func.count(func.distinct(EmissionsCrops.source_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EmissionsCrops.flag_id))).scalar()
