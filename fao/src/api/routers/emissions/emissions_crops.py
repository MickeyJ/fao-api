...
from .emissions_crops_config import EmissionsCropsConfig
...
config = EmissionsCropsConfig()

@router.get("/", summary="Get emissions crops data")
async def get_emissions_crops_data(
    request: Request,
    response: Response,
    ...
    """Get emissions crops data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EmissionsCrops, 
        model_name="EmissionsCrops",
        table_name="emissions_crops",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated emissions crops data")
async def get_emissions_crops_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EmissionsCrops))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in emissions_crops")
@cache_result(prefix="emissions_crops:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(EmissionsCrops, EmissionsCrops.area_code_id == AreaCodes.id)
...
@router.get("/item_codes", summary="Get item_codes in emissions_crops")
@cache_result(prefix="emissions_crops:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EmissionsCrops, EmissionsCrops.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in emissions_crops")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmissionsCrops, EmissionsCrops.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in emissions_crops")
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EmissionsCrops)
                .where(EmissionsCrops.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmissionsCrops)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in emissions_crops")
@cache_result(prefix="emissions_crops:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(EmissionsCrops)
        .group_by(EmissionsCrops.unit)
        .order_by(EmissionsCrops.unit)
    )
...
@router.get("/years", summary="Get available years in emissions_crops")
...
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EmissionsCrops.year,
                EmissionsCrops.year_code,
                func.count(EmissionsCrops.id).label('record_count')
            )
            .group_by(EmissionsCrops.year, EmissionsCrops.year_code)
            .order_by(EmissionsCrops.year_code)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of emissions_crops dataset")
@cache_result(prefix="emissions_crops:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsCrops.area_code_id)))
        .select_from(EmissionsCrops)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/emissions_crops/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsCrops.item_code_id)))
        .select_from(EmissionsCrops)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/emissions_crops/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsCrops.element_code_id)))
        .select_from(EmissionsCrops)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/emissions_crops/elements"
    }

    source_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsCrops.source_code_id)))
        .select_from(EmissionsCrops)
    ).scalar() or 0

    overview["dimensions"]["sources"] = {
        "count": source_code_ids,
        "endpoint": f"/emissions_crops/sources"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmissionsCrops.flag_id)))
        .select_from(EmissionsCrops)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/emissions_crops/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmissionsCrops.year).label('min_year'),
            func.max(EmissionsCrops.year).label('max_year'),
            func.count(func.distinct(EmissionsCrops.year)).label('year_count')
        )
        .select_from(EmissionsCrops)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/emissions_crops/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmissionsCrops.value).label('min_value'),
            func.max(EmissionsCrops.value).label('max_value'),
            func.avg(EmissionsCrops.value).label('avg_value')
        )
        .select_from(EmissionsCrops)
        .where(and_(EmissionsCrops.value > 0, EmissionsCrops.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/emissions_crops",
        "aggregate": f"/emissions_crops/aggregate",
        "summary": f"/emissions_crops/summary",
        "overview": f"/emissions_crops/overview",
        "area_codes": f"/emissions_crops/area_codes",
        "item_codes": f"/emissions_crops/item_codes",
        "elements": f"/emissions_crops/elements",
        "flags": f"/emissions_crops/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the emissions_crops endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmissionsCrops)).scalar()
        return {
            "status": "healthy",
            "dataset": "emissions_crops",
            "records": result
