...
@router.get("/")
@cache_result(prefix="emissions_livestock", ttl=86400, exclude_params=["response", "db"])
def get_emissions_livestock(
    response: Response,
    ...
    """
    Get emissions livestock data with filters and pagination.
    ...
    # Main table columns
    for col in EmissionsLivestock.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmissionsLivestock, 'id'):
                columns.append(EmissionsLivestock.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmissionsLivestock)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EmissionsLivestock.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EmissionsLivestock.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EmissionsLivestock.element_code_id == Elements.id)
    query = query.outerjoin(Sources, EmissionsLivestock.source_code_id == Sources.id)
    query = query.outerjoin(Flags, EmissionsLivestock.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EmissionsLivestock.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmissionsLivestock.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmissionsLivestock.year == year)
    if year_min is not None:
        conditions.append(EmissionsLivestock.year >= year_min)
    if year_max is not None:
        conditions.append(EmissionsLivestock.year <= year_max)
    if unit is not None:
        conditions.append(EmissionsLivestock.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmissionsLivestock.unit == unit_exact)
    if value is not None:
        conditions.append(EmissionsLivestock.value == value)
    if value_min is not None:
        conditions.append(EmissionsLivestock.value >= value_min)
    if value_max is not None:
        conditions.append(EmissionsLivestock.value <= value_max)
    if note is not None:
        conditions.append(EmissionsLivestock.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EmissionsLivestock.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmissionsLivestock, field_name):
                        order_by_clauses.append(getattr(EmissionsLivestock, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmissionsLivestock, sort_field):
                        order_by_clauses.append(getattr(EmissionsLivestock, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmissionsLivestock.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmissionsLivestock)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmissionsLivestock.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EmissionsLivestock.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EmissionsLivestock.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Sources, EmissionsLivestock.source_code_id == Sources.id)
    count_query = count_query.outerjoin(Flags, EmissionsLivestock.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_emissions_livestock'))
...
@router.get("/areas")
@cache_result(prefix="emissions_livestock:areas", ttl=604800)
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(AreaCodes.source_dataset == 'emissions_livestock')
...
@router.get("/items")
@cache_result(prefix="emissions_livestock:items", ttl=604800)
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(ItemCodes.source_dataset == 'emissions_livestock')
...
@router.get("/elements")
@cache_result(prefix="emissions_livestock:elements", ttl=604800)
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Elements.source_dataset == 'emissions_livestock')
...
@router.get("/sources")
@cache_result(prefix="emissions_livestock:sources", ttl=604800)
def get_available_sources(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Sources.source_dataset == 'emissions_livestock')
...
@router.get("/flags")
@cache_result(prefix="emissions_livestock:flags", ttl=604800)
def get_data_quality_summary(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            Flags.flag,
            Flags.description,
            func.count(EmissionsLivestock.id).label('record_count')
        )
        .join(EmissionsLivestock, Flags.id == EmissionsLivestock.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EmissionsLivestock.id).desc())
    )
...
@router.get("/years")
@cache_result(prefix="emissions_livestock:years", ttl=604800)
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            EmissionsLivestock.year,
            func.count(EmissionsLivestock.id).label('record_count')
        )
        .group_by(EmissionsLivestock.year)
        .order_by(EmissionsLivestock.year)
    )
    ...
    if not years_data:
        return {"dataset": "emissions_livestock", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="emissions_livestock:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmissionsLivestock.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EmissionsLivestock.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EmissionsLivestock.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EmissionsLivestock.element_code_id))).scalar()
    summary["unique_sources"] = db.query(func.count(func.distinct(EmissionsLivestock.source_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EmissionsLivestock.flag_id))).scalar()
