...
from .emissions_livestock_config import EmissionsLivestockConfig
...
config = EmissionsLivestockConfig()

@router.get("/", summary="Get emissions livestock data")
async def get_emissions_livestock_data(
    request: Request,
    response: Response,
    ...
    """Get emissions livestock data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EmissionsLivestock, 
        model_name="EmissionsLivestock",
...
@router.get("/aggregate", summary="Get aggregated emissions livestock data")
async def get_emissions_livestock_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EmissionsLivestock))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in emissions_livestock")
@cache_result(prefix="emissions_livestock:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(EmissionsLivestock, EmissionsLivestock.area_code_id == AreaCodes.id)
...
@router.get("/item_codes", summary="Get item_codes in emissions_livestock")
@cache_result(prefix="emissions_livestock:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EmissionsLivestock, EmissionsLivestock.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in emissions_livestock")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmissionsLivestock, EmissionsLivestock.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in emissions_livestock")
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(EmissionsLivestock.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmissionsLivestock)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in emissions_livestock")
@cache_result(prefix="emissions_livestock:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EmissionsLivestock.unit,
            func.count(EmissionsLivestock.id).label('record_count')
        )
        .select_from(EmissionsLivestock)
        .group_by(EmissionsLivestock.unit)
        .order_by(EmissionsLivestock.unit)
    )
...
@router.get("/years", summary="Get available years in emissions_livestock")
    ...
    if include_counts:
        query = (
            select(
                ...
                EmissionsLivestock.year_code,
                func.count(EmissionsLivestock.id).label('record_count')
            )
            .group_by(EmissionsLivestock.year, EmissionsLivestock.year_code)
            .order_by(EmissionsLivestock.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EmissionsLivestock.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of emissions_livestock dataset")
@cache_result(prefix="emissions_livestock:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsLivestock.area_code_id)))
        .select_from(EmissionsLivestock)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/emissions_livestock/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsLivestock.item_code_id)))
        .select_from(EmissionsLivestock)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/emissions_livestock/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsLivestock.element_code_id)))
        .select_from(EmissionsLivestock)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/emissions_livestock/elements"
    }

    source_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsLivestock.source_code_id)))
        .select_from(EmissionsLivestock)
    ).scalar() or 0

    overview["dimensions"]["sources"] = {
        "count": source_code_ids,
        "endpoint": f"/emissions_livestock/sources"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmissionsLivestock.flag_id)))
        .select_from(EmissionsLivestock)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/emissions_livestock/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmissionsLivestock.year).label('min_year'),
            func.max(EmissionsLivestock.year).label('max_year'),
            func.count(func.distinct(EmissionsLivestock.year)).label('year_count')
        )
        .select_from(EmissionsLivestock)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/emissions_livestock/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmissionsLivestock.value).label('min_value'),
            func.max(EmissionsLivestock.value).label('max_value'),
            func.avg(EmissionsLivestock.value).label('avg_value')
        )
        .select_from(EmissionsLivestock)
        .where(and_(EmissionsLivestock.value > 0, EmissionsLivestock.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/emissions_livestock",
        "aggregate": f"/emissions_livestock/aggregate",
        "summary": f"/emissions_livestock/summary",
        "overview": f"/emissions_livestock/overview",
        "area_codes": f"/emissions_livestock/area_codes",
        "item_codes": f"/emissions_livestock/item_codes",
        "elements": f"/emissions_livestock/elements",
        "flags": f"/emissions_livestock/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the emissions_livestock endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmissionsLivestock)).scalar()
        return {
            "status": "healthy",
            "dataset": "emissions_livestock",
            "records": result
