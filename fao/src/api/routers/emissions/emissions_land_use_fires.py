...
@router.get("/")
@cache_result(prefix="emissions_land_use_fires", ttl=86400, exclude_params=["response", "db"])
def get_emissions_land_use_fires(
    response: Response,
    ...
    """
    Get emissions land use fires data with filters and pagination.
    ...
    # Main table columns
    for col in EmissionsLandUseFires.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmissionsLandUseFires, 'id'):
                columns.append(EmissionsLandUseFires.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmissionsLandUseFires)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EmissionsLandUseFires.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EmissionsLandUseFires.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EmissionsLandUseFires.element_code_id == Elements.id)
    query = query.outerjoin(Sources, EmissionsLandUseFires.source_code_id == Sources.id)
    query = query.outerjoin(Flags, EmissionsLandUseFires.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EmissionsLandUseFires.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmissionsLandUseFires.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmissionsLandUseFires.year == year)
    if year_min is not None:
        conditions.append(EmissionsLandUseFires.year >= year_min)
    if year_max is not None:
        conditions.append(EmissionsLandUseFires.year <= year_max)
    if unit is not None:
        conditions.append(EmissionsLandUseFires.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmissionsLandUseFires.unit == unit_exact)
    if value is not None:
        conditions.append(EmissionsLandUseFires.value == value)
    if value_min is not None:
        conditions.append(EmissionsLandUseFires.value >= value_min)
    if value_max is not None:
        conditions.append(EmissionsLandUseFires.value <= value_max)
    if note is not None:
        conditions.append(EmissionsLandUseFires.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EmissionsLandUseFires.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmissionsLandUseFires, field_name):
                        order_by_clauses.append(getattr(EmissionsLandUseFires, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmissionsLandUseFires, sort_field):
                        order_by_clauses.append(getattr(EmissionsLandUseFires, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmissionsLandUseFires.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmissionsLandUseFires)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmissionsLandUseFires.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EmissionsLandUseFires.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EmissionsLandUseFires.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Sources, EmissionsLandUseFires.source_code_id == Sources.id)
    count_query = count_query.outerjoin(Flags, EmissionsLandUseFires.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_emissions_land_use_fires'))
...
@router.get("/areas")
@cache_result(prefix="emissions_land_use_fires:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="emissions_land_use_fires:items", ttl=604800)
...
@router.get("/sources")
@cache_result(prefix="emissions_land_use_fires:sources", ttl=604800)
def get_available_sources(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Sources.source_dataset == 'emissions_land_use_fires')
...
@router.get("/years")
@cache_result(prefix="emissions_land_use_fires:years", ttl=604800)
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        select(
            EmissionsLandUseFires.year,
            func.count(EmissionsLandUseFires.id).label('record_count')
        )
        .group_by(EmissionsLandUseFires.year)
        .order_by(EmissionsLandUseFires.year)
    )
    ...
    if not years_data:
        return {"dataset": "emissions_land_use_fires", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="emissions_land_use_fires:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmissionsLandUseFires.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EmissionsLandUseFires.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EmissionsLandUseFires.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EmissionsLandUseFires.element_code_id))).scalar()
    summary["unique_sources"] = db.query(func.count(func.distinct(EmissionsLandUseFires.source_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EmissionsLandUseFires.flag_id))).scalar()
