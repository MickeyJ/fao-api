...
config = EmissionsLandUseFiresConfig()

@router.get("/", summary="Get emissions land use fires data")
async def get_emissions_land_use_fires_data(
    request: Request,
    response: Response,
    ...
    """Get emissions land use fires data with advanced filtering and pagination.
...
@router.get("/aggregate", summary="Get aggregated emissions land use fires data")
async def get_emissions_land_use_fires_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EmissionsLandUseFires)
                .where(EmissionsLandUseFires.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmissionsLandUseFires)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in emissions_land_use_fires")
@cache_result(prefix="emissions_land_use_fires:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EmissionsLandUseFires.unit,
            func.count(EmissionsLandUseFires.id).label('record_count')
        )
        .select_from(EmissionsLandUseFires)
        .group_by(EmissionsLandUseFires.unit)
        .order_by(EmissionsLandUseFires.unit)
    )
...
@router.get("/years", summary="Get available years in emissions_land_use_fires")
    ...
    if include_counts:
        query = (
            select(
                ...
                EmissionsLandUseFires.year_code,
                func.count(EmissionsLandUseFires.id).label('record_count')
            )
            .group_by(EmissionsLandUseFires.year, EmissionsLandUseFires.year_code)
            .order_by(EmissionsLandUseFires.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EmissionsLandUseFires.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of emissions_land_use_fires dataset")
@cache_result(prefix="emissions_land_use_fires:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsLandUseFires.area_code_id)))
        .select_from(EmissionsLandUseFires)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/emissions_land_use_fires/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsLandUseFires.item_code_id)))
        .select_from(EmissionsLandUseFires)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/emissions_land_use_fires/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsLandUseFires.element_code_id)))
        .select_from(EmissionsLandUseFires)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/emissions_land_use_fires/elements"
    }

    source_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsLandUseFires.source_code_id)))
        .select_from(EmissionsLandUseFires)
    ).scalar() or 0

    overview["dimensions"]["sources"] = {
        "count": source_code_ids,
        "endpoint": f"/emissions_land_use_fires/sources"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmissionsLandUseFires.flag_id)))
        .select_from(EmissionsLandUseFires)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/emissions_land_use_fires/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmissionsLandUseFires.year).label('min_year'),
            func.max(EmissionsLandUseFires.year).label('max_year'),
            func.count(func.distinct(EmissionsLandUseFires.year)).label('year_count')
        )
        .select_from(EmissionsLandUseFires)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/emissions_land_use_fires/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmissionsLandUseFires.value).label('min_value'),
            func.max(EmissionsLandUseFires.value).label('max_value'),
            func.avg(EmissionsLandUseFires.value).label('avg_value')
        )
        .select_from(EmissionsLandUseFires)
        .where(and_(EmissionsLandUseFires.value > 0, EmissionsLandUseFires.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/emissions_land_use_fires",
        "aggregate": f"/emissions_land_use_fires/aggregate",
        "summary": f"/emissions_land_use_fires/summary",
        "overview": f"/emissions_land_use_fires/overview",
        "area_codes": f"/emissions_land_use_fires/area_codes",
        "item_codes": f"/emissions_land_use_fires/item_codes",
        "elements": f"/emissions_land_use_fires/elements",
        "flags": f"/emissions_land_use_fires/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the emissions_land_use_fires endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmissionsLandUseFires)).scalar()
        return {
            "status": "healthy",
            "dataset": "emissions_land_use_fires",
            "records": result
