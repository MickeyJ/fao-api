...
@router.get("/")
@cache_result(prefix="emissions_pre_post_production", ttl=86400, exclude_params=["response", "db"])
def get_emissions_pre_post_production(
    response: Response,
    ...
    """
    Get emissions pre post production data with filters and pagination.
    ...
    # Main table columns
    for col in EmissionsPrePostProduction.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmissionsPrePostProduction, 'id'):
                columns.append(EmissionsPrePostProduction.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmissionsPrePostProduction)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EmissionsPrePostProduction.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EmissionsPrePostProduction.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EmissionsPrePostProduction.element_code_id == Elements.id)
    query = query.outerjoin(Flags, EmissionsPrePostProduction.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EmissionsPrePostProduction.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmissionsPrePostProduction.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmissionsPrePostProduction.year == year)
    if year_min is not None:
        conditions.append(EmissionsPrePostProduction.year >= year_min)
    if year_max is not None:
        conditions.append(EmissionsPrePostProduction.year <= year_max)
    if unit is not None:
        conditions.append(EmissionsPrePostProduction.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmissionsPrePostProduction.unit == unit_exact)
    if value is not None:
        conditions.append(EmissionsPrePostProduction.value == value)
    if value_min is not None:
        conditions.append(EmissionsPrePostProduction.value >= value_min)
    if value_max is not None:
        conditions.append(EmissionsPrePostProduction.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmissionsPrePostProduction, field_name):
                        order_by_clauses.append(getattr(EmissionsPrePostProduction, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmissionsPrePostProduction, sort_field):
                        order_by_clauses.append(getattr(EmissionsPrePostProduction, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmissionsPrePostProduction.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmissionsPrePostProduction)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmissionsPrePostProduction.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EmissionsPrePostProduction.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EmissionsPrePostProduction.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EmissionsPrePostProduction.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_emissions_pre_post_production'))
...
@router.get("/areas")
@cache_result(prefix="emissions_pre_post_production:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="emissions_pre_post_production:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EmissionsPrePostProduction.year)
    ...
    if not years_data:
        return {"dataset": "emissions_pre_post_production", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="emissions_pre_post_production:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmissionsPrePostProduction.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EmissionsPrePostProduction.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EmissionsPrePostProduction.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EmissionsPrePostProduction.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EmissionsPrePostProduction.flag_id))).scalar()
