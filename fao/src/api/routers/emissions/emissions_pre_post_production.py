...
from .emissions_pre_post_production_config import EmissionsPrePostProductionConfig
...
router = APIRouter(
    prefix="/emissions_pre_post_production",
    responses={404: {"description": "Not found"}},
)


config = EmissionsPrePostProductionConfig()

@router.get("/", summary="Get emissions pre post production data")
async def get_emissions_pre_post_production_data(
    request: Request,
    response: Response,
    ...
    """Get emissions pre post production data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EmissionsPrePostProduction, 
        model_name="EmissionsPrePostProduction",
        table_name="emissions_pre_post_production",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated emissions pre post production data")
async def get_emissions_pre_post_production_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EmissionsPrePostProduction))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EmissionsPrePostProduction, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EmissionsPrePostProduction, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EmissionsPrePostProduction, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in emissions_pre_post_production")
@cache_result(prefix="emissions_pre_post_production:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(EmissionsPrePostProduction.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(EmissionsPrePostProduction, EmissionsPrePostProduction.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'emissions_pre_post_production')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="emissions_pre_post_production",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in emissions_pre_post_production")
@cache_result(prefix="emissions_pre_post_production:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EmissionsPrePostProduction, EmissionsPrePostProduction.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'emissions_pre_post_production')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in emissions_pre_post_production")
@cache_result(prefix="emissions_pre_post_production:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmissionsPrePostProduction, EmissionsPrePostProduction.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'emissions_pre_post_production')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in emissions_pre_post_production")
@cache_result(prefix="emissions_pre_post_production:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EmissionsPrePostProduction, Flags.id == EmissionsPrePostProduction.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EmissionsPrePostProduction.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EmissionsPrePostProduction)
                .where(EmissionsPrePostProduction.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmissionsPrePostProduction)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in emissions_pre_post_production")
@cache_result(prefix="emissions_pre_post_production:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EmissionsPrePostProduction.unit,
            func.count(EmissionsPrePostProduction.id).label('record_count')
        )
        .select_from(EmissionsPrePostProduction)
        .group_by(EmissionsPrePostProduction.unit)
        .order_by(EmissionsPrePostProduction.unit)
    )
...
@router.get("/years", summary="Get available years in emissions_pre_post_production")
@cache_result(prefix="emissions_pre_post_production:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EmissionsPrePostProduction.year,
                EmissionsPrePostProduction.year_code,
                func.count(EmissionsPrePostProduction.id).label('record_count')
            )
            .group_by(EmissionsPrePostProduction.year, EmissionsPrePostProduction.year_code)
            .order_by(EmissionsPrePostProduction.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EmissionsPrePostProduction.year)
            .distinct()
            .order_by(EmissionsPrePostProduction.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of emissions_pre_post_production dataset")
@cache_result(prefix="emissions_pre_post_production:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsPrePostProduction.area_code_id)))
        .select_from(EmissionsPrePostProduction)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/emissions_pre_post_production/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsPrePostProduction.item_code_id)))
        .select_from(EmissionsPrePostProduction)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/emissions_pre_post_production/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsPrePostProduction.element_code_id)))
        .select_from(EmissionsPrePostProduction)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/emissions_pre_post_production/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmissionsPrePostProduction.flag_id)))
        .select_from(EmissionsPrePostProduction)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/emissions_pre_post_production/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmissionsPrePostProduction.year).label('min_year'),
            func.max(EmissionsPrePostProduction.year).label('max_year'),
            func.count(func.distinct(EmissionsPrePostProduction.year)).label('year_count')
        )
        .select_from(EmissionsPrePostProduction)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/emissions_pre_post_production/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmissionsPrePostProduction.value).label('min_value'),
            func.max(EmissionsPrePostProduction.value).label('max_value'),
            func.avg(EmissionsPrePostProduction.value).label('avg_value')
        )
        .select_from(EmissionsPrePostProduction)
        .where(and_(EmissionsPrePostProduction.value > 0, EmissionsPrePostProduction.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/emissions_pre_post_production",
        "aggregate": f"/emissions_pre_post_production/aggregate",
        "summary": f"/emissions_pre_post_production/summary",
        "overview": f"/emissions_pre_post_production/overview",
        "area_codes": f"/emissions_pre_post_production/area_codes",
        "item_codes": f"/emissions_pre_post_production/item_codes",
        "elements": f"/emissions_pre_post_production/elements",
        "flags": f"/emissions_pre_post_production/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the emissions_pre_post_production endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmissionsPrePostProduction)).scalar()
        return {
            "status": "healthy",
            "dataset": "emissions_pre_post_production",
            "records": result
