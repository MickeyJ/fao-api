...
from .emissions_drained_organic_soils_config import EmissionsDrainedOrganicSoilsConfig
...
router = APIRouter(
    prefix="/emissions_drained_organic_soils",
    responses={404: {"description": "Not found"}},
)


config = EmissionsDrainedOrganicSoilsConfig()

@router.get("/", summary="Get emissions drained organic soils data")
async def get_emissions_drained_organic_soils_data(
    request: Request,
    response: Response,
    ...
    """Get emissions drained organic soils data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EmissionsDrainedOrganicSoils, 
        model_name="EmissionsDrainedOrganicSoils",
        table_name="emissions_drained_organic_soils",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated emissions drained organic soils data")
async def get_emissions_drained_organic_soils_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EmissionsDrainedOrganicSoils))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EmissionsDrainedOrganicSoils, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EmissionsDrainedOrganicSoils, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EmissionsDrainedOrganicSoils, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in emissions_drained_organic_soils")
@cache_result(prefix="emissions_drained_organic_soils:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(EmissionsDrainedOrganicSoils.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(EmissionsDrainedOrganicSoils, EmissionsDrainedOrganicSoils.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'emissions_drained_organic_soils')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="emissions_drained_organic_soils",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in emissions_drained_organic_soils")
@cache_result(prefix="emissions_drained_organic_soils:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EmissionsDrainedOrganicSoils, EmissionsDrainedOrganicSoils.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'emissions_drained_organic_soils')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in emissions_drained_organic_soils")
@cache_result(prefix="emissions_drained_organic_soils:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmissionsDrainedOrganicSoils, EmissionsDrainedOrganicSoils.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'emissions_drained_organic_soils')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in emissions_drained_organic_soils")
@cache_result(prefix="emissions_drained_organic_soils:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EmissionsDrainedOrganicSoils, Flags.id == EmissionsDrainedOrganicSoils.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EmissionsDrainedOrganicSoils.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EmissionsDrainedOrganicSoils)
                .where(EmissionsDrainedOrganicSoils.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmissionsDrainedOrganicSoils)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in emissions_drained_organic_soils")
@cache_result(prefix="emissions_drained_organic_soils:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EmissionsDrainedOrganicSoils.unit,
            func.count(EmissionsDrainedOrganicSoils.id).label('record_count')
        )
        .select_from(EmissionsDrainedOrganicSoils)
        .group_by(EmissionsDrainedOrganicSoils.unit)
        .order_by(EmissionsDrainedOrganicSoils.unit)
    )
...
@router.get("/years", summary="Get available years in emissions_drained_organic_soils")
@cache_result(prefix="emissions_drained_organic_soils:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EmissionsDrainedOrganicSoils.year,
                EmissionsDrainedOrganicSoils.year_code,
                func.count(EmissionsDrainedOrganicSoils.id).label('record_count')
            )
            .group_by(EmissionsDrainedOrganicSoils.year, EmissionsDrainedOrganicSoils.year_code)
            .order_by(EmissionsDrainedOrganicSoils.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EmissionsDrainedOrganicSoils.year)
            .distinct()
            .order_by(EmissionsDrainedOrganicSoils.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of emissions_drained_organic_soils dataset")
@cache_result(prefix="emissions_drained_organic_soils:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsDrainedOrganicSoils.area_code_id)))
        .select_from(EmissionsDrainedOrganicSoils)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/emissions_drained_organic_soils/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsDrainedOrganicSoils.item_code_id)))
        .select_from(EmissionsDrainedOrganicSoils)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/emissions_drained_organic_soils/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsDrainedOrganicSoils.element_code_id)))
        .select_from(EmissionsDrainedOrganicSoils)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/emissions_drained_organic_soils/elements"
    }

    source_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsDrainedOrganicSoils.source_code_id)))
        .select_from(EmissionsDrainedOrganicSoils)
    ).scalar() or 0

    overview["dimensions"]["sources"] = {
        "count": source_code_ids,
        "endpoint": f"/emissions_drained_organic_soils/sources"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmissionsDrainedOrganicSoils.flag_id)))
        .select_from(EmissionsDrainedOrganicSoils)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/emissions_drained_organic_soils/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmissionsDrainedOrganicSoils.year).label('min_year'),
            func.max(EmissionsDrainedOrganicSoils.year).label('max_year'),
            func.count(func.distinct(EmissionsDrainedOrganicSoils.year)).label('year_count')
        )
        .select_from(EmissionsDrainedOrganicSoils)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/emissions_drained_organic_soils/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmissionsDrainedOrganicSoils.value).label('min_value'),
            func.max(EmissionsDrainedOrganicSoils.value).label('max_value'),
            func.avg(EmissionsDrainedOrganicSoils.value).label('avg_value')
        )
        .select_from(EmissionsDrainedOrganicSoils)
        .where(and_(EmissionsDrainedOrganicSoils.value > 0, EmissionsDrainedOrganicSoils.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/emissions_drained_organic_soils",
        "aggregate": f"/emissions_drained_organic_soils/aggregate",
        "summary": f"/emissions_drained_organic_soils/summary",
        "overview": f"/emissions_drained_organic_soils/overview",
        "area_codes": f"/emissions_drained_organic_soils/area_codes",
        "item_codes": f"/emissions_drained_organic_soils/item_codes",
        "elements": f"/emissions_drained_organic_soils/elements",
        "flags": f"/emissions_drained_organic_soils/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the emissions_drained_organic_soils endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmissionsDrainedOrganicSoils)).scalar()
        return {
            "status": "healthy",
            "dataset": "emissions_drained_organic_soils",
            "records": result
