...
@router.get("/")
@cache_result(prefix="emissions_drained_organic_soils", ttl=86400, exclude_params=["response", "db"])
def get_emissions_drained_organic_soils(
    response: Response,
    ...
    """
    Get emissions drained organic soils data with filters and pagination.
    ...
    # Main table columns
    for col in EmissionsDrainedOrganicSoils.__table__.columns:
    ...
    if include_all_reference_columns:
        # Add all reference columns
        for col in Sources.__table__.columns:
            if col.name not in ['id', 'created_at', 'updated_at', 'source_dataset']:
                col_alias = "sources_" + col.name
                column_map[col_alias] = col.label(col_alias)
    else:
        # Just key columns
        col_alias = "sources_source_code"
        column_map[col_alias] = Sources.source_code.label(col_alias)
        col_alias = "sources_source"
        column_map[col_alias] = Sources.source.label(col_alias)
    if include_all_reference_columns:
        # Add all reference columns
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmissionsDrainedOrganicSoils, 'id'):
                columns.append(EmissionsDrainedOrganicSoils.id)
    else:
        ...
        if include_all_reference_columns:
            for col in Sources.__table__.columns:
                if col.name not in ['id', 'created_at', 'updated_at', 'source_dataset']:
                    columns.append(col.label("sources_" + col.name))
        else:
            columns.append(Sources.source_code.label("sources_source_code"))
            columns.append(Sources.source.label("sources_source"))
        if include_all_reference_columns:
            for col in Flags.__table__.columns:
                if col.name not in ['id', 'created_at', 'updated_at', 'source_dataset']:
    ...
    # Build base query
    query = select(*columns).select_from(EmissionsDrainedOrganicSoils)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EmissionsDrainedOrganicSoils.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EmissionsDrainedOrganicSoils.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EmissionsDrainedOrganicSoils.element_code_id == Elements.id)
    query = query.outerjoin(Sources, EmissionsDrainedOrganicSoils.source_code_id == Sources.id)
    query = query.outerjoin(Flags, EmissionsDrainedOrganicSoils.flag_id == Flags.id)
    ...
    if source_code:
        conditions.append(Sources.source_code == source_code)
    if source:
        conditions.append(Sources.source.ilike("%" + source + "%"))
    if flag:
    ...
    if year_code is not None:
        conditions.append(EmissionsDrainedOrganicSoils.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmissionsDrainedOrganicSoils.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmissionsDrainedOrganicSoils.year == year)
    if year_min is not None:
        conditions.append(EmissionsDrainedOrganicSoils.year >= year_min)
    if year_max is not None:
        conditions.append(EmissionsDrainedOrganicSoils.year <= year_max)
    if unit is not None:
        conditions.append(EmissionsDrainedOrganicSoils.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmissionsDrainedOrganicSoils.unit == unit_exact)
    if value is not None:
        conditions.append(EmissionsDrainedOrganicSoils.value == value)
    if value_min is not None:
        conditions.append(EmissionsDrainedOrganicSoils.value >= value_min)
    if value_max is not None:
        conditions.append(EmissionsDrainedOrganicSoils.value <= value_max)
    if note is not None:
        conditions.append(EmissionsDrainedOrganicSoils.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EmissionsDrainedOrganicSoils.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmissionsDrainedOrganicSoils, field_name):
                        order_by_clauses.append(getattr(EmissionsDrainedOrganicSoils, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmissionsDrainedOrganicSoils, sort_field):
                        order_by_clauses.append(getattr(EmissionsDrainedOrganicSoils, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmissionsDrainedOrganicSoils.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmissionsDrainedOrganicSoils)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmissionsDrainedOrganicSoils.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EmissionsDrainedOrganicSoils.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EmissionsDrainedOrganicSoils.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Sources, EmissionsDrainedOrganicSoils.source_code_id == Sources.id)
    count_query = count_query.outerjoin(Flags, EmissionsDrainedOrganicSoils.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_emissions_drained_organic_soils'))
...
@router.get("/areas")
@cache_result(prefix="emissions_drained_organic_soils:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="emissions_drained_organic_soils:items", ttl=604800)
...
@router.get("/sources")
@cache_result(prefix="emissions_drained_organic_soils:sources", ttl=604800)
def get_available_sources(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Sources.source_dataset == 'emissions_drained_organic_soils')
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EmissionsDrainedOrganicSoils.year)
    ...
    if not years_data:
        return {"dataset": "emissions_drained_organic_soils", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="emissions_drained_organic_soils:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmissionsDrainedOrganicSoils.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EmissionsDrainedOrganicSoils.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EmissionsDrainedOrganicSoils.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EmissionsDrainedOrganicSoils.element_code_id))).scalar()
    summary["unique_sources"] = db.query(func.count(func.distinct(EmissionsDrainedOrganicSoils.source_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EmissionsDrainedOrganicSoils.flag_id))).scalar()
