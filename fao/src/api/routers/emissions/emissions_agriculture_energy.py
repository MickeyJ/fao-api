...
@router.get("/")
@cache_result(prefix="emissions_agriculture_energy", ttl=86400, exclude_params=["response", "db"])
def get_emissions_agriculture_energy(
    response: Response,
    ...
    """
    Get emissions agriculture energy data with filters and pagination.
    ...
    # Main table columns
    for col in EmissionsAgricultureEnergy.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmissionsAgricultureEnergy, 'id'):
                columns.append(EmissionsAgricultureEnergy.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmissionsAgricultureEnergy)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EmissionsAgricultureEnergy.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, EmissionsAgricultureEnergy.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, EmissionsAgricultureEnergy.element_code_id == Elements.id)
    query = query.outerjoin(Flags, EmissionsAgricultureEnergy.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EmissionsAgricultureEnergy.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmissionsAgricultureEnergy.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmissionsAgricultureEnergy.year == year)
    if year_min is not None:
        conditions.append(EmissionsAgricultureEnergy.year >= year_min)
    if year_max is not None:
        conditions.append(EmissionsAgricultureEnergy.year <= year_max)
    if unit is not None:
        conditions.append(EmissionsAgricultureEnergy.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmissionsAgricultureEnergy.unit == unit_exact)
    if value is not None:
        conditions.append(EmissionsAgricultureEnergy.value == value)
    if value_min is not None:
        conditions.append(EmissionsAgricultureEnergy.value >= value_min)
    if value_max is not None:
        conditions.append(EmissionsAgricultureEnergy.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmissionsAgricultureEnergy, field_name):
                        order_by_clauses.append(getattr(EmissionsAgricultureEnergy, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmissionsAgricultureEnergy, sort_field):
                        order_by_clauses.append(getattr(EmissionsAgricultureEnergy, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmissionsAgricultureEnergy.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmissionsAgricultureEnergy)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmissionsAgricultureEnergy.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EmissionsAgricultureEnergy.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EmissionsAgricultureEnergy.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EmissionsAgricultureEnergy.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_emissions_agriculture_energy'))
...
@router.get("/areas")
@cache_result(prefix="emissions_agriculture_energy:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="emissions_agriculture_energy:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EmissionsAgricultureEnergy.year)
    ...
    if not years_data:
        return {"dataset": "emissions_agriculture_energy", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="emissions_agriculture_energy:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmissionsAgricultureEnergy.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(EmissionsAgricultureEnergy.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(EmissionsAgricultureEnergy.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EmissionsAgricultureEnergy.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EmissionsAgricultureEnergy.flag_id))).scalar()
