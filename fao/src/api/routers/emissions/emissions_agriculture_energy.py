...
from .emissions_agriculture_energy_config import EmissionsAgricultureEnergyConfig
...
router = APIRouter(
    prefix="/emissions_agriculture_energy",
    responses={404: {"description": "Not found"}},
)


config = EmissionsAgricultureEnergyConfig()

@router.get("/", summary="Get emissions agriculture energy data")
async def get_emissions_agriculture_energy_data(
    request: Request,
    response: Response,
    ...
    """Get emissions agriculture energy data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EmissionsAgricultureEnergy, 
        model_name="EmissionsAgricultureEnergy",
...
@router.get("/aggregate", summary="Get aggregated emissions agriculture energy data")
async def get_emissions_agriculture_energy_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EmissionsAgricultureEnergy))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EmissionsAgricultureEnergy, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EmissionsAgricultureEnergy, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EmissionsAgricultureEnergy, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in emissions_agriculture_energy")
@cache_result(prefix="emissions_agriculture_energy:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(EmissionsAgricultureEnergy.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(EmissionsAgricultureEnergy, EmissionsAgricultureEnergy.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'emissions_agriculture_energy')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="emissions_agriculture_energy",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in emissions_agriculture_energy")
@cache_result(prefix="emissions_agriculture_energy:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(EmissionsAgricultureEnergy, EmissionsAgricultureEnergy.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'emissions_agriculture_energy')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in emissions_agriculture_energy")
@cache_result(prefix="emissions_agriculture_energy:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmissionsAgricultureEnergy, EmissionsAgricultureEnergy.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'emissions_agriculture_energy')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in emissions_agriculture_energy")
@cache_result(prefix="emissions_agriculture_energy:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EmissionsAgricultureEnergy, Flags.id == EmissionsAgricultureEnergy.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(EmissionsAgricultureEnergy.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(EmissionsAgricultureEnergy)
                .where(EmissionsAgricultureEnergy.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmissionsAgricultureEnergy)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in emissions_agriculture_energy")
@cache_result(prefix="emissions_agriculture_energy:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EmissionsAgricultureEnergy.unit,
            func.count(EmissionsAgricultureEnergy.id).label('record_count')
        )
        .select_from(EmissionsAgricultureEnergy)
        .group_by(EmissionsAgricultureEnergy.unit)
        .order_by(EmissionsAgricultureEnergy.unit)
    )
...
@router.get("/years", summary="Get available years in emissions_agriculture_energy")
@cache_result(prefix="emissions_agriculture_energy:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EmissionsAgricultureEnergy.year,
                EmissionsAgricultureEnergy.year_code,
                func.count(EmissionsAgricultureEnergy.id).label('record_count')
            )
            .group_by(EmissionsAgricultureEnergy.year, EmissionsAgricultureEnergy.year_code)
            .order_by(EmissionsAgricultureEnergy.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EmissionsAgricultureEnergy.year)
            .distinct()
            .order_by(EmissionsAgricultureEnergy.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of emissions_agriculture_energy dataset")
@cache_result(prefix="emissions_agriculture_energy:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsAgricultureEnergy.area_code_id)))
        .select_from(EmissionsAgricultureEnergy)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/emissions_agriculture_energy/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsAgricultureEnergy.item_code_id)))
        .select_from(EmissionsAgricultureEnergy)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/emissions_agriculture_energy/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmissionsAgricultureEnergy.element_code_id)))
        .select_from(EmissionsAgricultureEnergy)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/emissions_agriculture_energy/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmissionsAgricultureEnergy.flag_id)))
        .select_from(EmissionsAgricultureEnergy)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/emissions_agriculture_energy/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmissionsAgricultureEnergy.year).label('min_year'),
            func.max(EmissionsAgricultureEnergy.year).label('max_year'),
            func.count(func.distinct(EmissionsAgricultureEnergy.year)).label('year_count')
        )
        .select_from(EmissionsAgricultureEnergy)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/emissions_agriculture_energy/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmissionsAgricultureEnergy.value).label('min_value'),
            func.max(EmissionsAgricultureEnergy.value).label('max_value'),
            func.avg(EmissionsAgricultureEnergy.value).label('avg_value')
        )
        .select_from(EmissionsAgricultureEnergy)
        .where(and_(EmissionsAgricultureEnergy.value > 0, EmissionsAgricultureEnergy.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/emissions_agriculture_energy",
        "aggregate": f"/emissions_agriculture_energy/aggregate",
        "summary": f"/emissions_agriculture_energy/summary",
        "overview": f"/emissions_agriculture_energy/overview",
        "area_codes": f"/emissions_agriculture_energy/area_codes",
        "item_codes": f"/emissions_agriculture_energy/item_codes",
        "elements": f"/emissions_agriculture_energy/elements",
        "flags": f"/emissions_agriculture_energy/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the emissions_agriculture_energy endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmissionsAgricultureEnergy)).scalar()
        return {
            "status": "healthy",
            "dataset": "emissions_agriculture_energy",
            "records": result
