...
@router.get("/")
@cache_result(prefix="emissions_land_use_forests", ttl=86400, exclude_params=["response", "db"])
def get_emissions_land_use_forests(
    ...
    """
    Get emissions land use forests data with filters and pagination.
    ...
    # Main table columns
    for col in EmissionsLandUseForests.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmissionsLandUseForests, 'id'):
                columns.append(EmissionsLandUseForests.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmissionsLandUseForests)
    ...
    if year_code is not None:
        conditions.append(EmissionsLandUseForests.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmissionsLandUseForests.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmissionsLandUseForests.year == year)
    if year_min is not None:
        conditions.append(EmissionsLandUseForests.year >= year_min)
    if year_max is not None:
        conditions.append(EmissionsLandUseForests.year <= year_max)
    if unit is not None:
        conditions.append(EmissionsLandUseForests.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmissionsLandUseForests.unit == unit_exact)
    if value is not None:
        conditions.append(EmissionsLandUseForests.value == value)
    if value_min is not None:
        conditions.append(EmissionsLandUseForests.value >= value_min)
    if value_max is not None:
        conditions.append(EmissionsLandUseForests.value <= value_max)
    if note is not None:
        conditions.append(EmissionsLandUseForests.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EmissionsLandUseForests.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmissionsLandUseForests, field_name):
                        order_by_clauses.append(getattr(EmissionsLandUseForests, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmissionsLandUseForests, sort_field):
                        order_by_clauses.append(getattr(EmissionsLandUseForests, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmissionsLandUseForests.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmissionsLandUseForests)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmissionsLandUseForests.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, EmissionsLandUseForests.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, EmissionsLandUseForests.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Sources, EmissionsLandUseForests.source_code_id == Sources.id)
    count_query = count_query.outerjoin(Flags, EmissionsLandUseForests.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_emissions_land_use_forests'))
...
@router.get("/areas")
@cache_result(prefix="emissions_land_use_forests:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="emissions_land_use_forests:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EmissionsLandUseForests.year)
    ...
    if not years_data:
        return {"dataset": "emissions_land_use_forests", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="emissions_land_use_forests:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmissionsLandUseForests.id)).scalar()
