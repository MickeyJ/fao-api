...
async def get_emissions_land_use_forests_aggregated(
    ...
    # Build query
    query_builder = QueryBuilder(select(EmissionsLandUseForests))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EmissionsLandUseForests, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EmissionsLandUseForests, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EmissionsLandUseForests, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in emissions_land_use_forests")
...
@router.get("/item_codes", summary="Get item_codes in emissions_land_use_forests")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmissionsLandUseForests, EmissionsLandUseForests.element_code_id == Elements.id)
