...
@router.get("/")
@cache_result(prefix="employment_indicators_rural", ttl=86400, exclude_params=["response", "db"])
def get_employment_indicators_rural(
    ...
    """
    Get employment indicators rural data with filters and pagination.
    ...
    # Main table columns
    for col in EmploymentIndicatorsRural.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmploymentIndicatorsRural, 'id'):
                columns.append(EmploymentIndicatorsRural.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmploymentIndicatorsRural)
    ...
    if year_code is not None:
        conditions.append(EmploymentIndicatorsRural.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmploymentIndicatorsRural.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmploymentIndicatorsRural.year == year)
    if year_min is not None:
        conditions.append(EmploymentIndicatorsRural.year >= year_min)
    if year_max is not None:
        conditions.append(EmploymentIndicatorsRural.year <= year_max)
    if unit is not None:
        conditions.append(EmploymentIndicatorsRural.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmploymentIndicatorsRural.unit == unit_exact)
    if value is not None:
        conditions.append(EmploymentIndicatorsRural.value == value)
    if value_min is not None:
        conditions.append(EmploymentIndicatorsRural.value >= value_min)
    if value_max is not None:
        conditions.append(EmploymentIndicatorsRural.value <= value_max)
    if note is not None:
        conditions.append(EmploymentIndicatorsRural.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EmploymentIndicatorsRural.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmploymentIndicatorsRural, field_name):
                        order_by_clauses.append(getattr(EmploymentIndicatorsRural, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmploymentIndicatorsRural, sort_field):
                        order_by_clauses.append(getattr(EmploymentIndicatorsRural, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmploymentIndicatorsRural.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmploymentIndicatorsRural)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmploymentIndicatorsRural.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(Sources, EmploymentIndicatorsRural.source_code_id == Sources.id)
    count_query = count_query.outerjoin(Indicators, EmploymentIndicatorsRural.indicator_code_id == Indicators.id)
    count_query = count_query.outerjoin(Sexs, EmploymentIndicatorsRural.sex_code_id == Sexs.id)
    count_query = count_query.outerjoin(Elements, EmploymentIndicatorsRural.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EmploymentIndicatorsRural.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_employment_indicators_rural'))
...
@router.get("/areas")
@cache_result(prefix="employment_indicators_rural:areas", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(EmploymentIndicatorsRural.year)
    ...
    if not years_data:
        return {"dataset": "employment_indicators_rural", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="employment_indicators_rural:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(EmploymentIndicatorsRural.id)).scalar()
