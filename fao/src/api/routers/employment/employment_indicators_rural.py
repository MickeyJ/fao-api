...
from .employment_indicators_rural_config import EmploymentIndicatorsRuralConfig
...
config = EmploymentIndicatorsRuralConfig()

@router.get("/", summary="Get employment indicators rural data")
async def get_employment_indicators_rural_data(
    request: Request,
    response: Response,
    ...
    """Get employment indicators rural data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EmploymentIndicatorsRural, 
        model_name="EmploymentIndicatorsRural",
...
@router.get("/aggregate", summary="Get aggregated employment indicators rural data")
async def get_employment_indicators_rural_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EmploymentIndicatorsRural))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EmploymentIndicatorsRural, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EmploymentIndicatorsRural, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EmploymentIndicatorsRural, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in employment_indicators_rural")
@cache_result(prefix="employment_indicators_rural:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(EmploymentIndicatorsRural, EmploymentIndicatorsRural.area_code_id == AreaCodes.id)
...
@router.get("/elements", summary="Get elements in employment_indicators_rural")
@cache_result(prefix="employment_indicators_rural:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmploymentIndicatorsRural, EmploymentIndicatorsRural.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in employment_indicators_rural")
@cache_result(prefix="employment_indicators_rural:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(EmploymentIndicatorsRural, Flags.id == EmploymentIndicatorsRural.flag_id)
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(EmploymentIndicatorsRural.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmploymentIndicatorsRural)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in employment_indicators_rural")
@cache_result(prefix="employment_indicators_rural:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EmploymentIndicatorsRural.unit,
            func.count(EmploymentIndicatorsRural.id).label('record_count')
        )
        .select_from(EmploymentIndicatorsRural)
        .group_by(EmploymentIndicatorsRural.unit)
        .order_by(EmploymentIndicatorsRural.unit)
    )
...
@router.get("/years", summary="Get available years in employment_indicators_rural")
...
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                EmploymentIndicatorsRural.year,
                EmploymentIndicatorsRural.year_code,
                func.count(EmploymentIndicatorsRural.id).label('record_count')
            )
            .group_by(EmploymentIndicatorsRural.year, EmploymentIndicatorsRural.year_code)
            .order_by(EmploymentIndicatorsRural.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EmploymentIndicatorsRural.year)
            .distinct()
            .order_by(EmploymentIndicatorsRural.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of employment_indicators_rural dataset")
@cache_result(prefix="employment_indicators_rural:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsRural.area_code_id)))
        .select_from(EmploymentIndicatorsRural)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/employment_indicators_rural/area_codes"
    }

    source_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsRural.source_code_id)))
        .select_from(EmploymentIndicatorsRural)
    ).scalar() or 0

    overview["dimensions"]["sources"] = {
        "count": source_code_ids,
        "endpoint": f"/employment_indicators_rural/sources"
    }

    indicator_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsRural.indicator_code_id)))
        .select_from(EmploymentIndicatorsRural)
    ).scalar() or 0

    overview["dimensions"]["indicators"] = {
        "count": indicator_code_ids,
        "endpoint": f"/employment_indicators_rural/indicators"
    }

    sex_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsRural.sex_code_id)))
        .select_from(EmploymentIndicatorsRural)
    ).scalar() or 0

    overview["dimensions"]["sexs"] = {
        "count": sex_code_ids,
        "endpoint": f"/employment_indicators_rural/sexs"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsRural.element_code_id)))
        .select_from(EmploymentIndicatorsRural)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/employment_indicators_rural/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsRural.flag_id)))
        .select_from(EmploymentIndicatorsRural)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/employment_indicators_rural/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmploymentIndicatorsRural.year).label('min_year'),
            func.max(EmploymentIndicatorsRural.year).label('max_year'),
            func.count(func.distinct(EmploymentIndicatorsRural.year)).label('year_count')
        )
        .select_from(EmploymentIndicatorsRural)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/employment_indicators_rural/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmploymentIndicatorsRural.value).label('min_value'),
            func.max(EmploymentIndicatorsRural.value).label('max_value'),
            func.avg(EmploymentIndicatorsRural.value).label('avg_value')
        )
        .select_from(EmploymentIndicatorsRural)
        .where(and_(EmploymentIndicatorsRural.value > 0, EmploymentIndicatorsRural.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/employment_indicators_rural",
        "aggregate": f"/employment_indicators_rural/aggregate",
        "summary": f"/employment_indicators_rural/summary",
        "overview": f"/employment_indicators_rural/overview",
        "area_codes": f"/employment_indicators_rural/area_codes",
        "elements": f"/employment_indicators_rural/elements",
        "flags": f"/employment_indicators_rural/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the employment_indicators_rural endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmploymentIndicatorsRural)).scalar()
        return {
            "status": "healthy",
            "dataset": "employment_indicators_rural",
            "records": result
