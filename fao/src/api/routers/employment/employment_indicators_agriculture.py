...
from .employment_indicators_agriculture_config import EmploymentIndicatorsAgricultureConfig
...
config = EmploymentIndicatorsAgricultureConfig()

@router.get("/", summary="Get employment indicators agriculture data")
async def get_employment_indicators_agriculture_data(
    request: Request,
    response: Response,
    ...
    """Get employment indicators agriculture data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=EmploymentIndicatorsAgriculture, 
        model_name="EmploymentIndicatorsAgriculture",
        table_name="employment_indicators_agriculture",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated employment indicators agriculture data")
async def get_employment_indicators_agriculture_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(EmploymentIndicatorsAgriculture))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(EmploymentIndicatorsAgriculture, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(EmploymentIndicatorsAgriculture, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(EmploymentIndicatorsAgriculture, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in employment_indicators_agriculture")
@cache_result(prefix="employment_indicators_agriculture:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(EmploymentIndicatorsAgriculture, EmploymentIndicatorsAgriculture.area_code_id == AreaCodes.id)
...
@router.get("/elements", summary="Get elements in employment_indicators_agriculture")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(EmploymentIndicatorsAgriculture, EmploymentIndicatorsAgriculture.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in employment_indicators_agriculture")
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(EmploymentIndicatorsAgriculture.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(EmploymentIndicatorsAgriculture)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in employment_indicators_agriculture")
@cache_result(prefix="employment_indicators_agriculture:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            EmploymentIndicatorsAgriculture.unit,
            func.count(EmploymentIndicatorsAgriculture.id).label('record_count')
        )
        .select_from(EmploymentIndicatorsAgriculture)
        .group_by(EmploymentIndicatorsAgriculture.unit)
        .order_by(EmploymentIndicatorsAgriculture.unit)
    )
...
@router.get("/years", summary="Get available years in employment_indicators_agriculture")
    ...
    if include_counts:
        query = (
            select(
                ...
                EmploymentIndicatorsAgriculture.year_code,
                func.count(EmploymentIndicatorsAgriculture.id).label('record_count')
            )
            .group_by(EmploymentIndicatorsAgriculture.year, EmploymentIndicatorsAgriculture.year_code)
            .order_by(EmploymentIndicatorsAgriculture.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(EmploymentIndicatorsAgriculture.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of employment_indicators_agriculture dataset")
@cache_result(prefix="employment_indicators_agriculture:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsAgriculture.area_code_id)))
        .select_from(EmploymentIndicatorsAgriculture)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/employment_indicators_agriculture/area_codes"
    }

    source_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsAgriculture.source_code_id)))
        .select_from(EmploymentIndicatorsAgriculture)
    ).scalar() or 0

    overview["dimensions"]["sources"] = {
        "count": source_code_ids,
        "endpoint": f"/employment_indicators_agriculture/sources"
    }

    indicator_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsAgriculture.indicator_code_id)))
        .select_from(EmploymentIndicatorsAgriculture)
    ).scalar() or 0

    overview["dimensions"]["indicators"] = {
        "count": indicator_code_ids,
        "endpoint": f"/employment_indicators_agriculture/indicators"
    }

    sex_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsAgriculture.sex_code_id)))
        .select_from(EmploymentIndicatorsAgriculture)
    ).scalar() or 0

    overview["dimensions"]["sexs"] = {
        "count": sex_code_ids,
        "endpoint": f"/employment_indicators_agriculture/sexs"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsAgriculture.element_code_id)))
        .select_from(EmploymentIndicatorsAgriculture)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/employment_indicators_agriculture/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(EmploymentIndicatorsAgriculture.flag_id)))
        .select_from(EmploymentIndicatorsAgriculture)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/employment_indicators_agriculture/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(EmploymentIndicatorsAgriculture.year).label('min_year'),
            func.max(EmploymentIndicatorsAgriculture.year).label('max_year'),
            func.count(func.distinct(EmploymentIndicatorsAgriculture.year)).label('year_count')
        )
        .select_from(EmploymentIndicatorsAgriculture)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/employment_indicators_agriculture/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(EmploymentIndicatorsAgriculture.value).label('min_value'),
            func.max(EmploymentIndicatorsAgriculture.value).label('max_value'),
            func.avg(EmploymentIndicatorsAgriculture.value).label('avg_value')
        )
        .select_from(EmploymentIndicatorsAgriculture)
        .where(and_(EmploymentIndicatorsAgriculture.value > 0, EmploymentIndicatorsAgriculture.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/employment_indicators_agriculture",
        "aggregate": f"/employment_indicators_agriculture/aggregate",
        "summary": f"/employment_indicators_agriculture/summary",
        "overview": f"/employment_indicators_agriculture/overview",
        "area_codes": f"/employment_indicators_agriculture/area_codes",
        "elements": f"/employment_indicators_agriculture/elements",
        "flags": f"/employment_indicators_agriculture/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the employment_indicators_agriculture endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(EmploymentIndicatorsAgriculture)).scalar()
        return {
            "status": "healthy",
            "dataset": "employment_indicators_agriculture",
            "records": result
