...
@router.get("/")
@cache_result(prefix="employment_indicators_agriculture", ttl=86400, exclude_params=["response", "db"])
def get_employment_indicators_agriculture(
    ...
    """
    Get employment indicators agriculture data with filters and pagination.
    ...
    # Main table columns
    for col in EmploymentIndicatorsAgriculture.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(EmploymentIndicatorsAgriculture, 'id'):
                columns.append(EmploymentIndicatorsAgriculture.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(EmploymentIndicatorsAgriculture)
    
    # Add joins
    query = query.outerjoin(AreaCodes, EmploymentIndicatorsAgriculture.area_code_id == AreaCodes.id)
    query = query.outerjoin(Sources, EmploymentIndicatorsAgriculture.source_code_id == Sources.id)
    query = query.outerjoin(Indicators, EmploymentIndicatorsAgriculture.indicator_code_id == Indicators.id)
    query = query.outerjoin(Sexs, EmploymentIndicatorsAgriculture.sex_code_id == Sexs.id)
    query = query.outerjoin(Elements, EmploymentIndicatorsAgriculture.element_code_id == Elements.id)
    query = query.outerjoin(Flags, EmploymentIndicatorsAgriculture.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(EmploymentIndicatorsAgriculture.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(EmploymentIndicatorsAgriculture.year_code == year_code_exact)
    if year is not None:
        conditions.append(EmploymentIndicatorsAgriculture.year == year)
    if year_min is not None:
        conditions.append(EmploymentIndicatorsAgriculture.year >= year_min)
    if year_max is not None:
        conditions.append(EmploymentIndicatorsAgriculture.year <= year_max)
    if unit is not None:
        conditions.append(EmploymentIndicatorsAgriculture.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(EmploymentIndicatorsAgriculture.unit == unit_exact)
    if value is not None:
        conditions.append(EmploymentIndicatorsAgriculture.value == value)
    if value_min is not None:
        conditions.append(EmploymentIndicatorsAgriculture.value >= value_min)
    if value_max is not None:
        conditions.append(EmploymentIndicatorsAgriculture.value <= value_max)
    if note is not None:
        conditions.append(EmploymentIndicatorsAgriculture.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(EmploymentIndicatorsAgriculture.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(EmploymentIndicatorsAgriculture, field_name):
                        order_by_clauses.append(getattr(EmploymentIndicatorsAgriculture, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(EmploymentIndicatorsAgriculture, sort_field):
                        order_by_clauses.append(getattr(EmploymentIndicatorsAgriculture, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(EmploymentIndicatorsAgriculture.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(EmploymentIndicatorsAgriculture)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, EmploymentIndicatorsAgriculture.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(Sources, EmploymentIndicatorsAgriculture.source_code_id == Sources.id)
    count_query = count_query.outerjoin(Indicators, EmploymentIndicatorsAgriculture.indicator_code_id == Indicators.id)
    count_query = count_query.outerjoin(Sexs, EmploymentIndicatorsAgriculture.sex_code_id == Sexs.id)
    count_query = count_query.outerjoin(Elements, EmploymentIndicatorsAgriculture.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, EmploymentIndicatorsAgriculture.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_employment_indicators_agriculture'))
...
@router.get("/areas")
@cache_result(prefix="employment_indicators_agriculture:areas", ttl=604800)
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(AreaCodes.source_dataset == 'employment_indicators_agriculture')
...
@router.get("/sources")
@cache_result(prefix="employment_indicators_agriculture:sources", ttl=604800)
def get_available_sources(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Sources.source_dataset == 'employment_indicators_agriculture')
...
@cache_result(prefix="employment_indicators_agriculture:indicators", ttl=604800)
def get_available_indicators(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Indicators.source_dataset == 'employment_indicators_agriculture')
...
@router.get("/sexs")
@cache_result(prefix="employment_indicators_agriculture:sexs", ttl=604800)
def get_available_sexs(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Sexs.source_dataset == 'employment_indicators_agriculture')
...
@router.get("/elements")
@cache_result(prefix="employment_indicators_agriculture:elements", ttl=604800)
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .where(Elements.source_dataset == 'employment_indicators_agriculture')
...
@router.get("/flags")
@cache_result(prefix="employment_indicators_agriculture:flags", ttl=604800)
...
@router.get("/years")
@cache_result(prefix="employment_indicators_agriculture:years", ttl=604800)
...
@router.get("/summary")
@cache_result(prefix="employment_indicators_agriculture:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    summary["unique_sources"] = db.query(func.count(func.distinct(EmploymentIndicatorsAgriculture.source_code_id))).scalar()
    summary["unique_indicators"] = db.query(func.count(func.distinct(EmploymentIndicatorsAgriculture.indicator_code_id))).scalar()
    summary["unique_sexs"] = db.query(func.count(func.distinct(EmploymentIndicatorsAgriculture.sex_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(EmploymentIndicatorsAgriculture.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(EmploymentIndicatorsAgriculture.flag_id))).scalar()
