...
@router.get("/")
@cache_result(prefix="forestry_pulp_paper_survey", ttl=86400, exclude_params=["response", "db"])
def get_forestry_pulp_paper_survey(
    response: Response,
    ...
    """
    Get forestry pulp paper survey data with filters and pagination.
    ...
    # Main table columns
    for col in ForestryPulpPaperSurvey.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(ForestryPulpPaperSurvey, 'id'):
                columns.append(ForestryPulpPaperSurvey.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(ForestryPulpPaperSurvey)
    
    # Add joins
    query = query.outerjoin(AreaCodes, ForestryPulpPaperSurvey.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, ForestryPulpPaperSurvey.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, ForestryPulpPaperSurvey.element_code_id == Elements.id)
    query = query.outerjoin(Flags, ForestryPulpPaperSurvey.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(ForestryPulpPaperSurvey.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(ForestryPulpPaperSurvey.year_code == year_code_exact)
    if year is not None:
        conditions.append(ForestryPulpPaperSurvey.year == year)
    if year_min is not None:
        conditions.append(ForestryPulpPaperSurvey.year >= year_min)
    if year_max is not None:
        conditions.append(ForestryPulpPaperSurvey.year <= year_max)
    if unit is not None:
        conditions.append(ForestryPulpPaperSurvey.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(ForestryPulpPaperSurvey.unit == unit_exact)
    if value is not None:
        conditions.append(ForestryPulpPaperSurvey.value == value)
    if value_min is not None:
        conditions.append(ForestryPulpPaperSurvey.value >= value_min)
    if value_max is not None:
        conditions.append(ForestryPulpPaperSurvey.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(ForestryPulpPaperSurvey, field_name):
                        order_by_clauses.append(getattr(ForestryPulpPaperSurvey, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(ForestryPulpPaperSurvey, sort_field):
                        order_by_clauses.append(getattr(ForestryPulpPaperSurvey, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(ForestryPulpPaperSurvey.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(ForestryPulpPaperSurvey)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, ForestryPulpPaperSurvey.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, ForestryPulpPaperSurvey.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, ForestryPulpPaperSurvey.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, ForestryPulpPaperSurvey.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_forestry_pulp_paper_survey'))
...
@router.get("/areas")
@cache_result(prefix="forestry_pulp_paper_survey:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="forestry_pulp_paper_survey:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(ForestryPulpPaperSurvey.year)
    ...
    if not years_data:
        return {"dataset": "forestry_pulp_paper_survey", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="forestry_pulp_paper_survey:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(ForestryPulpPaperSurvey.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(ForestryPulpPaperSurvey.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(ForestryPulpPaperSurvey.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(ForestryPulpPaperSurvey.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(ForestryPulpPaperSurvey.flag_id))).scalar()
