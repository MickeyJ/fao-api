...
from .forestry_pulp_paper_survey_config import ForestryPulpPaperSurveyConfig
...
router = APIRouter(
    prefix="/forestry_pulp_paper_survey",
    responses={404: {"description": "Not found"}},
)


config = ForestryPulpPaperSurveyConfig()

@router.get("/", summary="Get forestry pulp paper survey data")
async def get_forestry_pulp_paper_survey_data(
    request: Request,
    response: Response,
    ...
    """Get forestry pulp paper survey data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=ForestryPulpPaperSurvey, 
        model_name="ForestryPulpPaperSurvey",
        table_name="forestry_pulp_paper_survey",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated forestry pulp paper survey data")
async def get_forestry_pulp_paper_survey_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(ForestryPulpPaperSurvey))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(ForestryPulpPaperSurvey, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(ForestryPulpPaperSurvey, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(ForestryPulpPaperSurvey, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in forestry_pulp_paper_survey")
@cache_result(prefix="forestry_pulp_paper_survey:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(ForestryPulpPaperSurvey.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(ForestryPulpPaperSurvey, ForestryPulpPaperSurvey.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'forestry_pulp_paper_survey')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="forestry_pulp_paper_survey",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in forestry_pulp_paper_survey")
@cache_result(prefix="forestry_pulp_paper_survey:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(ForestryPulpPaperSurvey, ForestryPulpPaperSurvey.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'forestry_pulp_paper_survey')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in forestry_pulp_paper_survey")
@cache_result(prefix="forestry_pulp_paper_survey:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(ForestryPulpPaperSurvey, ForestryPulpPaperSurvey.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'forestry_pulp_paper_survey')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in forestry_pulp_paper_survey")
@cache_result(prefix="forestry_pulp_paper_survey:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(ForestryPulpPaperSurvey, Flags.id == ForestryPulpPaperSurvey.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(ForestryPulpPaperSurvey.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(ForestryPulpPaperSurvey)
                .where(ForestryPulpPaperSurvey.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(ForestryPulpPaperSurvey)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in forestry_pulp_paper_survey")
@cache_result(prefix="forestry_pulp_paper_survey:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            ForestryPulpPaperSurvey.unit,
            func.count(ForestryPulpPaperSurvey.id).label('record_count')
        )
        .select_from(ForestryPulpPaperSurvey)
        .group_by(ForestryPulpPaperSurvey.unit)
        .order_by(ForestryPulpPaperSurvey.unit)
    )
...
@router.get("/years", summary="Get available years in forestry_pulp_paper_survey")
@cache_result(prefix="forestry_pulp_paper_survey:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                ForestryPulpPaperSurvey.year,
                ForestryPulpPaperSurvey.year_code,
                func.count(ForestryPulpPaperSurvey.id).label('record_count')
            )
            .group_by(ForestryPulpPaperSurvey.year, ForestryPulpPaperSurvey.year_code)
            .order_by(ForestryPulpPaperSurvey.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(ForestryPulpPaperSurvey.year)
            .distinct()
            .order_by(ForestryPulpPaperSurvey.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of forestry_pulp_paper_survey dataset")
@cache_result(prefix="forestry_pulp_paper_survey:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(ForestryPulpPaperSurvey.area_code_id)))
        .select_from(ForestryPulpPaperSurvey)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/forestry_pulp_paper_survey/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(ForestryPulpPaperSurvey.item_code_id)))
        .select_from(ForestryPulpPaperSurvey)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/forestry_pulp_paper_survey/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(ForestryPulpPaperSurvey.element_code_id)))
        .select_from(ForestryPulpPaperSurvey)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/forestry_pulp_paper_survey/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(ForestryPulpPaperSurvey.flag_id)))
        .select_from(ForestryPulpPaperSurvey)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/forestry_pulp_paper_survey/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(ForestryPulpPaperSurvey.year).label('min_year'),
            func.max(ForestryPulpPaperSurvey.year).label('max_year'),
            func.count(func.distinct(ForestryPulpPaperSurvey.year)).label('year_count')
        )
        .select_from(ForestryPulpPaperSurvey)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/forestry_pulp_paper_survey/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(ForestryPulpPaperSurvey.value).label('min_value'),
            func.max(ForestryPulpPaperSurvey.value).label('max_value'),
            func.avg(ForestryPulpPaperSurvey.value).label('avg_value')
        )
        .select_from(ForestryPulpPaperSurvey)
        .where(and_(ForestryPulpPaperSurvey.value > 0, ForestryPulpPaperSurvey.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/forestry_pulp_paper_survey",
        "aggregate": f"/forestry_pulp_paper_survey/aggregate",
        "summary": f"/forestry_pulp_paper_survey/summary",
        "overview": f"/forestry_pulp_paper_survey/overview",
        "area_codes": f"/forestry_pulp_paper_survey/area_codes",
        "item_codes": f"/forestry_pulp_paper_survey/item_codes",
        "elements": f"/forestry_pulp_paper_survey/elements",
        "flags": f"/forestry_pulp_paper_survey/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the forestry_pulp_paper_survey endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(ForestryPulpPaperSurvey)).scalar()
        return {
            "status": "healthy",
            "dataset": "forestry_pulp_paper_survey",
            "records": result
