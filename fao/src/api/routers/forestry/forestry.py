...
@router.get("/")
@cache_result(prefix="forestry", ttl=86400, exclude_params=["response", "db"])
def get_forestry(
    response: Response,
    ...
    """
    Get forestry data with filters and pagination.
    ...
    # Main table columns
    for col in Forestry.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Forestry, 'id'):
                columns.append(Forestry.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Forestry)
    
    # Add joins
    query = query.outerjoin(AreaCodes, Forestry.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, Forestry.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, Forestry.element_code_id == Elements.id)
    query = query.outerjoin(Flags, Forestry.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(Forestry.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(Forestry.year_code == year_code_exact)
    if year is not None:
        conditions.append(Forestry.year == year)
    if year_min is not None:
        conditions.append(Forestry.year >= year_min)
    if year_max is not None:
        conditions.append(Forestry.year <= year_max)
    if unit is not None:
        conditions.append(Forestry.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(Forestry.unit == unit_exact)
    if value is not None:
        conditions.append(Forestry.value == value)
    if value_min is not None:
        conditions.append(Forestry.value >= value_min)
    if value_max is not None:
        conditions.append(Forestry.value <= value_max)
    if note is not None:
        conditions.append(Forestry.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(Forestry.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Forestry, field_name):
                        order_by_clauses.append(getattr(Forestry, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Forestry, sort_field):
                        order_by_clauses.append(getattr(Forestry, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Forestry.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Forestry)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, Forestry.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, Forestry.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, Forestry.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, Forestry.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_forestry'))
...
@router.get("/areas")
@cache_result(prefix="forestry:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="forestry:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    if not years_data:
        return {"dataset": "forestry", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="forestry:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(Forestry.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(Forestry.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(Forestry.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(Forestry.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(Forestry.flag_id))).scalar()
