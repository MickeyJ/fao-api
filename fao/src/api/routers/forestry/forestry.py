...
from .forestry_config import ForestryConfig
...
@router.get("/", summary="Get forestry data")
async def get_forestry_data(
    request: Request,
    response: Response,
    ...
    """Get forestry data with advanced filtering and pagination.
...
@router.get("/aggregate", summary="Get aggregated forestry data")
async def get_forestry_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(Forestry))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in forestry")
@cache_result(prefix="forestry:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(Forestry.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(Forestry, Forestry.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'forestry')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in forestry")
@cache_result(prefix="forestry:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(Forestry, Forestry.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'forestry')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in forestry")
@cache_result(prefix="forestry:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(Forestry, Forestry.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'forestry')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in forestry")
@cache_result(prefix="forestry:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(Forestry, Flags.id == Forestry.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(Forestry.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(Forestry)
                .where(Forestry.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(Forestry)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in forestry")
@cache_result(prefix="forestry:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(Forestry)
        .group_by(Forestry.unit)
        .order_by(Forestry.unit)
    )
...
@router.get("/years", summary="Get available years in forestry")
@cache_result(prefix="forestry:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            ...
            .group_by(Forestry.year, Forestry.year_code)
            .order_by(Forestry.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(Forestry.year)
            .distinct()
            .order_by(Forestry.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of forestry dataset")
@cache_result(prefix="forestry:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(Forestry.area_code_id)))
        .select_from(Forestry)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/forestry/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(Forestry.item_code_id)))
        .select_from(Forestry)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/forestry/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(Forestry.element_code_id)))
        .select_from(Forestry)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/forestry/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(Forestry.flag_id)))
        .select_from(Forestry)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/forestry/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(Forestry.year).label('min_year'),
            func.max(Forestry.year).label('max_year'),
            func.count(func.distinct(Forestry.year)).label('year_count')
        )
        .select_from(Forestry)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/forestry/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(Forestry.value).label('min_value'),
            func.max(Forestry.value).label('max_value'),
            func.avg(Forestry.value).label('avg_value')
        )
        .select_from(Forestry)
        .where(and_(Forestry.value > 0, Forestry.value.is_not(None)))
    ).first()
    ...
    overview["endpoints"] = {
        "data": f"/forestry",
        "aggregate": f"/forestry/aggregate",
        "summary": f"/forestry/summary",
        "overview": f"/forestry/overview",
        "area_codes": f"/forestry/area_codes",
        "item_codes": f"/forestry/item_codes",
        "elements": f"/forestry/elements",
        "flags": f"/forestry/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the forestry endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(Forestry)).scalar()
        return {
            "status": "healthy",
            "dataset": "forestry",
            "records": result
