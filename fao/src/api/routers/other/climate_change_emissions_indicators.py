...
@router.get("/")
@cache_result(prefix="climate_change_emissions_indicators", ttl=86400, exclude_params=["response", "db"])
def get_climate_change_emissions_indicators(
    response: Response,
    ...
    """
    Get climate change emissions indicators data with filters and pagination.
    ...
    # Main table columns
    for col in ClimateChangeEmissionsIndicators.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(ClimateChangeEmissionsIndicators, 'id'):
                columns.append(ClimateChangeEmissionsIndicators.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(ClimateChangeEmissionsIndicators)
    
    # Add joins
    query = query.outerjoin(AreaCodes, ClimateChangeEmissionsIndicators.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, ClimateChangeEmissionsIndicators.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, ClimateChangeEmissionsIndicators.element_code_id == Elements.id)
    query = query.outerjoin(Flags, ClimateChangeEmissionsIndicators.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(ClimateChangeEmissionsIndicators.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(ClimateChangeEmissionsIndicators.year_code == year_code_exact)
    if year is not None:
        conditions.append(ClimateChangeEmissionsIndicators.year == year)
    if year_min is not None:
        conditions.append(ClimateChangeEmissionsIndicators.year >= year_min)
    if year_max is not None:
        conditions.append(ClimateChangeEmissionsIndicators.year <= year_max)
    if unit is not None:
        conditions.append(ClimateChangeEmissionsIndicators.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(ClimateChangeEmissionsIndicators.unit == unit_exact)
    if value is not None:
        conditions.append(ClimateChangeEmissionsIndicators.value == value)
    if value_min is not None:
        conditions.append(ClimateChangeEmissionsIndicators.value >= value_min)
    if value_max is not None:
        conditions.append(ClimateChangeEmissionsIndicators.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(ClimateChangeEmissionsIndicators, field_name):
                        order_by_clauses.append(getattr(ClimateChangeEmissionsIndicators, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(ClimateChangeEmissionsIndicators, sort_field):
                        order_by_clauses.append(getattr(ClimateChangeEmissionsIndicators, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(ClimateChangeEmissionsIndicators.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(ClimateChangeEmissionsIndicators)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, ClimateChangeEmissionsIndicators.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, ClimateChangeEmissionsIndicators.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, ClimateChangeEmissionsIndicators.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, ClimateChangeEmissionsIndicators.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_climate_change_emissions_indicators'))
...
@router.get("/areas")
@cache_result(prefix="climate_change_emissions_indicators:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="climate_change_emissions_indicators:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(ClimateChangeEmissionsIndicators.year)
    ...
    if not years_data:
        return {"dataset": "climate_change_emissions_indicators", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="climate_change_emissions_indicators:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(ClimateChangeEmissionsIndicators.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(ClimateChangeEmissionsIndicators.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(ClimateChangeEmissionsIndicators.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(ClimateChangeEmissionsIndicators.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(ClimateChangeEmissionsIndicators.flag_id))).scalar()
