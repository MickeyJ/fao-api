...
from .climate_change_emissions_indicators_config import ClimateChangeEmissionsIndicatorsConfig
...
router = APIRouter(
    prefix="/climate_change_emissions_indicators",
    responses={404: {"description": "Not found"}},
)


config = ClimateChangeEmissionsIndicatorsConfig()

@router.get("/", summary="Get climate change emissions indicators data")
async def get_climate_change_emissions_indicators_data(
    request: Request,
    response: Response,
    ...
    """Get climate change emissions indicators data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=ClimateChangeEmissionsIndicators, 
        model_name="ClimateChangeEmissionsIndicators",
        table_name="climate_change_emissions_indicators",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated climate change emissions indicators data")
async def get_climate_change_emissions_indicators_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(ClimateChangeEmissionsIndicators))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(ClimateChangeEmissionsIndicators, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(ClimateChangeEmissionsIndicators, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(ClimateChangeEmissionsIndicators, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in climate_change_emissions_indicators")
@cache_result(prefix="climate_change_emissions_indicators:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(ClimateChangeEmissionsIndicators.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(ClimateChangeEmissionsIndicators, ClimateChangeEmissionsIndicators.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'climate_change_emissions_indicators')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in climate_change_emissions_indicators")
@cache_result(prefix="climate_change_emissions_indicators:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(ClimateChangeEmissionsIndicators, ClimateChangeEmissionsIndicators.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'climate_change_emissions_indicators')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in climate_change_emissions_indicators")
@cache_result(prefix="climate_change_emissions_indicators:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(ClimateChangeEmissionsIndicators, ClimateChangeEmissionsIndicators.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'climate_change_emissions_indicators')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in climate_change_emissions_indicators")
@cache_result(prefix="climate_change_emissions_indicators:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(ClimateChangeEmissionsIndicators, Flags.id == ClimateChangeEmissionsIndicators.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(ClimateChangeEmissionsIndicators.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(ClimateChangeEmissionsIndicators)
                .where(ClimateChangeEmissionsIndicators.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(ClimateChangeEmissionsIndicators)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in climate_change_emissions_indicators")
@cache_result(prefix="climate_change_emissions_indicators:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            ClimateChangeEmissionsIndicators.unit,
            func.count(ClimateChangeEmissionsIndicators.id).label('record_count')
        )
        .select_from(ClimateChangeEmissionsIndicators)
        .group_by(ClimateChangeEmissionsIndicators.unit)
        .order_by(ClimateChangeEmissionsIndicators.unit)
    )
...
@router.get("/years", summary="Get available years in climate_change_emissions_indicators")
@cache_result(prefix="climate_change_emissions_indicators:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                ClimateChangeEmissionsIndicators.year,
                ClimateChangeEmissionsIndicators.year_code,
                func.count(ClimateChangeEmissionsIndicators.id).label('record_count')
            )
            .group_by(ClimateChangeEmissionsIndicators.year, ClimateChangeEmissionsIndicators.year_code)
            .order_by(ClimateChangeEmissionsIndicators.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(ClimateChangeEmissionsIndicators.year)
            .distinct()
            .order_by(ClimateChangeEmissionsIndicators.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of climate_change_emissions_indicators dataset")
@cache_result(prefix="climate_change_emissions_indicators:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(ClimateChangeEmissionsIndicators.area_code_id)))
        .select_from(ClimateChangeEmissionsIndicators)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/climate_change_emissions_indicators/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(ClimateChangeEmissionsIndicators.item_code_id)))
        .select_from(ClimateChangeEmissionsIndicators)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/climate_change_emissions_indicators/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(ClimateChangeEmissionsIndicators.element_code_id)))
        .select_from(ClimateChangeEmissionsIndicators)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/climate_change_emissions_indicators/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(ClimateChangeEmissionsIndicators.flag_id)))
        .select_from(ClimateChangeEmissionsIndicators)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/climate_change_emissions_indicators/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(ClimateChangeEmissionsIndicators.year).label('min_year'),
            func.max(ClimateChangeEmissionsIndicators.year).label('max_year'),
            func.count(func.distinct(ClimateChangeEmissionsIndicators.year)).label('year_count')
        )
        .select_from(ClimateChangeEmissionsIndicators)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/climate_change_emissions_indicators/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(ClimateChangeEmissionsIndicators.value).label('min_value'),
            func.max(ClimateChangeEmissionsIndicators.value).label('max_value'),
            func.avg(ClimateChangeEmissionsIndicators.value).label('avg_value')
        )
        .select_from(ClimateChangeEmissionsIndicators)
        .where(and_(ClimateChangeEmissionsIndicators.value > 0, ClimateChangeEmissionsIndicators.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/climate_change_emissions_indicators",
        "aggregate": f"/climate_change_emissions_indicators/aggregate",
        "summary": f"/climate_change_emissions_indicators/summary",
        "overview": f"/climate_change_emissions_indicators/overview",
        "area_codes": f"/climate_change_emissions_indicators/area_codes",
        "item_codes": f"/climate_change_emissions_indicators/item_codes",
        "elements": f"/climate_change_emissions_indicators/elements",
        "flags": f"/climate_change_emissions_indicators/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the climate_change_emissions_indicators endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(ClimateChangeEmissionsIndicators)).scalar()
        return {
            "status": "healthy",
            "dataset": "climate_change_emissions_indicators",
            "records": result
