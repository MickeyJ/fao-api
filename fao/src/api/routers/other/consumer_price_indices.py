...
from .consumer_price_indices_config import ConsumerPriceIndicesConfig
...
config = ConsumerPriceIndicesConfig()

@router.get("/", summary="Get consumer price indices data")
async def get_consumer_price_indices_data(
    request: Request,
    response: Response,
    ...
    """Get consumer price indices data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=ConsumerPriceIndices, 
        model_name="ConsumerPriceIndices",
        table_name="consumer_price_indices",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated consumer price indices data")
async def get_consumer_price_indices_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(ConsumerPriceIndices))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(ConsumerPriceIndices, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(ConsumerPriceIndices, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(ConsumerPriceIndices, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in consumer_price_indices")
@cache_result(prefix="consumer_price_indices:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(ConsumerPriceIndices, ConsumerPriceIndices.area_code_id == AreaCodes.id)
...
@router.get("/item_codes", summary="Get item_codes in consumer_price_indices")
@cache_result(prefix="consumer_price_indices:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(ConsumerPriceIndices, ConsumerPriceIndices.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in consumer_price_indices")
@cache_result(prefix="consumer_price_indices:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(ConsumerPriceIndices, ConsumerPriceIndices.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in consumer_price_indices")
@cache_result(prefix="consumer_price_indices:flags", ttl=604800)
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(ConsumerPriceIndices.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(ConsumerPriceIndices)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in consumer_price_indices")
@cache_result(prefix="consumer_price_indices:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            ConsumerPriceIndices.unit,
            func.count(ConsumerPriceIndices.id).label('record_count')
        )
        .select_from(ConsumerPriceIndices)
        .group_by(ConsumerPriceIndices.unit)
        .order_by(ConsumerPriceIndices.unit)
    )
...
@router.get("/years", summary="Get available years in consumer_price_indices")
    ...
    if include_counts:
        query = (
            select(
                ...
                ConsumerPriceIndices.year_code,
                func.count(ConsumerPriceIndices.id).label('record_count')
            )
            .group_by(ConsumerPriceIndices.year, ConsumerPriceIndices.year_code)
            .order_by(ConsumerPriceIndices.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(ConsumerPriceIndices.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of consumer_price_indices dataset")
@cache_result(prefix="consumer_price_indices:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(ConsumerPriceIndices.area_code_id)))
        .select_from(ConsumerPriceIndices)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/consumer_price_indices/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(ConsumerPriceIndices.item_code_id)))
        .select_from(ConsumerPriceIndices)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/consumer_price_indices/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(ConsumerPriceIndices.element_code_id)))
        .select_from(ConsumerPriceIndices)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/consumer_price_indices/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(ConsumerPriceIndices.flag_id)))
        .select_from(ConsumerPriceIndices)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/consumer_price_indices/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(ConsumerPriceIndices.year).label('min_year'),
            func.max(ConsumerPriceIndices.year).label('max_year'),
            func.count(func.distinct(ConsumerPriceIndices.year)).label('year_count')
        )
        .select_from(ConsumerPriceIndices)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/consumer_price_indices/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(ConsumerPriceIndices.value).label('min_value'),
            func.max(ConsumerPriceIndices.value).label('max_value'),
            func.avg(ConsumerPriceIndices.value).label('avg_value')
        )
        .select_from(ConsumerPriceIndices)
        .where(and_(ConsumerPriceIndices.value > 0, ConsumerPriceIndices.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/consumer_price_indices",
        "aggregate": f"/consumer_price_indices/aggregate",
        "summary": f"/consumer_price_indices/summary",
        "overview": f"/consumer_price_indices/overview",
        "area_codes": f"/consumer_price_indices/area_codes",
        "item_codes": f"/consumer_price_indices/item_codes",
        "elements": f"/consumer_price_indices/elements",
        "flags": f"/consumer_price_indices/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the consumer_price_indices endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(ConsumerPriceIndices)).scalar()
        return {
            "status": "healthy",
            "dataset": "consumer_price_indices",
            "records": result
