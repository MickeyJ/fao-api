...
@router.get("/")
@cache_result(prefix="consumer_price_indices", ttl=86400, exclude_params=["response", "db"])
def get_consumer_price_indices(
    ...
    """
    Get consumer price indices data with filters and pagination.
    ...
    # Main table columns
    for col in ConsumerPriceIndices.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(ConsumerPriceIndices, 'id'):
                columns.append(ConsumerPriceIndices.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(ConsumerPriceIndices)
    
    # Add joins
    query = query.outerjoin(AreaCodes, ConsumerPriceIndices.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, ConsumerPriceIndices.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, ConsumerPriceIndices.element_code_id == Elements.id)
    query = query.outerjoin(Flags, ConsumerPriceIndices.flag_id == Flags.id)
    ...
    if months_code is not None:
        conditions.append(ConsumerPriceIndices.months_code.ilike("%" + months_code + "%"))
    if months_code_exact is not None:
        conditions.append(ConsumerPriceIndices.months_code == months_code_exact)
    if months is not None:
        conditions.append(ConsumerPriceIndices.months.ilike("%" + months + "%"))
    if months_exact is not None:
        conditions.append(ConsumerPriceIndices.months == months_exact)
    if year_code is not None:
        conditions.append(ConsumerPriceIndices.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(ConsumerPriceIndices.year_code == year_code_exact)
    if year is not None:
        conditions.append(ConsumerPriceIndices.year == year)
    if year_min is not None:
        conditions.append(ConsumerPriceIndices.year >= year_min)
    if year_max is not None:
        conditions.append(ConsumerPriceIndices.year <= year_max)
    if unit is not None:
        conditions.append(ConsumerPriceIndices.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(ConsumerPriceIndices.unit == unit_exact)
    if value is not None:
        conditions.append(ConsumerPriceIndices.value == value)
    if value_min is not None:
        conditions.append(ConsumerPriceIndices.value >= value_min)
    if value_max is not None:
        conditions.append(ConsumerPriceIndices.value <= value_max)
    if note is not None:
        conditions.append(ConsumerPriceIndices.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(ConsumerPriceIndices.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(ConsumerPriceIndices, field_name):
                        order_by_clauses.append(getattr(ConsumerPriceIndices, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(ConsumerPriceIndices, sort_field):
                        order_by_clauses.append(getattr(ConsumerPriceIndices, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(ConsumerPriceIndices.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(ConsumerPriceIndices)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, ConsumerPriceIndices.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, ConsumerPriceIndices.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, ConsumerPriceIndices.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, ConsumerPriceIndices.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_consumer_price_indices'))
...
@router.get("/areas")
@cache_result(prefix="consumer_price_indices:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="consumer_price_indices:items", ttl=604800)
...
@router.get("/summary")
@cache_result(prefix="consumer_price_indices:summary", ttl=604800)
