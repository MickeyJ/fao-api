...
from .development_assistance_to_agriculture_config import DevelopmentAssistanceToAgricultureConfig
...
config = DevelopmentAssistanceToAgricultureConfig()

@router.get("/", summary="Get development assistance to agriculture data")
async def get_development_assistance_to_agriculture_data(
    request: Request,
    response: Response,
    ...
    # Filter parameters
    donor_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by donor_code code (comma-separated for multiple)"),
    donor: Optional[str] = Query(None, description="Filter by donor description (partial match)"),
    recipient_country_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by recipient_country_code code (comma-separated for multiple)"),
    recipient_country: Optional[str] = Query(None, description="Filter by recipient_country description (partial match)"),
    ...
    purpose_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by purpose_code code (comma-separated for multiple)"),
    purpose: Optional[str] = Query(None, description="Filter by purpose description (partial match)"),
    ...
    """Get development assistance to agriculture data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=DevelopmentAssistanceToAgriculture, 
        model_name="DevelopmentAssistanceToAgriculture",
    ...
    recipient_country_code = router_handler.clean_param(recipient_country_code, "multi")
    recipient_country = router_handler.clean_param(recipient_country, "like")
    ...
    purpose_code = router_handler.clean_param(purpose_code, "multi")
    purpose = router_handler.clean_param(purpose, "like")
...
@router.get("/aggregate", summary="Get aggregated development assistance to agriculture data")
async def get_development_assistance_to_agriculture_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(DevelopmentAssistanceToAgriculture))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(DevelopmentAssistanceToAgriculture, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(DevelopmentAssistanceToAgriculture, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(DevelopmentAssistanceToAgriculture, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/recipient_country_codes", summary="Get recipient_country_codes in development_assistance_to_agriculture")
@cache_result(prefix="development_assistance_to_agriculture:recipient_country_codes", ttl=604800)
async def get_available_recipient_country_codes(
    db: Session = Depends(get_db),
    search: Optional[str] = Query(None, description="Search recipient_country by name or code"),
):
    ...
    query = (
        ...
        .join(DevelopmentAssistanceToAgriculture, 
              DevelopmentAssistanceToAgriculture.partner_country_code_id == PartnerCountryCodes.id)
        .where(PartnerCountryCodes.source_dataset == 'development_assistance_to_agriculture')
    )
...
@router.get("/item_codes", summary="Get item_codes in development_assistance_to_agriculture")
@cache_result(prefix="development_assistance_to_agriculture:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(DevelopmentAssistanceToAgriculture, DevelopmentAssistanceToAgriculture.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in development_assistance_to_agriculture")
@cache_result(prefix="development_assistance_to_agriculture:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(DevelopmentAssistanceToAgriculture, DevelopmentAssistanceToAgriculture.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in development_assistance_to_agriculture")
@cache_result(prefix="development_assistance_to_agriculture:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(DevelopmentAssistanceToAgriculture, Flags.id == DevelopmentAssistanceToAgriculture.flag_id)
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(DevelopmentAssistanceToAgriculture.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(DevelopmentAssistanceToAgriculture)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in development_assistance_to_agriculture")
@cache_result(prefix="development_assistance_to_agriculture:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            DevelopmentAssistanceToAgriculture.unit,
            func.count(DevelopmentAssistanceToAgriculture.id).label('record_count')
        )
        .select_from(DevelopmentAssistanceToAgriculture)
        .group_by(DevelopmentAssistanceToAgriculture.unit)
        .order_by(DevelopmentAssistanceToAgriculture.unit)
    )
...
@router.get("/years", summary="Get available years in development_assistance_to_agriculture")
...
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                DevelopmentAssistanceToAgriculture.year,
                DevelopmentAssistanceToAgriculture.year_code,
                func.count(DevelopmentAssistanceToAgriculture.id).label('record_count')
            )
            .group_by(DevelopmentAssistanceToAgriculture.year, DevelopmentAssistanceToAgriculture.year_code)
            .order_by(DevelopmentAssistanceToAgriculture.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(DevelopmentAssistanceToAgriculture.year)
            .distinct()
            .order_by(DevelopmentAssistanceToAgriculture.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of development_assistance_to_agriculture dataset")
@cache_result(prefix="development_assistance_to_agriculture:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    donor_code_ids = db.execute(
        select(func.count(func.distinct(DevelopmentAssistanceToAgriculture.donor_code_id)))
        .select_from(DevelopmentAssistanceToAgriculture)
    ).scalar() or 0

    overview["dimensions"]["donors"] = {
        "count": donor_code_ids,
        "endpoint": f"/development_assistance_to_agriculture/donors"
    }

    recipient_country_code_ids = db.execute(
        select(func.count(func.distinct(DevelopmentAssistanceToAgriculture.recipient_country_code_id)))
        .select_from(DevelopmentAssistanceToAgriculture)
    ).scalar() or 0

    overview["dimensions"]["recipient_country_codes"] = {
        "count": recipient_country_code_ids,
        "endpoint": f"/development_assistance_to_agriculture/recipient_country_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(DevelopmentAssistanceToAgriculture.item_code_id)))
        .select_from(DevelopmentAssistanceToAgriculture)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/development_assistance_to_agriculture/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(DevelopmentAssistanceToAgriculture.element_code_id)))
        .select_from(DevelopmentAssistanceToAgriculture)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/development_assistance_to_agriculture/elements"
    }

    purpose_code_ids = db.execute(
        select(func.count(func.distinct(DevelopmentAssistanceToAgriculture.purpose_code_id)))
        .select_from(DevelopmentAssistanceToAgriculture)
    ).scalar() or 0

    overview["dimensions"]["purposes"] = {
        "count": purpose_code_ids,
        "endpoint": f"/development_assistance_to_agriculture/purposes"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(DevelopmentAssistanceToAgriculture.flag_id)))
        .select_from(DevelopmentAssistanceToAgriculture)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/development_assistance_to_agriculture/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(DevelopmentAssistanceToAgriculture.year).label('min_year'),
            func.max(DevelopmentAssistanceToAgriculture.year).label('max_year'),
            func.count(func.distinct(DevelopmentAssistanceToAgriculture.year)).label('year_count')
        )
        .select_from(DevelopmentAssistanceToAgriculture)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/development_assistance_to_agriculture/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(DevelopmentAssistanceToAgriculture.value).label('min_value'),
            func.max(DevelopmentAssistanceToAgriculture.value).label('max_value'),
            func.avg(DevelopmentAssistanceToAgriculture.value).label('avg_value')
        )
        .select_from(DevelopmentAssistanceToAgriculture)
        .where(and_(DevelopmentAssistanceToAgriculture.value > 0, DevelopmentAssistanceToAgriculture.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/development_assistance_to_agriculture",
        "aggregate": f"/development_assistance_to_agriculture/aggregate",
        "summary": f"/development_assistance_to_agriculture/summary",
        "overview": f"/development_assistance_to_agriculture/overview",
        "item_codes": f"/development_assistance_to_agriculture/item_codes",
        "elements": f"/development_assistance_to_agriculture/elements",
        "flags": f"/development_assistance_to_agriculture/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the development_assistance_to_agriculture endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(DevelopmentAssistanceToAgriculture)).scalar()
        return {
            "status": "healthy",
            "dataset": "development_assistance_to_agriculture",
            "records": result
