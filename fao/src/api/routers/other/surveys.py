...
@router.get("/")
@cache_result(prefix="surveys", ttl=86400, exclude_params=["response", "db"])
def get_surveys(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    survey_code: Optional[str] = Query(None, description="Filter by survey code (partial match)"),
    survey_code_exact: Optional[str] = Query(None, description="Filter by exact survey code"),
    survey: Optional[str] = Query(None, description="Filter by survey (partial match)"),
    survey_exact: Optional[str] = Query(None, description="Filter by exact survey"),
    ...
    """
    Get surveys data with filters and pagination.
    ...
    - survey_code: Partial match (case-insensitive)
    - survey_code_exact: Exact match
    - survey: Partial match (case-insensitive)
    - survey_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Surveys, 'id'):
                columns.append(Surveys.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Surveys)
    ...
    if survey_code is not None:
        conditions.append(Surveys.survey_code.ilike("%" + survey_code + "%"))
    if survey_code_exact is not None:
        conditions.append(Surveys.survey_code == survey_code_exact)
    if survey is not None:
        conditions.append(Surveys.survey.ilike("%" + survey + "%"))
    if survey_exact is not None:
        conditions.append(Surveys.survey == survey_exact)
    if source_dataset is not None:
        conditions.append(Surveys.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(Surveys.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Surveys, field_name):
                        order_by_clauses.append(getattr(Surveys, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Surveys, sort_field):
                        order_by_clauses.append(getattr(Surveys, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Surveys.id)
    ...
    base_url = str(router.url_path_for('get_surveys'))
    ...
    all_params = {
        ...
        'survey_code_exact': survey_code_exact,
        'survey': survey,
        'survey_exact': survey_exact,
