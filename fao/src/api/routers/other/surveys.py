...
from .surveys_config import SurveysConfig
...
@router.get("/", summary="Get surveys data")
async def get_surveys_data(
    request: Request,
    response: Response,
    ...
    # Filter parameters
    survey_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by survey code (comma-separated for multiple)"),
    ...
    """Get surveys data with advanced filtering and pagination.
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the surveys endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(Surveys)).scalar()
        return {
            "status": "healthy",
            "dataset": "surveys",
            "records": result
