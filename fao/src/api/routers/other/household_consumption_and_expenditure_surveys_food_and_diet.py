...
from .household_consumption_and_expenditure_surveys_food_and_diet_config import HouseholdConsumptionAndExpenditureSurveysFoodAndDietConfig
...
config = HouseholdConsumptionAndExpenditureSurveysFoodAndDietConfig()

@router.get("/", summary="Get household consumption and expenditure surveys food and diet data")
async def get_household_consumption_and_expenditure_surveys_food_and_diet_data(
    request: Request,
    response: Response,
    ...
    """Get household consumption and expenditure surveys food and diet data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=HouseholdConsumptionAndExpenditureSurveysFoodAndDiet, 
        model_name="HouseholdConsumptionAndExpenditureSurveysFoodAndDiet",
...
@router.get("/aggregate", summary="Get aggregated household consumption and expenditure surveys food and diet data")
async def get_household_consumption_and_expenditure_surveys_food_and_diet_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/elements", summary="Get elements in household_consumption_and_expenditure_surveys_food_and_diet")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet, HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in household_consumption_and_expenditure_surveys_food_and_diet")
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in household_consumption_and_expenditure_surveys_food_and_diet")
@cache_result(prefix="household_consumption_and_expenditure_surveys_food_and_diet:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.unit,
            func.count(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.id).label('record_count')
        )
        .select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
        .group_by(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.unit)
        .order_by(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.unit)
    )
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of household_consumption_and_expenditure_surveys_food_and_diet dataset")
@cache_result(prefix="household_consumption_and_expenditure_surveys_food_and_diet:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    survey_code_ids = db.execute(
        select(func.count(func.distinct(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.survey_code_id)))
        .select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
    ).scalar() or 0

    overview["dimensions"]["surveys"] = {
        "count": survey_code_ids,
        "endpoint": f"/household_consumption_and_expenditure_surveys_food_and_diet/surveys"
    }

    geographic_level_code_ids = db.execute(
        select(func.count(func.distinct(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.geographic_level_code_id)))
        .select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
    ).scalar() or 0

    overview["dimensions"]["geographic_levels"] = {
        "count": geographic_level_code_ids,
        "endpoint": f"/household_consumption_and_expenditure_surveys_food_and_diet/geographic_levels"
    }

    food_group_code_ids = db.execute(
        select(func.count(func.distinct(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.food_group_code_id)))
        .select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
    ).scalar() or 0

    overview["dimensions"]["food_groups"] = {
        "count": food_group_code_ids,
        "endpoint": f"/household_consumption_and_expenditure_surveys_food_and_diet/food_groups"
    }

    indicator_code_ids = db.execute(
        select(func.count(func.distinct(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.indicator_code_id)))
        .select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
    ).scalar() or 0

    overview["dimensions"]["indicators"] = {
        "count": indicator_code_ids,
        "endpoint": f"/household_consumption_and_expenditure_surveys_food_and_diet/indicators"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.element_code_id)))
        .select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/household_consumption_and_expenditure_surveys_food_and_diet/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.flag_id)))
        .select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/household_consumption_and_expenditure_surveys_food_and_diet/flags"
    }
    ...
    value_stats = db.execute(
        select(
            func.min(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.value).label('min_value'),
            func.max(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.value).label('max_value'),
            func.avg(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.value).label('avg_value')
        )
        .select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)
        .where(and_(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.value > 0, HouseholdConsumptionAndExpenditureSurveysFoodAndDiet.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/household_consumption_and_expenditure_surveys_food_and_diet",
        "aggregate": f"/household_consumption_and_expenditure_surveys_food_and_diet/aggregate",
        "summary": f"/household_consumption_and_expenditure_surveys_food_and_diet/summary",
        "overview": f"/household_consumption_and_expenditure_surveys_food_and_diet/overview",
        "elements": f"/household_consumption_and_expenditure_surveys_food_and_diet/elements",
        "flags": f"/household_consumption_and_expenditure_surveys_food_and_diet/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the household_consumption_and_expenditure_surveys_food_and_diet endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet)).scalar()
        return {
            "status": "healthy",
            "dataset": "household_consumption_and_expenditure_surveys_food_and_diet",
            "records": result
