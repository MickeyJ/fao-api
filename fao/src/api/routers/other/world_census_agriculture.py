...
async def get_world_census_agriculture_aggregated(
    ...
    # Build query
    query_builder = QueryBuilder(select(WorldCensusAgriculture))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(WorldCensusAgriculture, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(WorldCensusAgriculture, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(WorldCensusAgriculture, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in world_census_agriculture")
...
@router.get("/item_codes", summary="Get item_codes in world_census_agriculture")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(WorldCensusAgriculture, WorldCensusAgriculture.element_code_id == Elements.id)
