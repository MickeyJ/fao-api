...
from .geographic_levels_config import GeographicLevelsConfig
...
config = GeographicLevelsConfig()

@router.get("/", summary="Get geographic levels data")
async def get_geographic_levels_data(
    request: Request,
    response: Response,
    ...
    # Filter parameters
    geographic_level_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by geographic level code (comma-separated for multiple)"),
    ...
    """Get geographic levels data with advanced filtering and pagination.
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the geographic_levels endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(GeographicLevels)).scalar()
        return {
            "status": "healthy",
            "dataset": "geographic_levels",
            "records": result
        }
