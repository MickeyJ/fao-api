...
@router.get("/")
@cache_result(prefix="geographic_levels", ttl=86400, exclude_params=["response", "db"])
def get_geographic_levels(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    geographic_level_code: Optional[str] = Query(None, description="Filter by geographic level code (partial match)"),
    geographic_level_code_exact: Optional[str] = Query(None, description="Filter by exact geographic level code"),
    geographic_level: Optional[str] = Query(None, description="Filter by geographic level (partial match)"),
    geographic_level_exact: Optional[str] = Query(None, description="Filter by exact geographic level"),
    ...
    """
    Get geographic levels data with filters and pagination.
    ...
    - geographic_level_code: Partial match (case-insensitive)
    - geographic_level_code_exact: Exact match
    - geographic_level: Partial match (case-insensitive)
    - geographic_level_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(GeographicLevels, 'id'):
                columns.append(GeographicLevels.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(GeographicLevels)
    ...
    if geographic_level_code is not None:
        conditions.append(GeographicLevels.geographic_level_code.ilike("%" + geographic_level_code + "%"))
    if geographic_level_code_exact is not None:
        conditions.append(GeographicLevels.geographic_level_code == geographic_level_code_exact)
    if geographic_level is not None:
        conditions.append(GeographicLevels.geographic_level.ilike("%" + geographic_level + "%"))
    if geographic_level_exact is not None:
        conditions.append(GeographicLevels.geographic_level == geographic_level_exact)
    if source_dataset is not None:
        conditions.append(GeographicLevels.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(GeographicLevels.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(GeographicLevels, field_name):
                        order_by_clauses.append(getattr(GeographicLevels, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(GeographicLevels, sort_field):
                        order_by_clauses.append(getattr(GeographicLevels, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(GeographicLevels.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(GeographicLevels)
    ...
    base_url = str(router.url_path_for('get_geographic_levels'))
    ...
    all_params = {
        ...
        'geographic_level_code_exact': geographic_level_code_exact,
        'geographic_level': geographic_level,
        'geographic_level_exact': geographic_level_exact,
