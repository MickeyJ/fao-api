...
@router.get("/")
@cache_result(prefix="deflators", ttl=86400, exclude_params=["response", "db"])
def get_deflators(
    response: Response,
    ...
    """
    Get deflators data with filters and pagination.
    ...
    # Main table columns
    for col in Deflators.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Deflators, 'id'):
                columns.append(Deflators.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Deflators)
    
    # Add joins
    query = query.outerjoin(AreaCodes, Deflators.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, Deflators.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, Deflators.element_code_id == Elements.id)
    query = query.outerjoin(Flags, Deflators.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(Deflators.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(Deflators.year_code == year_code_exact)
    if year is not None:
        conditions.append(Deflators.year == year)
    if year_min is not None:
        conditions.append(Deflators.year >= year_min)
    if year_max is not None:
        conditions.append(Deflators.year <= year_max)
    if unit is not None:
        conditions.append(Deflators.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(Deflators.unit == unit_exact)
    if value is not None:
        conditions.append(Deflators.value == value)
    if value_min is not None:
        conditions.append(Deflators.value >= value_min)
    if value_max is not None:
        conditions.append(Deflators.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Deflators, field_name):
                        order_by_clauses.append(getattr(Deflators, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Deflators, sort_field):
                        order_by_clauses.append(getattr(Deflators, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Deflators.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Deflators)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, Deflators.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, Deflators.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, Deflators.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, Deflators.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_deflators'))
...
@router.get("/areas")
@cache_result(prefix="deflators:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="deflators:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    if not years_data:
        return {"dataset": "deflators", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="deflators:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(Deflators.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(Deflators.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(Deflators.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(Deflators.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(Deflators.flag_id))).scalar()
