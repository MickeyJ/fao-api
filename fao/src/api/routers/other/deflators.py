...
from .deflators_config import DeflatorsConfig
...
config = DeflatorsConfig()

@router.get("/", summary="Get deflators data")
async def get_deflators_data(
    request: Request,
    response: Response,
    ...
    """Get deflators data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=Deflators, 
        model_name="Deflators",
        table_name="deflators",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated deflators data")
async def get_deflators_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(Deflators))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in deflators")
@cache_result(prefix="deflators:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(Deflators.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(Deflators, Deflators.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'deflators')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in deflators")
@cache_result(prefix="deflators:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(Deflators, Deflators.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'deflators')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in deflators")
@cache_result(prefix="deflators:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(Deflators, Deflators.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'deflators')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in deflators")
@cache_result(prefix="deflators:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(Deflators, Flags.id == Deflators.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(Deflators.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(Deflators)
                .where(Deflators.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(Deflators)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in deflators")
@cache_result(prefix="deflators:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(Deflators)
        .group_by(Deflators.unit)
        .order_by(Deflators.unit)
    )
...
@router.get("/years", summary="Get available years in deflators")
@cache_result(prefix="deflators:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            ...
            .group_by(Deflators.year, Deflators.year_code)
            .order_by(Deflators.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(Deflators.year)
            .distinct()
            .order_by(Deflators.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of deflators dataset")
@cache_result(prefix="deflators:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(Deflators.area_code_id)))
        .select_from(Deflators)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/deflators/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(Deflators.item_code_id)))
        .select_from(Deflators)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/deflators/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(Deflators.element_code_id)))
        .select_from(Deflators)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/deflators/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(Deflators.flag_id)))
        .select_from(Deflators)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/deflators/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(Deflators.year).label('min_year'),
            func.max(Deflators.year).label('max_year'),
            func.count(func.distinct(Deflators.year)).label('year_count')
        )
        .select_from(Deflators)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/deflators/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(Deflators.value).label('min_value'),
            func.max(Deflators.value).label('max_value'),
            func.avg(Deflators.value).label('avg_value')
        )
        .select_from(Deflators)
        .where(and_(Deflators.value > 0, Deflators.value.is_not(None)))
    ).first()
    ...
    overview["endpoints"] = {
        "data": f"/deflators",
        "aggregate": f"/deflators/aggregate",
        "summary": f"/deflators/summary",
        "overview": f"/deflators/overview",
        "area_codes": f"/deflators/area_codes",
        "item_codes": f"/deflators/item_codes",
        "elements": f"/deflators/elements",
        "flags": f"/deflators/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the deflators endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(Deflators)).scalar()
        return {
            "status": "healthy",
            "dataset": "deflators",
            "records": result
