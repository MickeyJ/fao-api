...
@router.get("/")
@cache_result(prefix="elements", ttl=86400, exclude_params=["response", "db"])
def get_elements(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    element_code: Optional[str] = Query(None, description="Filter by element code (partial match)"),
    element_code_exact: Optional[str] = Query(None, description="Filter by exact element code"),
    element: Optional[str] = Query(None, description="Filter by element (partial match)"),
    element_exact: Optional[str] = Query(None, description="Filter by exact element"),
    ...
    """
    Get elements data with filters and pagination.
    ...
    - element_code: Partial match (case-insensitive)
    - element_code_exact: Exact match
    - element: Partial match (case-insensitive)
    - element_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Elements, 'id'):
                columns.append(Elements.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Elements)
    ...
    if element_code is not None:
        conditions.append(Elements.element_code.ilike("%" + element_code + "%"))
    if element_code_exact is not None:
        conditions.append(Elements.element_code == element_code_exact)
    if element is not None:
        conditions.append(Elements.element.ilike("%" + element + "%"))
    if element_exact is not None:
        conditions.append(Elements.element == element_exact)
    if source_dataset is not None:
        conditions.append(Elements.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(Elements.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Elements, field_name):
                        order_by_clauses.append(getattr(Elements, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Elements, sort_field):
                        order_by_clauses.append(getattr(Elements, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Elements.id)
    ...
    base_url = str(router.url_path_for('get_elements'))
    ...
    all_params = {
        ...
        'element_code_exact': element_code_exact,
        'element': element,
