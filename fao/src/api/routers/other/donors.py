...
@router.get("/")
@cache_result(prefix="donors", ttl=86400, exclude_params=["response", "db"])
def get_donors(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    donor_code: Optional[str] = Query(None, description="Filter by donor code (partial match)"),
    donor_code_exact: Optional[str] = Query(None, description="Filter by exact donor code"),
    donor: Optional[str] = Query(None, description="Filter by donor (partial match)"),
    donor_exact: Optional[str] = Query(None, description="Filter by exact donor"),
    donor_code_m49: Optional[str] = Query(None, description="Filter by donor code m49 (partial match)"),
    donor_code_m49_exact: Optional[str] = Query(None, description="Filter by exact donor code m49"),
    ...
    """
    Get donors data with filters and pagination.
    ...
    - donor_code: Partial match (case-insensitive)
    - donor_code_exact: Exact match
    - donor: Partial match (case-insensitive)
    - donor_exact: Exact match
    - donor_code_m49: Partial match (case-insensitive)
    - donor_code_m49_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Donors, 'id'):
                columns.append(Donors.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Donors)
    ...
    if donor_code is not None:
        conditions.append(Donors.donor_code.ilike("%" + donor_code + "%"))
    if donor_code_exact is not None:
        conditions.append(Donors.donor_code == donor_code_exact)
    if donor is not None:
        conditions.append(Donors.donor.ilike("%" + donor + "%"))
    if donor_exact is not None:
        conditions.append(Donors.donor == donor_exact)
    if donor_code_m49 is not None:
        conditions.append(Donors.donor_code_m49.ilike("%" + donor_code_m49 + "%"))
    if donor_code_m49_exact is not None:
        conditions.append(Donors.donor_code_m49 == donor_code_m49_exact)
    if source_dataset is not None:
        conditions.append(Donors.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(Donors.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Donors, field_name):
                        order_by_clauses.append(getattr(Donors, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Donors, sort_field):
                        order_by_clauses.append(getattr(Donors, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Donors.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Donors)
    ...
    base_url = str(router.url_path_for('get_donors'))
    ...
    all_params = {
        ...
        'donor_code_exact': donor_code_exact,
        'donor': donor,
        'donor_exact': donor_exact,
        'donor_code_m49': donor_code_m49,
        'donor_code_m49_exact': donor_code_m49_exact,
