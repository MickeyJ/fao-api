...
@router.get("/")
@cache_result(prefix="sexs", ttl=86400, exclude_params=["response", "db"])
def get_sexs(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    sex_code: Optional[str] = Query(None, description="Filter by sex code (partial match)"),
    sex_code_exact: Optional[str] = Query(None, description="Filter by exact sex code"),
    sex: Optional[str] = Query(None, description="Filter by sex (partial match)"),
    sex_exact: Optional[str] = Query(None, description="Filter by exact sex"),
    ...
    """
    Get sexs data with filters and pagination.
    ...
    - sex_code: Partial match (case-insensitive)
    - sex_code_exact: Exact match
    - sex: Partial match (case-insensitive)
    - sex_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Sexs, 'id'):
                columns.append(Sexs.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Sexs)
    ...
    if sex_code is not None:
        conditions.append(Sexs.sex_code.ilike("%" + sex_code + "%"))
    if sex_code_exact is not None:
        conditions.append(Sexs.sex_code == sex_code_exact)
    if sex is not None:
        conditions.append(Sexs.sex.ilike("%" + sex + "%"))
    if sex_exact is not None:
        conditions.append(Sexs.sex == sex_exact)
    if source_dataset is not None:
        conditions.append(Sexs.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(Sexs.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Sexs, field_name):
                        order_by_clauses.append(getattr(Sexs, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Sexs, sort_field):
                        order_by_clauses.append(getattr(Sexs, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Sexs.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Sexs)
    ...
    base_url = str(router.url_path_for('get_sexs'))
