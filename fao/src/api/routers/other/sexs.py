...
from .sexs_config import SexsConfig
...
@router.get("/", summary="Get sexs data")
async def get_sexs_data(
    request: Request,
    response: Response,
    ...
    # Filter parameters
    sex_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by sex code (comma-separated for multiple)"),
    ...
    """Get sexs data with advanced filtering and pagination.
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the sexs endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(Sexs)).scalar()
        return {
            "status": "healthy",
            "dataset": "sexs",
            "records": result
