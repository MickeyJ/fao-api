...
config = RecipientCountryCodesConfig()

@router.get("/", summary="Get recipient country codes data")
async def get_recipient_country_codes_data(
    request: Request,
    response: Response,
    ...
    """Get recipient country codes data with advanced filtering and pagination.
    ...
    recipient_country_code_m49 = router_handler.clean_param(recipient_country_code_m49, "like")
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the recipient_country_codes endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(RecipientCountryCodes)).scalar()
        return {
            "status": "healthy",
            "dataset": "recipient_country_codes",
            "records": result
        }
