...
@router.get("/")
@cache_result(prefix="recipient_country_codes", ttl=86400, exclude_params=["response", "db"])
def get_recipient_country_codes(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    recipient_country_code: Optional[str] = Query(None, description="Filter by recipient country code (partial match)"),
    recipient_country_code_exact: Optional[str] = Query(None, description="Filter by exact recipient country code"),
    recipient_country: Optional[str] = Query(None, description="Filter by recipient country (partial match)"),
    recipient_country_exact: Optional[str] = Query(None, description="Filter by exact recipient country"),
    recipient_country_code_m49: Optional[str] = Query(None, description="Filter by recipient country code m49 (partial match)"),
    recipient_country_code_m49_exact: Optional[str] = Query(None, description="Filter by exact recipient country code m49"),
    ...
    """
    Get recipient country codes data with filters and pagination.
    ...
    - recipient_country_code: Partial match (case-insensitive)
    - recipient_country_code_exact: Exact match
    - recipient_country: Partial match (case-insensitive)
    - recipient_country_exact: Exact match
    - recipient_country_code_m49: Partial match (case-insensitive)
    - recipient_country_code_m49_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(RecipientCountryCodes, 'id'):
                columns.append(RecipientCountryCodes.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(RecipientCountryCodes)
    ...
    if recipient_country_code is not None:
        conditions.append(RecipientCountryCodes.recipient_country_code.ilike("%" + recipient_country_code + "%"))
    if recipient_country_code_exact is not None:
        conditions.append(RecipientCountryCodes.recipient_country_code == recipient_country_code_exact)
    if recipient_country is not None:
        conditions.append(RecipientCountryCodes.recipient_country.ilike("%" + recipient_country + "%"))
    if recipient_country_exact is not None:
        conditions.append(RecipientCountryCodes.recipient_country == recipient_country_exact)
    if recipient_country_code_m49 is not None:
        conditions.append(RecipientCountryCodes.recipient_country_code_m49.ilike("%" + recipient_country_code_m49 + "%"))
    if recipient_country_code_m49_exact is not None:
        conditions.append(RecipientCountryCodes.recipient_country_code_m49 == recipient_country_code_m49_exact)
    if source_dataset is not None:
        conditions.append(RecipientCountryCodes.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(RecipientCountryCodes.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(RecipientCountryCodes, field_name):
                        order_by_clauses.append(getattr(RecipientCountryCodes, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(RecipientCountryCodes, sort_field):
                        order_by_clauses.append(getattr(RecipientCountryCodes, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(RecipientCountryCodes.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(RecipientCountryCodes)
    ...
    base_url = str(router.url_path_for('get_recipient_country_codes'))
    ...
    all_params = {
        ...
        'recipient_country_code_exact': recipient_country_code_exact,
        'recipient_country': recipient_country,
        'recipient_country_exact': recipient_country_exact,
        'recipient_country_code_m49': recipient_country_code_m49,
        'recipient_country_code_m49_exact': recipient_country_code_m49_exact,
