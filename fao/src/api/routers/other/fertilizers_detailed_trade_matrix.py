...
@router.get("/")
@cache_result(prefix="fertilizers_detailed_trade_matrix", ttl=86400, exclude_params=["response", "db"])
def get_fertilizers_detailed_trade_matrix(
    ...
    """
    Get fertilizers detailed trade matrix data with filters and pagination.
    ...
    # Main table columns
    for col in FertilizersDetailedTradeMatrix.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(FertilizersDetailedTradeMatrix, 'id'):
                columns.append(FertilizersDetailedTradeMatrix.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(FertilizersDetailedTradeMatrix)
    
    # Add joins
    query = query.outerjoin(ReporterCountryCodes, FertilizersDetailedTradeMatrix.reporter_country_code_id == ReporterCountryCodes.id)
    query = query.outerjoin(PartnerCountryCodes, FertilizersDetailedTradeMatrix.partner_country_code_id == PartnerCountryCodes.id)
    query = query.outerjoin(ItemCodes, FertilizersDetailedTradeMatrix.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, FertilizersDetailedTradeMatrix.element_code_id == Elements.id)
    query = query.outerjoin(Flags, FertilizersDetailedTradeMatrix.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(FertilizersDetailedTradeMatrix.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(FertilizersDetailedTradeMatrix.year_code == year_code_exact)
    if year is not None:
        conditions.append(FertilizersDetailedTradeMatrix.year == year)
    if year_min is not None:
        conditions.append(FertilizersDetailedTradeMatrix.year >= year_min)
    if year_max is not None:
        conditions.append(FertilizersDetailedTradeMatrix.year <= year_max)
    if unit is not None:
        conditions.append(FertilizersDetailedTradeMatrix.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(FertilizersDetailedTradeMatrix.unit == unit_exact)
    if value is not None:
        conditions.append(FertilizersDetailedTradeMatrix.value == value)
    if value_min is not None:
        conditions.append(FertilizersDetailedTradeMatrix.value >= value_min)
    if value_max is not None:
        conditions.append(FertilizersDetailedTradeMatrix.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(FertilizersDetailedTradeMatrix, field_name):
                        order_by_clauses.append(getattr(FertilizersDetailedTradeMatrix, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(FertilizersDetailedTradeMatrix, sort_field):
                        order_by_clauses.append(getattr(FertilizersDetailedTradeMatrix, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(FertilizersDetailedTradeMatrix.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(FertilizersDetailedTradeMatrix)
    
    # Add joins to count query
    count_query = count_query.outerjoin(ReporterCountryCodes, FertilizersDetailedTradeMatrix.reporter_country_code_id == ReporterCountryCodes.id)
    count_query = count_query.outerjoin(PartnerCountryCodes, FertilizersDetailedTradeMatrix.partner_country_code_id == PartnerCountryCodes.id)
    count_query = count_query.outerjoin(ItemCodes, FertilizersDetailedTradeMatrix.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, FertilizersDetailedTradeMatrix.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, FertilizersDetailedTradeMatrix.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_fertilizers_detailed_trade_matrix'))
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(FertilizersDetailedTradeMatrix.year)
    ...
    if not years_data:
        return {"dataset": "fertilizers_detailed_trade_matrix", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="fertilizers_detailed_trade_matrix:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(FertilizersDetailedTradeMatrix.id)).scalar()
