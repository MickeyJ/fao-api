...
from .sua_crops_livestock_config import SuaCropsLivestockConfig
...
router = APIRouter(
    prefix="/sua_crops_livestock",
    responses={404: {"description": "Not found"}},
)


config = SuaCropsLivestockConfig()

@router.get("/", summary="Get sua crops livestock data")
async def get_sua_crops_livestock_data(
    request: Request,
    response: Response,
    ...
    """Get sua crops livestock data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=SuaCropsLivestock, 
        model_name="SuaCropsLivestock",
        table_name="sua_crops_livestock",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated sua crops livestock data")
async def get_sua_crops_livestock_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(SuaCropsLivestock))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in sua_crops_livestock")
@cache_result(prefix="sua_crops_livestock:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(SuaCropsLivestock.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(SuaCropsLivestock, SuaCropsLivestock.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'sua_crops_livestock')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in sua_crops_livestock")
@cache_result(prefix="sua_crops_livestock:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(SuaCropsLivestock, SuaCropsLivestock.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'sua_crops_livestock')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in sua_crops_livestock")
@cache_result(prefix="sua_crops_livestock:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(SuaCropsLivestock, SuaCropsLivestock.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'sua_crops_livestock')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in sua_crops_livestock")
@cache_result(prefix="sua_crops_livestock:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(SuaCropsLivestock, Flags.id == SuaCropsLivestock.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(SuaCropsLivestock.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(SuaCropsLivestock)
                .where(SuaCropsLivestock.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(SuaCropsLivestock)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in sua_crops_livestock")
@cache_result(prefix="sua_crops_livestock:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            SuaCropsLivestock.unit,
            func.count(SuaCropsLivestock.id).label('record_count')
        )
        .select_from(SuaCropsLivestock)
        .group_by(SuaCropsLivestock.unit)
        .order_by(SuaCropsLivestock.unit)
    )
...
@router.get("/years", summary="Get available years in sua_crops_livestock")
@cache_result(prefix="sua_crops_livestock:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                SuaCropsLivestock.year,
                SuaCropsLivestock.year_code,
                func.count(SuaCropsLivestock.id).label('record_count')
            )
            .group_by(SuaCropsLivestock.year, SuaCropsLivestock.year_code)
            .order_by(SuaCropsLivestock.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(SuaCropsLivestock.year)
            .distinct()
            .order_by(SuaCropsLivestock.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of sua_crops_livestock dataset")
@cache_result(prefix="sua_crops_livestock:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(SuaCropsLivestock.area_code_id)))
        .select_from(SuaCropsLivestock)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/sua_crops_livestock/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(SuaCropsLivestock.item_code_id)))
        .select_from(SuaCropsLivestock)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/sua_crops_livestock/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(SuaCropsLivestock.element_code_id)))
        .select_from(SuaCropsLivestock)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/sua_crops_livestock/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(SuaCropsLivestock.flag_id)))
        .select_from(SuaCropsLivestock)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/sua_crops_livestock/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(SuaCropsLivestock.year).label('min_year'),
            func.max(SuaCropsLivestock.year).label('max_year'),
            func.count(func.distinct(SuaCropsLivestock.year)).label('year_count')
        )
        .select_from(SuaCropsLivestock)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/sua_crops_livestock/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(SuaCropsLivestock.value).label('min_value'),
            func.max(SuaCropsLivestock.value).label('max_value'),
            func.avg(SuaCropsLivestock.value).label('avg_value')
        )
        .select_from(SuaCropsLivestock)
        .where(and_(SuaCropsLivestock.value > 0, SuaCropsLivestock.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/sua_crops_livestock",
        "aggregate": f"/sua_crops_livestock/aggregate",
        "summary": f"/sua_crops_livestock/summary",
        "overview": f"/sua_crops_livestock/overview",
        "area_codes": f"/sua_crops_livestock/area_codes",
        "item_codes": f"/sua_crops_livestock/item_codes",
        "elements": f"/sua_crops_livestock/elements",
        "flags": f"/sua_crops_livestock/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the sua_crops_livestock endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(SuaCropsLivestock)).scalar()
        return {
            "status": "healthy",
            "dataset": "sua_crops_livestock",
            "records": result
