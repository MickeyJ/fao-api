...
from .macro_statistics_key_indicators_config import MacroStatisticsKeyIndicatorsConfig
...
config = MacroStatisticsKeyIndicatorsConfig()

@router.get("/", summary="Get macro statistics key indicators data")
async def get_macro_statistics_key_indicators_data(
    request: Request,
    response: Response,
    ...
    """Get macro statistics key indicators data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=MacroStatisticsKeyIndicators, 
        model_name="MacroStatisticsKeyIndicators",
        table_name="macro_statistics_key_indicators",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated macro statistics key indicators data")
async def get_macro_statistics_key_indicators_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(MacroStatisticsKeyIndicators))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(MacroStatisticsKeyIndicators, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(MacroStatisticsKeyIndicators, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(MacroStatisticsKeyIndicators, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in macro_statistics_key_indicators")
@cache_result(prefix="macro_statistics_key_indicators:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(MacroStatisticsKeyIndicators.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(MacroStatisticsKeyIndicators, MacroStatisticsKeyIndicators.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'macro_statistics_key_indicators')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
    ...
    return ResponseFormatter.format_metadata_response(
        dataset="macro_statistics_key_indicators",
        metadata_type="areas",
        total=total_count,
        items=[
            {
...
@router.get("/item_codes", summary="Get item_codes in macro_statistics_key_indicators")
@cache_result(prefix="macro_statistics_key_indicators:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(MacroStatisticsKeyIndicators, MacroStatisticsKeyIndicators.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'macro_statistics_key_indicators')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in macro_statistics_key_indicators")
@cache_result(prefix="macro_statistics_key_indicators:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(MacroStatisticsKeyIndicators, MacroStatisticsKeyIndicators.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'macro_statistics_key_indicators')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in macro_statistics_key_indicators")
@cache_result(prefix="macro_statistics_key_indicators:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(MacroStatisticsKeyIndicators, Flags.id == MacroStatisticsKeyIndicators.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(MacroStatisticsKeyIndicators.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(MacroStatisticsKeyIndicators)
                .where(MacroStatisticsKeyIndicators.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(MacroStatisticsKeyIndicators)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in macro_statistics_key_indicators")
@cache_result(prefix="macro_statistics_key_indicators:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    """Get all units of measurement used in this dataset."""
    query = (
        select(
            MacroStatisticsKeyIndicators.unit,
            func.count(MacroStatisticsKeyIndicators.id).label('record_count')
        )
        .select_from(MacroStatisticsKeyIndicators)
        .group_by(MacroStatisticsKeyIndicators.unit)
        .order_by(MacroStatisticsKeyIndicators.unit)
    )
...
@router.get("/years", summary="Get available years in macro_statistics_key_indicators")
@cache_result(prefix="macro_statistics_key_indicators:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                MacroStatisticsKeyIndicators.year,
                MacroStatisticsKeyIndicators.year_code,
                func.count(MacroStatisticsKeyIndicators.id).label('record_count')
            )
            .group_by(MacroStatisticsKeyIndicators.year, MacroStatisticsKeyIndicators.year_code)
            .order_by(MacroStatisticsKeyIndicators.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(MacroStatisticsKeyIndicators.year)
            .distinct()
            .order_by(MacroStatisticsKeyIndicators.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of macro_statistics_key_indicators dataset")
@cache_result(prefix="macro_statistics_key_indicators:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(MacroStatisticsKeyIndicators.area_code_id)))
        .select_from(MacroStatisticsKeyIndicators)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/macro_statistics_key_indicators/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(MacroStatisticsKeyIndicators.item_code_id)))
        .select_from(MacroStatisticsKeyIndicators)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/macro_statistics_key_indicators/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(MacroStatisticsKeyIndicators.element_code_id)))
        .select_from(MacroStatisticsKeyIndicators)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/macro_statistics_key_indicators/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(MacroStatisticsKeyIndicators.flag_id)))
        .select_from(MacroStatisticsKeyIndicators)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/macro_statistics_key_indicators/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(MacroStatisticsKeyIndicators.year).label('min_year'),
            func.max(MacroStatisticsKeyIndicators.year).label('max_year'),
            func.count(func.distinct(MacroStatisticsKeyIndicators.year)).label('year_count')
        )
        .select_from(MacroStatisticsKeyIndicators)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/macro_statistics_key_indicators/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(MacroStatisticsKeyIndicators.value).label('min_value'),
            func.max(MacroStatisticsKeyIndicators.value).label('max_value'),
            func.avg(MacroStatisticsKeyIndicators.value).label('avg_value')
        )
        .select_from(MacroStatisticsKeyIndicators)
        .where(and_(MacroStatisticsKeyIndicators.value > 0, MacroStatisticsKeyIndicators.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/macro_statistics_key_indicators",
        "aggregate": f"/macro_statistics_key_indicators/aggregate",
        "summary": f"/macro_statistics_key_indicators/summary",
        "overview": f"/macro_statistics_key_indicators/overview",
        "area_codes": f"/macro_statistics_key_indicators/area_codes",
        "item_codes": f"/macro_statistics_key_indicators/item_codes",
        "elements": f"/macro_statistics_key_indicators/elements",
        "flags": f"/macro_statistics_key_indicators/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the macro_statistics_key_indicators endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(MacroStatisticsKeyIndicators)).scalar()
        return {
            "status": "healthy",
            "dataset": "macro_statistics_key_indicators",
            "records": result
