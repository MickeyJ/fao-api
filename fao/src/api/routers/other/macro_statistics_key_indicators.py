...
@router.get("/")
@cache_result(prefix="macro_statistics_key_indicators", ttl=86400, exclude_params=["response", "db"])
def get_macro_statistics_key_indicators(
    ...
    """
    Get macro statistics key indicators data with filters and pagination.
    ...
    # Main table columns
    for col in MacroStatisticsKeyIndicators.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(MacroStatisticsKeyIndicators, 'id'):
                columns.append(MacroStatisticsKeyIndicators.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(MacroStatisticsKeyIndicators)
    ...
    if year_code is not None:
        conditions.append(MacroStatisticsKeyIndicators.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(MacroStatisticsKeyIndicators.year_code == year_code_exact)
    if year is not None:
        conditions.append(MacroStatisticsKeyIndicators.year == year)
    if year_min is not None:
        conditions.append(MacroStatisticsKeyIndicators.year >= year_min)
    if year_max is not None:
        conditions.append(MacroStatisticsKeyIndicators.year <= year_max)
    if unit is not None:
        conditions.append(MacroStatisticsKeyIndicators.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(MacroStatisticsKeyIndicators.unit == unit_exact)
    if value is not None:
        conditions.append(MacroStatisticsKeyIndicators.value == value)
    if value_min is not None:
        conditions.append(MacroStatisticsKeyIndicators.value >= value_min)
    if value_max is not None:
        conditions.append(MacroStatisticsKeyIndicators.value <= value_max)
    if note is not None:
        conditions.append(MacroStatisticsKeyIndicators.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(MacroStatisticsKeyIndicators.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(MacroStatisticsKeyIndicators, field_name):
                        order_by_clauses.append(getattr(MacroStatisticsKeyIndicators, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(MacroStatisticsKeyIndicators, sort_field):
                        order_by_clauses.append(getattr(MacroStatisticsKeyIndicators, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(MacroStatisticsKeyIndicators.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(MacroStatisticsKeyIndicators)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, MacroStatisticsKeyIndicators.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, MacroStatisticsKeyIndicators.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, MacroStatisticsKeyIndicators.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, MacroStatisticsKeyIndicators.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_macro_statistics_key_indicators'))
...
@router.get("/areas")
@cache_result(prefix="macro_statistics_key_indicators:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="macro_statistics_key_indicators:items", ttl=604800)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(MacroStatisticsKeyIndicators.year)
    ...
    if not years_data:
        return {"dataset": "macro_statistics_key_indicators", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="macro_statistics_key_indicators:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(MacroStatisticsKeyIndicators.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(MacroStatisticsKeyIndicators.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(MacroStatisticsKeyIndicators.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(MacroStatisticsKeyIndicators.element_code_id))).scalar()
    summary["unique_flags"] = db.query(func.count(func.distinct(MacroStatisticsKeyIndicators.flag_id))).scalar()
