...
from .item_codes_config import ItemCodesConfig
...
config = ItemCodesConfig()

@router.get("/", summary="Get item codes data")
async def get_item_codes_data(
    request: Request,
    response: Response,
    ...
    # Filter parameters
    item_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by item code (comma-separated for multiple)"),
    ...
    """Get item codes data with advanced filtering and pagination.
    ...
    item_code_cpc = router_handler.clean_param(item_code_cpc, "like")
    item_code_fbs = router_handler.clean_param(item_code_fbs, "like")
    item_code_sdg = router_handler.clean_param(item_code_sdg, "like")
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the item_codes endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(ItemCodes)).scalar()
        return {
            "status": "healthy",
            "dataset": "item_codes",
            "records": result
