...
@router.get("/")
@cache_result(prefix="item_codes", ttl=86400, exclude_params=["response", "db"])
def get_item_codes(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    item_code: Optional[str] = Query(None, description="Filter by item code (partial match)"),
    item_code_exact: Optional[str] = Query(None, description="Filter by exact item code"),
    item: Optional[str] = Query(None, description="Filter by item (partial match)"),
    item_exact: Optional[str] = Query(None, description="Filter by exact item"),
    item_code_cpc: Optional[str] = Query(None, description="Filter by item code cpc (partial match)"),
    item_code_cpc_exact: Optional[str] = Query(None, description="Filter by exact item code cpc"),
    item_code_fbs: Optional[str] = Query(None, description="Filter by item code fbs (partial match)"),
    item_code_fbs_exact: Optional[str] = Query(None, description="Filter by exact item code fbs"),
    item_code_sdg: Optional[str] = Query(None, description="Filter by item code sdg (partial match)"),
    item_code_sdg_exact: Optional[str] = Query(None, description="Filter by exact item code sdg"),
    ...
    """
    Get item codes data with filters and pagination.
    ...
    - item_code: Partial match (case-insensitive)
    - item_code_exact: Exact match
    - item: Partial match (case-insensitive)
    - item_exact: Exact match
    - item_code_cpc: Partial match (case-insensitive)
    - item_code_cpc_exact: Exact match
    - item_code_fbs: Partial match (case-insensitive)
    - item_code_fbs_exact: Exact match
    - item_code_sdg: Partial match (case-insensitive)
    - item_code_sdg_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(ItemCodes, 'id'):
                columns.append(ItemCodes.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(ItemCodes)
    ...
    if item_code is not None:
        conditions.append(ItemCodes.item_code.ilike("%" + item_code + "%"))
    if item_code_exact is not None:
        conditions.append(ItemCodes.item_code == item_code_exact)
    if item is not None:
        conditions.append(ItemCodes.item.ilike("%" + item + "%"))
    if item_exact is not None:
        conditions.append(ItemCodes.item == item_exact)
    if item_code_cpc is not None:
        conditions.append(ItemCodes.item_code_cpc.ilike("%" + item_code_cpc + "%"))
    if item_code_cpc_exact is not None:
        conditions.append(ItemCodes.item_code_cpc == item_code_cpc_exact)
    if item_code_fbs is not None:
        conditions.append(ItemCodes.item_code_fbs.ilike("%" + item_code_fbs + "%"))
    if item_code_fbs_exact is not None:
        conditions.append(ItemCodes.item_code_fbs == item_code_fbs_exact)
    if item_code_sdg is not None:
        conditions.append(ItemCodes.item_code_sdg.ilike("%" + item_code_sdg + "%"))
    if item_code_sdg_exact is not None:
        conditions.append(ItemCodes.item_code_sdg == item_code_sdg_exact)
    if source_dataset is not None:
        conditions.append(ItemCodes.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(ItemCodes.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(ItemCodes, field_name):
                        order_by_clauses.append(getattr(ItemCodes, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(ItemCodes, sort_field):
                        order_by_clauses.append(getattr(ItemCodes, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(ItemCodes.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(ItemCodes)
    ...
    base_url = str(router.url_path_for('get_item_codes'))
    ...
    all_params = {
        ...
        'item_code_cpc_exact': item_code_cpc_exact,
        'item_code_fbs': item_code_fbs,
        'item_code_fbs_exact': item_code_fbs_exact,
        'item_code_sdg': item_code_sdg,
        'item_code_sdg_exact': item_code_sdg_exact,
