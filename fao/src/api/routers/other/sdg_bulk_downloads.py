...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get AreaCodes in sdg_bulk_downloads")
...
@router.get("/item_codes", summary="Get ItemCodes in sdg_bulk_downloads")
...
async def get_available_elements(
    ...
    if include_distribution:
        query = (
            ...
            .select_from(Elements)
            .join(SdgBulkDownloads, Elements.id == SdgBulkDownloads.element_id)
            .where(Elements.source_dataset == 'sdg_bulk_downloads')
            .group_by(
                Elements.element_code,
                Elements.element,
