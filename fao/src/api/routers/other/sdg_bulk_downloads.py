...
@router.get("/")
@cache_result(prefix="sdg_bulk_downloads", ttl=86400, exclude_params=["response", "db"])
def get_sdg_bulk_downloads(
    response: Response,
    ...
    """
    Get sdg bulk downloads data with filters and pagination.
    ...
    # Main table columns
    for col in SdgBulkDownloads.__table__.columns:
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(SdgBulkDownloads, 'id'):
                columns.append(SdgBulkDownloads.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(SdgBulkDownloads)
    
    # Add joins
    query = query.outerjoin(AreaCodes, SdgBulkDownloads.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, SdgBulkDownloads.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, SdgBulkDownloads.element_code_id == Elements.id)
    query = query.outerjoin(Flags, SdgBulkDownloads.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(SdgBulkDownloads.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(SdgBulkDownloads.year_code == year_code_exact)
    if year is not None:
        conditions.append(SdgBulkDownloads.year == year)
    if year_min is not None:
        conditions.append(SdgBulkDownloads.year >= year_min)
    if year_max is not None:
        conditions.append(SdgBulkDownloads.year <= year_max)
    if unit is not None:
        conditions.append(SdgBulkDownloads.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(SdgBulkDownloads.unit == unit_exact)
    if value is not None:
        conditions.append(SdgBulkDownloads.value == value)
    if value_min is not None:
        conditions.append(SdgBulkDownloads.value >= value_min)
    if value_max is not None:
        conditions.append(SdgBulkDownloads.value <= value_max)
    if note is not None:
        conditions.append(SdgBulkDownloads.note.ilike("%" + note + "%"))
    if note_exact is not None:
        conditions.append(SdgBulkDownloads.note == note_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(SdgBulkDownloads, field_name):
                        order_by_clauses.append(getattr(SdgBulkDownloads, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(SdgBulkDownloads, sort_field):
                        order_by_clauses.append(getattr(SdgBulkDownloads, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(SdgBulkDownloads.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(SdgBulkDownloads)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, SdgBulkDownloads.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, SdgBulkDownloads.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, SdgBulkDownloads.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, SdgBulkDownloads.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_sdg_bulk_downloads'))
...
@router.get("/areas")
@cache_result(prefix="sdg_bulk_downloads:areas", ttl=604800)
def get_available_areas(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(SdgBulkDownloads, AreaCodes.id == SdgBulkDownloads.area_code_id)
...
@router.get("/items")
@cache_result(prefix="sdg_bulk_downloads:items", ttl=604800)
def get_available_items(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(SdgBulkDownloads, ItemCodes.id == SdgBulkDownloads.item_code_id)
...
def get_available_elements(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .join(SdgBulkDownloads, Elements.id == SdgBulkDownloads.element_code_id)
...
def get_temporal_coverage(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .group_by(SdgBulkDownloads.year)
    ...
    if not years_data:
        return {"dataset": "sdg_bulk_downloads", "message": "No temporal data available"}
...
@router.get("/summary")
@cache_result(prefix="sdg_bulk_downloads:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    """Get comprehensive summary of this dataset"""
    total_records = db.query(func.count(SdgBulkDownloads.id)).scalar()
    ...
    summary["unique_areas"] = db.query(func.count(func.distinct(SdgBulkDownloads.area_code_id))).scalar()
    summary["unique_items"] = db.query(func.count(func.distinct(SdgBulkDownloads.item_code_id))).scalar()
    summary["unique_elements"] = db.query(func.count(func.distinct(SdgBulkDownloads.element_code_id))).scalar()
