...
@router.get("/")
@cache_result(prefix="currencies", ttl=86400, exclude_params=["response", "db"])
def get_currencies(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    iso_currency_code: Optional[str] = Query(None, description="Filter by iso currency code (partial match)"),
    iso_currency_code_exact: Optional[str] = Query(None, description="Filter by exact iso currency code"),
    currency: Optional[str] = Query(None, description="Filter by currency (partial match)"),
    currency_exact: Optional[str] = Query(None, description="Filter by exact currency"),
    ...
    """
    Get currencies data with filters and pagination.
    ...
    - iso_currency_code: Partial match (case-insensitive)
    - iso_currency_code_exact: Exact match
    - currency: Partial match (case-insensitive)
    - currency_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Currencies, 'id'):
                columns.append(Currencies.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Currencies)
    ...
    if iso_currency_code is not None:
        conditions.append(Currencies.iso_currency_code.ilike("%" + iso_currency_code + "%"))
    if iso_currency_code_exact is not None:
        conditions.append(Currencies.iso_currency_code == iso_currency_code_exact)
    if currency is not None:
        conditions.append(Currencies.currency.ilike("%" + currency + "%"))
    if currency_exact is not None:
        conditions.append(Currencies.currency == currency_exact)
    if source_dataset is not None:
        conditions.append(Currencies.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(Currencies.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Currencies, field_name):
                        order_by_clauses.append(getattr(Currencies, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Currencies, sort_field):
                        order_by_clauses.append(getattr(Currencies, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Currencies.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Currencies)
    ...
    base_url = str(router.url_path_for('get_currencies'))
    ...
    all_params = {
        ...
        'iso_currency_code_exact': iso_currency_code_exact,
        'currency': currency,
