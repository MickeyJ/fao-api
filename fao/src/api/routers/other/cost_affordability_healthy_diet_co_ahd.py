...
from .cost_affordability_healthy_diet_co_ahd_config import CostAffordabilityHealthyDietCoAhdConfig
...
config = CostAffordabilityHealthyDietCoAhdConfig()

@router.get("/", summary="Get cost affordability healthy diet co ahd data")
async def get_cost_affordability_healthy_diet_co_ahd_data(
    request: Request,
    response: Response,
    ...
    release_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by release_code code (comma-separated for multiple)"),
    release: Optional[str] = Query(None, description="Filter by release description (partial match)"),
    ...
    """Get cost affordability healthy diet co ahd data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=CostAffordabilityHealthyDietCoAhd, 
        model_name="CostAffordabilityHealthyDietCoAhd",
    ...
    release_code = router_handler.clean_param(release_code, "multi")
    release = router_handler.clean_param(release, "like")
...
@router.get("/aggregate", summary="Get aggregated cost affordability healthy diet co ahd data")
async def get_cost_affordability_healthy_diet_co_ahd_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(CostAffordabilityHealthyDietCoAhd))
    filter_count = 0
    joined_tables = set()
    ...
    # Add grouping
    group_columns = [getattr(CostAffordabilityHealthyDietCoAhd, f) for f in group_fields]
    query_builder.add_grouping(group_columns)
    
    # Add aggregations
    for agg_config in agg_configs:
        field = agg_config['field']
        if not hasattr(CostAffordabilityHealthyDietCoAhd, field):
            raise HTTPException(400, f"Invalid aggregation field: {field}")
        
        column = getattr(CostAffordabilityHealthyDietCoAhd, field)
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in cost_affordability_healthy_diet_co_ahd")
@cache_result(prefix="cost_affordability_healthy_diet_co_ahd:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(CostAffordabilityHealthyDietCoAhd, CostAffordabilityHealthyDietCoAhd.area_code_id == AreaCodes.id)
...
@router.get("/item_codes", summary="Get item_codes in cost_affordability_healthy_diet_co_ahd")
@cache_result(prefix="cost_affordability_healthy_diet_co_ahd:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(CostAffordabilityHealthyDietCoAhd, CostAffordabilityHealthyDietCoAhd.item_code_id == ItemCodes.id)
...
@router.get("/elements", summary="Get elements in cost_affordability_healthy_diet_co_ahd")
...
async def get_available_elements(
    ...
    query = (
        ...
        .join(CostAffordabilityHealthyDietCoAhd, CostAffordabilityHealthyDietCoAhd.element_code_id == Elements.id)
...
@router.get("/flags", summary="Get flags in cost_affordability_healthy_diet_co_ahd")
...
async def get_available_flags(
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                ...
                .where(CostAffordabilityHealthyDietCoAhd.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(CostAffordabilityHealthyDietCoAhd)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in cost_affordability_healthy_diet_co_ahd")
@cache_result(prefix="cost_affordability_healthy_diet_co_ahd:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(CostAffordabilityHealthyDietCoAhd)
        .group_by(CostAffordabilityHealthyDietCoAhd.unit)
        .order_by(CostAffordabilityHealthyDietCoAhd.unit)
    )
...
@router.get("/years", summary="Get available years in cost_affordability_healthy_diet_co_ahd")
    ...
    if include_counts:
        query = (
            select(
                ...
                CostAffordabilityHealthyDietCoAhd.year_code,
                func.count(CostAffordabilityHealthyDietCoAhd.id).label('record_count')
            )
            .group_by(CostAffordabilityHealthyDietCoAhd.year, CostAffordabilityHealthyDietCoAhd.year_code)
            .order_by(CostAffordabilityHealthyDietCoAhd.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(CostAffordabilityHealthyDietCoAhd.year)
            .distinct()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of cost_affordability_healthy_diet_co_ahd dataset")
@cache_result(prefix="cost_affordability_healthy_diet_co_ahd:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(CostAffordabilityHealthyDietCoAhd.area_code_id)))
        .select_from(CostAffordabilityHealthyDietCoAhd)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/cost_affordability_healthy_diet_co_ahd/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(CostAffordabilityHealthyDietCoAhd.item_code_id)))
        .select_from(CostAffordabilityHealthyDietCoAhd)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/cost_affordability_healthy_diet_co_ahd/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(CostAffordabilityHealthyDietCoAhd.element_code_id)))
        .select_from(CostAffordabilityHealthyDietCoAhd)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/cost_affordability_healthy_diet_co_ahd/elements"
    }

    release_code_ids = db.execute(
        select(func.count(func.distinct(CostAffordabilityHealthyDietCoAhd.release_code_id)))
        .select_from(CostAffordabilityHealthyDietCoAhd)
    ).scalar() or 0

    overview["dimensions"]["releases"] = {
        "count": release_code_ids,
        "endpoint": f"/cost_affordability_healthy_diet_co_ahd/releases"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(CostAffordabilityHealthyDietCoAhd.flag_id)))
        .select_from(CostAffordabilityHealthyDietCoAhd)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/cost_affordability_healthy_diet_co_ahd/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(CostAffordabilityHealthyDietCoAhd.year).label('min_year'),
            func.max(CostAffordabilityHealthyDietCoAhd.year).label('max_year'),
            func.count(func.distinct(CostAffordabilityHealthyDietCoAhd.year)).label('year_count')
        )
        .select_from(CostAffordabilityHealthyDietCoAhd)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/cost_affordability_healthy_diet_co_ahd/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(CostAffordabilityHealthyDietCoAhd.value).label('min_value'),
            func.max(CostAffordabilityHealthyDietCoAhd.value).label('max_value'),
            func.avg(CostAffordabilityHealthyDietCoAhd.value).label('avg_value')
        )
        .select_from(CostAffordabilityHealthyDietCoAhd)
        .where(and_(CostAffordabilityHealthyDietCoAhd.value > 0, CostAffordabilityHealthyDietCoAhd.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/cost_affordability_healthy_diet_co_ahd",
        "aggregate": f"/cost_affordability_healthy_diet_co_ahd/aggregate",
        "summary": f"/cost_affordability_healthy_diet_co_ahd/summary",
        "overview": f"/cost_affordability_healthy_diet_co_ahd/overview",
        "area_codes": f"/cost_affordability_healthy_diet_co_ahd/area_codes",
        "item_codes": f"/cost_affordability_healthy_diet_co_ahd/item_codes",
        "elements": f"/cost_affordability_healthy_diet_co_ahd/elements",
        "flags": f"/cost_affordability_healthy_diet_co_ahd/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the cost_affordability_healthy_diet_co_ahd endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(CostAffordabilityHealthyDietCoAhd)).scalar()
        return {
            "status": "healthy",
            "dataset": "cost_affordability_healthy_diet_co_ahd",
            "records": result
