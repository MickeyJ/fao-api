...
@router.get("/")
@cache_result(prefix="sources", ttl=86400, exclude_params=["response", "db"])
def get_sources(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    source_code: Optional[str] = Query(None, description="Filter by source code (partial match)"),
    source_code_exact: Optional[str] = Query(None, description="Filter by exact source code"),
    source: Optional[str] = Query(None, description="Filter by source (partial match)"),
    source_exact: Optional[str] = Query(None, description="Filter by exact source"),
    ...
    """
    Get sources data with filters and pagination.
    ...
    - source_code: Partial match (case-insensitive)
    - source_code_exact: Exact match
    - source: Partial match (case-insensitive)
    - source_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Sources, 'id'):
                columns.append(Sources.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Sources)
    ...
    if source_code is not None:
        conditions.append(Sources.source_code.ilike("%" + source_code + "%"))
    if source_code_exact is not None:
        conditions.append(Sources.source_code == source_code_exact)
    if source is not None:
        conditions.append(Sources.source.ilike("%" + source + "%"))
    if source_exact is not None:
        conditions.append(Sources.source == source_exact)
    if source_dataset is not None:
        conditions.append(Sources.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(Sources.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Sources, field_name):
                        order_by_clauses.append(getattr(Sources, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Sources, sort_field):
                        order_by_clauses.append(getattr(Sources, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Sources.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Sources)
    ...
    base_url = str(router.url_path_for('get_sources'))
    ...
    all_params = {
        ...
        'source_code_exact': source_code_exact,
        'source': source,
