...
@router.get("/")
@cache_result(prefix="area_codes", ttl=86400, exclude_params=["response", "db"])
def get_area_codes(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    area_code: Optional[str] = Query(None, description="Filter by area code (partial match)"),
    area_code_exact: Optional[str] = Query(None, description="Filter by exact area code"),
    area: Optional[str] = Query(None, description="Filter by area (partial match)"),
    area_exact: Optional[str] = Query(None, description="Filter by exact area"),
    area_code_m49: Optional[str] = Query(None, description="Filter by area code m49 (partial match)"),
    area_code_m49_exact: Optional[str] = Query(None, description="Filter by exact area code m49"),
    ...
    """
    Get area codes data with filters and pagination.
    ...
    - area_code: Partial match (case-insensitive)
    - area_code_exact: Exact match
    - area: Partial match (case-insensitive)
    - area_exact: Exact match
    - area_code_m49: Partial match (case-insensitive)
    - area_code_m49_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(AreaCodes, 'id'):
                columns.append(AreaCodes.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(AreaCodes)
    ...
    if area_code is not None:
        conditions.append(AreaCodes.area_code.ilike("%" + area_code + "%"))
    if area_code_exact is not None:
        conditions.append(AreaCodes.area_code == area_code_exact)
    if area is not None:
        conditions.append(AreaCodes.area.ilike("%" + area + "%"))
    if area_exact is not None:
        conditions.append(AreaCodes.area == area_exact)
    if area_code_m49 is not None:
        conditions.append(AreaCodes.area_code_m49.ilike("%" + area_code_m49 + "%"))
    if area_code_m49_exact is not None:
        conditions.append(AreaCodes.area_code_m49 == area_code_m49_exact)
    if source_dataset is not None:
        conditions.append(AreaCodes.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(AreaCodes.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(AreaCodes, field_name):
                        order_by_clauses.append(getattr(AreaCodes, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(AreaCodes, sort_field):
                        order_by_clauses.append(getattr(AreaCodes, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(AreaCodes.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(AreaCodes)
    ...
    base_url = str(router.url_path_for('get_area_codes'))
    ...
    all_params = {
        ...
        'offset': offset,
        'area_code': area_code,
        'area_code_exact': area_code_exact,
        'area': area,
        'area_exact': area_exact,
        'area_code_m49': area_code_m49,
        'area_code_m49_exact': area_code_m49_exact,
