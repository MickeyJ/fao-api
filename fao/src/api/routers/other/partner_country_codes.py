...
@router.get("/")
@cache_result(prefix="partner_country_codes", ttl=86400, exclude_params=["response", "db"])
def get_partner_country_codes(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    partner_country_code: Optional[str] = Query(None, description="Filter by partner country code (partial match)"),
    partner_country_code_exact: Optional[str] = Query(None, description="Filter by exact partner country code"),
    partner_countries: Optional[str] = Query(None, description="Filter by partner countries (partial match)"),
    partner_countries_exact: Optional[str] = Query(None, description="Filter by exact partner countries"),
    partner_country_code_m49: Optional[str] = Query(None, description="Filter by partner country code m49 (partial match)"),
    partner_country_code_m49_exact: Optional[str] = Query(None, description="Filter by exact partner country code m49"),
    ...
    """
    Get partner country codes data with filters and pagination.
    ...
    - partner_country_code: Partial match (case-insensitive)
    - partner_country_code_exact: Exact match
    - partner_countries: Partial match (case-insensitive)
    - partner_countries_exact: Exact match
    - partner_country_code_m49: Partial match (case-insensitive)
    - partner_country_code_m49_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(PartnerCountryCodes, 'id'):
                columns.append(PartnerCountryCodes.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(PartnerCountryCodes)
    ...
    if partner_country_code is not None:
        conditions.append(PartnerCountryCodes.partner_country_code.ilike("%" + partner_country_code + "%"))
    if partner_country_code_exact is not None:
        conditions.append(PartnerCountryCodes.partner_country_code == partner_country_code_exact)
    if partner_countries is not None:
        conditions.append(PartnerCountryCodes.partner_countries.ilike("%" + partner_countries + "%"))
    if partner_countries_exact is not None:
        conditions.append(PartnerCountryCodes.partner_countries == partner_countries_exact)
    if partner_country_code_m49 is not None:
        conditions.append(PartnerCountryCodes.partner_country_code_m49.ilike("%" + partner_country_code_m49 + "%"))
    if partner_country_code_m49_exact is not None:
        conditions.append(PartnerCountryCodes.partner_country_code_m49 == partner_country_code_m49_exact)
    if source_dataset is not None:
        conditions.append(PartnerCountryCodes.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(PartnerCountryCodes.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(PartnerCountryCodes, field_name):
                        order_by_clauses.append(getattr(PartnerCountryCodes, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(PartnerCountryCodes, sort_field):
                        order_by_clauses.append(getattr(PartnerCountryCodes, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(PartnerCountryCodes.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(PartnerCountryCodes)
    ...
    base_url = str(router.url_path_for('get_partner_country_codes'))
    ...
    all_params = {
        ...
        'partner_country_code_exact': partner_country_code_exact,
        'partner_countries': partner_countries,
        'partner_countries_exact': partner_countries_exact,
        'partner_country_code_m49': partner_country_code_m49,
        'partner_country_code_m49_exact': partner_country_code_m49_exact,
