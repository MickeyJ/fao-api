...
from .purposes_config import PurposesConfig
...
config = PurposesConfig()

@router.get("/", summary="Get purposes data")
async def get_purposes_data(
    request: Request,
    response: Response,
    ...
    # Filter parameters
    purpose_code: Optional[Union[str, List[str]]] = Query(None, description="Filter by purpose code (comma-separated for multiple)"),
    ...
    """Get purposes data with advanced filtering and pagination.
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the purposes endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(Purposes)).scalar()
        return {
            "status": "healthy",
            "dataset": "purposes",
            "records": result
