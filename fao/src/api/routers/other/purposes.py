...
@router.get("/")
@cache_result(prefix="purposes", ttl=86400, exclude_params=["response", "db"])
def get_purposes(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    purpose_code: Optional[str] = Query(None, description="Filter by purpose code (partial match)"),
    purpose_code_exact: Optional[str] = Query(None, description="Filter by exact purpose code"),
    purpose: Optional[str] = Query(None, description="Filter by purpose (partial match)"),
    purpose_exact: Optional[str] = Query(None, description="Filter by exact purpose"),
    ...
    """
    Get purposes data with filters and pagination.
    ...
    - purpose_code: Partial match (case-insensitive)
    - purpose_code_exact: Exact match
    - purpose: Partial match (case-insensitive)
    - purpose_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Purposes, 'id'):
                columns.append(Purposes.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Purposes)
    ...
    if purpose_code is not None:
        conditions.append(Purposes.purpose_code.ilike("%" + purpose_code + "%"))
    if purpose_code_exact is not None:
        conditions.append(Purposes.purpose_code == purpose_code_exact)
    if purpose is not None:
        conditions.append(Purposes.purpose.ilike("%" + purpose + "%"))
    if purpose_exact is not None:
        conditions.append(Purposes.purpose == purpose_exact)
    if source_dataset is not None:
        conditions.append(Purposes.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(Purposes.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Purposes, field_name):
                        order_by_clauses.append(getattr(Purposes, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Purposes, sort_field):
                        order_by_clauses.append(getattr(Purposes, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Purposes.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Purposes)
    ...
    base_url = str(router.url_path_for('get_purposes'))
    ...
    all_params = {
        ...
        'purpose_code_exact': purpose_code_exact,
        'purpose': purpose,
