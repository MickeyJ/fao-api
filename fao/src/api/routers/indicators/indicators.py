...
@router.get("/")
@cache_result(prefix="indicators", ttl=86400, exclude_params=["response", "db"])
def get_indicators(
    response: Response,
    limit: int = Query(100, le=1000, ge=1, description="Maximum records to return"),
    offset: int = Query(0, ge=0, description="Number of records to skip"),
    # Dynamic column filters based on model
    indicator_code: Optional[str] = Query(None, description="Filter by indicator code (partial match)"),
    indicator_code_exact: Optional[str] = Query(None, description="Filter by exact indicator code"),
    indicator: Optional[str] = Query(None, description="Filter by indicator (partial match)"),
    indicator_exact: Optional[str] = Query(None, description="Filter by exact indicator"),
    ...
    """
    Get indicators data with filters and pagination.
    ...
    - indicator_code: Partial match (case-insensitive)
    - indicator_code_exact: Exact match
    - indicator: Partial match (case-insensitive)
    - indicator_exact: Exact match
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Indicators, 'id'):
                columns.append(Indicators.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Indicators)
    ...
    if indicator_code is not None:
        conditions.append(Indicators.indicator_code.ilike("%" + indicator_code + "%"))
    if indicator_code_exact is not None:
        conditions.append(Indicators.indicator_code == indicator_code_exact)
    if indicator is not None:
        conditions.append(Indicators.indicator.ilike("%" + indicator + "%"))
    if indicator_exact is not None:
        conditions.append(Indicators.indicator == indicator_exact)
    if source_dataset is not None:
        conditions.append(Indicators.source_dataset.ilike("%" + source_dataset + "%"))
    if source_dataset_exact is not None:
        conditions.append(Indicators.source_dataset == source_dataset_exact)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Indicators, field_name):
                        order_by_clauses.append(getattr(Indicators, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Indicators, sort_field):
                        order_by_clauses.append(getattr(Indicators, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Indicators.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Indicators)
    ...
    base_url = str(router.url_path_for('get_indicators'))
    ...
    all_params = {
        ...
        'indicator_code_exact': indicator_code_exact,
        'indicator': indicator,
        'indicator_exact': indicator_exact,
