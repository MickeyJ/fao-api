...
from .prices_archive_config import PricesArchiveConfig
...
router = APIRouter(
    prefix="/prices_archive",
    responses={404: {"description": "Not found"}},
)


config = PricesArchiveConfig()

@router.get("/", summary="Get prices archive data")
async def get_prices_archive_data(
    request: Request,
    response: Response,
    ...
    """Get prices archive data with advanced filtering and pagination.
    ...
    router_handler = DatasetRouterHandler(
        db=db, 
        model=PricesArchive, 
        model_name="PricesArchive",
        table_name="prices_archive",
        request=request, 
        response=response, 
        config=config
    )
...
@router.get("/aggregate", summary="Get aggregated prices archive data")
async def get_prices_archive_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(PricesArchive))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in prices_archive")
@cache_result(prefix="prices_archive:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        select(
            ...
            func.count(PricesArchive.id).label('record_count')
        )
        .select_from(AreaCodes)
        .join(PricesArchive, PricesArchive.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'prices_archive')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in prices_archive")
@cache_result(prefix="prices_archive:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(PricesArchive, PricesArchive.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'prices_archive')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in prices_archive")
@cache_result(prefix="prices_archive:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(PricesArchive, PricesArchive.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'prices_archive')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in prices_archive")
@cache_result(prefix="prices_archive:flags", ttl=604800)
async def get_available_flags(
    ...
    query = (
        ...
        .join(PricesArchive, Flags.id == PricesArchive.flag_id)
        .group_by(Flags.flag, Flags.description)
        .order_by(func.count(PricesArchive.id).desc())
    )
    ...
    for flag in flags:
        ...
        if include_distribution:
            ...
            count = db.execute(
                select(func.count())
                .select_from(PricesArchive)
                .where(PricesArchive.flag_id == flag.id)
            ).scalar() or 0
    ...
    if include_distribution:
        ...
        total_records = db.execute(
            select(func.count()).select_from(PricesArchive)
        ).scalar() or 0
...
@router.get("/units", summary="Get units of measurement in prices_archive")
@cache_result(prefix="prices_archive:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(PricesArchive)
        .group_by(PricesArchive.unit)
        .order_by(PricesArchive.unit)
    )
...
@router.get("/years", summary="Get available years in prices_archive")
@cache_result(prefix="prices_archive:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            select(
                PricesArchive.year,
                PricesArchive.year_code,
                func.count(PricesArchive.id).label('record_count')
            )
            .group_by(PricesArchive.year, PricesArchive.year_code)
            .order_by(PricesArchive.year_code)
        )
        results = db.execute(query).all()
        ...
        query = (
            select(PricesArchive.year)
            .distinct()
            .order_by(PricesArchive.year)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of prices_archive dataset")
@cache_result(prefix="prices_archive:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(PricesArchive.area_code_id)))
        .select_from(PricesArchive)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/prices_archive/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(PricesArchive.item_code_id)))
        .select_from(PricesArchive)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/prices_archive/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(PricesArchive.element_code_id)))
        .select_from(PricesArchive)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/prices_archive/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(PricesArchive.flag_id)))
        .select_from(PricesArchive)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/prices_archive/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(PricesArchive.year).label('min_year'),
            func.max(PricesArchive.year).label('max_year'),
            func.count(func.distinct(PricesArchive.year)).label('year_count')
        )
        .select_from(PricesArchive)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/prices_archive/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(PricesArchive.value).label('min_value'),
            func.max(PricesArchive.value).label('max_value'),
            func.avg(PricesArchive.value).label('avg_value')
        )
        .select_from(PricesArchive)
        .where(and_(PricesArchive.value > 0, PricesArchive.value.is_not(None)))
    ).first()
    ...
    # Available endpoints
    overview["endpoints"] = {
        "data": f"/prices_archive",
        "aggregate": f"/prices_archive/aggregate",
        "summary": f"/prices_archive/summary",
        "overview": f"/prices_archive/overview",
        "area_codes": f"/prices_archive/area_codes",
        "item_codes": f"/prices_archive/item_codes",
        "elements": f"/prices_archive/elements",
        "flags": f"/prices_archive/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the prices_archive endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(PricesArchive)).scalar()
        return {
            "status": "healthy",
            "dataset": "prices_archive",
            "records": result
