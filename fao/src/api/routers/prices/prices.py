...
from .prices_config import PricesConfig
...
@router.get("/", summary="Get prices data")
async def get_prices_data(
    ...
    """Get prices data with advanced filtering and pagination.
...
@router.get("/aggregate", summary="Get aggregated prices data")
async def get_prices_aggregated(
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
    # Grouping
    ...
    # Build query
    query_builder = QueryBuilder(select(Prices))
    filter_count = 0
    joined_tables = set()
...
# ========== Metadata Endpoints ==========
# ----------------------------------------
@router.get("/area_codes", summary="Get area_codes in prices")
@cache_result(prefix="prices:area_codes", ttl=604800)
async def get_available_area_codes(
    ...
    query = (
        ...
        .join(Prices, Prices.area_code_id == AreaCodes.id)
        .where(AreaCodes.source_dataset == 'prices')
        .group_by(
            AreaCodes.area_code,
            AreaCodes.area,
            AreaCodes.area_code_m49,
...
@router.get("/item_codes", summary="Get item_codes in prices")
@cache_result(prefix="prices:item_codes", ttl=604800)
async def get_available_item_codes(
    ...
    query = (
        ...
        .join(Prices, Prices.item_code_id == ItemCodes.id)
        .where(ItemCodes.source_dataset == 'prices')
        .group_by(
            ItemCodes.item_code,
            ItemCodes.item,
            ItemCodes.item_code_cpc,
...
@router.get("/elements", summary="Get elements in prices")
@cache_result(prefix="prices:elements", ttl=604800)
async def get_available_elements(
    ...
    query = (
        ...
        .join(Prices, Prices.element_code_id == Elements.id)
        .where(Elements.source_dataset == 'prices')
        .group_by(
            Elements.element_code,
            Elements.element,
        )
...
@router.get("/flags", summary="Get flags in prices")
@cache_result(prefix="prices:flags", ttl=604800)
...
@router.get("/units", summary="Get units of measurement in prices")
@cache_result(prefix="prices:units", ttl=604800)
async def get_available_units(db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .select_from(Prices)
        .group_by(Prices.unit)
        .order_by(Prices.unit)
    )
...
@router.get("/years", summary="Get available years in prices")
@cache_result(prefix="prices:years", ttl=604800)
async def get_available_years(
    ...
    if include_counts:
        query = (
            ...
            .group_by(Prices.year, Prices.year_code)
            .order_by(Prices.year_code)
        )
        results = db.execute(query).all()
...
# -----------------------------------------------
@router.get("/overview", summary="Get complete overview of prices dataset")
@cache_result(prefix="prices:overview", ttl=3600)
async def get_dataset_overview(db: Session = Depends(get_db)):
    ...
    area_code_ids = db.execute(
        select(func.count(func.distinct(Prices.area_code_id)))
        .select_from(Prices)
    ).scalar() or 0

    overview["dimensions"]["area_codes"] = {
        "count": area_code_ids,
        "endpoint": f"/prices/area_codes"
    }

    item_code_ids = db.execute(
        select(func.count(func.distinct(Prices.item_code_id)))
        .select_from(Prices)
    ).scalar() or 0

    overview["dimensions"]["item_codes"] = {
        "count": item_code_ids,
        "endpoint": f"/prices/item_codes"
    }

    element_code_ids = db.execute(
        select(func.count(func.distinct(Prices.element_code_id)))
        .select_from(Prices)
    ).scalar() or 0

    overview["dimensions"]["elements"] = {
        "count": element_code_ids,
        "endpoint": f"/prices/elements"
    }

    flag_ids = db.execute(
        select(func.count(func.distinct(Prices.flag_id)))
        .select_from(Prices)
    ).scalar() or 0

    overview["dimensions"]["flags"] = {
        "count": flag_ids,
        "endpoint": f"/prices/flags"
    }
    
    # Year range
    year_stats = db.execute(
        select(
            func.min(Prices.year).label('min_year'),
            func.max(Prices.year).label('max_year'),
            func.count(func.distinct(Prices.year)).label('year_count')
        )
        .select_from(Prices)
    ).first()
    ...
    overview["dimensions"]["years"] = {
        ...
        "endpoint": f"/prices/years"
    }
    
    # Value statistics
    value_stats = db.execute(
        select(
            func.min(Prices.value).label('min_value'),
            func.max(Prices.value).label('max_value'),
            func.avg(Prices.value).label('avg_value')
        )
        .select_from(Prices)
        .where(and_(Prices.value > 0, Prices.value.is_not(None)))
    ).first()
    ...
    overview["endpoints"] = {
        "data": f"/prices",
        "aggregate": f"/prices/aggregate",
        "summary": f"/prices/summary",
        "overview": f"/prices/overview",
        "area_codes": f"/prices/area_codes",
        "item_codes": f"/prices/item_codes",
        "elements": f"/prices/elements",
        "flags": f"/prices/flags",
    }
...
async def health_check(db: Session = Depends(get_db)):
    """Check if the prices endpoint is healthy."""
    try:
        # Try to execute a simple query
        result = db.execute(select(func.count()).select_from(Prices)).scalar()
        return {
            "status": "healthy",
            "dataset": "prices",
            "records": result
