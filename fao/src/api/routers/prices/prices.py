...
@router.get("/")
@cache_result(prefix="prices", ttl=86400, exclude_params=["response", "db"])
def get_prices(
    response: Response,
    ...
    """
    Get prices data with filters and pagination.
    ...
    if requested_fields:
        ...
        for field_name in requested_fields:
            ...
            elif field_name == 'id' and hasattr(Prices, 'id'):
                columns.append(Prices.id)
    else:
    ...
    # Build base query
    query = select(*columns).select_from(Prices)
    
    # Add joins
    query = query.outerjoin(AreaCodes, Prices.area_code_id == AreaCodes.id)
    query = query.outerjoin(ItemCodes, Prices.item_code_id == ItemCodes.id)
    query = query.outerjoin(Elements, Prices.element_code_id == Elements.id)
    query = query.outerjoin(Flags, Prices.flag_id == Flags.id)
    ...
    if year_code is not None:
        conditions.append(Prices.year_code.ilike("%" + year_code + "%"))
    if year_code_exact is not None:
        conditions.append(Prices.year_code == year_code_exact)
    if year is not None:
        conditions.append(Prices.year == year)
    if year_min is not None:
        conditions.append(Prices.year >= year_min)
    if year_max is not None:
        conditions.append(Prices.year <= year_max)
    if months_code is not None:
        conditions.append(Prices.months_code.ilike("%" + months_code + "%"))
    if months_code_exact is not None:
        conditions.append(Prices.months_code == months_code_exact)
    if months is not None:
        conditions.append(Prices.months.ilike("%" + months + "%"))
    if months_exact is not None:
        conditions.append(Prices.months == months_exact)
    if unit is not None:
        conditions.append(Prices.unit.ilike("%" + unit + "%"))
    if unit_exact is not None:
        conditions.append(Prices.unit == unit_exact)
    if value is not None:
        conditions.append(Prices.value == value)
    if value_min is not None:
        conditions.append(Prices.value >= value_min)
    if value_max is not None:
        conditions.append(Prices.value <= value_max)
        ...
        for sort_field in sort.split(','):
            ...
            if sort_field:  # Skip empty strings
                if sort_field.startswith('-'):
                    ...
                    if hasattr(Prices, field_name):
                        order_by_clauses.append(getattr(Prices, field_name).desc())
                else:
                    # Ascending order
                    if hasattr(Prices, sort_field):
                        order_by_clauses.append(getattr(Prices, sort_field))
        
        if order_by_clauses:
            query = query.order_by(*order_by_clauses)
    else:
        # Default ordering by ID for consistent pagination
        query = query.order_by(Prices.id)
    
    # Get total count with filters applied
    count_query = select(func.count()).select_from(Prices)
    
    # Add joins to count query
    count_query = count_query.outerjoin(AreaCodes, Prices.area_code_id == AreaCodes.id)
    count_query = count_query.outerjoin(ItemCodes, Prices.item_code_id == ItemCodes.id)
    count_query = count_query.outerjoin(Elements, Prices.element_code_id == Elements.id)
    count_query = count_query.outerjoin(Flags, Prices.flag_id == Flags.id)
    ...
    base_url = str(router.url_path_for('get_prices'))
...
@router.get("/areas")
@cache_result(prefix="prices:areas", ttl=604800)
...
@router.get("/items")
@cache_result(prefix="prices:items", ttl=604800)
...
@router.get("/summary")
@cache_result(prefix="prices:summary", ttl=604800)
def get_dataset_summary(db: Session = Depends(get_db)):
    ...
    summary["unique_flags"] = db.query(func.count(func.distinct(Prices.flag_id))).scalar()
