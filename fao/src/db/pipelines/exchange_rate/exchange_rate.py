...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "exchange_rate"  # This dataset's name
    ...
    # Generate hash IDs for ISO Currency Code
    df['iso_currency_code_id'] = df['ISO Currency Code'].apply(
        lambda val: generate_numeric_id(
            {
                'ISO Currency Code': str(val),
                'source_dataset': dataset_name,
            },
            ["ISO Currency Code", "source_dataset"]
        ) if pd.notna(val) and str(val).strip() else None
    )
    ...
    columns_to_drop = [col for col in ["Area", "Area Code", "Area Code (M49)", "Currency", "Element", "Element Code", "Flag", "ISO Currency Code"] if col in df.columns]
    if columns_to_drop:
