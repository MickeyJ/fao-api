...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "employment_indicators_agriculture"  # This dataset's name
    ...
    # Generate hash IDs for Source Code
    df['source_code_id'] = df['Source Code'].apply(
        lambda val: generate_numeric_id(
            {
                'Source Code': str(val),
                'source_dataset': dataset_name,
            },
            ["Source Code", "source_dataset"]
        ) if pd.notna(val) and str(val).strip() else None
    )
    ...
    columns_to_drop = [col for col in ["Area", "Area Code", "Area Code (M49)", "Element", "Element Code", "Flag", "Indicator", "Indicator Code", "Sex", "Sex Code", "Source", "Source Code"] if col in df.columns]
    if columns_to_drop:
...
def insert(df: pd.DataFrame, session: Session):
    ...
    for chunk_idx, start_idx in enumerate(range(0, total_rows, chunk_size)):
        ...
        if records:
            try:
                stmt = pg_insert(EmploymentIndicatorsAgriculture).values(records)
