...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "household_consumption_and_expenditure_surveys_food_and_diet"  # This dataset's name
    ...
    columns_to_drop = [col for col in ["Element", "Element Code", "Flag", "Food Group", "Food Group Code", "Geographic Level", "Geographic Level Code", "Indicator", "Indicator Code", "Survey", "Survey Code"] if col in df.columns]
    if columns_to_drop:
...
def insert(df: pd.DataFrame, session: Session):
    ...
    for chunk_idx, start_idx in enumerate(range(0, total_rows, chunk_size)):
        ...
        if records:
            try:
                stmt = pg_insert(HouseholdConsumptionAndExpenditureSurveysFoodAndDiet).values(records)
