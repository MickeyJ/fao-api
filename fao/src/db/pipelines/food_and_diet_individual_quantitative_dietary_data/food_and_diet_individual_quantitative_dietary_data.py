...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "food_and_diet_individual_quantitative_dietary_data"  # This dataset's name
    ...
    # Generate hash IDs for Population Group Code
    ...
    columns_to_drop = [col for col in ["Element", "Element Code", "Flag", "Food Group", "Food Group Code", "Geographic Level", "Geographic Level Code", "Indicator", "Indicator Code", "Population Group", "Sex", "Sex Code", "Survey", "Survey Code"] if col in df.columns]
    if columns_to_drop:
...
def insert(df: pd.DataFrame, session: Session):
    ...
    for chunk_idx, start_idx in enumerate(range(0, total_rows, chunk_size)):
        ...
        if records:
            try:
                stmt = pg_insert(FoodAndDietIndividualQuantitativeDietaryData).values(records)
