...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "commodity_balances_non_food_2013_old_methodology"  # This dataset's name
...
def insert(df: pd.DataFrame, session: Session):
    ...
    for chunk_idx, start_idx in enumerate(range(0, total_rows, chunk_size)):
        ...
        if records:
            try:
                stmt = pg_insert(CommodityBalancesNonFood2013OldMethodology).values(records)
