...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "cost_affordability_healthy_diet_co_ahd"  # This dataset's name
    ...
    # Generate hash IDs for Release Code
    df['release_code_id'] = df['Release Code'].apply(
        lambda val: generate_numeric_id(
            {
                'Release Code': str(val),
                'source_dataset': dataset_name,
            },
            ["Release Code", "source_dataset"]
        ) if pd.notna(val) and str(val).strip() else None
    )
    ...
    columns_to_drop = [col for col in ["Area", "Area Code", "Area Code (M49)", "Element", "Element Code", "Flag", "Item", "Item Code", "Release", "Release Code"] if col in df.columns]
    if columns_to_drop:
...
def insert(df: pd.DataFrame, session: Session):
    ...
    for chunk_idx, start_idx in enumerate(range(0, total_rows, chunk_size)):
        ...
        if records:
            try:
                stmt = pg_insert(CostAffordabilityHealthyDietCoAhd).values(records)
