...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "environment_temperature_change"  # This dataset's name
    ...
    columns_to_drop = [col for col in ["Area", "Area Code", "Area Code (M49)", "Element", "Element Code", "Flag"] if col in df.columns]
    if columns_to_drop:
...
def insert(df: pd.DataFrame, session: Session):
    ...
    for chunk_idx, start_idx in enumerate(range(0, total_rows, chunk_size)):
        ...
        if records:
            try:
                stmt = pg_insert(EnvironmentTemperatureChange).values(records)
