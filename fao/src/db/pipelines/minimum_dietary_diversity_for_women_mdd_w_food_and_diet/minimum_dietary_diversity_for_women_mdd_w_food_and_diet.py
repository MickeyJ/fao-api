...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "minimum_dietary_diversity_for_women_mdd_w_food_and_diet"  # This dataset's name
...
def insert(df: pd.DataFrame, session: Session):
    ...
    for chunk_idx, start_idx in enumerate(range(0, total_rows, chunk_size)):
        ...
        if records:
            try:
                stmt = pg_insert(MinimumDietaryDiversityForWomenMddWFoodAndDiet).values(records)
