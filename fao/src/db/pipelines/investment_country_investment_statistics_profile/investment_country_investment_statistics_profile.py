...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataset_name = "investment_country_investment_statistics_profile"  # This dataset's name
...
def insert(df: pd.DataFrame, session: Session):
    ...
    for chunk_idx, start_idx in enumerate(range(0, total_rows, chunk_size)):
        ...
        if records:
            try:
                stmt = pg_insert(InvestmentCountryInvestmentStatisticsProfile).values(records)
